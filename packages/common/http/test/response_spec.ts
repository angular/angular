...

{
  describe("HttpResponse", () => {
    ...
    it("can be checked by HttpStatusCode enum", () => {
      const okResponse = new HttpResponse({ status: 200 });
      const notFoundResponse = new HttpResponse({ status: 404 });
      expect(okResponse.status).toEqual(HttpStatusCode.Ok);
      expect(notFoundResponse.status).toEqual(HttpStatusCode.NotFound);
    });
  });
}
