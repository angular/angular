    ...
    if (req.method === 'JSONP') {
      throw new RuntimeError(
          ...
          (typeof ngDevMode === 'undefined' || ngDevMode) &&
              `Attempted to construct Jsonp request without enabling support for it. To fix the problem, either add the \`withJsonpSupport()\` call (if \`provideHttpClient()\` is used) or import the \`HttpClientJsonpModule\` in the root NgModule.`);
    }
