 ...
 * @publicApi
 */
let FetchBackend = class FetchBackend {
  constructor() {
          ...
          if (request.reportProgress) {
            partialText =
              request.responseType === 'text'
                ? (partialText ?? '') +
                  (decoder ?? (decoder = new TextDecoder())).decode(value, {stream: true})
                : undefined;
...
FetchBackend = __decorate([Injectable()], FetchBackend);
export {FetchBackend};
/**
...
function warningOptionsMessage(req) {
  if (req.credentials && req.withCredentials) {
    console.warn(
      formatRuntimeError(
        2819 /* RuntimeErrorCode.WITH_CREDENTIALS_OVERRIDES_EXPLICIT_CREDENTIALS */,
