...
export function main() {
  describe('ngLoopFrom', () => {
    ...
    it('should support injecting `NgFor` and get an instance of `NgForOf`', async(() => {
      fixture = TestBed.overrideComponent(TestComponent, {set: {template:
        `<ng-template ngLoop [ngLoopTo]='2' let-i test></ng-template>`
...
function createTestComponent(from: number | undefined, to: number, by?: number, precision?: number): ComponentFixture<TestComponent> {
  let loopString = 'let idx';
  if (from !== undefined)
    loopString += ' from start';
  if (to !== undefined)
    loopString += ' to end';
  if (by !== undefined)
    loopString += ' by step';
  if (precision !== undefined)
    loopString += ' precision: precision';

  const cmp = TestBed.overrideComponent(TestComponent, {set: {template:
    `<div><span *ngLoop="${loopString}">{{idx}};</span></div>`
  }})
  .createComponent(TestComponent);
