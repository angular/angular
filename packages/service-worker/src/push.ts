...
import {EVENT_PUSH_MESSAGE, NgswCommChannel} from './low_level';
 ...
 * @experimental
 */
export class NgswPush {
  readonly messages: Observable<object>;
  readonly subscription: Observable<PushSubscription|null>;

  private pushManager: Observable<PushManager>private subscriptionChanges:
      Subject<PushSubscription|null> = new Subject<PushSubscription|null>();

  constructor(private sw: NgswCommChannel) {
    this.messages = op_map.call(
        this.sw.eventsOfType(EVENT_PUSH_MESSAGE), (message: {data: object}) => message.data);

    this.pushManager =
        op_map.call(this.sw.registration, (registration: ServiceWorkerRegistration) => {
          return registration.pushManager;
        }) as Observable<PushManager>;

    const workerDrivenSubscriptions = op_switchMap.call(
        this.pushManager, (pm: PushManager) => pm.getSubscription().then(sub => { return sub; }))
                                          as Observable<PushSubscription|null>;
  ...
  requestSubscription(options: {serverPublicKey: string}): Promise<PushSubscription> {
    ...
    const subscribe =
        op_switchMap.call(this.pushManager, (pm: PushManager) => pm.subscribe(pushOptions))
            as Observable<PushSubscription>;
