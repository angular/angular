...
(function () {
  ...
  describe('Driver', () => {
    ...
    it('should bypass serviceworker on ngsw-bypass parameter', async () => {
      ...
      await makeRequest(
        scope,
        '/bar&ngsw-bypass=true.txt?testparam=test&ngSW-BYPASS=SOMETHING&testparam2=test'
      );
    ...
    describe('cache naming', () => {
      ...
      const getClientAssignments = async (sw: SwTestHarness, baseHref: string) => {
        const cache = ((await sw.caches.open(
          `ngsw:${baseHref}:db:control`
      ...
      it('does not affect caches from other scopes', async () => {
        ...
        expect(barCacheNames).toEqual([...cacheKeysFor('/foo/'), ...cacheKeysFor('/bar/')]);
      ...
      it('updates existing caches for same scope', async () => {
        ...
        const fooScope2 = await initializeSwFor(
          '/foo/',
          await fooScope.caches.dehydrate(),
          serverUpdate
        );
    ...
    describe('cleanupOldSwCaches()', () => {
      ...
      it('should delete other caches even if deleting one of them fails', async () => {
        ...
        const deleteSpy = spyOn(scope.caches, 'delete').and.callFake((cacheName: string) =>
