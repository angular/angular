  ...
  return {
    ...
    'bazelOptions': {
      ...
      'nodeModulesPrefix': path.join(
        require.resolve('npm/node_modules/typescript/package.json'),
        '../../'
      ),
    },
    'files': options.files,
    'angularCompilerOptions': {
      ...
      'expectedOut': [
        ...
        ...options.compilationTargetSrc.map((src) =>
          srcToExpectedOut(src, 'ngfactory.js', options)
        ),
        ...options.compilationTargetSrc.map((src) =>
          srcToExpectedOut(src, 'ngfactory.d.ts', options)
        ),
        ...options.compilationTargetSrc.map((src) =>
          srcToExpectedOut(src, 'ngsummary.js', options)
        ),
        ...options.compilationTargetSrc.map((src) =>
          srcToExpectedOut(src, 'ngsummary.d.ts', options)
        ),
        ...options.compilationTargetSrc.map((src) =>
          srcToExpectedOut(src, 'ngsummary.json', options)
        ),
      ],
    },
  };
