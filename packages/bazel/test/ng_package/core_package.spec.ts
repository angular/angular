...
describe('@angular/core ng_package', () => {
  ...
  describe('primary entry-point', () => {
    describe('package.json', () => {
      ...
      it('should contain metadata for ng update', () => {
        ...
        expect(JSON.parse(shx.cat(packageJson))['ng-update']['packageGroup']).toContain(
          '@angular/core'
        );
      });
    });
    ...
    obsoleteInIvy('metadata files are no longer needed or produced in Ivy').describe(
      'angular metadata',
      () => {
    ...
    describe('fesm2015', () => {
      ...
      it('should have a source map', () => {
        expect(shx.cat('fesm2015/core.js.map')).toContain(
          `{"version":3,"file":"core.js","sources":`
        );
      });

      it('should have the version info in the header', () => {
        expect(shx.cat('fesm2015/core.js')).toMatch(
          /@license Angular v\d+\.\d+\.\d+(?!-PLACEHOLDER)/
        );
      });

      obsoleteInIvy('we no longer need to export private symbols').it(
        'should have been built from the generated bundle index',
        () => {
    ...
    describe('umd', () => {
      ...
      it('should have the version info in the header', () => {
        expect(shx.cat('bundles/core.umd.js')).toMatch(
          /@license Angular v\d+\.\d+\.\d+(?!-PLACEHOLDER)/
        );
      });
  ...
  describe('secondary entry-point', () => {
    ...
    obsoleteInIvy('metadata files are no longer needed or produced in Ivy').describe(
      'angular metadata file',
      () => {
        it("should have a 'redirect' metadata.json file next to the d.ts file", () => {
          expect(shx.cat('testing.metadata.json')).toContain(
    ...
    describe('fesm2015', () => {
      ...
      it('should have a source map', () => {
        expect(shx.cat('fesm2015/testing.js.map')).toContain(
          `{"version":3,"file":"testing.js","sources":`
        );
      });

      it('should have the version info in the header', () => {
        expect(shx.cat('fesm2015/testing.js')).toMatch(
          /@license Angular v\d+\.\d+\.\d+(?!-PLACEHOLDER)/
        );
      });
    });
    ...
    describe('fesm5', () => {
      ...
      it('should have a source map', () => {
        expect(shx.cat('fesm5/testing.js.map')).toContain(
          `{"version":3,"file":"testing.js","sources":`
        );
      });
    });
    ...
    describe('umd', () => {
      ...
      it('should have an AMD name', () => {
        expect(shx.cat('bundles/core-testing.umd.js')).toContain("define('@angular/core/testing'");
      });
