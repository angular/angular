...
describe('ng-add schematic', () => {
  ...
  it('should create yarn.lock if it does not exist', () => {
    host = schematicRunner.runSchematic('ng-add', defaultOptions, host);
    expect(host.files).toContain('/yarn.lock');
    expect(host.readContent('/yarn.lock')).toBe('');
  });

  it('should not overwrite existing yarn.lock', () => {
    expect(host.exists('/yarn.lock')).toBe(false);
    host.create('/yarn.lock', 'lock file content');
    expect(host.files).toContain('/yarn.lock');
    host = schematicRunner.runSchematic('ng-add', defaultOptions, host);
    expect(host.files).toContain('/yarn.lock');
    expect(host.readContent('/yarn.lock')).toBe('lock file content');
  });
