 ...
 * This function updates the tsconfig.json to suppress Bazel warnings about
...
function updateTsconfigJson(): Rule {
  return (host: Tree, context: SchematicContext) => {
    ...
    const bazelOptions = findPropertyInAstObject(ast, 'bazelOptions');
    const recorder = host.beginUpdate(tsconfigPath);
    recorder.insertRight(
        0, '// Warning: Some compiler options are controlled by Bazel and changing them \n' +
            '// in this file has no effect. If you do need to change these settings, set \n' +
            '// "suppressTsconfigOverrideWarnings" to false.\n');
    if (!isJsonAstObject(bazelOptions)) {
      const indent = 2;
      appendPropertyInAstObject(
          recorder, ast, 'bazelOptions', {
            suppressTsconfigOverrideWarnings: true,
          },
          indent);
    } else {
      const suppressWarnings =
          findPropertyInAstObject(bazelOptions, 'suppressTsconfigOverrideWarnings');
      const indent = 4;
      if (suppressWarnings) {
        replacePropertyInAstObject(
            recorder, bazelOptions, 'suppressTsconfigOverrideWarnings', true, indent);
      } else {
