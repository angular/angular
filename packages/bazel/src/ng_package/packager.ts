...
function main(args: string[]): number {
  ...
  function writeEsmFile(file: string, suffix: string, outDir: string) {
    function relPath(file: string, suffix: string) {
      if (suffix) {
        ...
        const root = suffix
          ? file.substr(0, file.lastIndexOf(`${suffix}/`) + suffix.length + 1)
          : binDir;
  ...
  dtsBundles.forEach((bundleFile) => {
    ...
    const content =
      licenseBanner +
      '\n' +
      readTypingsAndStripAmdModule(bundleFile).replace(
        /(\/\*\*\s+\*\s\@license(((?!\*\/).|\s)*)\*\/)/gm,
        ''
      );
  ...
  Object.keys(modulesManifest).forEach((entryPointPackageName) => {
    ...
    if (metadataFilePath) {
      createMetadataReexportFile(
        entryPointName,
        modulesManifest[entryPointPackageName]['metadata'],
    ...
    createTypingsReexportFile(
      entryPointName,
      licenseBanner,
      modulesManifest[entryPointPackageName]['typings']
    );
  ...
  function copyFile(file: string, baseDir: string, relative = '.') {
    ...
    const outFile = path.posix.join(
      dir,
      path.basename(file.endsWith('.mjs') ? file.replace(/\.mjs$/, '.js') : file)
    );
    shx.mkdir('-p', dir);
    shx.cp(file, outFile);
  ...
  function amendPackageJson(
    packageJson: string,
    parsedPackage: {[key: string]: string},
...
`;
  ...
  function rewireMetadata(metadataPath: string, typingsPath: string): string {
    ...
    let typingsRelativePath = normalizeSeparators(
      path.relative(path.dirname(metadataPath), typingsPath)
    );
    ...
    if (metadata.exports) {
      ...
      metadata.exports = metadata.exports.filter(
        (e: {from: string}) => !e.from.match(relativePathRegex)
      );
    }
