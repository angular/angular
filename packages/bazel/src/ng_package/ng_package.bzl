...
def _ng_package_impl(ctx):
    ...
    for dep in deps_in_package:
        ...
        if hasattr(dep, "dts_bundler"):
            typings.append(dep.dts_bundler)
    ...
    for m in flat_module_metadata:
        ...
        # typings output file which would eventually be generated by packager.ts
        typings_files = m.typings_file.path.replace(".bundle.d.ts", ".d.ts")

        metadata_arg[m.module_name] = {
            "index": typings_files.replace(".d.ts", ".js"),
            "typings": typings_files,
    ...
    packager_args.add_joined([s.path for s in typings], join_with = ",", omit_if_empty = False)
