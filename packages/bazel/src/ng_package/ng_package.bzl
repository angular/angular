...
def _ng_package_impl(ctx):
    ...
    for m in flat_module_metadata:
        packager_inputs.extend([m.metadata_file, m.typings_file])
        # typings output file which would eventually be generated by packager.ts
        typings_out_path = m.typings_file.path.replace(".bundle.d.ts", ".d.ts")

        metadata_arg[m.module_name] = {
            "index": typings_out_path.replace(".d.ts", ".js"),
            "typings": typings_out_path,
            "typings_bundle": m.typings_file.path,
