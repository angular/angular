...
async function loadModuleInterop<T>(moduleName: string): Promise<T> {
  const importModule = name => new Function('m', `return import(m);`)(name);
  let exports: Partial<T>&{default?: T};
  try {
    exports = await importModule(moduleName);
  } catch (e) {
    try {
      exports = await importModule(`${moduleName}.js`);
    } catch {
      exports = await importModule(`${moduleName}/index.js`);
    }
  }
