...
export class ClusterExecutor implements Executor {
  ...
  async execute(analyzeFn: AnalyzeEntryPointsFn, createCompileFn: CreateCompileFn): Promise<void> {
    if (cluster.isMaster) {
      this.logger.debug(
      ...
      const master =
          new ClusterMaster(this.workerCount, this.logger, this.pkgJsonUpdater, analyzeFn);
      return master.run();
    } else {
