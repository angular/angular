...
export class ModuleWithProvidersAnalyzer {
      ...
      fns &&
        fns.forEach((fn) => {
          ...
          if (this.processDts) {
            ...
            const typeParam =
              (dtsFn.type &&
                ts.isTypeReferenceNode(dtsFn.type) &&
                dtsFn.type.typeArguments &&
                dtsFn.type.typeArguments[0]) ||
              null;
  ...
  private getDtsDeclarationForFunction(fn: ModuleWithProvidersFunction) {
    ...
    if (containerClass) {
      ...
      dtsFn =
        dtsClass && ts.isClassDeclaration(dtsClass)
...
): declaration is ts.FunctionDeclaration | ts.MethodDeclaration {
