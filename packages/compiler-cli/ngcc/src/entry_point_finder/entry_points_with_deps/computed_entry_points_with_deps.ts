...
export class ComputedEntryPointsWithDeps implements EntryPointsWithDeps {
  constructor(
   ...
   *
   * @param entryPointPath the path to the entry-point whose information and dependencies are to
   *     be retrieved or computed.
   *
  ...
  private computePackagePath(entryPointPath: AbsoluteFsPath): AbsoluteFsPath {
    ...
    if (entryPointPath.startsWith(this.sourceDirectory)) {
      const packagePath =
          this.computePackagePathFromContainingPath(entryPointPath, this.sourceDirectory);
      if (packagePath !== null) {
        return packagePath;
      }
    }
  ...
  /**
   * Search down to the `entryPointPath` from the `containingPath` for the first `package.json`
   * that we come to. This is the path to the entry-point's containing package. For example if
   ...
   *
   * To account for nested `node_modules` we actually start the search at the last `node_modules`
   * in the `entryPointPath` that is below the `containingPath`. E.g. if `containingPath` is
   * `/a/b/c` and `entryPointPath` is `/a/b/c/d/node_modules/x/y/z`, we start the search at
