...
export class DtsRenderer {
  constructor(
    private dtsFormatter: RenderingFormatter,
  ...
  renderProgram(
    ...
    if (this.bundle.dts) {
      ...
      dtsFiles.forEach((renderInfo, file) =>
        renderedFiles.push(...this.renderDtsFile(file, renderInfo))
      );
    }
  ...
  renderDtsFile(dtsFile: ts.SourceFile, renderInfo: DtsRenderInfo): FileToWrite[] {
    ...
    this.dtsFormatter.addModuleWithProvidersParams(
      outputText,
      renderInfo.moduleWithProviders,
      importManager
    );
    this.dtsFormatter.addExports(
      outputText,
      dtsFile.fileName,
      renderInfo.privateExports,
      importManager,
      dtsFile
    );
    this.dtsFormatter.addImports(
      outputText,
      importManager.getAllImports(dtsFile.fileName),
      dtsFile
    );
  ...
  private getTypingsFilesToRender(
    ...
    privateDeclarationsAnalyses: PrivateDeclarationsAnalyses,
    moduleWithProvidersAnalyses: ModuleWithProvidersAnalyses | null
  ): Map<ts.SourceFile, DtsRenderInfo> {
    ...
    if (privateDeclarationsAnalyses.length) {
      ...
      const renderInfo = dtsMap.has(dtsEntryPoint)
        ? dtsMap.get(dtsEntryPoint)!
        : new DtsRenderInfo();
