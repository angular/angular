...
runInEachFileSystem(() => {
  describe('AsyncLocker', () => {
    describe('lock()', () => {
      ...
      it('should guard the `fn()` with calls to `write()` and `remove()`, even if it throws', async () => {
        let error: string = '';
        const fs = getFileSystem();
        const log: string[] = [];
        const lockFile = new MockLockFile(fs, log);
      ...
      it('should error if another process does not release the lock-file before this times out', async () => {
        const fs = getFileSystem();
        const log: string[] = [];
        const lockFile = new MockLockFile(fs, log);
        const logger = new MockLogger();
