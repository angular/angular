...
runInEachFileSystem(() => {
  describe('entry point bundle', () => {
    ...
    it('should resolve JavaScript sources instead of declaration files if they are adjacent', () => {
      ...
      expect(esm5bundle.src.program.getSourceFiles().map((sf) => sf.fileName)).toEqual(
      ...
      expect(esm5bundle.dts!.program.getSourceFiles().map((sf) => sf.fileName)).toEqual(
        jasmine.arrayWithExactContents(
          [
    ...
    describe('including equivalently named, internally imported, src files in the typings program', () => {
      it('does include internal .d.ts files if `mirrorDtsFromSrc` is true', () => {
        ...
        const esm5bundle = makeEntryPointBundle(
          ...
          /* mirrorDtsFromSrc */ true
        );
        expect(esm5bundle.src.program.getSourceFiles().map((sf) => sf.fileName)).toContain(
          absoluteFrom('/node_modules/test/internal.js')
        );
        expect(esm5bundle.dts!.program.getSourceFiles().map((sf) => sf.fileName)).toContain(
          absoluteFrom('/node_modules/test/internal.d.ts')
        );
      });
      ...
      it('should work when the .d.ts files are in a different tree than the sources', () => {
        ...
        expect(esm5bundle.src.program.getSourceFiles().map((sf) => sf.fileName)).toContain(
          absoluteFrom('/node_modules/internal/esm2015/src/internal.js')
        );
        expect(esm5bundle.dts!.program.getSourceFiles().map((sf) => sf.fileName)).toContain(
          absoluteFrom('/node_modules/internal/src/internal.d.ts')
        );
      });
      ...
      it('ignores internal .d.ts files if `mirrorDtsFromSrc` is false', () => {
        ...
        expect(esm5bundle.dts!.program.getSourceFiles().map((sf) => sf.fileName)).not.toContain(
          absoluteFrom('/node_modules/test/internal.d.ts')
        );
      });
    });
