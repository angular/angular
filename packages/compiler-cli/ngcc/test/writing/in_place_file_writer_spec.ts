...
runInEachFileSystem(() => {
  describe('InPlaceFileWriter', () => {
    ...
    it('should create backups of all files that previously existed', () => {
      ...
      expect(fs.readFile(_('/package/path/top-level.js.__ivy_ngcc_bak'))).toEqual(
        'ORIGINAL TOP LEVEL'
      );
      expect(fs.readFile(_('/package/path/folder-1/file-1.js.__ivy_ngcc_bak'))).toEqual(
        'ORIGINAL FILE 1'
      );
      ...
      expect(fs.readFile(_('/package/path/folder-2/file-4.js.__ivy_ngcc_bak'))).toEqual(
        'ORIGINAL FILE 4'
      );
