...
export function makeTestEntryPoint(
  ...
  packageName: string = entryPointName,
  config?: TestConfig
): EntryPoint {
  return {
    name: entryPointName,
...
export function makeTestEntryPointBundle(
  ...
  isCore: boolean,
  srcRootNames: AbsoluteFsPath[],
  dtsRootNames?: AbsoluteFsPath[],
  config?: TestConfig,
  ...
  const dts = dtsRootNames
    ? makeTestDtsBundleProgram(dtsRootNames[0], entryPoint.package, isCore)
    : null;
...
export function makeTestBundleProgram(
  ...
  additionalFiles?: AbsoluteFsPath[]
): BundleProgram {
