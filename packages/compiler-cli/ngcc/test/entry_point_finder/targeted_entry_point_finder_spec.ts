    ...
    describe('findEntryPoints()', () => {
      it('should find a single entry-point with no dependencies', () => {
        ...
        const finder = new TargetedEntryPointFinder(
          ...
          logger,
          resolver,
          _Abs('/sub_entry_points/node_modules'),
          targetPath,
          undefined
        );
        const {entryPoints} = finder.findEntryPoints();
        expect(dumpEntryPointPaths(basePath, entryPoints)).toEqual([['common', 'common']]);
      });
      ...
      it('should handle external node_modules folders (e.g. in a yarn workspace) for dependencies', () => {
        ...
        const lib2 = createPackage(_Abs('/project/node_modules/lib1/node_modules'), 'lib2', [
          'lib3/entry-point',
        ]);
      ...
      it('should handle external node_modules folders (e.g. in a yarn workspace) for scoped dependencies', () => {
        ...
        const lib3EntryPoint = createPackage(
          _Abs('/project/node_modules/@scope/lib3'),
          'entry-point',
          ['lib4/entry-point']
        );
        const lib4 = createPackage(_Abs('/project/node_modules/@scope/lib3/node_modules'), 'lib4');
        const lib4EntryPoint = createPackage(
          _Abs('/project/node_modules/@scope/lib3/node_modules/lib4'),
          'entry-point'
        );
      ...
      it('should handle pathMappings that map to files or non-existent directories', () => {
        ...
        const {entryPoints} = finder.findEntryPoints();
        expect(dumpEntryPointPaths(basePath, entryPoints)).toEqual([['test', 'test']]);
      });
    ...
    describe('targetNeedsProcessingOrCleaning()', () => {
      ...
      it('should false if the target path has no typings', () => {
        ...
        loadTestFiles([
          ...
          {
            name: _Abs(
              '/no_valid_entry_points/node_modules/some_package/some_package.metadata.json'
            ),
      ...
      describe('[compileAllFormats: false]', () => {
        ...
        it('should return false if the first of the properties to consider (that actually appear in the package.json) has been processed', () => {
          ...
          expect(finder.targetNeedsProcessingOrCleaning(['fesm2015', 'esm5'], false)).toBe(false);
        });
      });
    });
