  ...
  `,
    ...
    describe('getDecoratorsOfDeclaration()', () => {
      ...
      it('should ignore decorator elements that are not object literals', () => {
        ...
        expect(decorators[0]).toEqual(
          jasmine.objectContaining<Decorator>({name: 'Directive'})
        );
      });
    ...
    describe('getMembersOfClass()', () => {
      ...
      it('should find Object.defineProperty members on a class', () => {
        ...
        expect(
          (setter.implementation as ts.FunctionExpression).body.statements[0].getText()
        ...
        expect(
          (getter.implementation as ts.FunctionExpression).body.statements[0].getText()
        ).toEqual('return null;');

        const [combinedSetter, combinedGetter] = members.filter(
          (member) => member.name === 'setterAndGetter'
        );
      ...
      it('should ignore the prototype pseudo-static property on class imported from typings files', () => {
        ...
        const classNode = getDeclaration(
          bundle.program,
          UNWANTED_PROTOTYPE_EXPORT_FILE.name,
          'SomeParam',
          isNamedClassDeclaration
        );
        const members = host.getMembersOfClass(classNode);
    ...
    describe('getDeclarationOfIdentifier()', () => {
      ...
      const createTestForTsHelper = (
        ...
        const node = getDeclaration(program, srcFile.name, varName, ts.isVariableDeclaration);
      ...
      it('should return the declaration of an externally defined identifier', () => {
        ...
        const identifierOfDirective = ((classDecorators[0].node as ts.ObjectLiteralExpression)
          .properties[0] as ts.PropertyAssignment).initializer as ts.Identifier;
      ...
      it('should return the correct declaration for an inner function identifier inside an ES5 IIFE', () => {
        const superGetDeclarationOfIdentifierSpy = spyOn(
          Esm2015ReflectionHost.prototype,
        ...
        const innerDeclaration = (((outerDeclaration.initializer as ts.ParenthesizedExpression)
          .expression as ts.CallExpression).expression as ts.FunctionExpression).body
      ...
      it('should return the correct outer declaration for an aliased inner class declaration inside an ES5 IIFE', () => {
        ...
        const forRootMethod = (((expectedDeclaration.initializer as ts.ParenthesizedExpression)
          .expression as ts.CallExpression).expression as ts.FunctionExpression).body
          .statements[2] as ts.ExpressionStatement;
        const identifier = (((((forRootMethod.expression as ts.BinaryExpression)
          .right as ts.FunctionExpression).body.statements[0] as ts.ReturnStatement)
          .expression as ts.ObjectLiteralExpression).properties[0] as ts.PropertyAssignment)
      ...
      it('should recognize TypeScript helpers (as function declarations)', () => {
        ...
        const testForHelper = createTestForTsHelper(bundle.program, host, file, (helperName) =>
      ...
      it('should recognize imported TypeScript helpers (named imports)', () => {
        ...
        const testForHelper = createTestForTsHelper(bundle.program, host, testFile, (helperName) =>
          getDeclaration(bundle.program, tslibFile.name, helperName, ts.isFunctionDeclaration)
        );
    ...
    describe('getExportsOfModule()', () => {
      it('should return a map of all the exports from a given module', () => {
        ...
        const values = Array.from(exportDeclarations!.values()).map((declaration) => [
          declaration.node!.getText(),
          declaration.viaModule,
        ]);
    ...
    describe('getDtsDeclaration()', () => {
      it('should find the dts declaration that has the same relative path to the source file', () => {
        ...
        const host = createHost(
          bundle,
          new Esm5ReflectionHost(new MockLogger(), false, bundle, dts)
        );
    ...
    describe('getModuleWithProvidersFunctions', () => {
      ...
      it('should find every static method on exported classes that return an object that looks like a ModuleWithProviders object', () => {
        ...
        expect(fn.map((fn) => [fn.declaration.getText(), fn.ngModule.node.name.text])).toEqual([
          ...
          ['function() { return { ngModule: ExternalModule }; }', 'ExternalModule'],
    ...
    describe('getEndOfClass()', () => {
      it('should return the last static property of the class', () => {
        ...
        const classSymbol = host.findClassSymbols(
          bundle.program.getSourceFile(SOME_DIRECTIVE_FILE.name)!
        )[0];
