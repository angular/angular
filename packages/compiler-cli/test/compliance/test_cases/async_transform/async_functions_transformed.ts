...
export const fnExpression = async function(a: number, b: Promise<string>): Promise<number|string> {
  const x = await a;
  if (x > 10) {
    return await 200;
  }
...
export class Test {
  ...
  protected methodExpression =
      async function(a: number, b: Promise<string>): Promise<number|string> {
    const x = await a;
    if (x > 10) {
      return await 200;
    }
