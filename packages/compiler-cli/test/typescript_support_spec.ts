...
describe('checkVersion', () => {
  ...
  it('should handle a TypeScript version < the minimum supported one', () => {
    expect(() => checkVersion('2.4.1', MIN_TS_VERSION, MAX_TS_VERSION)).toThrowError(
      versionError('2.4.1'),
    );
    expect(() => checkVersion('2.7.1', MIN_TS_VERSION, MAX_TS_VERSION)).toThrowError(
      versionError('2.7.1'),
    );
  });

  it('should handle a TypeScript version > the maximum supported one', () => {
    expect(() => checkVersion('2.9.0', MIN_TS_VERSION, MAX_TS_VERSION)).toThrowError(
      versionError('2.9.0'),
    );
    expect(() => checkVersion('2.8.0', MIN_TS_VERSION, MAX_TS_VERSION)).toThrowError(
      versionError('2.8.0'),
    );
  });

  it('should throw when an out-of-bounds pre-release version is used', () => {
    expect(() => checkVersion('2.7.0-beta', MIN_TS_VERSION, MAX_TS_VERSION)).toThrowError(
      versionError('2.7.0-beta'),
    );
    expect(() => checkVersion('2.8.5-rc.3', MIN_TS_VERSION, MAX_TS_VERSION)).toThrowError(
      versionError('2.8.5-rc.3'),
    );
  });
