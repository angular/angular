...
describe('ng program', () => {
  ...
  function compile(
    ...
    rootNames?: string[],
    host?: CompilerHost
  ): {program: ng.Program; emitResult: ts.EmitResult} {
  ...
  describe('reuse of old program', () => {
    it('should reuse generated code for libraries from old programs', () => {
      ...
      expect(
        p1
          .getTsProgram()
          .getSourceFiles()
          .some((sf) => /node_modules\/lib\/.*\.ngfactory\.ts$/.test(sf.fileName))
      ).toBe(true);
      expect(
        p1
          .getTsProgram()
          .getSourceFiles()
          .some((sf) => /node_modules\/lib2\/.*\.ngfactory.*$/.test(sf.fileName))
      ).toBe(false);
      ...
      expect(
        p2
          .getTsProgram()
          .getSourceFiles()
          .some((sf) => /node_modules\/lib\/.*\.ngfactory.*$/.test(sf.fileName))
      ).toBe(false);
      expect(
        p2
          .getTsProgram()
      ...
      expect(
        p3
          .getTsProgram()
          .getSourceFiles()
          .some((sf) => /node_modules\/lib2\/.*\.ngfactory\.ts$/.test(sf.fileName))
      ).toBe(true);
    ...
    it('should only emit changed files', () => {
      ...
      let ngFactoryContent = written.get(
        path.posix.join(testSupport.basePath, 'built/src/index.ngfactory.js')
      );
    ...
    it('should store library summaries on emit', () => {
      ...
      const p1 = compile().program;
      expect(
        Array.from(p1.getLibrarySummaries().values()).some((sf) =>
          /node_modules\/lib\/index\.ngfactory\.d\.ts$/.test(sf.fileName)
        )
      ).toBe(true);
      expect(
        Array.from(p1.getLibrarySummaries().values()).some((sf) =>
          /node_modules\/lib\/index\.ngsummary\.json$/.test(sf.fileName)
        )
      ).toBe(true);
      expect(
        Array.from(p1.getLibrarySummaries().values()).some((sf) =>
          /node_modules\/lib\/index\.d\.ts$/.test(sf.fileName)
        )
      ).toBe(true);

      expect(
        Array.from(p1.getLibrarySummaries().values()).some((sf) => /src\/main.*$/.test(sf.fileName))
      ).toBe(false);
    });

    describe('verify that program structure is reused within tsc in order to speed up incremental compilation', () => {
  ...
  it('should not typecheck templates if skipTemplateCodegen is set but fullTemplateTypeCheck is not', () => {
    ...
    const host = ng.createCompilerHost({options});
    const program = ng.createProgram({
      rootNames: [path.resolve(testSupport.basePath, 'src/main.ts')],
      options,
      host,
    });
    expectNoDiagnosticsInProgram(options, program);
  ...
  it('should emit also empty generated files depending on the options', () => {
    ...
    function assertGenFile(
      fileName: string,
      checks: {originalFileName: string; shouldBe: ShouldBe}
    ) {
    ...
    assertGenFile('built/src/util.ngfactory.js', {
      originalFileName: 'src/util.ts',
      shouldBe: ShouldBe.Empty,
    });
    assertGenFile('built/src/util.ngfactory.d.ts', {
      originalFileName: 'src/util.ts',
      shouldBe: ShouldBe.EmptyExport,
    });
    assertGenFile('built/src/util.ngsummary.js', {
      originalFileName: 'src/util.ts',
      shouldBe: ShouldBe.Empty,
    });
    assertGenFile('built/src/util.ngsummary.d.ts', {
      originalFileName: 'src/util.ts',
      shouldBe: ShouldBe.EmptyExport,
    });
    assertGenFile('built/src/util.ngsummary.json', {
      originalFileName: 'src/util.ts',
      shouldBe: ShouldBe.NoneEmpty,
    });

    // Note: we always fill non shim and shim style files as they might
    // be shared by component with and without ViewEncapsulation.
    assertGenFile('built/src/main.css.ngstyle.js', {
      originalFileName: 'src/main.ts',
      shouldBe: ShouldBe.NoneEmpty,
    });
    assertGenFile('built/src/main.css.ngstyle.d.ts', {
      originalFileName: 'src/main.ts',
      shouldBe: ShouldBe.EmptyExport,
    });
    // Note: this file is not empty as we actually generated code for it
    assertGenFile('built/src/main.css.shim.ngstyle.js', {
      originalFileName: 'src/main.ts',
      shouldBe: ShouldBe.NoneEmpty,
    });
    assertGenFile('built/src/main.css.shim.ngstyle.d.ts', {
      originalFileName: 'src/main.ts',
      shouldBe: ShouldBe.EmptyExport,
    });
  });
  ...
  it('should not emit /// references in .d.ts files', () => {
    ...
    const dts = fs
      .readFileSync(path.resolve(testSupport.basePath, 'built', 'src', 'main.d.ts'))
      .toString();
  ...
  describe('listLazyRoutes', () => {
    ...
    function createProgram(rootNames: string[], overrideOptions: ng.CompilerOptions = {}) {
      ...
      const host = ng.createCompilerHost({options});
      const program = ng.createProgram({
        rootNames: rootNames.map((p) => path.resolve(testSupport.basePath, p)),
        options,
        host,
      });
      ...
      expect(normalizeRoutes(program.listLazyRoutes())).toEqual([
        {
          module: {
            name: 'MainModule',
            filePath: path.posix.join(testSupport.basePath, 'src/main.ts'),
          },
          referencedModule: {
            name: 'ChildModule',
  ...
  it('should report errors for ts and ng errors on emit with noEmitOnError=true', () => {
    ...
    const errorDiags = program1
      .emit()
      .diagnostics.filter((d) => d.category === ts.DiagnosticCategory.Error);
    expect(stripAnsi(formatDiagnostics(errorDiags))).toContain(
      `src/main.ts:5:13 - error TS2322: Type '1' is not assignable to type 'string'.`
    );
    expect(stripAnsi(formatDiagnostics(errorDiags))).toContain(
  ...
  describe('errors', () => {
    ...
    it(
      'should be able report structural errors with noResolve:true and generateCodeForLibraries:false ' +
