...
3. To allow any property add 'NO_ERRORS_SCHEMA' to the '@Component.schemas' of this component.`);
    ...
    describe('schema checking with custom elements manifests', () => {
      ...
      it('should preserve exact names in translated property interpolation', () => {
        const translatedManifest = structuredClone(manifest);
        translatedManifest.modules[0].declarations[0].members.push({
          kind: 'field',
          name: 'for',
          type: {text: 'string'},
        });
        env.write('custom-elements.json', JSON.stringify(translatedManifest));
        env.write(
          'test.ts',
          `
          import {Component} from '@angular/core';
          @Component({
            template: '<my-button for="{{target}}" i18n-for tabindex="{{index}}" i18n-tabindex></my-button><label for="{{target}}" i18n-for></label>',
          })
          export class TranslatedProperties { target = 'input'; index = 0; }
        `,
        );
        ...
        expect(output).toMatch(/ɵɵi18nAttributes\(1, \d+, \["for"\]\)/);
        expect(output).toMatch(/ɵɵi18nAttributes\(3, \d+\)/);
      });
      ...
      it('should resolve manifests through module specifiers', () => {
        ...
        // An unimportable class produces a warning and uses HTMLElement for the instance type.
        env.write(
      ...
      it('should summarize repeated warnings by default and expand them with customElementsManifestsDiagnostics', () => {
        ...
        // The default mode groups warnings by manifest and kind.
      ...
      it('should warn about and skip declarations with native tag names', () => {
        ...
        const diags = env.driveDiagnostics();
        expect(diags.length).toBe(2);
        // Skip the invalid declaration with a warning.
        expect(diags[0].code).toBe(
          ngErrorCode(ErrorCode.CONFIG_CUSTOM_ELEMENTS_MANIFEST_INVALID_TAG_NAME),
        );
        expect(diags[0].category).toBe(ts.DiagnosticCategory.Warning);
        expect(diags[0].messageText).toContain(`'marquee'`);
        // The native element must not gain the declared property.
      ...
      it('should warn about duplicate tags across manifests and use the first declaration', () => {
        ...
        expect(diags[0].category).toBe(ts.DiagnosticCategory.Warning);
        // The first manifest defines label. The later declaration of other is ignored.
        expect(diags[1].code).toBe(ngErrorCode(ErrorCode.SCHEMA_INVALID_ATTRIBUTE));
        expect(diags[1].messageText).toContain(
      ...
      describe('binding value type checking', () => {
        ...
        for (const initialValue of ['1', "'wrong'"]) {
          it(`should check the value of a two-way bound signal initialized to ${initialValue}`, () => {
            const module = typedManifest.modules[0];
            env.write(
              'custom-elements.json',
              JSON.stringify({
                ...typedManifest,
                    ...
                    declarations: [
                      {
                        ...
                        events: [{name: 'countChange', type: {text: 'any'}}],
                      },
                    ],
                  },
                ],
            ...
            env.write(
              ...
              @Component({template: '<my-button [(count)]="value"></my-button>'})
              export class SignalBinding { value = signal(${initialValue}); }
            `,
            );
        ...
        for (const parameters of ['<T>', '<T = string>']) {
          it(`should handle custom element instance type parameters ${parameters} in strict templates`, () => {
            env.write(
              'node_modules/@my/elements/package.json',
              JSON.stringify({
                name: '@my/elements',
            ...
            env.write(
              ...
              `export {GenericElement} from './generic';`,
            );
            env.write(
              'node_modules/@my/elements/generic.d.ts',
              `export declare class GenericElement${parameters} extends HTMLElement { count: number; }`,
            );
            env.write(
              'node_modules/@my/elements/custom-elements.json',
              JSON.stringify({
                    ...
                    exports: [
                      {
                        ...
                        declaration: {name: 'GenericElement'},
                      },
                    ],
                  },
                ],
            ...
            env.tsconfig({strictTemplates: true, customElementsManifests: ['@my/elements']});
            writeComponent(
              `<generic-element #element [count]="1">{{element.title}}</generic-element>`,
            );
            const diagnostics = env.driveDiagnostics();
            if (parameters === '<T>') {
              expect(diagnostics.length).toBe(1);
              expect(diagnostics[0].category).toBe(ts.DiagnosticCategory.Warning);
              expect(diagnostics[0].messageText).toContain('require type arguments');
            } else {
              expect(diagnostics).toEqual([]);
            }
            writeComponent(
              `<generic-element #element [count]="\\'invalid\\'">{{element.count}}</generic-element>`,
            );
            const invalid = env.driveDiagnostics();
            expect(invalid.some((diagnostic) => diagnostic.code === 2314)).toBe(false);
            expect(
              invalid.some((diagnostic) =>
                String(diagnostic.messageText).includes("not assignable to type 'number'"),
              ),
            ).toBe(true);
            expect(
              invalid.some((diagnostic) =>
                String(diagnostic.messageText).includes(
                  "'count' does not exist on type 'HTMLElement'",
                ),
              ),
            ).toBe(parameters === '<T>');
          });
        ...
        it('should warn and type local references as HTMLElement when the declared class is not importable', () => {
          ...
          // Use HTMLElement for the reference and report the unresolved module as a configuration warning.
        ...
        it('should warn and fall back to existence checking for references to packages without types', () => {
          ...
          // Report the unresolved reference as a configuration warning without template errors.
          const diags = env.driveDiagnostics();
          expect(diags.length).toBe(1);
          expect(diags[0].code).toBe(
            ngErrorCode(ErrorCode.CONFIG_CUSTOM_ELEMENTS_MANIFEST_UNRESOLVABLE_TYPE_REFERENCE),
        ...
        it('should recognize definition-only custom-element tags without suppressing member checks', () => {
          ...
          // Recognize the tag and continue reporting unknown property bindings.
        ...
        it('should warn and fall back to event inference for references to packages without types', () => {
          ...
          // Report the unresolved reference as a configuration warning and infer $event from the DOM.
          const diags = env.driveDiagnostics();
          expect(diags.length).toBe(1);
          expect(diags[0].code).toBe(
            ngErrorCode(ErrorCode.CONFIG_CUSTOM_ELEMENTS_MANIFEST_UNRESOLVABLE_TYPE_REFERENCE),
