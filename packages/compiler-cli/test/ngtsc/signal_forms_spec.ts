  ...
  describe('signal forms', () => {
    ...
    it('should check that the field is present', () => {
      ...
      expect(extractMessage(diags[0])).toBe(
        `Type 'null' is not assignable to type '(() => CompatFieldState<any, string | number>) & object'.`,
      );
    });
    ...
    it('should check that the field type is correct', () => {
      ...
      expect(diags.length).toBe(1);
      expect(extractMessage(diags[0])).toBe(
        `Type 'string' is not assignable to type '(() => CompatFieldState<any, string | number>) & object'.`,
      );
    });
