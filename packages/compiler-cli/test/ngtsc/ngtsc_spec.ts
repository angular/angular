...
function allTests(os: string) {
  describe('ngtsc behavioral tests', () => {
    ...
    `);
      ...
      const dtsContents = env.getContents('test.d.ts');
      const expectedDirectiveDeclaration =
          `static ɵdir: i0.ɵɵDirectiveDeclaration<TestBase, never, never, { "input": { "publicName": "input"; "required": false; }; }, {}, never, never, false, never>;`;
      expect(dtsContents).toContain(expectedDirectiveDeclaration);
    });
  ...
  `);
    ...
    it('should generate the correct declaration for class members decorated with @Input', () => {
      env.write('test.ts', `
        ...
        export class TestDir {
          ...
          @Input({publicName: 'aliasedRequiredWithAlias', required: true}) requiredWithAlias: any;
        }
      `);
      ...
      expect(dtsContents)
          .toContain(
              ...
              '{ "noArgs": { "publicName": "noArgs"; "required": false; }; "stringArg": ' +
              '{ "publicName": "aliasedStringArg"; "required": false; }; "requiredNoAlias": ' +
              '{ "publicName": "requiredNoAlias"; "required": true; }; "requiredWithAlias": ' +
              '{ "publicName": "aliasedRequiredWithAlias"; "required": true; }; }, {}, never, never, false, never>;');
    });
    ...
    it('should generate the correct declaration for directives using the `inputs` array', () => {
      env.write('test.ts', `
        ...
        @Directive({
          ...
          inputs: [
            ...
            {name: 'plainLiteral'},
            {name: 'aliasedLiteral', publicName: 'alisedLiteralAlias'},
            {name: 'requiredLiteral', required: true},
            {name: 'requiredAlisedLiteral', publicName: 'requiredAlisedLiteralAlias', required: true}
          ]
        })
        export class TestDir {
          plainLiteral: any;
      ...
      expect(dtsContents)
          .toContain(
              'static ɵdir: i0.ɵɵDirectiveDeclaration<TestDir, "[dir]", never, ' +
              '{ "plain": { "publicName": "plain"; "required": false; }; ' +
              '"withAlias": { "publicName": "aliasedWithAlias"; "required": false; }; ' +
              '"plainLiteral": { "publicName": "plainLiteral"; "required": false; }; ' +
              '"aliasedLiteral": { "publicName": "alisedLiteralAlias"; "required": false; }; ' +
              '"requiredLiteral": { "publicName": "requiredLiteral"; "required": true; }; ' +
              '"requiredAlisedLiteral": { "publicName": "requiredAlisedLiteralAlias"; "required": true; }; }, {}, never, never, false, never>;');
    });
