...
function i18nMigrationMappingTest() {
  ...
  return __NGTOOLS_PRIVATE_API_2.migrateI18n(options)
      .then((files: string[]) => {
        ...
        const mappingFile = 'mapping_v1_to_v0.json';
        const allFiles = glob.sync(path.join(srcPath, '**/ /*'), {nodir: true});
        ...
        assert.deepEqual(
            ...
            JSON.parse(fs.readFileSync(path.join(srcPath, mappingFile), 'utf8')),
            `Expected correct mapping file to have been generated`);
        console.log(`done, no errors.`);
      })
      .catch(onRejected('Migration failed'));
}
