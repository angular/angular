...
export class TypeScriptNodeEmitter {
  ...
  createCommentStatement(sourceFile: ts.SourceFile, comment: string): ts.Statement {
    ...
    ts.setSyntheticLeadingComments(commentStmt, [
...
export function updateSourceFile(
  ...
  const prefixStatements = module.statements.filter(
    (statement) => !(statement instanceof ClassStmt)
  );
  ...
  const prefix: ts.Statement[] = prefixStatements.map((statement) =>
    statement.visitStatement(converter, sourceFile)
  );
    ...
    if (node.kind == ts.SyntaxKind.ClassDeclaration) {
      ...
      if (name) {
        ...
        if (classStatement) {
          ...
          const classMemberHolder = converter.visitDeclareClassStmt(
            classStatement
          ) as ts.ClassDeclaration;
          const newMethods = classMemberHolder.members.filter(
  ...
  classNames.size == 0 ||
    error(
      `${classNames.size == 1 ? 'Class' : 'Classes'} "${Array.from(classNames.keys()).join(
        ', '
      )}" not generated`
    );
  ...
  if (imports && imports.length) {
    ...
    newStatements = [
      ...newStatements.slice(0, index),
      ...imports,
      ...prefix,
      ...newStatements.slice(index),
    ];
  } else {
...
// NodeEmitterVisitor
export type RecordedNode<T extends ts.Node = ts.Node> =
  | (T & {
      __recorded: any;
    })
  | null;
...
export class NodeEmitterVisitor implements StatementVisitor, ExpressionVisitor {
  ...
  getReexports(): ts.Statement[] {
    return Array.from(this._reexports.entries()).map(([exportedFilePath, reexports]) =>
      ts.createExportDeclaration(
        ...
        /* modifiers */ undefined,
        ts.createNamedExports(reexports.map(({name, as}) => ts.createExportSpecifier(name, as))),
    ...
    const visitNode = (tsNode: ts.Node) => {
      ...
      if (ngNode) {
        ...
        if (range) {
          if (
            !lastRange ||
            range.source != lastRange.source ||
            range.pos != lastRange.pos ||
  ...
  visitDeclareVarStmt(stmt: DeclareVarStmt) {
    if (
      stmt.hasModifier(StmtModifier.Exported) &&
      stmt.value instanceof ExternalExpr &&
      !stmt.type
    ) {
    ...
    const varDeclList = ts.createVariableDeclarationList([
    ...
    if (stmt.hasModifier(StmtModifier.Exported)) {
      ...
      const tsVarStmt = this.record(
        stmt,
        ts.createVariableStatement(/* modifiers */ [], varDeclList)
      );
  ...
  visitDeclareFunctionStmt(stmt: DeclareFunctionStmt) {
    return this.record(
      ...
      ts.createFunctionDeclaration(
        ...
        /* type */ undefined,
        this._visitStatements(stmt.statements)
      )
    );
  }
  ...
  visitReturnStmt(stmt: ReturnStatement) {
    return this.record(
      stmt,
      ts.createReturn(stmt.value ? stmt.value.visitExpression(this, null) : undefined)
    );
  }
  ...
  visitDeclareClassStmt(stmt: ClassStmt) {
    ...
    const fields = stmt.fields.map((field) => {
      const property = ts.createProperty(
        /* decorators */ undefined,
        /* modifiers */ translateModifiers(field.modifiers),
        field.name,
        /* questionToken */ undefined,
        /* type */ undefined,
        field.initializer == null ? ts.createNull() : field.initializer.visitExpression(this, null)
      );
    ...
    const getters = stmt.getters.map((getter) =>
      ts.createGetAccessor(
        ...
        /* parameters */ [],
        /* type */ undefined,
        this._visitStatements(getter.body)
      )
    );
    ...
    const constructor =
      (stmt.constructorMethod && [
        ts.createConstructor(
          ...
          /* modifiers */ undefined,
          /* parameters */
          stmt.constructorMethod.params.map((p) =>
            ts.createParameter(
              /* decorators */ undefined,
              /* modifiers */ undefined,
              /* dotDotDotToken */ undefined,
    ...
    const methods = stmt.methods
      ...
      .map((method) =>
        ts.createMethod(
          ...
          /* astriskToken */ undefined,
          method.name! /* guarded by filter */,
          /* questionToken */ undefined,
          /* typeParameters */ undefined,
          method.params.map((p) =>
            ts.createParameter(
          ...
          /* type */ undefined,
          this._visitStatements(method.body)
        )
      );
    return this.record(
      stmt,
  ...
  visitIfStmt(stmt: IfStmt) {
    return this.record(
      ...
      ts.createIf(
        ...
        this._visitStatements(stmt.trueCase),
  ...
  visitWriteVarExpr(expr: WriteVarExpr): RecordedNode<ts.BinaryExpression> {
    return this.record(
      expr,
      ts.createAssignment(ts.createIdentifier(expr.name), expr.value.visitExpression(this, null))
    );
  }
    ...
    return this.record(
      ...
      ts.createAssignment(
        ts.createElementAccess(
          expr.receiver.visitExpression(this, null),
  ...
  visitInstantiateExpr(expr: InstantiateExpr): RecordedNode<ts.NewExpression> {
    return this.record(
      expr,
      ts.createNew(
        expr.classExpr.visitExpression(this, null),
        /* typeArguments */ undefined,
        expr.args.map((arg) => arg.visitExpression(this, null))
      )
    );
    ...
    return this.record(
      ...
      ts.createParen(
        ts.createConditional(
          expr.condition.visitExpression(this, null),
  ...
  visitNotExpr(expr: NotExpr): RecordedNode<ts.PrefixUnaryExpression> {
    return this.record(
      expr,
      ts.createPrefix(ts.SyntaxKind.ExclamationToken, expr.condition.visitExpression(this, null))
    );
  }
  ...
  visitFunctionExpr(expr: FunctionExpr) {
    return this.record(
      ...
      ts.createFunctionExpression(
        ...
        /* type */ undefined,
        this._visitStatements(expr.statements)
      )
    );
  }
  ...
  visitBinaryOperatorExpr(
    ...
    const binary = ts.createBinary(
      expr.lhs.visitExpression(this, null),
      binaryOperator,
  ...
  visitLiteralArrayExpr(expr: LiteralArrayExpr): RecordedNode<ts.ArrayLiteralExpression> {
    return this.record(
      expr,
      ts.createArrayLiteral(expr.entries.map((entry) => entry.visitExpression(this, null)))
    );
  }
  ...
  visitCommaExpr(expr: CommaExpr): RecordedNode<ts.Expression> {
    return this.record(
      expr,
      expr.parts
        .map((e) => e.visitExpression(this, null))
        .reduce<ts.Expression | null>(
          (left, right) => (left ? ts.createBinary(left, ts.SyntaxKind.CommaToken, right) : right),
          null
        )
    );
  }
