...
function visitClassDeclaration(
  ...
  const members = clazz.members.map((member) => {
    ...
    if (
      host.getDecoratorsOfDeclaration(member)?.some((d) => isAngularDecorator(d, 'Input', isCore))
    ) {
      return member;
    }
    ...
    const newDecorator = ctx.factory.createDecorator(
      ctx.factory.createCallExpression(
        ctx.factory.createPropertyAccessExpression(
          ...
          ts.setOriginalNode(ctx.factory.createIdentifier('Input'), classDecoratorIdentifier),
        ),
        undefined,
        [
          ctx.factory.createAsExpression(
            ctx.factory.createObjectLiteralExpression(
              Object.entries(fields).map(([name, value]) =>
                ctx.factory.createPropertyAssignment(name, value),
              ),
            ),
