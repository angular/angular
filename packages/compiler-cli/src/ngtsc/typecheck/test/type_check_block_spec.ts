...
describe('type check blocks', () => {
  ...
  it('should handle attribute values for directive inputs', () => {
    ...
    expect(tcb(TEMPLATE, DIRECTIVES))
        .toContain('_t1: i0.DirA = null! as i0.DirA; _t1.inputA = ("value");');
  });
...
`;
    ...
    it('should use coercion types if declared', () => {
      ...
      expect(tcb(TEMPLATE, DIRECTIVES))
          .toContain(
              'var _t1: typeof i0.Dir.ngAcceptInputType_fieldA = null! as typeof i0.Dir.ngAcceptInputType_fieldA; ' +
              '_t1 = (((this).foo));');
    });
  });
  ...
  it('should only generate directive declarations that have bindings or are referenced', () => {
    ...
    const block = tcb(TEMPLATE, DIRECTIVES);
    expect(block).toContain('var _t1: i0.HasInput = null! as i0.HasInput');
    expect(block).toContain('_t1.input = (((this).value));');
    expect(block).toContain('var _t2: i0.HasOutput = null! as i0.HasOutput');
    expect(block).toContain('_t2["output"]');
    expect(block).toContain('var _t4: i0.HasReference = null! as i0.HasReference');
  ...
  it('should only apply property bindings to directives', () => {
    ...
    const block = tcb(TEMPLATE, DIRECTIVES);
    expect(block).not.toContain('var _t1: Dir = null! as Dir;');
...
`;
    ...
    expect(tcb(TEMPLATE, DIRECTIVES))
        .toContain(
            'var _t2: i0.DirB = null! as i0.DirB; ' +
            'var _t1 = _t2; ' +
            'var _t3: i0.DirA = null! as i0.DirA; ' +
            '_t3.inputA = (_t1); ' +
            'var _t4 = _t3; ' +
            '_t2.inputA = (_t4);');
  });
  ...
  it('should assign restricted properties to temp variables by default', () => {
    ...
    expect(tcb(TEMPLATE, DIRECTIVES))
        .toContain(
            'var _t1: i0.Dir = null! as i0.Dir; ' +
            'var _t2: (typeof _t1)["fieldA"] = null! as (typeof _t1)["fieldA"]; ' +
            '_t2 = (((this).foo)); ');
  });
  ...
  it('should handle a single property bound to multiple fields, where one of them is coerced', () => {
    ...
    expect(tcb(TEMPLATE, DIRECTIVES))
        .toContain(
            'var _t1: typeof i0.Dir.ngAcceptInputType_field1 = null! as typeof i0.Dir.ngAcceptInputType_field1; ' +
            'var _t2: i0.Dir = null! as i0.Dir; ' +
  ...
  it('should use transform type if an input has one', () => {
    ...
    expect(block).toContain(
        'var _t1: boolean | string = null! as boolean | string; ' +
        '_t1 = (((this).expr));');
  });
  ...
  it('should use `any` type for type constructors with bound generic params ' +
    ...
    `;
       ...
       expect(renderedTcb).toContain(`var _t1: i0.Dir<any, any> = null! as i0.Dir<any, any>;`);
  ...
  describe('host directives', () => {
    it('should generate bindings to host directive inputs/outputs', () => {
      ...
      const block = tcb(TEMPLATE, DIRECTIVES);
      expect(block).toContain('var _t1: i0.HostDir = null! as i0.HostDir');
    ...
    it('should generate bindings to an input from a multi-level host directive', () => {
      ...
      const block = tcb(TEMPLATE, DIRECTIVES);
      expect(block).toContain('var _t1: i0.MultiLevelHostDir = null! as i0.MultiLevelHostDir;');
    ...
    it('should generate references to host directives', () => {
      ...
      const block = tcb(TEMPLATE, DIRECTIVES);
      expect(block).toContain('var _t2: i0.HostA = null! as i0.HostA;');
      expect(block).toContain('var _t4: i0.HostB = null! as i0.HostB;');
    ...
    it('should generate bindings to the same input both from the host and host input', () => {
      ...
      const block = tcb(TEMPLATE, DIRECTIVES);
      expect(block).toContain('var _t1: i0.HostDir = null! as i0.HostDir');
      expect(block).toContain('var _t2: i0.DirA = null! as i0.DirA;');
  ...
  describe('conditional blocks', () => {
    ...
    it('should generate a switch block inside a template', () => {
      ...
      expect(tcb(TEMPLATE))
          .toContain(
              'var _t1: any = null! as any; { var _t2 = (_t1.exp); _t2(); ' +
              'if ((_t2()) === "one") { "" + ((this).one()); } ' +
  ...
  describe('for loop blocks', () => {
    ...
    it('should generate a for block with implicit variables', () => {
      ...
      const result = tcb(TEMPLATE);
      expect(result).toContain('for (const _t1 of ((this).items)!) {');
      expect(result).toContain('var _t2: number = null! as number;');
      ...
      expect(result).toContain('var _t7: number = null! as number;');
    ...
    it('should read variable from a parent for loop', () => {
      ...
      const result = tcb(TEMPLATE);
      expect(result).toContain(
          'for (const _t1 of ((this).items)!) { var _t2: number = null! as number;');
      expect(result).toContain('"" + (_t1) + (_t2)');
      expect(result).toContain(
          'for (const _t3 of ((_t1).items)!) { var _t4: number = null! as number;');
