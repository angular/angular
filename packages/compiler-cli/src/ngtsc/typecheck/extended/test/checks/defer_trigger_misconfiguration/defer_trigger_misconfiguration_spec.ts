...
runInEachFileSystem(() => {
  describe('DeferTriggerMisconfiguration', () => {
    ...
    it('should emit when on immediate coexists with other mains', () => {
      const diags = getDiags(`@defer (on immediate; on timer(100ms) ; on viewport(ref) ) { <div></div> }`);
    ...
    it('should emit when prefetch identical to main viewport/interaction/hover', () => {
      const diags = getDiags(`@defer (on viewport(ref); prefetch on viewport(ref)) { <div></div> }`);
      expect(diags.length).toBe(1);
      expect(diags[0].category).toBe(ts.DiagnosticCategory.Warning);
      expect(diags[0].code).toBe(ngErrorCode(ErrorCode.DEFER_TRIGGER_MISCONFIGURATION));
    });
    ...
    it('should not emit when main is viewport(ref) and prefetch is viewport without reference', () => {
      const diags = getDiags(`@defer (on viewport(ref); prefetch on viewport) { <div></div> } @placeholder { <div></div> }`);
      expect(diags.length).toBe(0);
    });

    it('should not emit when main is interaction(refA) and prefetch is interaction(refB)', () => {
      const diags = getDiags(`@defer (on interaction(refA); prefetch on interaction(refB)) { <div></div> } @placeholder { <div></div> }`);
      expect(diags.length).toBe(0);
    });

    it('should not emit when viewport triggers share the same rootMargin but use different trigger references', () => {
      const diags = getDiags(`@defer (on viewport({trigger: refA , rootMargin: '100px'}); prefetch on viewport({trigger: refB , rootMargin: '100px'})) { <div></div> } @placeholder { <div></div> }`);
      expect(diags.length).toBe(0);
    });

    it('should emit when viewport triggers have identical rootMargin options', () => {
      const diags = getDiags(`@defer (on viewport({rootMargin: '100px'}); prefetch on viewport({rootMargin: '100px'})) { <div></div> } @placeholder { <div></div> }`);
      expect(diags.length).toBe(1);
      expect(diags[0].category).toBe(ts.DiagnosticCategory.Warning);
      expect(diags[0].code).toBe(ngErrorCode(ErrorCode.DEFER_TRIGGER_MISCONFIGURATION));
    });
    ...
    it('should emit when viewport triggers have identical empty options', () => {
      const diags = getDiags(`@defer (on viewport({}); prefetch on viewport({})) { <div></div> } @placeholder { <div></div> }`);
      expect(diags.length).toBe(1);
      expect(diags[0].category).toBe(ts.DiagnosticCategory.Warning);
      expect(diags[0].code).toBe(ngErrorCode(ErrorCode.DEFER_TRIGGER_MISCONFIGURATION));
    });
    ...
    it('should not emit when viewport triggers have different rootMargin options', () => {
      const diags = getDiags(`@defer (on viewport({rootMargin: '100px'}); prefetch on viewport({rootMargin: '200px'})) { <div></div> } @placeholder { <div></div> }`);
      expect(diags.length).toBe(0);
    });

    it('should not emit when one viewport has options and the other does not', () => {
      const diags = getDiags(`@defer (on viewport({rootMargin: '100px'}); prefetch on viewport) { <div></div> } @placeholder { <div></div> }`);
      expect(diags.length).toBe(0);
    });
