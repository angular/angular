...
runInEachFileSystem(() => {
  describe('UninvokedTrackFunctionCheck', () => {
    ...
    it('should produce a diagnostic when a track function in a @for block is not invoked', () => {
      ...
      expect(diags[0].messageText).toBe(generateDiagnosticText('trackByName', 'item'));
    });

    it('should not produce a warning when track is set to a property read', () => {
      const diags = setupTestComponent(
        `
...
function generateDiagnosticText(method: string, itemName: string): string {
  return `The track function in @for block should be invoked: ${method}(${itemName})`;
}
