    ...
    for (const attr of boundAttrs) {
      ...
      for (const {fieldName, required, transformType, isSignal, isTwoWayBinding} of attr.inputs) {
        ...
        if (isSignal) {
          ...
          //    using a TypeScript trick: `Parameters<InputSignal[BrandSymbol]>[0]`.
          ...
          let type: ts.TypeNode = ts.factory.createKeywordTypeNode(ts.SyntaxKind.AnyKeyword);
          const lhs = (target as ts.ElementAccessExpression).expression;
          const symbol = (target as ts.ElementAccessExpression).argumentExpression;
          ...
          const inputType = ts.factory.createIndexedAccessTypeNode(
            ts.factory.createTypeQueryNode(lhs.expression),
            ts.factory.createLiteralTypeNode(ts.factory.createStringLiteral(lhs.name.text)),
          );
          ...
          if (
            ...
            const restrictedAccessExpr = lhs;
            addParseSpanInfo(restrictedAccessExpr, attr.keySpan!);
            this.scope.addStatement(ts.factory.createExpressionStatement(restrictedAccessExpr));
          }

          // Creating the type of the LHS expression
          const symbolType = ts.factory.createTypeQueryNode(symbol as ts.Identifier);
          type = ts.factory.createIndexedAccessTypeNode(
            ts.factory.createTypeReferenceNode('Parameters', [
              ts.factory.createIndexedAccessTypeNode(inputType, symbolType),
            ]),
