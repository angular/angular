...
class TcbElementOp extends TcbOp {
  /**
   * The statement generated by this operation is only used for type-inference of the DOM element's
   * type and won't report diagnostics by itself, so the operation is marked as optional to avoid
   * generating statements for DOM elements that are never referenced.
   */
  readonly optional = true;
