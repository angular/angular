...
export function tsCreateElement(...tagNames: string[]): ts.Expression {
  ...
  if (tagNames.length === 1) {
    ...
    // we create an expression in the form of `null! as 'tag-1' | 'tag-2' | 'tag-3'`.
    const expression = ts.factory.createNonNullExpression(ts.factory.createNull());
    ...
    arg = ts.factory.createAsExpression(expression, type);
  }
