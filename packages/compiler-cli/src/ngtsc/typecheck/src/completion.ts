...
export class CompletionEngine {
  ...
  getGlobalCompletions(context: TmplAstTemplate|null, node: AST|TmplAstNode): GlobalCompletion
    ...
    if (node instanceof PropertyRead && node.receiver instanceof ImplicitReceiver) {
      ...
      if (nodeLocation) {
        nodeContext = {
          shimPath: this.shimPath,
          positionInShimFile: nodeLocation.parent.pos,
        };
      }
    }
  ...
  getStringLiteralCompletionLocation(expr: LiteralPrimitive|TextAttribute): ShimLocation|null {
    if (this.expressionCompletionCache.has(expr)) {
      return this.expressionCompletionCache.get(expr)!;
    }

    let tsExpr: ts.StringLiteral|null = null;
    if (expr instanceof TextAttribute) {
      const strNode = findFirstMatchingNode(this.tcb, {
        filter: ts.isParenthesizedExpression,
        withSpan: expr.sourceSpan,
      });
      if (strNode && ts.isStringLiteral(strNode.expression)) {
    ...
    const res: ShimLocation = {
      shimPath: this.shimPath,
      positionInShimFile: tsExpr.getEnd() - 1,
    };
    this.expressionCompletionCache.set(expr, res);
    return res;
  }
