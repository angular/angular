...
import {AbsoluteFsPath, absoluteFrom, absoluteFromSourceFile, basename} from '../../file_system';
...
export class FactoryGenerator implements PerFileShimGenerator, FactoryTracker {
  ...
  generateShimForFile(sf: ts.SourceFile, genFilePath: AbsoluteFsPath): ts.SourceFile {
    ...
    if (sf.statements.length > 0) {
      ...
      const leadingTriviaWidth = firstStatement.getLeadingTriviaWidth(sf);
      if (leadingTriviaWidth > 0) {
        comment = firstStatement.getFullText(sf).substr(0, leadingTriviaWidth);
      }
    }
