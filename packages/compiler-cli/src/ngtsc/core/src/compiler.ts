...
export class NgCompiler {
  ...
  /**
   * Loads the manifests configured via the `customElementsManifests` option. Manifest schemas
   * affect every template in the program, so each manifest resolution path is recorded as a
   * resource dependency of every source file: incremental build hosts then invalidate all source
   * files when a manifest changes and include the manifests in their resource watch set.
   */
...
function verifyCustomElementsManifestsOption(options: NgCompilerOptions): ts.Diagnostic[] {
  ...
  const mode = (options as {customElementsManifestsDiagnostics?: unknown})
...
function getCustomElementsManifestsDiagnosticsMode(
  ...
  const mode = (options as {customElementsManifestsDiagnostics?: unknown})
    .customElementsManifestsDiagnostics;
  return mode === 'verbose' ? 'verbose' : 'summary';
}
