...
export class ReferenceEmitter {
  ...
  emit(
    ...
    throw new Error(
      `Unable to write a reference to ${nodeNameForError(ref.node)} in ${
        ref.node.getSourceFile().fileName
...
export class LocalIdentifierStrategy implements ReferenceEmitStrategy {
  emit(
    ...
    if (
      importFlags & ImportFlags.ForceNewImport &&
...
export class AbsoluteModuleStrategy implements ReferenceEmitStrategy {
  ...
  emit(
    ...
    if (symbolName === null) {
      ...
      // triggered.
      throw new Error(
        `Symbol ${ref.debugName} declared in ${
          getSourceFile(ref.node).fileName
        } is not exported from ${specifier} (import into ${context.fileName})`
      );
    }
  ...
  protected enumerateExportsOfModule(
    specifier: string,
    fromFile: string
  ): Map<ts.Declaration, string> | null {
