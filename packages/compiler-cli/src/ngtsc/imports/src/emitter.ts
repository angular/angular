 ...
 */
export class NodeModulesReferenceEmitStrategy implements ReferenceEmitStrategy {
  /**
  ...
  emit(ref: Reference<ts.Node>, context: ts.SourceFile, importMode: ImportMode): Expression|null {
    const container = getSourceFile(ref.node);
    if (container.fileName.indexOf('/node_modules/') === -1) {
      return null;
    }
    if (ref.bestGuessOwningModule === null) {
      // There is no module name available for this Reference, meaning it was tracked via a relative
      // path, which makes no sense.
      throw new Error('Debug assert: relative reference into node_modules?');
    } else if (!isDeclaration(ref.node)) {
      // It's not possible to import something which isn't a declaration.
      throw new Error('Debug assert: absolute reference to non-declaration?')
    }
    ...
    if (symbolName === null) {
      ...
      // triggered.
      throw new Error(`Symbol ${ref.debugName} is not exported from ${specifier}`);
    }
...
/**
 * A `ReferenceEmitStrategy` which will refer to declarations via relative paths.
 *
 * This is trickier than it sounds, as the two files may be in different root directories in the
 * project. Simply calculating a file system relative path between the two is not sufficient.
 * Instead, `LogicalProjectPath`s are used.
 */
export class RelativeReferenceEmitStrategy implements ReferenceEmitStrategy {
  ...
  emit(ref: Reference<ts.Node>, context: ts.SourceFile): Expression|null {
    ...
    if (destPath === null) {
      throw new Error(
          `debug assert: attempt to import ${destSf.fileName} but it's outside the program?`);
 ...
 */
export class FileToModuleReferenceEmitStrategy implements ReferenceEmitStrategy {
