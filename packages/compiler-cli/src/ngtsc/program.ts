...
import {FileToModuleHost, ImportRewriter, LocalIdentifierStrategy, ModuleResolver, NodeModulesReferenceEmitStrategy, NoopImportRewriter, R3SymbolsImportRewriter, Reference, ReferenceEmitter, RelativeReferenceEmitStrategy} from './imports';
import {FileToModuleReferenceEmitStrategy} from './imports/src/emitter';
...
export class NgtscProgram implements api.Program {
  ...
  private makeCompilation(): IvyCompilation {
    ...
    if (this.fileToModuleHost === null) {
      // The CompilerHost doesn't have fileNameToModuleName, so build an NPM-centric reference
      // resolution strategy.
      this.refEmitter = new ReferenceEmitter([
        // First, try to use local identifiers if available.
        new LocalIdentifierStrategy(),
        // Next, check if the reference is being written into node_modules, and generate an absolute
        // import if so.
        new NodeModulesReferenceEmitStrategy(this.tsProgram, checker, this.options, this.host),
        // Finally, use relative paths between the modules.
        new RelativeReferenceEmitStrategy(checker, new LogicalFileSystem(this.rootDirs)),
      ]);
    } else {
      ...
      this.refEmitter = new ReferenceEmitter([
        ...
        new FileToModuleReferenceEmitStrategy(checker, this.fileToModuleHost),
      ]);
    }
