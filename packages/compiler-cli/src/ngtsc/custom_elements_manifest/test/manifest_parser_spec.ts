...
describe('parseCustomElementsManifest', () => {
  ...
  it('should compute check types for trustworthy type text', () => {
    ...
    expect(properties.get('count')!.checkType?.checkType).toBe('number');
    ...
    expect(properties.get('items')!.checkType?.checkType).toBeUndefined();
    expect(properties.get('mixed')!.checkType?.checkType).toBe('boolean | number');
    expect(properties.get('untyped')!.checkType?.checkType).toBeUndefined();
    ...
    expect(schemaFor(result, 'my-button')!.attributes).toEqual([
      ...
      {
        name: 'mixed',
        checkType: jasmine.objectContaining({checkType: 'boolean | number'}),
  ...
  it('should report unusable member and event types', () => {
    ...
    expect(result.schemas[0].properties[0].checkType?.checkType).toBeUndefined();
    expect(result.schemas[0].events[0].checkType?.checkType).toBeUndefined();
    expect(result.warnings.map((warning) => warning.subject)).toEqual([
      'my-element.variant',
      'my-element.change',
    ]);
  ...
  it('should support global type references', () => {
    ...
    expect(result.schemas[0].events[0].checkType?.checkType).toBe('KeyboardEvent');
  });
