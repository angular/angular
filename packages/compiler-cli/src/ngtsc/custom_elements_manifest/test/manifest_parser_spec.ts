...
/**
 * Keeps concise fixtures that exercise unrelated parser behavior structurally conforming. Tests
 * for malformed declaration/export relationships call `parseCustomElementsManifest` directly.
 */
...
describe('parseCustomElementsManifest', () => {
          ...
          declarations: [
            {
              ...
              members: [
                ...
                // `readonly` was only formalized in schema v2, but v1-era producers emitted it.
              ...
              attributes: [
                ...
                // Dangling fieldName: the attribute remains registered, but the invalid
                // relationship is not projected.
  ...
  it('should parse schema v2 manifests, including their v2-only fields', () => {
    ...
    // Readonly fields are not bindable; the tagless custom-element mixin produces no schema.
    expect(schema.properties.map((property) => property.name)).toEqual(['label']);
    expect(schema.attributes).toEqual([
      {name: 'label', fieldName: 'label', deprecated: 'Use slotted content.'},
    ]);
    expect(schema.events).toEqual([
  ...
  it('should compute check types for trustworthy type text', () => {
    ...
    // Named type without references: coarse tag only, with producer feedback that the declared
    // type could not be used for template checking.
  ...
  it('should treat an explicit attribute type as authoritative', () => {
    ...
    expect(attributes.get('mode')).toEqual({name: 'mode', fieldName: 'mode'});
    expect(result.warnings.map((warning) => warning.subject)).toEqual([
      'my-button.size',
      'my-button.variant',
      'my-button.mode',
  ...
  it("should prefer the declaration's home module for a barrel re-exported element instance type", () => {
    // Mirrors the @rhds/elements barrel pattern: the element class is exported exact-name from
    // both its own module and a sibling barrel that re-exports it. The home-module export is
    // authoritative, so selection is unambiguous rather than failing closed to HTMLElement.
    const result = parseCustomElementsManifest(
      JSON.stringify({
        schemaVersion: '1.0.0',
        modules: [
