...
describe('computeCheckType', () => {
  it('should report the reason rejected declared type text cannot be used', () => {
    const failureFor = (text: string): CheckTypeFailure | null => {
      ...
      computeCheckType({text}, null, null, (reportedFailure) => {
  ...
  describe('tier 1: self-contained type text', () => {
    ...
    it('should accept identifier property names in inline object types', () => {
      ...
      // Semicolons remain forbidden by the character whitelist; property-name support does not
      // broaden the accepted punctuation boundary.
  ...
  describe('tier 2: index-less references', () => {
    ...
    it('should reject index-less references to names nested in compound type text', () => {
      // Per the specification, an index-less reference names the entire type string. Nested
      // occurrences require exact `start`/`end` indices; producers that omit them are
      // nonconforming and Angular deliberately does not compensate (decision 2026-07-17).
      ...
      // An index-less reference among indexed ones fails the whole text the same way.
      expect(
        check('First | Second', [
          {name: 'First', package: 'first-pkg'},
    ...
    it('should keep string-literal-only text self-contained regardless of references', () => {
      // 'MyItem' inside a string literal is data, not a type reference; the text is tier-1.
