...
import {parseTypeText, TYPE_TEXT_ALIAS_PREFIX} from './type_text';
 ...
 *
 * 1. *Self-contained* text whose identifiers are all TypeScript built-in type keywords (e.g.
 *    `boolean`, `'a' | 'b'`, `string[]`). Identifier property names in object types are syntax
 *    rather than type references and are also accepted. Returned as-is.
 * 2. Text whose named types are located by the manifest's `type.references` entries. Each
 *    reference must exactly cover an identifier occurrence — via its `start`/`end` indices, or
 *    index-less only in the specification's whole-text form where the entire type string is the
 *    referenced name — and provide a usable bare package specifier; the covered span is replaced
 *    with an inline `import("<package>").<Name>` type query, producing self-contained text.
 *
...
export function computeCheckType(
  ...
  reportFailure?: (failure: CheckTypeFailure) => void,
): string | null {
  ...
  if (text.trim().length === 0) {
    return failCheckType('emptyTypeText', reportFailure);
  }
  if (text.length > MAX_TYPE_TEXT_LENGTH) {
    return failCheckType('typeTextTooLong', reportFailure);
  }

  const scan = scanTypeText(text);
  if (scan === null) {
    return failCheckType('unsupportedTypeText', reportFailure);
  }
  ...
  const namedIdentifiers = scan.identifiers.filter(
    (id) =>
  ...
  if (namedIdentifiers.length === 0) {
    ...
    return isSyntacticallyValidType(result)
      ? result
      : failCheckType('unsupportedTypeText', reportFailure);
  }
  ...
  for (const identifier of namedIdentifiers) {
    ...
    if (replacement === null) {
      return failCheckType('unusableTypeReference', reportFailure);
    }
    substitutions.push({
...
export type CheckTypeFailure =
  'emptyTypeText' | 'typeTextTooLong' | 'unsupportedTypeText' | 'unusableTypeReference';

function failCheckType(
  failure: CheckTypeFailure,
  reportFailure: ((failure: CheckTypeFailure) => void) | undefined,
): null {
...
const ALLOWED_IDENTIFIERS = new Set<string>(TYPE_KEYWORDS);
...
/**
 * Returns the original-text spans occupied by identifier names on object type properties.
 * Property names are declarations, not references to types, so requiring a CEM `type.references`
 * entry for them would reject ordinary object types such as `{value: string}`.
 *
 * This is intentionally limited to `PropertySignature` names. Method names, index-signature
 * parameters, computed names and locally scoped identifiers remain subject to the scanner's
 * reference rules.
 */
 ...
 * string." Producers that omit indices for names nested in compound types are nonconforming;
 * Angular deliberately does not compensate for them (a previous unique-occurrence compatibility
 * extension was removed by decision on 2026-07-17), so such references fail closed and the fix
 * belongs upstream in the producer's manifest generator.
 */
function findCoveringReferenceReplacement(
  ...
  for (const reference of references) {
    ...
    if (reference['start'] !== undefined || reference['end'] !== undefined) {
      ...
      // Index-less reference: only the specification's whole-text form is accepted — the entire
      // type string is the referenced symbol. Names nested in compound text require exact
      // `start`/`end` indices.
      continue;
    }
...
/**
 * Returns whether a specifier names one npm package rather than a relative path or package
 * subpath. Besides bounding the emitted import type syntax, rejecting dot-like names prevents
 * values such as `.` and `..` from acquiring relative-module semantics in a generated TCB.
 */
