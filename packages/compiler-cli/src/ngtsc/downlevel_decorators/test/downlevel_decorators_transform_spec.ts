...
import {absoluteFrom, AbsoluteFsPath, FileSystem, getFileSystem} from '../../file_system';
import {runInEachFileSystem} from '../../file_system/testing';
import {TypeScriptReflectionHost} from '../../reflection';
import {NgtscTestCompilerHost} from '../../testing';
import {getDownlevelDecoratorsTransform} from '../src/downlevel_decorators_transform';

runInEachFileSystem(() => {
  describe('downlevel decorator transform', () => {
    let fs: FileSystem;
    let host: NgtscTestCompilerHost;
    let diagnostics: ts.Diagnostic[];
    let isClosureEnabled: boolean;
    let TEST_FILE_INPUT: AbsoluteFsPath;
    let TEST_FILE_OUTPUT: AbsoluteFsPath;
    let TEST_FILE_DTS_OUTPUT: AbsoluteFsPath;

    beforeEach(() => {
      diagnostics = [];
      fs = getFileSystem();
      fs.ensureDir(fs.resolve('/'));
      fs.writeFile(absoluteFrom('/dom_globals.d.ts'), `
        declare class HTMLElement {};
        declare class Document {};
      `);
      host = new NgtscTestCompilerHost(fs);
      isClosureEnabled = false;

      TEST_FILE_INPUT = absoluteFrom('/test.ts');
      TEST_FILE_OUTPUT = absoluteFrom(`/test.js`);
      TEST_FILE_DTS_OUTPUT = absoluteFrom(`/test.d.ts`);
    });

    function transform(
        contents: string, compilerOptions: ts.CompilerOptions = {},
        preTransformers: ts.TransformerFactory<ts.SourceFile>[] = []) {
      fs.writeFile(TEST_FILE_INPUT, contents);
    ...
    it('should downlevel Angular-decorated class member but not preserve type', () => {
      fs.writeFile(absoluteFrom('/other-file.ts'), `export class MyOtherClass {}`);
      const {output} = transform(`
       import {Input} from '@angular/core';
    ...
    it('should not retain unused type imports due to decorator downleveling with ' +
       ...
       () => {
         fs.writeFile(absoluteFrom('/external.ts'), `
    ...
    it('should allow for type-only references to be removed with `emitDecoratorMetadata` from custom decorators',
       () => {
         fs.writeFile(absoluteFrom('/external-interface.ts'), `
    ...
    describe('transforming multiple files', () => {
      it('should work correctly for multiple files that import distinct declarations', () => {
        fs.writeFile(absoluteFrom('/foo_service.d.ts'), `
         export declare class Foo {};
       `);
        fs.writeFile(absoluteFrom('/foo.ts'), `
         import {Injectable} from '@angular/core';
       ...
       `);
        ...
        fs.writeFile(absoluteFrom('/bar_service.d.ts'), `
         export declare class Bar {};
       `);
        fs.writeFile(absoluteFrom('/bar.ts'), `
         import {Injectable} from '@angular/core';
       ...
       `);
        ...
        expect(fs.readFile(absoluteFrom('/foo.js')))
            .toContain(`import { Foo } from './foo_service';`);
        expect(fs.readFile(absoluteFrom('/bar.js')))
            .toContain(`import { Bar } from './bar_service';`);
      });
      ...
      it('should not result in a stack overflow for a large number of files', () => {
        ...
        // number of source files was reached. This test verifies that emit succeeds even when
        // there's lots of source files. See https://github.com/angular/angular/issues/40276.
        fs.writeFile(absoluteFrom('/foo.d.ts'), `
         export declare class Foo {};
       `);
        ...
        program.emit(undefined, (fileName, outputText) => {
          ...
          // as we want to abort on the first failure, if any. This avoids as many as
          // `numberOfFiles` expectation failures, which would bloat the test output.
