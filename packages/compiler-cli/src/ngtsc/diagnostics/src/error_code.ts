...
export enum ErrorCode {
  ...
  /**
   * Raised (as a warning) when a Custom Elements Manifest provides type metadata that cannot be
   * safely used for template checking, including ambiguous JavaScript export mappings. The
   * declaration remains available and affected checks use safe fallbacks.
   */
  CONFIG_CUSTOM_ELEMENTS_MANIFEST_UNUSABLE_TYPE = 4013,

  /**
   * Raised (as a warning) when an individually recoverable manifest entry violates a consumed
   * CEM structural relationship. Unrelated valid declarations remain available.
   */
  ...
  /**
   * Raised when a `@defer` block is missing a `name` parameter but `deferredImports` is an object.
   */
  DEFER_BLOCK_MISSING_NAME_PARAMETER = 11100,

  /**
   * Raised when a `@defer` block specifies a `name` parameter that does not match any entry in `deferredImports`.
   */
  DEFER_BLOCK_UNKNOWN_NAME_PARAMETER = 11101,

  /**
   * Raised when a `@defer` block specifies a `name` parameter but `deferredImports` is not an object.
   */
  DEFER_BLOCK_INVALID_NAME_PARAMETER = 11102,
}
