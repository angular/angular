...
export interface NgTools_InternalApi_NG2_CodeGen_Options {
  basePath: string; compilerOptions: ts.CompilerOptions; program: ts.Program; host: ts.CompilerHost;
...
export interface NgTools_InternalApi_NG2_ListLazyRoutes_Options {
  program: ts.Program; host: ts.CompilerHost; angularCompilerOptions: AngularCompilerOptions;
  entryModule: string;
...
export class NgTools_InternalApi_NG_2 {
  ...
  static codeGen(options: NgTools_InternalApi_NG2_CodeGen_Options): Promise<any>{
    ...
    const resolver = createModuleFilenameResolver(options.host, ngOptions);
    const ngCompilerHost = new CompilerHost(options.program, ngOptions, options.host, resolver);
    ngCompilerHost.loadResource = options.readResource;
    ...
    const codeGenerator = CodeGenerator.create(
        options.angularCompilerOptions, cliOptions, options.program, options.host, ngCompilerHost);
   ...
   * @private
   */
  static listLazyRoutes(
      options:
          NgTools_InternalApi_NG2_ListLazyRoutes_Options): NgTools_InternalApi_NG_2_LazyRouteMap{
      const ngOptions = options.angularCompilerOptions; const program = options.program;

      const resolver = createModuleFilenameResolver(options.host, ngOptions);
      const ngCompilerHost = new CompilerHost(program, ngOptions, options.host, resolver);
  ...
  static extractI18n(options: NgTools_InternalApi_NG2_ExtractI18n_Options): Promise<any>{
      ...
      const extractor = Extractor.create(
          options.angularCompilerOptions, options.program, options.host, locale, ngCompilerHost);
