 ...
 */
import {ChangeDetectionStrategy, compileComponentFromMetadata, ConstantPool, DeclarationListEmitMode, DEFAULT_INTERPOLATION_CONFIG, ForwardRefHandling, InterpolationConfig, makeBindingParser, outputAst as o, parseTemplate, R3ComponentMetadata, R3DeclareComponentMetadata, R3DeclareUsedDirectiveMetadata, R3DeclareUsedPipeMetadata, R3PartialDeclaration, R3UsedDeclarationKind, R3UsedDeclarationMetadata, R3UsedDirectiveMetadata, R3UsedPipeMetadata, ViewEncapsulation} from '@angular/compiler';
...
function makeUsedDirectiveMetadata<TExpression>(
    directiveExpr: AstObject<R3DeclareUsedDirectiveMetadata, TExpression>,
    typeExpr: o.WrappedNodeExpr<TExpression>,
    isComponentByDefault: true|null = null): R3UsedDirectiveMetadata {
  return {
  ...
  private toR3ComponentMeta(metaObj: AstObject<R3DeclareComponentMetadata, TExpression>):
    ...
    if (metaObj.has('components')) {
      declarations.push(...metaObj.getArray('components').map(dir => {
        ...
        return makeUsedDirectiveMetadata(dirExpr, typeExpr, /* isComponentByDefault */ true);
      }));
    }
    if (metaObj.has('directives')) {
      declarations.push(...metaObj.getArray('directives').map(dir => {
        const dirExpr = dir.getObject();
        const typeExpr = extractDeclarationTypeExpr(dirExpr.getValue('type'));
        return makeUsedDirectiveMetadata(dirExpr, typeExpr);
      }));
    }
    ...
    if (metaObj.has('dependencies')) {
      for (const dep of metaObj.getArray('dependencies')) {
        ...
        switch (depObj.getString('kind')) {
          case 'directive':
          case 'component':
            declarations.push(makeUsedDirectiveMetadata(depObj, typeExpr));
            break;
          case 'pipe':
            const pipeObj =
