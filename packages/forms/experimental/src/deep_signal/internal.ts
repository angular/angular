...
import {ReactiveNode, COMPUTED_NODE, ComputedNode, Version} from '@angular/core/primitives/signals';
...
export const DEEP_SIGNAL_NODE: Omit<DeepSignalNode, 'computation' | 'parentNode'> =
  /* @__PURE__ */ (() => {
    return {
      ...
      rawDirty: false,
      get dirty(): boolean {
        if (isNotAffectedByCurrentUpdate(this as DeepSignalNode)) {
 ...
 */
function isNotAffectedByCurrentUpdate(node: DeepSignalNode): boolean {
  ...
  // that were the case, we'd already be dirty from that notification.
  return false;
}
