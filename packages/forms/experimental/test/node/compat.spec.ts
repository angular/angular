...
describe('Forms compat', () => {
  ...
  describe('validation', () => {
    it('picks up the validation from a new form control', () => {
      ...
      expect(f.age().errors()).toEqual([
        ValidationError.custom({kind: 'min', context: {min: 10, actual: 5}}),
      ]);
      f().value.set({age: new FormControl(10), name: 'lol'});
    ...
    it('picks up the multiple errors from a new form control', () => {
      ...
      expect(f.mistake().errors()).toEqual([
        ValidationError.custom({kind: 'pirojok-the-error', context: {meow: true}}),
        ValidationError.custom({kind: 'error-1', context: 'error-1-content'}),
        ValidationError.custom({kind: 'error-2', context: 'error-2-content'}),
      ]);
    });
  });
  ...
  describe('validation', () => {
    it('supports async validation', async () => {
      ...
      expect(f.age().pending()).toBeFalse();
      expect(f.age().errors()).toEqual([
        ValidationError.custom({kind: 'incorrect-cat', context: 123}),
      ]);
    });
  });
