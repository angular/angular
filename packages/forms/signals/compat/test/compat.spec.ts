...
import {CompatValidationError} from '../src/compat_validation_error';
import {compatForm} from '../src/compat_form';
...
describe('Forms compat', () => {
  ...
  describe('validation', () => {
    it('picks up the validation from a new form control', () => {
      ...
      expect(f.age().errors()).toEqual([
        new CompatValidationError({kind: 'min', context: {min: 10, actual: 5}}),
      ]);
      f().value.set({age: new FormControl(10), name: 'lol'});
    ...
    it('allows to manually set errors', () => {
      ...
      expect(f().errors()).toEqual([new CompatValidationError({kind: 'meow', context: true})]);
    });
    ...
    it('picks up the multiple errors from a new form control', () => {
      ...
      expect(f.mistake().errors()).toEqual([
        new CompatValidationError({kind: 'pirojok-the-error', context: {meow: true}}),
        new CompatValidationError({kind: 'error-1', context: 'error-1-content'}),
        new CompatValidationError({kind: 'error-2', context: 'error-2-content'}),
      ]);
    });
    ...
    it('supports async validation', async () => {
      ...
      expect(f.age().pending()).toBeFalse();
      expect(f.age().errors()).toEqual([
        new CompatValidationError({kind: 'incorrect-cat', context: 123}),
      ]);
    });
  });
  ...
  describe('state propagation', () => {
    ...
    it('picks up submittedState from parent', async () => {
      ...
      const result = submit(f as unknown as Field<void>, () => {
        return promise;
      });
  ...
  describe('FormGroup', () => {
    ...
    it('is propagates validity from child controls, but not errors', () => {
      const cat = signal(
        new FormGroup({
          name: new FormControl('pirojok-the-cat'),
