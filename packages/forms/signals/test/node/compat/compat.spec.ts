...
import {CompatFieldTree, CompatFieldState} from '../../../compat/src/compat_types';
...
describe('Forms compat', () => {
  ...
  describe('FormGroup', () => {
    ...
    it('uses the actual value, not rawValue', () => {
      ...
      expect(f().value()).toEqual({
        name: 'pirojok-the-cat',
        // no `age: 10,`, because the control is disabled
      });
  ...
  describe('type tests', () => {
    it('allows generic CompatValidationError', () => {
      ...
      const min = error.context.min;
      // @ts-expect-error
      const notMin: string = error.context.min;
    });

    it('Allows setting T value for non compat fields', () => {
      function setCompatValue<T>(f: CompatFieldTree<T>, value: T) {
        // @ts-expect-error
        f().value.set(value);
      }

      function setValueCompatState<T>(field: CompatFieldState<T>, value: T) {
