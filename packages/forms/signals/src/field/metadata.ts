...
export class FieldMetadataState {
  ...
  get<T>(key: MetadataKey<unknown, unknown, T>): T {
    ...
    if (!this.metadata.has(key)) {
      const logic = this.node.logicNode.logic.getMetadata(key as MetadataKey<unknown, unknown>);
  ...
  has(key: MetadataKey<any, any>): boolean {
