...
export abstract class AbstractControl<TValue = any, TRawValue extends TValue = TValue> {
  ...
  markAllAsTouched(opts: {emitEvent?: boolean} = {}): void {
    this._markAsTouched({onlySelf: true, emitEvent: opts.emitEvent}, this);
  ...
  _updateValueAndValidity(
    ...
    if (opts.emitEvent !== false) {
      this._events.next(new ValueChangeEvent<TValue>(this.value, changedControl));
      this._events.next(new StatusChangeEvent(this.status, changedControl));
  ...
  /** @internal */
  _enable(opts: {onlySelf?: boolean, emitEvent?: boolean}): void {
  ...
  _disable(opts: {onlySelf?: boolean, emitEvent?: boolean}, changedControl: AbstractControl): void {
    ...
    if (opts.emitEvent !== false) {
      this._events.next(new StatusChangeEvent(this.status, changedControl));
      this._events.next(new ValueChangeEvent(this.value, changedControl));
      (this.valueChanges as EventEmitter<TValue>).emit(this.value);
      (this.statusChanges as EventEmitter<FormControlStatus>).emit(this.status);
    }
  ...
  /** @internal */
  _markAsDirty(opts: {onlySelf?: boolean, emitEvent?: boolean}, changedControl: AbstractControl):
      void {
    ...
    if (changed && opts.emitEvent !== false) {
      this._events.next(new PristineChangeEvent(false, changedControl));
    }
  }

  /** @internal */
  _markAsPristine(opts: {onlySelf?: boolean, emitEvent?: boolean}, changedControl: AbstractControl):
      void {
    ...
    if (changed && opts.emitEvent !== false) {
      this._events.next(new PristineChangeEvent(true, changedControl));
    }
  }
  ...
  /** @internal */
  _markAsTouched(opts: {onlySelf?: boolean, emitEvent?: boolean}, changedControl: AbstractControl):
      void {
    ...
    if (changed && opts.emitEvent !== false) {
      this._events.next(new TouchedChangeEvent(true, changedControl));
    }
  }

  /** @internal */
  _markAsUntouched(
      opts: {onlySelf?: boolean, emitEvent?: boolean}, changedControl: AbstractControl|null): void {
    const changed = this.touched === true;
    (this as Writable<this>).touched = false;
    this._pendingTouched = false;

    this._forEachChild((control: AbstractControl) => {
      control._markAsUntouched({onlySelf: true, emitEvent: opts.emitEvent}, control);
    });

    if (this._parent && !opts.onlySelf) {
      this._parent._updateTouched(opts, changedControl);
    }
    if (changedControl && changed && opts.emitEvent !== false) {
      this._events.next(new TouchedChangeEvent(false, changedControl));
    }
  }
  /** @internal */
