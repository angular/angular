...
export class FormGroup<TControl extends {
   ...
   */
  constructor(
      public controls: TypedOrUntyped<TControl, TControl, {[key: string]: AbstractControl}>,
  ...
  removeControl<S extends string>(name: KeyIsRemovable<TControl, S>, options: {
    ...
    if ((this.controls as any)[name])
      this.controls[name as keyof TControl]._registerOnCollectionChange(() => {});
  ...
  override patchValue(value: FormGroupValue<TControl>, options: {
    ...
    (Object.keys(value) as Array<keyof TControl>).forEach(name => {
      if ((this.controls as any)[name]) {
        (this.controls as any)[name]!.patchValue(
  ...
  override _find(name: string|number): AbstractControl|null {
    return this.controls.hasOwnProperty(name as string) ? this.controls[name as keyof TControl] :
                                                          null;
  }
}
 ...
 * the same value type.
 *
 * `FormRecord` is very similar to {@see FormGroup}, except it enforces that all controls in the group have the same type, and accepts an open-ended, dynamically changing set of controls.
 *
 * @publicApi
 */
export class FormRecord<TControl extends AbstractControl<Value<TControl>, RawValue<TControl>> =
  ...
  /**
   * Creates a new `FormRecord` instance.
   *
   * @param controls A collection of child controls. The key for each child is the name
   * under which it is registered.
   *
   ...
   */
  constructor(
      public override controls: {[key: string]: TControl},
      validatorOrOpts?: ValidatorFn|ValidatorFn[]|AbstractControlOptions|null,
      asyncValidator?: AsyncValidatorFn|AsyncValidatorFn[]|null) {
    super(
        controls, pickValidators(validatorOrOpts),
   ...
   */
  override addControl(name: string, control: TControl, options: {emitEvent?: boolean} = {}): void {
    this.registerControl(name, control);
    this.updateValueAndValidity({emitEvent: options.emitEvent});
    this._onCollectionChange();
  }
   ...
   */
  override setControl(name: string, control: TControl, options: {emitEvent?: boolean} = {}): void {
    if (this.controls[name]) this.controls[name]._registerOnCollectionChange(() => {});
    delete (this.controls[name]);
    if (control) this.registerControl(name, control);
    this.updateValueAndValidity({emitEvent: options.emitEvent});
   ...
   */
  override setValue(value: {[key: string]: Value<TControl>}, options: {
    onlySelf?: boolean,
    emitEvent?: boolean
  } = {}): void {
    assertAllValuesPresent(this, true, value);
   ...
   */
  override patchValue(value: {[key: string]: Value<TControl>}, options: {
    onlySelf?: boolean,
    emitEvent?: boolean
  } = {}): void {
    // Even though the `value` argument type doesn't allow `null` and `undefined` values, the
    // `patchValue` can be called recursively and inner data structures might have these values, so
    // we just ignore such cases when a field containing FormRecord instance receives `null` or
    // `undefined` as a value.
    if (value == null /* both `null` and `undefined` */) return;
   ...
   * {@see FormGroup#reset}
   */
  override reset(value: {[key: string]: Value<TControl>} = {}, options: {
    onlySelf?: boolean,
    emitEvent?: boolean
  } = {}): void {
   ...
   */
  override getRawValue(): {[key: string]: RawValue<TControl>} {
  ...
  /** @internal */
  override _reduceChildren<T>(initValue: T, fn: any): T {
    let res = initValue;
    this._forEachChild((control: AbstractControl, name: string) => {
      res = fn(res, control, name);
    });
