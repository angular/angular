...

{
  describe('MockBackend', () => {
    ...
    it('should allow responding after subscription', inject(
      [AsyncTestCompleter],
    ...
    it('should allow subscribing after responding', inject(
      [AsyncTestCompleter],
      (async: AsyncTestCompleter) => {
        const connection: MockConnection = backend.createConnection(sampleRequest1);
        connection.mockRespond(sampleResponse1);
    ...
    it('should allow responding after subscription with an error', inject(
      [AsyncTestCompleter],
      (async: AsyncTestCompleter) => {
        const connection: MockConnection = backend.createConnection(sampleRequest1);
    ...
    it('should not throw when there are no unresolved requests', inject(
      [AsyncTestCompleter],
      (async: AsyncTestCompleter) => {
        const connection: MockConnection = backend.createConnection(sampleRequest1);
        connection.response.subscribe(() => {
    ...
    xit('should throw when there are unresolved requests', inject(
      [AsyncTestCompleter],
      (async: AsyncTestCompleter) => {
        const connection: MockConnection = backend.createConnection(sampleRequest1);
        connection.response.subscribe(() => {
    ...
    it('should work when requests are resolved out of order', inject(
      [AsyncTestCompleter],
      (async: AsyncTestCompleter) => {
    ...
    xit('should allow double subscribing', inject(
      [AsyncTestCompleter],
      (async: AsyncTestCompleter) => {
        ...
        const responseObservable: ReplaySubject<Response> = backend.createConnection(sampleRequest1)
          .response;
        responseObservable.subscribe((res) => expect(res.text()).toBe('response1'));
        responseObservable.subscribe(
          (res) => expect(res.text()).toBe('response2'),
          null!,
          async.done
        );
      }
