      ...
      it('should report error if loaded without invoking callback', inject(
        ...
        (async: AsyncTestCompleter) => {
          ...
          connection.response.subscribe(
            () => async.fail('Response listener should not be called'),
            (err: Response) => {
      ...
      it('should throw if request method is not GET', () => {
        [
          ...
          const req = new Request(
            base.merge(new RequestOptions({url: 'https://google.com', method: method})) as any
          );
          expect(() =>
            new (JSONPConnection as any)(req, new MockBrowserJsonp()).response.subscribe()
          ).toThrowError();
        });
      });
