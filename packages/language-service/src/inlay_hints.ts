...
export function getInlayHintsForTemplate(
  ...
  function getEventType(
    ...
    const symbol = ttc.getSymbolOfNode(event, typeCheckInfo.declaration);
    let isDirectiveOrComponentOutput = false;
    ...
    if (symbol && symbol.kind === SymbolKind.Output) {
      ...
      isDirectiveOrComponentOutput = outputSymbol.bindings.some(
        (binding) => binding.target.kind === SymbolKind.Directive,
      );
      if (!isDirectiveOrComponentOutput) {
        // Native DOM events can still appear as Output symbols in TCB.
        // For these, prefer DOM event map typing to avoid verbose generic signatures.
      } else if (outputSymbol.bindings.length > 0) {
    ...
    if (!symbol || symbol.kind !== SymbolKind.Output || !isDirectiveOrComponentOutput) {
