...
import {createDisplayParts, scriptElementKindForSymbolDisplayKind, SYMBOL_PUNC, SYMBOL_SPACE, SYMBOL_TEXT, SymbolDisplayInfoKind} from './display_parts';
...
export class QuickInfoBuilder {
  ...
  private getQuickInfoForBindingSymbol(
    ...
    const kind = symbol.kind === SymbolKind.Input ? SymbolDisplayInfoKind.PROPERTY :
                                                    SymbolDisplayInfoKind.EVENT;
  ...
  private getQuickInfoForElementSymbol(symbol: ElementSymbol): ts.QuickInfo {
    ...
    return createQuickInfo(
        templateNode.name, SymbolDisplayInfoKind.ELEMENT, getTextSpanOfNode(templateNode),
  ...
  private getQuickInfoForVariableSymbol(symbol: VariableSymbol, node: TmplAstNode|AST):
    ...
    return createQuickInfo(
        symbol.declaration.name, SymbolDisplayInfoKind.VARIABLE, getTextSpanOfNode(node),
  ...
  private getQuickInfoForReferenceSymbol(symbol: ReferenceSymbol, node: TmplAstNode|AST):
    ...
    const documentation = this.getDocumentationFromTypeDefAtLocation(symbol.shimLocation);
    return createQuickInfo(
        symbol.declaration.name, SymbolDisplayInfoKind.REFERENCE, getTextSpanOfNode(node),
        undefined /* containerName */, this.typeChecker.typeToString(symbol.tsType), documentation);
  }
  ...
  private getQuickInfoForPipeSymbol(symbol: ExpressionSymbol, node: TmplAstNode|AST): ts.QuickInfo
    ...
    return quickInfo === undefined ? undefined :
                                     updateQuickInfoKind(quickInfo, SymbolDisplayInfoKind.PIPE);
  }
  ...
  private getQuickInfoForDirectiveSymbol(dir: DirectiveSymbol, node: TmplAstNode|AST):
      ts.QuickInfo {
    const kind =
        dir.isComponent ? SymbolDisplayInfoKind.COMPONENT : SymbolDisplayInfoKind.DIRECTIVE;
...
function updateQuickInfoKind(quickInfo: ts.QuickInfo, kind: SymbolDisplayInfoKind): ts.QuickInfo {
...
function createNgTemplateQuickInfo(node: TmplAstNode|AST): ts.QuickInfo {
  return createQuickInfo(
      'ng-template',
      SymbolDisplayInfoKind.TEMPLATE,
      getTextSpanOfNode(node),
      /** containerName */ undefined,
      /** type */ undefined,
      [{
...
export function createQuickInfo(
    name: string, kind: SymbolDisplayInfoKind, textSpan: ts.TextSpan, containerName?: string,
  ...
  return {
    kind: scriptElementKindForSymbolDisplayKind(kind),
