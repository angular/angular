 ...
 */
import {AST, BindingPipe, ImplicitReceiver, MethodCall, ThisImplicitReceiver, TmplAstBoundAttribute, TmplAstNode, TmplAstTextAttribute} from '@angular/compiler';
...
function isDollarAny(node: TmplAstNode|AST): node is MethodCall {
  return node instanceof MethodCall && node.receiver instanceof ImplicitReceiver &&
      !(node.receiver instanceof ThisImplicitReceiver) && node.name === '$any' &&
      node.args.length === 1;
}
