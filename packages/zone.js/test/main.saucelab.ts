...
if ((window as any)[(Zone as any).__symbol__('setTimeout')]) {
  ...
  browserPatchedPromise = System.import('/base/build/test/browser-zone-saucelab-setup').then(() => {
...
browserPatchedPromise.then(() => {
  let testFrameworkPatch = typeof (window as any).Mocha !== 'undefined' ?
      '/base/build/test-env-setup-saucelab-mocha' :
      '/base/build/test/test-env-setup-saucelab-jasmine';
