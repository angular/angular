...
interface ZoneType {
  ...
  /**
   * Zone awaiter util function to handle native `async/await` transformation.
   * Zone.js can not monkey patch native `async/await`, so we still need to
   * make transformation of `async/await`, and this Zone.js `__awaiter`
   * transformation spec use `generator` and `native async/await` instead of
   * `Javascript Promise` generated by `ES2015`. It improves the performance
   * and also make the debug more easier.
   */
  __awaiter<T>(thisArg: any, _arguments: any, generator: () => Generator<T|Promise<T>>): Promise<T>;
}
