...
export function patchOnProperties(obj: any, properties: string[] | null, prototype?: any) {
  if (properties) {
    for (const property of properties) {
      patchProperty(obj, 'on' + property, prototype);
    }
  } else {
    const onProperties = [];
    ...
    for (const onProperty of onProperties) {
      patchProperty(obj, onProperty, prototype);
    }
  }
}
...
let copySymbolPropertiesImpl: ((source: any, destination: any) => void) | null = null;

export function setCopySymbolPropertiesImplForNodeJS(): void {
  if (copySymbolPropertiesImpl) {
    return;
  }

  // This is a tree-shakable implementation that excludes the
  // `copySymbolProperties` function from browser bundles.
  copySymbolPropertiesImpl = (source: any, destination: any) => {
...
export function patchMethod(
  ...
  if (proto && (!(delegate = proto[delegateName]) || !proto.hasOwnProperty(delegateName))) {
    ...
    if (isPropertyWritable(desc)) {
      ...
      copySymbolPropertiesImpl?.(delegate, proto[name]);
    }
  }
  return delegate;
}
