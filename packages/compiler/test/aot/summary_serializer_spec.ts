...
import {
  ...
  serializeSummaries,
} from '@angular/compiler/src/aot/summary_serializer';
...

{
  describe('summary serializer', () => {
    ...
    function init(
      summaries: {[filePath: string]: string} = {},
      ...
      symbolResolver = new StaticSymbolResolver(
        new MockStaticSymbolResolverHost(metadata),
        symbolCache,
        summaryResolver
      );
    }
    ...
    it('should serialize various data correctly', () => {
        ...
        [
          {
            ...
            metadata: {
              ...
              aStaticSymbolWithMembers: symbolCache.get('/tmp/some_symbol.ts', 'someName', [
                'someMember',
              ]),
            },
          },
      ...
      const summaries = deserializeSummaries(
        symbolCache,
        summaryResolver,
        'someFile.d.ts',
        serializedData.json
      ...
      expect(summaries[0].metadata).toEqual({
        ...
        aStaticSymbolWithMembers: symbolCache.get('/tmp/some_symbol.d.ts', 'someName', [
          'someMember',
        ]),
      });
      ...
      expect(summaries[1].type!.type.reference).toBe(
        symbolCache.get('/tmp/some_service.d.ts', 'SomeService')
      );
    });
    ...
    it('should automatically add exported directives / pipes of NgModules that are not source files', () => {
      ...
      expect(summaries[2].symbol).toBe(symbolCache.get('/tmp/external.d.ts', 'SomeExternalPipe'));

      const reexportSummaries = deserializeSummaries(
        symbolCache,
        summaryResolver,
        'someFile.d.ts',
        serializedReexport.json
      ).summaries;
      expect(reexportSummaries.length).toBe(4);
      expect(reexportSummaries[0].symbol).toBe(
        symbolCache.get('/tmp/some_reexport.d.ts', 'ReexportModule')
      );
      expect(reexportSummaries[1].symbol).toBe(
        symbolCache.get('/tmp/some_module.d.ts', 'SomeModule')
      );
      expect(reexportSummaries[2].symbol).toBe(
        symbolCache.get('/tmp/external.d.ts', 'SomeExternalDir')
      );
      expect(reexportSummaries[3].symbol).toBe(
        symbolCache.get('/tmp/external.d.ts', 'SomeExternalPipe')
      );
    });
    ...
    it('should automatically add the metadata of referenced symbols that are not in the source files', () => {
      ...
      expect(summaries[1].metadata).toEqual([
        symbolCache.get('/tmp/external_svc.d.ts', 'SomeService'),
      ]);
      ...
      expect(summaries[2].type!.type.reference).toBe(
        symbolCache.get('/tmp/external_svc.d.ts', 'SomeService')
      );
    ...
    it('should use existing reexports for "importAs" for symbols of libraries', () => {
      ...
      const importAs = deserializeSummaries(
        symbolCache,
        summaryResolver,
        'someFile.d.ts',
        serialized.json
          ...
          [
            {
              ...
              metadata: {
                ...
                expression: {
                  __symbolic: 'resolved',
                  symbol: symbolCache.get('/tmp/test2.ts', 'ref'),
                },
              },
            },
          ],
    ...
    describe('symbol re-exports enabled', () => {
      ...
      it('should create reexports in the ngfactory for symbols of libraries', () => {
        ...
        const deserialized = deserializeSummaries(
          symbolCache,
          summaryResolver,
          'someFile.d.ts',
          serialized.json
    ...
    it('should not create reexports in the ngfactory for external symbols', () => {
      ...
      expect(deserialized.importAs.length).toBe(
        0,
        'Expected no symbols that can be imported from a re-exported location'
      );
    });
  });
}
