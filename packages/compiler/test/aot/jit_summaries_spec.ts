...
describe('aot summaries for jit', () => {
  ...
  function compileApp(
    rootDir: MockDirectory,
    options: {useSummaries?: boolean} & AotCompilerOptions = {}
  ): {genFiles: GeneratedFile[]; outDir: MockDirectory} {
    return compile([rootDir, options.useSummaries ? angularSummaryFiles : angularFiles], {
      ...options,
  ...
  it('should create @Injectable summaries', () => {
    ...
    const genFile = compileApp(rootDir).genFiles.find(
      (f) => f.genFileUrl === '/app/app.module.ngsummary.ts'
    )!;
  ...
  it(
    'should create and use reexports for imported NgModules ' +
    ...
    () => {
      ...
      const lib2ModuleNgSummary = lib2Gen.find(
        (f) => f.genFileUrl === '/lib2/module.ngsummary.ts'
      )!;
      const lib2ReexportNgSummary = lib2Gen.find(
        (f) => f.genFileUrl === '/lib2/reexport.ngsummary.ts'
      )!;

      // ngsummaries should add reexports for imported NgModules from a direct dependency
      expect(toTypeScript(lib2ModuleNgSummary)).toContain(
      ...
      expect(toTypeScript(lib2ReexportNgSummary)).toContain(
      ...
      const lib3ModuleNgSummary = lib3Gen.find(
        (f) => f.genFileUrl === '/lib3/module.ngsummary.ts'
      )!;
      const lib3ReexportNgSummary = lib3Gen.find(
        (f) => f.genFileUrl === '/lib3/reexport.ngsummary.ts'
      )!;
      ...
      expect(lib3ModuleNgSummarySource).toContain(`import * as i5 from '/lib2/reexport.ngsummary'`);
      expect(lib3ModuleNgSummarySource).toMatch(
      ...
      expect(lib3ModuleNgSummarySource).toContain(
      ...
      expect(toTypeScript(lib3ReexportNgSummary)).toContain(
  ...
  it('should not create reexports for external symbols imported by NgModules', () => {
    ...
    expect(lib3ModuleNgSummarySource).toMatch(
      /export function Lib3ModuleNgSummary\(\).*modules:\[{reference:i1\.Lib1Module,/s
    );
    expect(lib3ModuleNgSummarySource).toMatch(
      /export function Lib3ModuleNgSummary\(\).*reference:i3\.ReexportModule,/s
    );
