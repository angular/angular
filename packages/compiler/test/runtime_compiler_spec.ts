      ...
      describe('never resolving loader', () => {
        ...
        it('should throw when using a templateUrl that has not been compiled before', async(() => {
          ...
          TestBed.compileComponents().then(() => {
            expect(() => TestBed.createComponent(SomeCompWithUrlTemplate)).toThrowError(
        ...
        it('should throw when using a templateUrl in a nested component that has not been compiled before', () => {
          ...
          TestBed.compileComponents().then(() => {
            expect(() => TestBed.createComponent(SomeComp)).toThrowError(
  ...
  describe('RuntimeCompiler', () => {
    ...
    beforeEach(fakeAsync(
      inject(
        ...
        (
          _compiler: Compiler,
          _resourceLoader: SpyResourceLoader,
          _dirResolver: MockDirectiveResolver,
          _injector: Injector
        ) => {
          compiler = _compiler;
    ...
    describe('compileModuleSync', () => {
      it('should throw when using a templateUrl that has not been compiled before', () => {
        ...
        expect(() => compiler.compileModuleSync(SomeModule)).toThrowError(
          `Can't compile synchronously as ${stringify(
            SomeCompWithUrlTemplate
          )} is still being loaded!`
        );
      ...
      it('should throw when using a templateUrl in a nested component that has not been compiled before', () => {
        ...
        dirResolver.setDirective(
          ChildComp,
          new Component({selector: 'child-cmp', templateUrl: '/someTpl.html'})
        );
        expect(() => compiler.compileModuleSync(SomeModule)).toThrowError(
          `Can't compile synchronously as ${stringify(ChildComp)} is still being loaded!`
        );
      ...
      it('should allow to use templateUrl components that have been loaded before', fakeAsync(() => {
        @NgModule({
          declarations: [SomeCompWithUrlTemplate],
          entryComponents: [SomeCompWithUrlTemplate],
        })
