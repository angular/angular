...

{
  describe('DirectiveNormalizer', () => {
    ...
    beforeEach(() => {
      resourceLoaderSpy = jasmine
        .createSpy('get')
        .and.callFake((url: string) => `resource(${url})`);
      const resourceLoader = {get: resourceLoaderSpy};
      TestBed.configureCompiler({
        providers: [
          ...TEST_COMPILER_PROVIDERS,
          {provide: ResourceLoader, useValue: resourceLoader},
        ],
      });
    });

    describe('normalizeTemplate', () => {
      it('should throw if no template was specified', inject(
        [DirectiveNormalizer],
        (normalizer: DirectiveNormalizer) => {
          expect(() => normalizeTemplate(normalizer, {})).toThrowError(
            'No template specified for component SomeComp'
          );
        }
      ));
      it('should throw if template is not a string', inject(
        [DirectiveNormalizer],
        (normalizer: DirectiveNormalizer) => {
          expect(() => normalizeTemplate(normalizer, {template: <any>{}})).toThrowError(
            'The template specified for component SomeComp is not a string'
          );
        }
      ));
      it('should throw if templateUrl is not a string', inject(
        [DirectiveNormalizer],
        (normalizer: DirectiveNormalizer) => {
          expect(() => normalizeTemplate(normalizer, {templateUrl: <any>{}})).toThrowError(
            'The templateUrl specified for component SomeComp is not a string'
          );
        }
      ));
      it('should throw if both template and templateUrl are defined', inject(
        [DirectiveNormalizer],
        (normalizer: DirectiveNormalizer) => {
          expect(() =>
            normalizeTemplate(normalizer, {
              template: '',
              templateUrl: '',
            })
      ...
      it('should throw if preserveWhitespaces is not a boolean', inject(
        [DirectiveNormalizer],
        (normalizer: DirectiveNormalizer) => {
          expect(() =>
            normalizeTemplate(normalizer, {
    ...
    describe('inline template', () => {
      it('should store the template', inject(
        [DirectiveNormalizer],
        (normalizer: DirectiveNormalizer) => {
      ...
      it('should resolve styles on the annotation against the moduleUrl', inject(
        [DirectiveNormalizer],
        (normalizer: DirectiveNormalizer) => {
          const template = <CompileTemplateMetadata>(
            normalizeTemplate(normalizer, {template: '', styleUrls: ['test.css']})
          );
          expect(template.styleUrls).toEqual(['package:some/module/test.css']);
        }
      ));

      it('should resolve styles in the template against the moduleUrl and add them to the styles', inject(
        [DirectiveNormalizer],
        (normalizer: DirectiveNormalizer) => {
          const template = <CompileTemplateMetadata>normalizeTemplate(normalizer, {
          ...
          expect(template.styles).toEqual([
            'direct',
            'template ',
            'resource(package:some/module/test.css)',
          ]);
        }
      ));

      it('should use ViewEncapsulation.Emulated by default', inject(
        [DirectiveNormalizer],
        (normalizer: DirectiveNormalizer) => {
          const template = <CompileTemplateMetadata>(
            normalizeTemplate(normalizer, {template: '', styleUrls: ['test.css']})
      ...
      it('should use default encapsulation provided by CompilerConfig', inject(
    ...
    it('should load a template from a url that is resolved against moduleUrl', inject(
      [DirectiveNormalizer],
      (normalizer: DirectiveNormalizer) => {
        const template = <CompileTemplateMetadata>(
          normalizeTemplate(normalizer, {templateUrl: 'sometplurl.html', styleUrls: ['test.css']})
        );
    ...
    it('should resolve styles in the template against the templateUrl and add them to the styles', inject(
      [DirectiveNormalizer],
      (normalizer: DirectiveNormalizer) => {
        ...
        const template = <CompileTemplateMetadata>(
          normalizeTemplate(normalizer, {templateUrl: 'tpl/sometplurl.html', styles: ['direct']})
        );
        expect(template.styles).toEqual([
          'direct',
          'template ',
          'resource(package:some/module/tpl/test.css)',
        ]);
      }
    ));

    describe('externalStylesheets', () => {
      it('should load an external stylesheet', inject(
        [DirectiveNormalizer],
        (normalizer: DirectiveNormalizer) => {
          const template = <CompileTemplateMetadata>normalizeTemplate(normalizer, {
            template: '',
          ...
          expect(template.externalStylesheets[0]).toEqual(
      ...
      it('should load stylesheets referenced by external stylesheets and inline them', inject(
        [DirectiveNormalizer],
        (normalizer: DirectiveNormalizer) => {
          resourceLoaderSpy.and.callFake((url: string) => {
            switch (url) {
    ...
    describe('caching', () => {
      it('should work for templateUrl', inject(
        [DirectiveNormalizer],
        (normalizer: DirectiveNormalizer) => {
          const prenormMeta = {
    ...
    describe('normalizeLoadedTemplate', () => {
      it('should store the viewEncapsulation in the result', inject(
        [DirectiveNormalizer],
        (normalizer: DirectiveNormalizer) => {
      ...
      it('should use preserveWhitespaces setting from compiler config if none provided', inject(
        [DirectiveNormalizer, CompilerConfig],
        (normalizer: DirectiveNormalizer, config: CompilerConfig) => {
          const template = <CompileTemplateMetadata>normalizeTemplate(normalizer, {template: ''});
          expect(template.preserveWhitespaces).toBe(config.preserveWhitespaces);
        }
      ));

      it('should store the preserveWhitespaces=false in the result', inject(
        [DirectiveNormalizer],
        (normalizer: DirectiveNormalizer) => {
          const template = <CompileTemplateMetadata>(
            normalizeTemplate(normalizer, {preserveWhitespaces: false, template: ''})
          );
          expect(template.preserveWhitespaces).toBe(false);
        }
      ));

      it('should store the preserveWhitespaces=true in the result', inject(
        [DirectiveNormalizer],
        (normalizer: DirectiveNormalizer) => {
          const template = <CompileTemplateMetadata>(
            normalizeTemplate(normalizer, {preserveWhitespaces: true, template: ''})
          );
          expect(template.preserveWhitespaces).toBe(true);
        }
      ));

      it('should keep the template as html', inject(
        [DirectiveNormalizer],
        (normalizer: DirectiveNormalizer) => {
          const template = <CompileTemplateMetadata>normalizeTemplate(normalizer, {
            template: 'a',
      ...
      it('should collect ngContent', inject(
        [DirectiveNormalizer],
        (normalizer: DirectiveNormalizer) => {
          const template = <CompileTemplateMetadata>normalizeTemplate(normalizer, {
      ...
      it('should normalize ngContent wildcard selector', inject(
        [DirectiveNormalizer],
        (normalizer: DirectiveNormalizer) => {
          const template = <CompileTemplateMetadata>normalizeTemplate(normalizer, {
            template:
      ...
      it('should collect top level styles in the template', inject(
        [DirectiveNormalizer],
        (normalizer: DirectiveNormalizer) => {
          const template = <CompileTemplateMetadata>normalizeTemplate(normalizer, {
      ...
      it('should collect styles inside elements', inject(
        [DirectiveNormalizer],
        (normalizer: DirectiveNormalizer) => {
          const template = <CompileTemplateMetadata>normalizeTemplate(normalizer, {
      ...
      it('should collect styleUrls in the template and add them to the styles', inject(
        [DirectiveNormalizer],
        (normalizer: DirectiveNormalizer) => {
          const template = <CompileTemplateMetadata>normalizeTemplate(normalizer, {
      ...
      it('should collect styleUrls in elements and add them to the styles', inject(
        [DirectiveNormalizer],
        (normalizer: DirectiveNormalizer) => {
          const template = <CompileTemplateMetadata>normalizeTemplate(normalizer, {
      ...
      it('should ignore link elements with non stylesheet rel attribute', inject(
        [DirectiveNormalizer],
        (normalizer: DirectiveNormalizer) => {
          const template = <CompileTemplateMetadata>normalizeTemplate(normalizer, {
      ...
      it('should ignore link elements with absolute urls but non package: scheme', inject(
        [DirectiveNormalizer],
        (normalizer: DirectiveNormalizer) => {
          const template = <CompileTemplateMetadata>normalizeTemplate(normalizer, {
      ...
      it('should extract @import style urls and add them to the styles', inject(
        [DirectiveNormalizer],
        (normalizer: DirectiveNormalizer) => {
          const template = <CompileTemplateMetadata>normalizeTemplate(normalizer, {
      ...
      it('should not resolve relative urls in inline styles', inject(
        [DirectiveNormalizer],
        (normalizer: DirectiveNormalizer) => {
          const template = <CompileTemplateMetadata>normalizeTemplate(normalizer, {
      ...
      it('should resolve relative style urls in styleUrls', inject(
        [DirectiveNormalizer],
        (normalizer: DirectiveNormalizer) => {
          const template = <CompileTemplateMetadata>normalizeTemplate(normalizer, {
            styleUrls: ['test.css'],
      ...
      it('should resolve relative style urls in styleUrls with http directive url', inject(
        [DirectiveNormalizer],
        (normalizer: DirectiveNormalizer) => {
          const template = <CompileTemplateMetadata>normalizeTemplate(normalizer, {
      ...
      it('should normalize ViewEncapsulation.Emulated to ViewEncapsulation.None if there are no styles nor stylesheets', inject(
        [DirectiveNormalizer],
        (normalizer: DirectiveNormalizer) => {
          const template = <CompileTemplateMetadata>normalizeTemplate(normalizer, {
      ...
      it('should ignore ng-content in elements with ngNonBindable', inject(
        [DirectiveNormalizer],
        (normalizer: DirectiveNormalizer) => {
          const template = <CompileTemplateMetadata>normalizeTemplate(normalizer, {
      ...
      it('should still collect <style> in elements with ngNonBindable', inject(
        [DirectiveNormalizer],
        (normalizer: DirectiveNormalizer) => {
          const template = <CompileTemplateMetadata>normalizeTemplate(normalizer, {
