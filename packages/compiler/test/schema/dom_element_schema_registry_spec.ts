...
import {CustomElementsManifestIndex} from '../../src/schema/custom_elements_manifest_schema';
import {
  _ATTR_TO_PROP,
  ...
  describe('custom element schemas', () => {
    ...
    beforeEach(() => {
      customRegistry = new DomElementSchemaRegistry(
        new CustomElementsManifestIndex([
          {
            tagName: 'my-button',
            properties: [
              {name: 'label'},
          ...
          {tagName: 'my-icon', properties: [], attributes: [], events: []},
        ]),
      );
    });
