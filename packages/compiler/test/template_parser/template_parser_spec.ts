    ...
    function() {
          ...
          TestBed.configureCompiler({
            providers: [
              {provide: Console, useValue: console}, {
                ...
                useValue: new CompilerConfig({enableLegacyTemplate: true})
              }
            ],
          });
        });
      ...
      describe(
          'TemplateParser', () => {
            beforeEach(() => {
              TestBed.configureCompiler(
                  {providers: [TEST_COMPILER_PROVIDERS, MOCK_SCHEMA_REGISTRY]});
            });
...
Binding to attribute 'onEvent' is disallowed for security reasons ("<my-component [ERROR ->][attr.onEvent]="bar"></my-component>"): TestComp@0:14`);
                ...
                it('should parse bound properties via {{...}} and not report them as attributes',
                   () => {
                     expect(humanizeTplAst(parse('<div prop="{{v}}">', []))).toEqual([
                       [ElementAst, 'div'],
                       [
                         BoundElementPropertyAst, PropertyBindingType.Property, 'prop', '{{ v }}',
                         null
                       ]
                     ]);
                   });
                ...
                it('should parse bound properties via bind-animate- and not report them as attributes',
                   () => {
                     expect(humanizeTplAst(
                                parse('<div bind-animate-someAnimation="value2">', [], [], [])))
                         .toEqual([
                           [ElementAst, 'div'],
                           [
                             BoundElementPropertyAst, PropertyBindingType.Animation,
                             'someAnimation', 'value2', null
                           ]
                         ]);
                   });
                ...
                it('should parse bound properties via [@] and not report them as attributes',
                   () => {
                     expect(humanizeTplAst(parse('<div [@someAnimation]="value2">', [], [], [])))
                         .toEqual([
                           [ElementAst, 'div'],
                           [
                             BoundElementPropertyAst, PropertyBindingType.Animation,
              ...
              describe('events', () => {
                ...
                it('should allow events on explicit embedded templates that are emitted by a directive',
                   () => {
                     ...
                     expect(humanizeTplAst(parse('<ng-template (e)="f"></ng-template>', [dirA])))
                         .toEqual([
                           [EmbeddedTemplateAst],
                ...
                it('should order directives by the directives array in the View and match them only once',
                   () => {
                     ...
                     expect(humanizeTplAst(parse('<div a c b a b>', [dirA, dirB, dirC]))).toEqual([
                       [ElementAst, 'div'], [AttrAst, 'a', ''], [AttrAst, 'c', ''],
                       [AttrAst, 'b', ''], [AttrAst, 'a', ''], [AttrAst, 'b', ''],
                       [DirectiveAst, dirA], [DirectiveAst, dirB], [DirectiveAst, dirC]
                     ]);
                   });
                ...
                it('should locate directives in inline templates', () => {
                  const dirTemplate =
                      compileDirectiveMetadataCreate({
                        selector: 'template',
                        type: createTypeMeta(
                            {reference: {filePath: someModuleUrl, name: 'onTemplate'}})
                      }).toSummary();
              ...
              describe('providers', () => {
                ...
                it('should mark queried providers as eager', () => {
                  const provider0 = createProvider('service0');
                  const provider1 = createProvider('service1');
                  const dirA = createDir(
                      '[dirA]', {providers: [provider0, provider1], queries: ['service0']});
                   ...
                   () => {
                     const pizzaTestDirective =
                         compileDirectiveMetadataCreate({
                           selector: 'pizza-test',
                           type: createTypeMeta(
                               {reference: {filePath: someModuleUrl, name: 'Pizza'}}),
                           exportAs: 'pizza, cheeseSauceBread'
                         }).toSummary();

                     const template =
                         '<pizza-test #food="pizza" #yum="cheeseSauceBread"></pizza-test>';

                     expect(humanizeTplAst(parse(template, [pizzaTestDirective]))).toEqual([
                       [ElementAst, 'pizza-test'],
                       [
                         ReferenceAst, 'food',
                         createTokenForReference(pizzaTestDirective.type.reference)
                       ],
                       [
                         ReferenceAst, 'yum',
                         createTokenForReference(pizzaTestDirective.type.reference)
...
Reference "#a" is defined several times ("<div #a></div><div [ERROR ->]#a></div>"): TestComp@0:19`);
                   ...
                   () => {
                     ...
                     const chocolateDirective =
                         compileDirectiveMetadataCreate({
                           selector: '[chocolate]',
                           type: createTypeMeta(
                               {reference: {filePath: someModuleUrl, name: 'Chocolate'}}),
                           exportAs: 'chocolate'
                         }).toSummary();

                     const template = '<div dessert-pizza chocolate #snack="chocolate"></div>';
                     const compileTemplate = () =>
                         parse(template, [pizzaDirective, chocolateDirective]);
                ...
                it('should not throw error when there is same reference name in different templates',
                   () => {
                     expect(
                         () => parse(
                             '<div #a><ng-template #a><span>OK</span></ng-template></div>', []))
                         .not.toThrowError();
                   });
              ...
              describe('explicit templates', () => {
                ...
                it('should create embedded templates for <ng-template> elements regardless the namespace',
                   () => {
                     expect(humanizeTplAst(parse('<svg><ng-template></ng-template></svg>', [])))
                         .toEqual([
                           [ElementAst, ':svg:svg'],
                           [EmbeddedTemplateAst],
                         ]);
                ...
                it('should not locate directives in variables', () => {
                  ...
                  expect(humanizeTplAst(parse('<ng-template let-a="b"></ng-template>', [dirA])))
                      .toEqual([
                        [EmbeddedTemplateAst],
                        [VariableAst, 'a', 'b'],
                      ]);
              ...
              describe('inline templates', () => {
                ...
                it('should parse variables via let ...', () => {
                  ...
                  expect(humanizeTplAst(parse('<div data-*ngIf="let a=b">', [
                  ]))).toEqual(targetAst);
                });
                ...
                it('should parse variables via as ...', () => {
                  ...
                  expect(humanizeTplAst(parse('<div *ngIf="expr as local">', [ngIf])))
                      .toEqual(targetAst);
                });
                ...
                describe('directives', () => {
                  it('should locate directives in property bindings', () => {
                    ...
                    expect(humanizeTplAst(parse('<div template="a b" b>', [dirA, dirB]))).toEqual([
                      ...
                      [BoundDirectivePropertyAst, 'a', 'b'], [ElementAst, 'div'],
                      [AttrAst, 'b', ''], [DirectiveAst, dirB]
                    ]);
                  });
            ...
            describe('content projection', () => {
              ...
              function createComp(
                ...
                return compileDirectiveMetadataCreate({
                         ...
                         template:
                             compileTemplateMetadata({ngContentSelectors: ngContentSelectors})
                       })
                    .toSummary();
              }
              ...
              describe('project text nodes', () => {
                it('should project text nodes with wildcard selector', () => {
                  expect(humanizeContentProjection(
                             parse('<div>hello</div>', [createComp('div', ['*'])])))
                      .toEqual([
                        ['div', null],
                        ['#text(hello)', 0],
                      ]);
              ...
              describe('embedded templates', () => {
                it('should project embedded templates with wildcard selector', () => {
                  expect(humanizeContentProjection(parse(
                             '<div><ng-template></ng-template></div>', [createComp('div', ['*'])])))
                      .toEqual([
                        ['div', null],
                        ['template', 0],
                      ]);
              ...
              it('should project children of components with ngNonBindable', () => {
                expect(humanizeContentProjection(parse(
                           '<div ngNonBindable>{{hello}}<span></span></div>',
                           [createComp('div', ['*'])])))
...
Property binding a not used by any directive on an embedded template. Make sure that the property name is spelled correctly and all directives are listed in the "@NgModule.declarations". ("[ERROR ->]<div *a="b"></div>"): TestComp@0:0`);
            ...
            describe('ignore elements', () => {
              ...
              it('should convert <ng-content> elements into regular elements inside of elements with ngNonBindable',
                 () => {
                   expect(humanizeTplAst(
                              parse('<div ngNonBindable><ng-content></ng-content>a</div>', [])))
                       .toEqual([
            ...
            describe('source spans', () => {
              ...
              it('should support directive in namespace', () => {
                ...
                expect(humanizeTplAstSourceSpans(parse(
                           '<svg><circle /><use xlink:href="Port" /></svg>', [tagSel, attrSel])))
                    .toEqual([
      ...
      describe('whitespaces removal', () => {
        ...
        it('should remove whitespace between ICU expansions when not preserving whitespaces',
           () => {
             ...
             // ICU expansions are converted to `<ng-container>` tags and all blank text nodes are
             // reomved
