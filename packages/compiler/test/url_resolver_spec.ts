  ...
  describe('UrlResolver', () => {
    ...
    describe('absolute base url', () => {
      ...
      it('should replace the base path', () => {
        ...
        expect(resolver.resolve('http://www.foo.com/baz', './bar')).toEqual(
      ...
      it('should support ".." in the path', () => {
        ...
        expect(resolver.resolve('http://www.foo.com/1/2/3/', '../../bar')).toEqual(
          'http://www.foo.com/1/bar'
        );
        expect(resolver.resolve('http://www.foo.com/1/2/3/', '../biz/bar')).toEqual(
          'http://www.foo.com/1/2/biz/bar'
        );
        expect(resolver.resolve('http://www.foo.com/1/2/baz', '../../bar')).toEqual(
          'http://www.foo.com/bar'
        );
      });

      it('should ignore the base path when the url has a scheme', () => {
        expect(resolver.resolve('http://www.foo.com', 'http://www.bar.com')).toEqual(
          'http://www.bar.com'
        );
      });
    ...
    describe('packages', () => {
      it('should resolve a url based on the application package', () => {
        resolver = new UrlResolver('my_packages_dir');
        expect(resolver.resolve(null!, 'package:some/dir/file.txt')).toEqual(
          'my_packages_dir/some/dir/file.txt'
        );
        expect(resolver.resolve(null!, 'some/dir/file.txt')).toEqual('some/dir/file.txt');
      });

      it('should contain a default value of "/" when nothing is provided', inject(
        [UrlResolver],
      ...
      it('should resolve a package value when present within the baseurl', () => {
        ...
        expect(resolver.resolve('package:some_dir/', 'matias.html')).toEqual(
