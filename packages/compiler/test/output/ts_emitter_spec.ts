...

{
  ...
  describe('TypeScriptEmitter', () => {
    ...
    it('should declare variables', () => {
      ...
      expect(emitStmt(someVar.set(o.literal(1)).toDeclStmt(null, [o.StmtModifier.Final]))).toEqual(
        `const someVar:any = 1;`
      );
      expect(
        emitStmt(someVar.set(o.literal(1)).toDeclStmt(null, [o.StmtModifier.Exported]))
      ).toEqual(`export var someVar:any = 1;`);
      expect(emitStmt(someVar.set(o.literal(1)).toDeclStmt(o.INT_TYPE))).toEqual(
        `var someVar:number = 1;`
      );
      expect(emitStmt(someVar.set(o.literal(1)).toDeclStmt(o.INFERRED_TYPE))).toEqual(
        `var someVar = 1;`
      );
    });
    ...
    describe('declare variables with ExternExpressions as values', () => {
      it('should create no reexport if the identifier is in the same module', () => {
        ...
        expect(
          emitStmt(
            someVar
              .set(o.importExpr(sameModuleIdentifier))
      ...
      it('should create no reexport if the variable is not exported', () => {
        expect(emitStmt(someVar.set(o.importExpr(externalModuleIdentifier)).toDeclStmt())).toEqual(
          [
            ...
            `var someVar:any = i0.someExternalId;`,
          ].join('\n')
        );
      });
      ...
      it('should create no reexport if the variable is typed', () => {
        expect(
          emitStmt(
            someVar
              .set(o.importExpr(externalModuleIdentifier))
      ...
      it('should create a reexport', () => {
        expect(
          ...
          [`export {someExternalId as someVar} from 'somePackage/someOtherPath';`, ``].join('\n')
        );
      });

      it('should create multiple reexports from the same file', () => {
        const someVar2 = o.variable('someVar2');
        const externalModuleIdentifier2 = new o.ExternalReference(
          anotherModuleUrl,
          'someExternalId2',
          null
        );
        expect(
          emitStmt([
            someVar
              ...
              .toDeclStmt(null, [o.StmtModifier.Exported]),
            someVar2
              .set(o.importExpr(externalModuleIdentifier2))
              .toDeclStmt(null, [o.StmtModifier.Exported]),
          ])
        ).toEqual(
          [
    ...
    it('should read and write variables', () => {
      ...
      expect(emitStmt(someVar.set(o.variable('someOtherVar').set(o.literal(1))).toStmt())).toEqual(
        `someVar = (someOtherVar = 1);`
      );
    });

    it('should read and write keys', () => {
      expect(emitStmt(o.variable('someMap').key(o.variable('someKey')).toStmt())).toEqual(
        `someMap[someKey];`
      );
      expect(
        emitStmt(o.variable('someMap').key(o.variable('someKey')).set(o.literal(1)).toStmt())
      ).toEqual(`someMap[someKey] = 1;`);
    });

    it('should read and write properties', () => {
      expect(emitStmt(o.variable('someObj').prop('someProp').toStmt())).toEqual(
        `someObj.someProp;`
      );
      expect(emitStmt(o.variable('someObj').prop('someProp').set(o.literal(1)).toStmt())).toEqual(
        `someObj.someProp = 1;`
      );
    });
    ...
    it('should invoke functions and methods and constructors', () => {
      ...
      expect(
        emitStmt(
          o
            .variable('someObj')
            .callMethod('someMethod', [o.literal(1)])
            .toStmt()
        )
    ...
    it('should support builtin methods', () => {
      ...
      expect(
        emitStmt(
          o
            .variable('fn')
            .callMethod(o.BuiltinMethod.Bind, [o.variable('someObj')])
            .toStmt()
        )
    ...
    it('should support external identifiers', () => {
      ...
      expect(emitStmt(o.importExpr(externalModuleIdentifier).toStmt())).toEqual(
        [`import * as i0 from 'somePackage/someOtherPath';`, `i0.someExternalId;`].join('\n')
      );
    });
    ...
    it('should support function expressions', () => {
      ...
      expect(
        emitStmt(o.fn([], [new o.ReturnStatement(o.literal(1))], o.INT_TYPE).toStmt())
    ...
    it('should support function statements', () => {
      ...
      expect(
        emitStmt(new o.DeclareFunctionStmt('someFn', [], [], null, [o.StmtModifier.Exported]))
      ).toEqual(['export function someFn():void {', '}'].join('\n'));
      expect(
        emitStmt(
          new o.DeclareFunctionStmt('someFn', [], [new o.ReturnStatement(o.literal(1))], o.INT_TYPE)
        )
    ...
    it('should support if stmt', () => {
      ...
      expect(emitStmt(new o.IfStmt(o.variable('cond'), [trueCase], [falseCase]))).toEqual(
        ['if (cond) {', '  trueCase();', '} else {', '  falseCase();', '}'].join('\n')
      );
    });
    ...
    it('should support try/catch', () => {
      ...
      const catchStmt = o
        .variable('catchFn')
        .callFn([o.CATCH_ERROR_VAR, o.CATCH_STACK_VAR])
        .toStmt();
    ...
    describe('classes', () => {
      ...
      it('should support declaring classes', () => {
        ...
        expect(
          emitStmt(
            new o.ClassStmt('SomeClass', null!, [], [], null!, [], [o.StmtModifier.Exported])
          )
      ...
      it('should support declaring constructors', () => {
        ...
        expect(
          ...
          ['class SomeClass {', '  constructor() {', '    super(someParam);', '  }', '}'].join('\n')
        );
        expect(
          emitStmt(
            new o.ClassStmt(
          ...
          ['class SomeClass {', '  constructor() {', '    this.someMethod();', '  }', '}'].join(
            '\n'
          )
        );
      });
      ...
      it('should support declaring fields', () => {
        expect(
          emitStmt(
            new o.ClassStmt('SomeClass', null!, [new o.ClassField('someField')], [], null!, [])
          )
        ...
        expect(
          emitStmt(
            new o.ClassStmt(
              'SomeClass',
              null!,
              [new o.ClassField('someField', o.INT_TYPE)],
              [],
              null!,
              []
            )
        ...
        expect(
          emitStmt(
            new o.ClassStmt(
              ...
              null!,
              [],
              [new o.ClassGetter('someGetter', [], null!, [o.StmtModifier.Private])],
              null!,
              []
            )
          )
      ...
      it('should support methods', () => {
        expect(
          emitStmt(
            new o.ClassStmt('SomeClass', null!, [], [], null!, [
        ...
        expect(
          ...
          ['class SomeClass {', '  someMethod():void {', '    this.someMethod();', '  }', '}'].join(
            '\n'
          )
        );
      });
    ...
    it('should support builtin types', () => {
      ...
      expect(emitStmt(writeVarExpr.toDeclStmt(o.DYNAMIC_TYPE))).toEqual(
        'var a:any = (null as any);'
      );
      expect(emitStmt(writeVarExpr.toDeclStmt(o.BOOL_TYPE))).toEqual(
        'var a:boolean = (null as any);'
      );
      expect(emitStmt(writeVarExpr.toDeclStmt(o.INT_TYPE))).toEqual(
        'var a:number = (null as any);'
      );
      expect(emitStmt(writeVarExpr.toDeclStmt(o.NUMBER_TYPE))).toEqual(
        'var a:number = (null as any);'
      );
      expect(emitStmt(writeVarExpr.toDeclStmt(o.STRING_TYPE))).toEqual(
        'var a:string = (null as any);'
      );
      expect(emitStmt(writeVarExpr.toDeclStmt(o.FUNCTION_TYPE))).toEqual(
        'var a:Function = (null as any);'
      );
    });

    it('should support external types', () => {
      const writeVarExpr = o.variable('a').set(o.NULL_EXPR);
      expect(emitStmt(writeVarExpr.toDeclStmt(o.importType(sameModuleIdentifier)))).toEqual(
        'var a:someLocalId = (null as any);'
      );
      ...
      expect(
        emitStmt(
          o
            .variable('a')
            .set(o.NULL_EXPR)
            .toDeclStmt(o.expressionType(o.variable('b')))
        )
    ...
    it('should support combined types', () => {
      const writeVarExpr = o.variable('a').set(o.NULL_EXPR);
      expect(emitStmt(writeVarExpr.toDeclStmt(new o.ArrayType(null!)))).toEqual(
        'var a:any[] = (null as any);'
      );
      expect(emitStmt(writeVarExpr.toDeclStmt(new o.ArrayType(o.INT_TYPE)))).toEqual(
        'var a:number[] = (null as any);'
      );

      expect(emitStmt(writeVarExpr.toDeclStmt(new o.MapType(null)))).toEqual(
        'var a:{[key: string]:any} = (null as any);'
      );
      expect(emitStmt(writeVarExpr.toDeclStmt(new o.MapType(o.INT_TYPE)))).toEqual(
        'var a:{[key: string]:number} = (null as any);'
      );
    });
    ...
    describe('comments', () => {
      ...
      it('should support multiline comments', () => {
        expect(emitStmt(new o.CommentStmt('Multiline comment', true))).toBe(
          '/* Multiline comment */'
        );
      ...
      it('should support JSDoc comments', () => {
        expect(emitStmt(new o.JSDocCommentStmt([{text: 'Intro comment'}]))).toBe(
          `/**\n * Intro comment\n */`
        );
        expect(
          emitStmt(new o.JSDocCommentStmt([{tagName: o.JSDocTagName.Desc, text: 'description'}]))
        ).toBe(`/**\n * @desc description\n */`);
        expect(
          emitStmt(
    ...
    describe('emitter context', () => {
      it('should be able to back to the generating span', () => {
        ...
        const returnSpan = new ParseSourceSpan(
          new ParseLocation(file, 100, 10, 10),
          new ParseLocation(file, 200, 20, 10)
        );
        const referenceSpan = new ParseSourceSpan(
          new ParseLocation(file, 150, 15, 10),
          new ParseLocation(file, 175, 17, 10)
        );
        const statements = [
          new o.ClassStmt('SomeClass', null, [], [], new o.ClassMethod(null, [], []), [
            new o.ClassMethod(
              'someMethod',
              [new o.FnParam('a', o.INT_TYPE)],
              [
        ...
        expect(spanOf('someVar', returnLoc)).toEqual(
          referenceSpan,
          'return span calculated incorrectly'
        );
      });
    });
  });
