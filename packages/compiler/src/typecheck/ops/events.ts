    ...
    for (const output of this.outputs) {
      ...
      if (output.type === ParsedEventType.LegacyAnimation) {
        ...
        if (manifestCheckType !== null) {
          // `TemplateSymbolBuilder` locates unclaimed event bindings through a source-mapped
          // `addEventListener` access. Manifest events use an explicit parameter type instead of
          // calling that API, so emit a non-invoked access as symbol metadata for language-service
          // quick info and navigation. Use `document` rather than the manifest instance type so a
          // malformed exported class cannot introduce an unrelated property-access diagnostic.
