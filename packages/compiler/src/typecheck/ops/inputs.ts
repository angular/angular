...
import {
  getCustomElementsManifestAttributeCheckType,
...
export class TcbUnclaimedInputsOp extends TcbOp {
  ...
  private checkCustomElementTextAttribute(attribute: TextAttribute): void {
    ...
    const checkType = getCustomElementsManifestAttributeCheckType(
      this.tcb.env.config,
      this.target.name,
      attribute.name,
    );
    ...
    const expression = new TcbExpr(`(${TcbExpr.quoteAndEscape(attribute.value)})`).addParseSpanInfo(
