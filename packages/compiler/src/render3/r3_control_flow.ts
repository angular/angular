...
const ALLOWED_FOR_LOOP_LET_VARIABLES = new Set<keyof t.ForLoopBlockContext>([
  '$index',
  '$first',
  '$last',
  '$even',
...
function parseLetParameter(
  ...
  for (const part of parts) {
    ...
    const variableName = (
      expressionParts.length === 2 ? expressionParts[1].trim() : ''
    ) as keyof t.ForLoopBlockContext;
