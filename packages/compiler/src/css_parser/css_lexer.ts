...
export function generateErrorMessage(
  ...
  return (
    `${message} at column ${row}:${column} in expression [` +
...
export class CssScanner {
  ...
  _scan(): CssToken | null {
    ...
    if (
      digitA ||
      (isModifier && (peekPeek == chars.$PERIOD || digitB)) ||
  ...
  scanString(): CssToken | null {
    if (
      this.assertCondition(
        isStringStart(this.peek, this.peekPeek),
  ...
  scanIdentifier(): CssToken | null {
    if (
      this.assertCondition(
        isIdentifierStart(this.peek, this.peekPeek),
  ...
  error(
    message: string,
    errorTokenValue: string | null = null,
    doNotAdvance: boolean = false
  ): CssToken {
