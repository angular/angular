  ...
  for (const op of ops) {
    ...
    switch (op.kind) {
      ...
      case ir.OpKind.Listener:
        const listenerFn = reifyListenerHandler(unit, op.handlerFnName!, op.handlerOps);
        ir.OpList.replace(
            op,
            ng.listener(
                op.name,
...
function reifyListenerHandler(
    unit: CompilationUnit, name: string, handlerOps: ir.OpList<ir.UpdateOp>): o.FunctionExpr {
