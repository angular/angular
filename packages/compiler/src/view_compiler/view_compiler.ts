...
import {
  ...
  depDef,
  lifecycleHookToNodeFlag,
  providerDef,
} from './provider_compiler';
    ...
    if (!component.isHost) {
      ...
      if (template.animations && template.animations.length) {
        customRenderData.push(
          new o.LiteralMapEntry(
            'animation',
            convertValueToOutputAst(outputCtx, template.animations),
            true
          )
        );
      }
      ...
      outputCtx.statements.push(
        renderComponentVar
          .set(
            o
              .importExpr(Identifiers.createRendererType2)
              .callFn([
          ...
          .toDeclStmt(o.importType(Identifiers.RendererType2), [
...
class ViewBuilder implements TemplateAstVisitor, LocalResolver {
  ...
  visitAll(variables: VariableAst[], astNodes: TemplateAst[]) {
    ...
    if (!this.parent) {
      this.component.viewQueries.forEach((query, queryIndex) => {
        ...
        this.nodes.push(() => ({
          ...
          nodeFlags: flags,
          nodeDef: o
            .importExpr(Identifiers.queryDef)
            .callFn([
              o.literal(flags),
              o.literal(queryId),
              new o.LiteralMapExpr([
                new o.LiteralMapEntry(query.propertyName, o.literal(bindingType), false),
              ]),
            ]),
        }));
      });
    ...
    if (this.parent && (astNodes.length === 0 || needsAdditionalRootNode(astNodes))) {
      ...
      this.nodes.push(() => ({
        ...
        nodeDef: o
          .importExpr(Identifiers.anchorDef)
          .callFn([o.literal(NodeFlags.None), o.NULL_EXPR, o.NULL_EXPR, o.literal(0)]),
      }));
    }
  }
  ...
  build(targetStatements: o.Statement[] = []): o.Statement[] {
    ...
    const viewFactory = new o.DeclareFunctionStmt(
      this.viewName,
      [new o.FnParam(LOG_VAR.name!)],
      [
        new o.ReturnStatement(
          o
            .importExpr(Identifiers.viewDef)
            .callFn([
              o.literal(viewFlags),
  ...
  visitNgContent(ast: NgContentAst, context: any): any {
    ...
    this.nodes.push(() => ({
      ...
      nodeDef: o
        .importExpr(Identifiers.ngContentDef)
  ...
  visitText(ast: TextAst, context: any): any {
    ...
    this.nodes.push(() => ({
      ...
      nodeDef: o
        .importExpr(Identifiers.textDef)
        .callFn([
  ...
  visitBoundText(ast: BoundTextAst, context: any): any {
    ...
    const updateRendererExpressions = inter.expressions.map((expr, bindingIndex) =>
  ...
  visitElement(ast: ElementAst, context: any): any {
    ...
    if (elName) {
      ...
      if (hostBindings.length) {
        updateRendererExpressions = hostBindings.map((hostBinding, bindingIndex) =>
          this._preprocessUpdateExpression({
        ...
        inputDefs = hostBindings.map((hostBinding) =>
          elementBindingDef(hostBinding.inputAst, hostBinding.dirAst)
        );
      }
      outputDefs = usedEvents.map(([target, eventName]) =>
        o.literalArr([o.literal(target), o.literal(eventName)])
      );
    }
    ...
    this.nodes[nodeIndex] = () => ({
      ...
      nodeFlags: NodeFlags.TypeElement | flags,
      nodeDef: o
        .importExpr(Identifiers.elementDef)
        .callFn([
          o.literal(checkIndex),
          o.literal(flags),
          queryMatchesExpr,
  ...
  private _visitElementOrTemplate(
    ...
    hostBindings: {
      context: o.Expression;
      inputAst: BoundElementPropertyAst;
      dirAst: DirectiveAst;
    }[];
  ...
  private _visitDirective(
    ...
    let {flags, queryMatchExprs, providerExpr, depsExpr} = this._visitProviderOrDirective(
      providerAst,
      queryMatches
    );
    ...
    if (dirAst.inputs.length || (flags & (NodeFlags.DoCheck | NodeFlags.OnInit)) > 0) {
      updateDirectiveExpressions = dirAst.inputs.map((input, bindingIndex) =>
        this._preprocessUpdateExpression({
          nodeIndex,
          bindingIndex,
    ...
    const dirContextExpr = o
      .importExpr(Identifiers.nodeValue)
      .callFn([VIEW_VAR, o.literal(nodeIndex)]);
    ...
    this.nodes[nodeIndex] = () => ({
      ...
      nodeDef: o
        .importExpr(Identifiers.directiveDef)
        .callFn([
          o.literal(checkIndex),
          o.literal(flags),
  ...
  private _addProviderNode(data: {
    ...
    this.nodes.push(() => ({
      ...
      nodeDef: o
        .importExpr(Identifiers.providerDef)
        .callFn([
  ...
  private _visitProviderOrDirective(
    providerAst: ProviderAst,
    queryMatches: QueryMatch[]
  ): {
    flags: NodeFlags;
    ...
    const {providerExpr, depsExpr, flags: providerFlags, tokenExpr} = providerDef(
      this.outputCtx,
      providerAst
    );
    return {
  ...
  getLocal(name: string): o.Expression | null {
    ...
    for (
      let currBuilder: ViewBuilder | null = this;
      currBuilder;
  ...
  private _createLiteralArrayConverter(
    sourceSpan: ParseSourceSpan,
    argCount: number
  ): BuiltinConverter {
    if (argCount === 0) {
    ...
    this.nodes.push(() => ({
      ...
      nodeDef: o
        .importExpr(Identifiers.pureArrayDef)
        .callFn([o.literal(checkIndex), o.literal(argCount)]),
    }));
  ...
  private _createLiteralMapConverter(
    ...
    this.nodes.push(() => ({
      ...
      nodeDef: o.importExpr(Identifiers.pureObjectDef).callFn([o.literal(checkIndex), map]),
    }));

    return (args: o.Expression[]) => callCheckStmt(checkIndex, args);
  }

  private _createPipeConverter(
    ...
    if (pipe.pure) {
      ...
      this.nodes.push(() => ({
        ...
        nodeDef: o
          .importExpr(Identifiers.purePipeDef)
          .callFn([o.literal(checkIndex), o.literal(argCount)]),
      }));
  ...
  private _createPipe(sourceSpan: ParseSourceSpan | null, pipe: CompilePipeSummary): number {
    ...
    this.nodes.push(() => ({
      ...
      nodeDef: o
        .importExpr(Identifiers.pipeDef)
        .callFn([
          o.literal(flags),
          this.outputCtx.importExpr(pipe.type.reference),
          o.literalArr(depExprs),
        ]),
    }));
    return nodeIndex;
  }
  ...
  private _createNodeExpressions(): {
    ...
    const nodeDefExprs = this.nodes.map((factory, nodeIndex) => {
      ...
      const logWithNodeDef =
        nodeFlags & NodeFlags.CatRenderNode
    ...
    function createUpdateStatements(
      ...
      const exprs = expressions.map(({sourceSpan, context, value}) => {
        ...
        updateStmts.push(
          ...stmts.map((stmt: o.Statement) =>
            o.applySourceSpanToStatementIfNeeded(stmt, sourceSpan)
          )
        );
      ...
      if (expressions.length || allowEmptyExprs) {
        updateStmts.push(
          o.applySourceSpanToStatementIfNeeded(callCheckStmt(nodeIndex, exprs).toStmt(), sourceSpan)
        );
      }
      return updateStmts;
    }
  ...
  private _createElementHandleEventFn(
    ...
    handlers.forEach(({context, eventAst, dirAst}) => {
      ...
      const {stmts, allowDefault} = convertActionBinding(
        nameResolver,
        context,
        eventAst.handler,
        bindingId
      ...
      handleEventStmts.push(
        o.applySourceSpanToStatementIfNeeded(
...
function elementBindingDef(inputAst: BoundElementPropertyAst, dirAst: DirectiveAst): o.Expression {
  ...
  switch (inputType) {
    case PropertyBindingType.Attribute:
      return o.literalArr([
        o.literal(BindingFlags.TypeElementAttribute),
        o.literal(inputAst.name),
        o.literal(inputAst.securityContext),
      ]);
    case PropertyBindingType.Property:
      return o.literalArr([
        o.literal(BindingFlags.TypeProperty),
        o.literal(inputAst.name),
        o.literal(inputAst.securityContext),
      ]);
    case PropertyBindingType.Animation:
      const bindingType =
        ...
        (dirAst && dirAst.directive.isComponent
          ? BindingFlags.SyntheticHostProperty
          : BindingFlags.SyntheticProperty);
      return o.literalArr([
        o.literal(bindingType),
        o.literal('@' + inputAst.name),
        o.literal(inputAst.securityContext),
      ]);
    case PropertyBindingType.Class:
      return o.literalArr([
        o.literal(BindingFlags.TypeElementClass),
        o.literal(inputAst.name),
        o.NULL_EXPR,
      ]);
    case PropertyBindingType.Style:
      return o.literalArr([
        o.literal(BindingFlags.TypeElementStyle),
        o.literal(inputAst.name),
...
function fixedAttrsDef(elementAst: ElementAst): o.Expression {
  ...
  return o.literalArr(
    Object.keys(mapResult)
      .sort()
      .map((attrName) => o.literalArr([o.literal(attrName), o.literal(mapResult[attrName])]))
  );
}
...
function callCheckStmt(nodeIndex: number, exprs: o.Expression[]): o.Expression {
  if (exprs.length > 10) {
    return CHECK_VAR.callFn([
      VIEW_VAR,
      o.literal(nodeIndex),
      o.literal(ArgumentType.Dynamic),
      o.literalArr(exprs),
    ]);
  } else {
    return CHECK_VAR.callFn([
...
function callUnwrapValue(nodeIndex: number, bindingIdx: number, expr: o.Expression): o.Expression {
  return o
    .importExpr(Identifiers.unwrapValue)
    .callFn([VIEW_VAR, o.literal(nodeIndex), o.literal(bindingIdx), expr]);
}

function elementEventNameAndTarget(
  eventAst: BoundEventAst,
  dirAst: DirectiveAst | null
