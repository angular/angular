...
export class DomElementSchemaRegistry extends ElementSchemaRegistry {
  ...
  // NOTE: this heuristic runs before any Custom Elements Manifest schema is consulted, so a
  // manifest that legitimately declares a bindable `on*`-prefixed property (e.g. `onDark`) gets
  // its binding rejected with this message. Ecosystem impact is currently one known field; any
  // fix belongs in a general revision of this heuristic that considers whether a manifest or
  // CUSTOM_ELEMENTS_SCHEMA element actually owns the property — not in CEM-specific code.
