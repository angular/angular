...
export class ProviderElementContext {
  ...
  constructor(
    ...
    private _directiveAsts: DirectiveAst[],
    attrs: AttrAst[],
    refs: ReferenceAst[],
    isTemplate: boolean,
    ...
    if (isTemplate) {
      const templateRefId = createTokenForExternalReference(
        this.viewContext.reflector,
  ...
  get transformedDirectiveAsts(): DirectiveAst[] {
    const sortedProviderTypes = this.transformProviders.map(
      (provider) => provider.token.identifier
    );
  ...
  private _addQueryReadsTo(
    ...
    defaultValue: CompileTokenMetadata,
  ...
  private _getOrCreateLocalProvider(
    requestingProviderType: ProviderAstType,
    token: CompileTokenMetadata,
    eager: boolean
  ): ProviderAst | null {
    ...
    const transformedProviders = resolvedProvider.providers.map((provider) => {
      ...
      if (provider.useExisting != null) {
        ...
        transformedDeps = deps.map(
          (dep) => this._getDependency(resolvedProvider.providerType, dep, eager)!
        );
  ...
  private _getLocalDependency(
    ...
    eager: boolean = false
  ): CompileDiDependencyMetadata | null {
    if (dep.isAttribute) {
...
export class NgModuleProviderAnalyzer {
  ...
  private _getDependency(
    dep: CompileDiDependencyMetadata,
    eager: boolean = false,
    requestorSourceSpan: ParseSourceSpan
  ): CompileDiDependencyMetadata {
...
function _transformProviderAst(
  ...
  {eager, providers}: {eager: boolean; providers: CompileProviderMetadata[]}
): ProviderAst {
...
function _resolveProvidersFromDirectives(
  ...
  sourceSpan: ParseSourceSpan,
  targetErrors: ParseError[]
): Map<any, ProviderAst> {
  const providersByToken = new Map<any, ProviderAst>();
  directives.forEach((directive) => {
    const dirProvider: CompileProviderMetadata = {
  ...
  const directivesWithComponentFirst = directives
...
function _resolveProviders(
  ...
  providers.forEach((provider) => {
    ...
    if (resolvedProvider != null && !!resolvedProvider.multiProvider !== !!provider.multi) {
      targetErrors.push(
        new ProviderError(
          `Mixing multi and non multi provider is not possible for token ${tokenName(
    ...
    if (!resolvedProvider) {
      const lifecycleHooks =
        provider.token.identifier && (<CompileTypeMetadata>provider.token.identifier).lifecycleHooks
...
function _getViewQueries(component: CompileDirectiveMetadata): Map<any, QueryWithId[]> {
  ...
  if (component.viewQueries) {
    component.viewQueries.forEach((query) =>
      _addQueryToTokenMap(viewQueries, {meta: query, queryId: viewQueryId++})
    );
  }
  return viewQueries;
}
...
function _getContentQueries(
  ...
  directives.forEach((directive, directiveIndex) => {
    if (directive.queries) {
      directive.queries.forEach((query) =>
        _addQueryToTokenMap(contentQueries, {meta: query, queryId: contentQueryId++})
      );
    }
  });
