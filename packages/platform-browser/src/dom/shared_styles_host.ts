...
export class SharedStylesHost implements OnDestroy {
  ...
  constructor(
    ...
    this.inline.set(doc.head, new Map());
    this.external.set(doc.head, new Map());

    addServerStyles(doc, appId, this.inline.get(doc.head)!, this.external.get(doc.head)!);
  ...
  /**
   * Checks if a shadow root is an active IsolatedShadowDom shadow root.
   */
  isIsolatedShadowRoot(shadowRoot: ShadowRoot): boolean {
    return this.shadowRoots.includes(shadowRoot);
  }
  ...
  private addUsageToTarget<T extends HTMLElement>(
    ...
    if (shouldAddToHosts && styleRoot === this.doc.head && this.hosts.size > 0) {
  ...
  addShadowRoot(shadowRoot: ShadowRoot): void {
    ...
    if (ngDevMode && this.shadowRoots.includes(shadowRoot)) {
