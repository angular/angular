...
export class SharedStylesHost implements OnDestroy {
  ...
  protected removeUsage<T extends HTMLElement>(
    ...
    if (typeof ngDevMode !== 'undefined' && ngDevMode && !rootMap) {
      throw new Error(`Attempting to remove styles which are not in the provided \`StyleRoot\`.`);
    }
    const record = rootMap!.get(value);
  ...
  anyStyleRootContains(styles: string[], urls?: string[]): boolean {
    for (const style of styles) {
      for (const inline of this.inline.values()) {
        if (inline.has(style)) return true;
      }
    }
    ...
    if (urls) {
      for (const url of urls) {
        for (const external of this.external.values()) {
          if (external.has(url)) return true;
        }
      }
    }
