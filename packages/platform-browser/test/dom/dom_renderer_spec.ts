...
describe('DefaultDomRendererV2', () => {
  ...
  it('child components of shadow components should have their styles applied within isolated shadow DOM', () => {
    const fixture = TestBed.createComponent(IsolatedShadowComponentParentApp);
    fixture.detectChanges();
    ...
    // For isolated shadow DOM, child component styles should not be in document head
    // (they should only be in the shadow root)
    expect(hasEmulatedStyles(document)).toBe(false);
    expect(hasNoneStyles(document)).toBe(false);
  });

  it('shadow components with projected components should apply correct styles when using IsolatedShadowDom', async () => {
    ...
    // Verify that projected component styles are in the shadow root
    expect(hasEmulatedStyles(shadowRoot)).toBe(true);
