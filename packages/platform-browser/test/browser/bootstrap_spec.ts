  ...
  describe('bootstrap factory method', () => {
    ...
    modifiedInIvy('bootstrapping non-Component throws in View Engine').isEnabled &&
      it('should throw if bootstrapped Directive is not a Component', inject(
        [AsyncTestCompleter],
        (done: AsyncTestCompleter) => {
          const logger = new MockConsole();
          const errorHandler = new ErrorHandler();
          (errorHandler as any)._console = logger as any;
          expect(() =>
            bootstrap(HelloRootDirectiveIsNotCmp, [{provide: ErrorHandler, useValue: errorHandler}])
    ...
    it('should throw if no element is found', inject(
      [AsyncTestCompleter],
      (async: AsyncTestCompleter) => {
        const logger = new MockConsole();
        const errorHandler = new ErrorHandler();
        ...
        bootstrap(NonExistentComp, [{provide: ErrorHandler, useValue: errorHandler}]).then(
          ...
          (reason) => {
            expect(reason.message).toContain(
              'The selector "non-existent" did not match any elements'
            );
            async.done();
            return null;
          }
    ...
    if (getDOM().supportsDOMEvents()) {
      it('should forward the error to promise when bootstrap fails', inject(
        [AsyncTestCompleter],
        (async: AsyncTestCompleter) => {
          const logger = new MockConsole();
          const errorHandler = new ErrorHandler();
      ...
      it('should invoke the default exception handler when bootstrap fails', inject(
        [AsyncTestCompleter],
        (async: AsyncTestCompleter) => {
          const logger = new MockConsole();
          const errorHandler = new ErrorHandler();
          ...
          refPromise.then(null, (reason) => {
            expect(logger.res[0].join('#')).toContain(
              'ERROR#Error: The selector "non-existent" did not match any elements'
            );
            async.done();
            return null;
          });
    ...
    it('should set platform name to browser', inject(
      [AsyncTestCompleter],
      (async: AsyncTestCompleter) => {
        const refPromise = bootstrap(HelloRootCmp, testProviders);
        refPromise.then((ref) => {
    ...
    it('should throw a descriptive error if BrowserModule is installed again via a lazily loaded module', inject(
      [AsyncTestCompleter],
      (async: AsyncTestCompleter) => {
    ...
    it('should not crash if change detection is invoked when the root component is disposed', inject(
      [AsyncTestCompleter],
      (async: AsyncTestCompleter) => {
    ...
    it('should unregister change detectors when components are disposed', inject(
      [AsyncTestCompleter],
      (async: AsyncTestCompleter) => {
    ...
    it('should make the provided bindings available to the application component', inject(
      [AsyncTestCompleter],
      (async: AsyncTestCompleter) => {
    ...
    it('should not override locale provided during bootstrap', inject(
      [AsyncTestCompleter],
      (async: AsyncTestCompleter) => {
    ...
    it('should avoid cyclic dependencies when root component requires Lifecycle through DI', inject(
      [AsyncTestCompleter],
      (async: AsyncTestCompleter) => {
    ...
    it('should run platform initializers', inject(
      [Log, AsyncTestCompleter],
      (log: Log, async: AsyncTestCompleter) => {
    ...
    it('should remove styles when transitioning from a server render', inject(
      [AsyncTestCompleter],
      (async: AsyncTestCompleter) => {
        @Component({
          selector: 'root',
        ...
        platform.bootstrapModule(TestModule).then(() => {
          const styles: HTMLElement[] = Array.prototype.slice.apply(
    ...
    describe('change detection', () => {
      ...
      it('should be triggered for all bootstrapped components in case change happens in one of them', inject(
        [AsyncTestCompleter],
        (async: AsyncTestCompleter) => {
          @NgModule({
            imports: [BrowserModule],
      ...
      it('should work in isolation for each component bootstrapped individually', inject(
        [AsyncTestCompleter],
        (async: AsyncTestCompleter) => {
