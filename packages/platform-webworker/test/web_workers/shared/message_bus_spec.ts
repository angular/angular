...

{
  ...
  describe('MessageBus', () => {
    ...
    it('should pass messages in the same channel from sink to source', inject(
      [AsyncTestCompleter],
    ...
    it('should keep channels independent', inject(
      [AsyncTestCompleter],
      (async: AsyncTestCompleter) => {
  ...
  describe('PostMessageBusSink', () => {
    ...
    it(
      ...
      withModule({providers: [{provide: NgZone, useClass: MockNgZone}]}).inject(
    ...
    it('should send messages immediately when run outside the zone', inject(
      [AsyncTestCompleter, NgZone],
      (async: AsyncTestCompleter, zone: MockNgZone) => {
        bus = createConnectedMessageBus();
        setup(false, zone);
