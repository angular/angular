  ...
  describe('platform-server integration', () => {
    ...
    describe('render', () => {
      ...
      [true, false].forEach((zoneless: boolean) => {
        it(`should render a ${zoneless ? 'zoneless' : 'zone'} app with \`createApplication\``, async () => {
          const output = await renderApplication(
            async (context) => {
              const appRef = await createApplication({providers: []}, context);
        ...
        it(`using platform should work (${zoneless ? 'zoneless' : 'zone'})`, async () => {
          const platform = platformServer([
            {
              provide: INITIAL_CONFIG,
