function getComputedStylePixel(element: any, property: string, fontSize?: any) {
  var
      // Internet Explorer sometimes struggles to read currentStyle until the element's document is
      // accessed.
      value = element.document &&
          element.currentStyle[property].match(/([\d\.]+)(%|cm|em|in|mm|pc|pt|)/) ||
      [0, 0, ''],
      ...
      suffix == 'in' ? size * 96 :
                       suffix == 'mm' ? size * 0.3937 * 96 / 10 :
                                        suffix == 'pc' ? size * 12 * 96 / 72 :
                                                         suffix == 'pt' ? size * 96 / 72 : size;
}

function setShortStyleProperty(style: ɵCSSStyleDeclaration, property: string) {
  var borderSuffix = property == 'border' ? 'Width' : '', t = property + 'Top' + borderSuffix,
      r = property + 'Right' + borderSuffix, b = property + 'Bottom' + borderSuffix,
...
export class ɵCSSStyleDeclaration {
  ...
  constructor(element: any) {
    var style = this, currentStyle = element.currentStyle,
        fontSize = getComputedStylePixel(element, 'fontSize'), property;

    for (property in currentStyle) {
      Array.prototype.push.call(
          style, property == 'styleFloat' ? 'float' : property.replace(/[A-Z]/, unCamelCase));
  ...
  getPropertyPriority() { throw new Error('NotSupportedError: DOM Exception 9'); }
  // <CSSStyleDeclaration>.getPropertyValue
  getPropertyValue(property: string) {
    return this[property.replace(/-\w/g, function(match) { return match[1].toUpperCase(); })];
  }
  // <CSSStyleDeclaration>.item
  item(index: string) { return this[index]; }
