...
function setDomTypes() {
  ...
  Object.assign(global, jsdom.Element);
  (global as any)['KeyboardEvent'] = jsdom.Event;
}
...
/**
 * DOM Adapter for the server platform based on https://github.com/jsdom/jsdom.
 */
export class JsdomAdapter extends BrowserDomAdapter {
  static makeCurrent() {
    setDomTypes();
    setRootDomAdapter(new JsdomAdapter());
  }
  ...
  getDefaultDocument(): Document {
    if (!JsdomAdapter.defaultDoc) {
      JsdomAdapter.defaultDoc = new jsdom().window.document;
    }
    return JsdomAdapter.defaultDoc;
  }
  ...
  isTextNode(node: any): boolean { return node.nodeType === JsdomAdapter.defaultDoc.TEXT_NODE; }
  isCommentNode(node: any): boolean {
    return node.nodeType === JsdomAdapter.defaultDoc.COMMENT_NODE;
  }
  isElementNode(node: any): boolean {
    return node ? node.nodeType === JsdomAdapter.defaultDoc.ELEMENT_NODE : false;
  }
