 ...
 * A special animation control binding called `@$disabled` can be placed on an element which will
 then
 * disable animations for any inner animation triggers situated within the element.
 *
 ...
 * When an area of the template is set to have animations disabled, **all** inner components will
 also have their
 * animations disabled as well. This means that all animations for an angular application can be
 disabled by placing
 * `@$disabled` on the topmost Angular component.
 *
 * ```html
 * <app-root [@$disabled]="true">
 *   <!-- now all the animations application-wide are disabled! -->
 * </app-root>
 * ```
 *
 * The flag can also be placed using `@HostBinding`:
 *
 * ```ts
 * @Component({
 *   selector: 'app-root',
 ...
 * Despite inner animations being disabled, a parent animation can {@link query query} for inner
 elements located
 in
 * disabled areas of the template and still animate them as it sees fit. This is also the case for
 when
 * a sub animation is queried by a parent and then later animated using {@link animateChild
 animateChild}.
 *
 * @experimental Animation support is experimental.
 */
