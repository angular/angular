import {
  ...
  describe('dispatcher test.ts', () => {
    it('dispatches to registered EventInfo handler', () => {
      ...
      let actionFlow!: ActionFlow;
      const actionHandler = wrapActionFlowHandler((flow: ActionFlow) => {
        actionFlow = flow;
        expect(flow.branches().get(ActionFlowBranch.MAIN)).toBe(1);
      ...
      expect(actionFlow.branches().get('fakebranch')).toBe(1);
      expect(actionFlow).not.toBeNull();
      expect(actionFlow.getType()).toBe('foo_bar');
      expect(actionFlow.event()?.type).toBe(createMockClickEvent().type);
      expect(actionFlow.node()).toBe(actionElement);
    });
