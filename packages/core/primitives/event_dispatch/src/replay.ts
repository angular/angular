...
export function createUiEvent(original: Event, opt_eventType?: string): Event {
  ...
  (event as Writeable<FocusEvent>).relatedTarget =
    (original as FocusEvent).relatedTarget || null;
...
export function createEvent(base: unknown, opt_eventType?: string): Event {
  ...
  if (isKeyboardEvent(eventType)) {
    ...
    (event as {originalTimestamp?: number | null} | null)!.originalTimestamp =
      original.timeStamp;
  } else {
