 ...
 * @param e The event.
 * @return Whether the given event is a middle click.
 */
...
export function isActionKeyEvent(e: Event): boolean {
  ...
  const hasType =
    el.tagName.toUpperCase() !== 'INPUT' || !!(el as HTMLInputElement).type;
...
export function isSpaceKeyEvent(e: Event): boolean {
  ...
  const el = getTarget(e);
  const elementName = (
    (el as HTMLInputElement).type || el.tagName
  ).toUpperCase();
...
export function recreateTouchEventAsClick(event: TouchEvent): MouseEvent {
  const click: {
    -readonly [P in keyof MouseEvent]?: MouseEvent[P];
...
function isNativelyActivatable(el: Element): boolean {
  return (
    ...
    (!!(el as HTMLInputElement).type &&
      (el as HTMLInputElement).type.toUpperCase() === 'FILE')
  );
}
