...
export class Dispatcher {
  ...
  dispatch(eventInfo: EventInfo, isGlobalDispatch?: boolean): EventInfo | void {
    ...
    if (
      ...
      !skipStopPropagation(eventInfoWrapper)
    ) {
  ...
  unregisterHandler(namespace: string, name: string) {
    const fullName = namespace
      ? namespace + Char.NAMESPACE_ACTION_SEPARATOR + name
      : name;
  ...
  private scheduleEventReplay() {
    if (
      ...
      !this.eventReplayer ||
      this.queuedEventInfos.length === 0
    ) {
      return;
    }
...
function resolveA11yEvent(
  ...
  if (isA11yClickEvent(eventInfoWrapper, isGlobalDispatch)) {
    ...
    if (!isGlobalDispatch) {
      ...
      // events.
      eventInfoWrapper.getEvent()[AccessibilityAttribute.SKIP_A11Y_CHECK] =
        true;
      // Since globally dispatched events will get handled by the dispatcher,
      // don't have the event contract dispatch it again.
      eventInfoWrapper.getEvent()[AccessibilityAttribute.SKIP_GLOBAL_DISPATCH] =
        true;
      return false;
    }
  }
...
function shouldPreventDefault(eventInfoWrapper: EventInfoWrapper): boolean {
  ...
  if (
    jsactionEvent.shouldCallPreventDefaultOnNativeHtmlControl(
      eventInfoWrapper.getEvent(),
    )
  ) {
    return true;
