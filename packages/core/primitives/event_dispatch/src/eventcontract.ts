...
export class EventContract implements UnrenamedEventContract {
  ...
  private handleEventInfo(
    ...
    if (
      ...
      !eventInfoLib.getEvent(eventInfo)[
        AccessibilityAttribute.SKIP_GLOBAL_DISPATCH
      ]
    ) {
      const globalEventInfo: eventInfoLib.EventInfo =
        eventInfoLib.cloneEventInfo(eventInfo);
  ...
  private populateAction(eventInfo: eventInfoLib.EventInfo) {
    ...
    if (EventContract.USE_EVENT_PATH) {
      ...
      let actionElement: Element | null =
        eventInfoLib.getTargetElement(eventInfo);
      while (
        actionElement &&
        actionElement !== eventInfoLib.getContainer(eventInfo)
      ) {
        this.populateActionOnElement(actionElement, eventInfo);
        ...
        if (actionElement.parentNode?.nodeName !== '#document-fragment') {
          ...
          actionElement =
            (actionElement.parentNode as ShadowRoot | null)?.host ?? null;
        }
      }
    }
    ...
    if (!jsactionAttribute) {
      ...
      if (!actionMap) {
        ...
        for (let idx = 0; idx < values.length; idx++) {
          ...
          const type = hasColon
            ? stringTrim(value.substr(0, colon))
            : DEFAULT_EVENT_TYPE;
