...
export interface ReactiveNode {
   ...
   *
   * While recomputing is false, producersTail.nextProducer is guaranteed to be undefined.
   * Otherwise, producersTail.nextProducer will point to either
   * - the next producer that was accessed during the last run of this consumer (assuming we are reading the same producers)
   * - the previous list of producers that we didn't reuse, so that we can remove them all at once after we finish recomputing.
   */
  recomputing: boolean;
...
export function producerAccessed(node: ReactiveNode): void {
  ...
  const prevProducer = consumer.producersTail;

  // If the last producer we accessed is the same as the current one, we can skip adding a new
  // link
  if (prevProducer !== undefined && prevProducer.producer === producer) {
    prevProducer.lastReadVersion = producer.version;
    return;
  }

  let nextProducer: ReactiveLink | undefined = undefined;
  const recursedCheck = consumer.recomputing;
  if (recursedCheck) {
    ...
    nextProducer = prevProducer !== undefined ? prevProducer.nextProducer : consumer.producers;
    if (nextProducer !== undefined && nextProducer.producer === producer) {
      ...
      consumer.producersTail = nextProducer;
      nextProducer.lastReadVersion = producer.version;
      return;
    }
  }

  const prevConsumer = producer.consumersTail;
  ...
  if (
    ...
    prevConsumer.consumer === consumer &&
    ...
    (!recursedCheck || isValidLink(prevConsumer, consumer))
  ) {
    // If we found a link to the consumer, we can just update the last read version and return.
    prevConsumer.lastReadVersion = producer.version;
    return;
  }
  ...
  const newLink = {
    ...
    prevConsumer: prevConsumer,
    lastReadVersion: producer.version,
    nextConsumer: undefined,
  };
  consumer.producersTail = newLink;
  if (prevProducer !== undefined) {
    prevProducer.nextProducer = newLink;
  } else {
    consumer.producers = newLink;
  }

  if (isLive) {
    producerAddLiveConsumer(producer, newLink);
  }
}
