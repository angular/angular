...
export function findUninitializedPropertiesToCombine(
  ...
  const inlinableParameters = findInlinableParameterReferences(constructor, localTypeChecker);
 ...
 */
function isIdentifierReferencingProperty(
  node: ts.Node,
  localTypeChecker: ts.TypeChecker,
...
function findInlinableParameterReferences(
  ...
  function walk(node: ts.Node, canReferenceThis: boolean) {
    const property = isIdentifierReferencingProperty(
      node,
      localTypeChecker,
      eligibleNames,
