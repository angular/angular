...
export function findRendererReferences(
  ...
  rendererImport: ts.NamedImports
) {
  ...
  const forwardRefSpecifier = forwardRefImport
    ? findImportSpecifier(forwardRefImport.elements, 'forwardRef')
    : null;
  ...
  ts.forEachChild(sourceFile, function visitNode(node: ts.Node) {
    if (
      ...
      isReferenceToImport(typeChecker, node.type, importSpecifier)
    ) {
      typedNodes.add(node);
      ...
      if (
        ...
        const rendererIdentifier = findRendererIdentifierInForwardRef(
          typeChecker,
          node,
          importSpecifier
        );
...
export function findCoreImport(
  sourceFile: ts.SourceFile,
  symbolName: string
...
function findRendererIdentifierInForwardRef(
  ...
  rendererImport: ts.ImportSpecifier | null
): ts.Identifier | null {
