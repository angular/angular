  ...
  return async (tree: Tree) => {
    ...
    if (!allPaths.length) {
      throw new SchematicsException(
          'Could not find any tsconfig file. Cannot run the BrowserTransferStateModule migration.');
    }

    for (const tsconfigPath of allPaths) {
      runBrowserTransferStateModuleMigration(tree, tsconfigPath, basePath);
    }
  };
}

function runBrowserTransferStateModuleMigration(
