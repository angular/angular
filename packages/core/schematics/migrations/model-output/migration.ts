...
> {
  ...
  private migrateModelProperty(
    ...
    let inputArgs;
    if (isRequired) {
      let options = call.arguments.length > 0 ? call.arguments[0].getText() : '{}';
      if (options.startsWith('{') && options.endsWith('}')) {
        const inner = options.slice(1, -1).trim();
        inputArgs = `{alias: '${modelName}'${inner ? `, ${inner}` : ''}}`;
      } else {
        inputArgs = `{...${options}, alias: '${modelName}'}`;
      }
    } else {
      const initialValue = call.arguments.length > 0 ? call.arguments[0].getText() : 'undefined';
      let options = call.arguments.length > 1 ? call.arguments[1].getText() : '{}';
      if (options.startsWith('{') && options.endsWith('}')) {
        const inner = options.slice(1, -1).trim();
        options = `{alias: '${modelName}'${inner ? `, ${inner}` : ''}}`;
      } else {
        options = `{...${options}, alias: '${modelName}'}`;
      }
      inputArgs = `${initialValue}, ${options}`;
    }
