...
function replaceProviderWithNewApi({
  ...
  sourceFile: ts.SourceFile; node: ts.Node; provider: ProviderInfo; changeTracker: ChangeTracker;
}) {
  ...
  const newNamedImports = ts.factory.updateNamedImports(namedImports, [
    ...
    ...(!hasProvideInitializeFunction ? [createImportSpecifier(provideInitializerFunctionName)] :
                                        []),
  ]);
