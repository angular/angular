...
> {
  ...
  async analyze(info: ProgramInfo): Promise<Serializable<CommonModuleCompilationUnitData>> {
  ...
  private analyzeComponentDecorator(
    ...
    for (const property of config.properties) {
      ...
      if (name === 'template') {
        ...
        isStandalone = property.initializer.kind === ts.SyntaxKind.TrueKeyword;
      }
    }
  ...
  async stats(globalMetadata: CommonModuleCompilationUnitData) {
    const stats = {
      counters: {
  ...
  async migrate(globalData: CommonModuleCompilationUnitData) {
    return {
