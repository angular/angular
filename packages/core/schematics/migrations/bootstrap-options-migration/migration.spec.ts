    ...
    [
      ...
      describe(`${platformBrowserFn}().bootstrapModule`, () => {
        ...
        it('should keep compiler options if present', async () => {
          ...
          ${platformBrowserFn}().bootstrapModule(AppModule, { compilerOptions: { preserveWhitespaces: true }, ngZoneRunCoalescing: true });
        `,
            },
            {
              name: absoluteFrom('/app/app.module.ts'),
        ...
        `;
          ...
          ${platformBrowserFn}().bootstrapModule(AppModule, { applicationProviders: [provideZoneChangeDetection({ runCoalescing: true })], compilerOptions: { preserveWhitespaces: true }, });
        `;
          expect(mainActual.replace(/\s+/g, ''))
            .withContext(diffText(mainExpected, mainActual))
            .toEqual(mainExpected.replace(/\s+/g, ''));
