...
> {
  ...
  private analyzeBootstrapApplication(
    ...
    if (optionsNode) {
      ...
      if (ts.isObjectLiteralExpression(optionsNode)) {
        ...
        if (isMergeApplicationConfig(optionsNode, typeChecker)) {
          // We're skipping this option as the base config we already be migrated
          return;
        }
        optionLiteral = getObjectLiteralFromIdentifier(optionsNode, typeChecker);
        if (!optionLiteral) {
          throw new Error(
            `Could not find options object for identifier ${optionsNode.text} in ${sourceFile.fileName}`,
          );
        }
        optionProjectFile = projectFile(optionLiteral.getSourceFile(), info);
      } else {
        throw new Error('unsupported optionsNode: ' + optionsNode.getText());
      }

      addProvidersToBootstrapOption(optionProjectFile, optionLiteral, providersText, replacements);
    } else {
  ...
  private analyzeBootstrapModule(
    ...
    // Even though we remove the option argument, we still need to understand the options
    const optionsNode = node.arguments[1];
    const options =
  ...
  private analyzeTestBedInitEnvironment(
    ...
    const ngModules = node.arguments[0];
    ...
    const insertPosition = node.parent.getStart() - 1;
    addZoneCDModule(ZONE_CD_PROVIDER, moduleProjectFile, insertPosition, replacements);
    insertZoneCDModule(ngModules, moduleProjectFile, typeChecker, replacements);
  }
}
...
function addProvidersToNgModule(
  ...
  const insertPosition = ngModule.parent.getStart() - 1;
  ...
  if (importsNode && ts.isPropertyAssignment(importsNode)) {
    insertZoneCDModule(importsNode.initializer, projectFile, typechecker, replacements);
  } else {
...
function isMergeApplicationConfig(node: ts.Node, typeChecker: ts.TypeChecker): boolean {
  ...
  return (
    ...
    decl.initializer.expression.escapedText === 'mergeApplicationConfig'
  );
}
