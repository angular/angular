...
> {
  ...
  private analyzeBootstrapModule(
    ...
    const optionsNode = node.arguments[1];
    let hasOptionObject = !!optionsNode;
    const removeAllOptions = () => {
      hasOptionObject = false;
      replacements.push(
        new Replacement(
          file,
          new TextUpdate({
    ...
    let hasOnlyBootstrapOptions = onlyBootstrapOptions(optionsNode);
    ...
    if (hasExistingChangeDetectionProvider) {
      if (hasOnlyBootstrapOptions) {
        removeAllOptions();
      } else {
        removePropertiesFromLiteral(file, optionsNode, BOOTSTRAP_OPTIONS, replacements);
      }
      return;
    }

    // Let's try to understand the bootstrap options.
    const options = optionsNode ? evaluator.evaluate(optionsNode) : null;

    let zoneCdProvider = ZONE_CD_PROVIDER;
    let zoneInstanceProvider: string | null = null;

    if (options instanceof Map) {
      if ([...options.keys()].every((k) => SAFE_TO_REMOVE_OPTIONS.includes(k))) {
        // In this case it's safe to remove the entire options object.
        removeAllOptions();
      } else {
        removePropertiesFromLiteral(file, optionsNode, SAFE_TO_REMOVE_OPTIONS, replacements);
      }
      ...
      replacements.push(
        new Replacement(
          ...
          new TextUpdate({
            ...
            end: optionsNode.getStart() + 1,
            toInsert: `applicationProviders: [${providers.join(', ')}],`,
          }),
        ),
      );
    } else {
