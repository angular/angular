...
export class TslintUpdateRecorder implements UpdateRecorder {
  ...
  addBaseClassDecorator(node: ts.ClassDeclaration, decoratorText: string) {
    this.failures.push(new RuleFailure(
        this.sourceFile, node.getStart(), 0, `Base class needs to be decorated with "@Directive".`,
  ...
  updateModuleDeclarations(node: ts.ArrayLiteralExpression, newDeclarations: string): void {
    const fix = [
      Replacement.deleteText(node.getStart(), node.getWidth()),
      Replacement.appendText(node.getStart(), newDeclarations),
    ];
    this.failures.push(new RuleFailure(
        this.sourceFile, node.getStart(), node.getEnd(),
        `Module needs to have the following declarations: "${newDeclarations}"`, this.ruleName,
        fix));
  }
