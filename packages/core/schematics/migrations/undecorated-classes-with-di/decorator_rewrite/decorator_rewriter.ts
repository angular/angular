...
export class DecoratorRewriter {
  ...
  rewrite(ngDecorator: NgDecorator, newSourceFile: ts.SourceFile): ts.Decorator {
    ...
    return this._createDecorator(
      ts.createCall(
        this.importManager.addImportToSourceFile(
          newSourceFile,
          ngDecorator.name,
          ngDecorator.moduleName
        ),
        undefined,
        [ts.updateObjectLiteral(metadata, [...this.newProperties, ...this.nonCopyableProperties])]
      )
    );
  }
  ...
  private _sanitizeMetadataProperty(
    ...
    try {
      return ts.transform(prop, [
        (ctx) => this.importRewriterFactory.create(ctx, this.newSourceFile!),
      ]).transformed[0];
    } catch (e) {
