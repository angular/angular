...
describe('Renderer to Renderer2 migration', () => {
  ...
  describe('import renaming', () => {
    ...
    it('should change aliased Renderer imports to Renderer2', async () => {
      ...
      await runMigration();
      expect(tree.readContent('/index.ts')).toContain(
        `import { Component, Renderer2 as RenamedRenderer } from '@angular/core';`
      );
    });
  ...
  describe('type renaming', () => {
    ...
    it('should change type of method parameter from Renderer to Renderer2', async () => {
      ...
      await runMigration();
      expect(tree.readContent('/index.ts')).toContain(
        'disable(renderer: Renderer2, element: HTMLElement, isDisabled: boolean)'
      );
    });
  ...
  describe('helper insertion', () => {
    it('should only declare each helper once per file', async () => {
      ...
      expect(content.match(/function __ngRendererCreateElementHelper\(/g)!.length).toBe(
        1,
        'Expected exactly one helper for createElement.'
      );
    });
    ...
    it('should be able to handle multiple helpers per file', async () => {
      ...
      expect(content.match(/function __ngRendererCreateTextHelper\(/g)!.length).toBe(
        1,
        'Expected exactly one helper for createElement.'
      );
      expect(content.match(/function __ngRendererCreateElementHelper\(/g)!.length).toBe(
        1,
        'Expected exactly one helper for createText.'
      );
      expect(content.match(/function __ngRendererCreateTemplateAnchorHelper\(/g)!.length).toBe(
        1,
        'Expected exactly one helper for createTemplateAnchor.'
      );
    });
    ...
    it('should create the __ngRendererSplitNamespaceHelper', async () => {
      ...
      expect(stripWhitespace(tree.readContent('/index.ts'))).toContain(
        stripWhitespace(`
  ...
  describe('setElementProperty migration', () => {
    it('should migrate setElementProperty calls', async () => {
      ...
      await runMigration();
      expect(tree.readContent('/index.ts')).toContain(
        `this._renderer.setProperty(this._element.nativeElement, 'disabled', true);`
      );
    });
  });
  ...
  describe('listenGlobal migration', () => {
    it('should migrate listenGlobal calls', async () => {
      ...
      await runMigration();
      expect(tree.readContent('/index.ts')).toContain(
        `this._renderer.listen('window', 'resize', () => console.log('resized'));`
      );
    });
  });
  ...
  describe('setElementStyle migration', () => {
    ...
    it('should migrate calls with null or undefined value for last argument to a removeStyle call', async () => {
      writeFile(
        '/index.ts',
        `
        import { Renderer, Component, ElementRef } from '@angular/core';
    ...
    it('should migrate calls with a variable third argument whose value can be inferred', async () => {
      writeFile(
        '/index.ts',
        `
        import { Renderer, Component, ElementRef } from '@angular/core';
  ...
  describe('invokeElementMethod migration', () => {
    it('should migrate calls to a direct method call if the method name and arguments are static', async () => {
      writeFile(
        '/index.ts',
        `
        import { Renderer, Component, ElementRef } from '@angular/core';
    ...
    it('should migrate calls to a property access if the method name or arguments are dynamic', async () => {
      writeFile(
        '/index.ts',
        `
        import { Renderer, Component, ElementRef } from '@angular/core';
  ...
  describe('createViewRoot migration', () => {
    it('should replace createViewRoot calls with a reference to the first argument', async () => {
      ...
      expect(tree.readContent('/index.ts')).toMatch(
        /createRoot\(\) \{\s+return this\._element\.nativeElement;\s+\}/
      );
    });
  });
  ...
  describe('destroyView migration', () => {
    it('should migrate to calls to the __ngRendererDestroyViewHelper', async () => {
      ...
      expect(tree.readContent('/index.ts')).toContain(
        '__ngRendererDestroyViewHelper(this._renderer, allNodes);'
      );
    });
  ...
  describe('detachView migration', () => {
    it('should migrate to calls to the __ngRendererDetachViewHelper', async () => {
      ...
      expect(tree.readContent('/index.ts')).toContain(
        '__ngRendererDetachViewHelper(this._renderer, rootNodes);'
      );
    });
