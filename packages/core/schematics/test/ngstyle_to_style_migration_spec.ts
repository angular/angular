...
describe('NgStyle migration', () => {
  ...
  function runMigration(options?: {path?: string; migrateObjectReferences?: boolean}) {
  ...
  describe('No change cases', () => {
    ...
    it('should change empty ngStyle binding', async () => {
      writeFile(
        '/app.component.ts',
        `
        import {Component} from '@angular/core';
...
describe('NgStyle migration migrateObjectReferences option', () => {
  let runner: SchematicTestRunner;
  let host: TempScopedNodeJsSyncHost;
  let tree: UnitTestTree;
  ...
  it('should migrate a simple object reference', async () => {
    ...
    await runMigration({migrateObjectReferences: true});
