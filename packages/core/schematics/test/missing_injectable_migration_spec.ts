...
describe('Missing injectable migration', () => {
    ...
    it(
      ...
      async () => {
        ...
        expect(tree.readContent('/index.ts')).toMatch(
          /@Injectable\(\)\s+export class MySecondService/
        );
  ...
  function createTests(
    ...
    it(`should migrate object literal provider in ${type} to explicit value provider`, async () => {
      ...
      expect(tree.readContent('/index.ts')).toContain(
        `${propName}: [{ provide: MyService, useValue: undefined }]`
      );
    ...
    it(`should migrate object literal provider with forwardRef in ${type}`, async () => {
      ...
      expect(tree.readContent('/index.ts')).toMatch(/@Injectable\(\)\s+export class MyService/);
      expect(tree.readContent('/index.ts')).toContain(
        `{ ${type}, forwardRef, Injectable } from '@angular/core`
      );
    });
    ...
    it(
      `should not migrate references for providers with "useExisting" in ${type}, but migrate ` +
      ...
      `
        ...
        expect(warnOutput.length).toBe(0);
        expect(tree.readContent('/index.ts')).toMatch(
          /@angular\/core';\s+@Injectable\(\)\s+export class MyService/
        );
        expect(tree.readContent('/index.ts')).toMatch(/MyService {}\s+export class MyToken/);
      }
    );
    ...
    it(`should migrate multiple mixed providers in same ${type}`, async () => {
      ...
      expect(tree.readContent('/index.ts')).toContain(
        `{ provide: ServiceB, useValue: undefined },`
      );
    });
    ...
    it(`should migrate multiple nested providers in same ${type}`, async () => {
      ...
      expect(tree.readContent('/index.ts')).toContain(
        `{ provide: ServiceD, useValue: undefined },`
      );
    });
    ...
    it('should migrate providers referenced through identifier', async () => {
      ...
      expect(tree.readContent('/index.ts')).toContain(
        `{ provide: ServiceC, useValue: undefined },`
      );
    });
    ...
    it('should migrate providers created through static analyzable function call', async () => {
      ...
      expect(tree.readContent('/index.ts')).toContain(
        `ServiceB, { provide: ServiceC, useValue: undefined }),`
      );
    });
    ...
    it(`should migrate provider once if referenced in multiple ${type} definitions`, async () => {
      ...
      expect(tree.readContent('/second.ts')).toContain(
        `{ ${type}, Injectable } from '@angular/core`
      );
    });

    it(
      `should only migrate empty object provider literal once if referenced multiple times ` +
        `in ${type} definitions`,
      async () => {
        writeFile(
          '/provider.ts',
      ...
      `
        ...
        expect(tree.readContent('/provider.ts')).toContain(
          `const PROVIDER = { provide: MyService, useValue: undefined };`
        );
      }
    );
    ...
    it('should create new import for @Injectable when migrating provider', async () => {
      ...
      expect(tree.readContent('/service.ts')).toMatch(
        /@Injectable\(\)\s+export class MySecondService/
      );
      expect(tree.readContent('/service.ts')).toMatch(
    ...
    it('should re-use existing namespace import for importing @Injectable when migrating provider', async () => {
      writeFile(
        '/index.ts',
        `
      ...
      expect(warnOutput.length).toBe(0);
      expect(tree.readContent('/index.ts')).toMatch(
        /@core.Injectable\(\)\s+export class MyService/
      );
    });
    ...
    it('should create new import for injectable after full end of last import statement', async () => {
      ...
      expect(tree.readContent('/service.ts')).toMatch(/@Injectable\(\)\s+export class MyService/);
      expect(tree.readContent('/service.ts')).toMatch(
        /'b'; \/\/ some comment\s+import { Injectable } from "@angular\/core";/
      );
    });
    ...
    it('should remove @Inject decorator for providers which are migrated', async () => {
      ...
      expect(warnOutput.length).toBe(0);
      expect(tree.readContent('/service.ts')).toMatch(
        /core';\s+@Injectable\(\)\s+export class MyService/
      );
