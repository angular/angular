...
describe('dynamic queries migration', () => {
  ...
  it('should remove the options object from a dynamic ViewChild query that only has one property', async () => {
    writeFile(
      '/index.ts',
      `
  ...
  it('should remove the options object from a dynamic ContentChild query that only has one property', async () => {
    writeFile(
      '/index.ts',
      `
  ...
  it('should only remove the `static` flag from a ViewChild query if it has more than one property', async () => {
    writeFile(
      '/index.ts',
      `
  ...
  it('should only remove the `static` flag from a ContentChild query if it has more than one property', async () => {
    writeFile(
      '/index.ts',
      `
    ...
    await runMigration();
    expect(tree.readContent('/index.ts')).toContain(
      `@ContentChild('child', { read: ElementRef }) child: ElementRef;`
    );
  });
