...
describe('projection', () => {
  ...
  it('should support moving non projected light dom around', () => {
    ...
    @Directive({selector: '[manual]', standalone: false})
  ...
  describe('projectable nodes', () => {
    ...
    @Component({selector: 'with-content', template: '', standalone: false})
    class WithContentCmpt {
      @ViewChild('ref', {static: true}) directiveRef: any;
    }

    @Component({selector: 're-project', template: '<ng-content></ng-content>', standalone: false})
    class ReProjectCmpt {}

    @Directive({selector: '[insert]', standalone: false})
    ...
    @Directive({selector: '[delayedInsert]', exportAs: 'delayedInsert', standalone: false})
...
@Component({selector: 'main', template: '', standalone: false})
class MainComp {
  text: string = '';
}

@Component({selector: 'other', template: '', standalone: false})
class OtherComp {
  text: string = '';
}
...
@Component({selector: 'empty', template: '', standalone: false})
class Empty {}
...
@Directive({selector: '[project]', standalone: false})
...
@Component({selector: 'cmp-d', template: `<i>{{tagName}}</i>`, standalone: false})
class CmpD {
  tagName: string;
...
@Component({selector: 'cmp-c', template: `<b>{{tagName}}</b>`, standalone: false})
class CmpC {
  tagName: string;
  constructor(elementRef: ElementRef) {
    this.tagName = elementRef.nativeElement.tagName.toLowerCase();
...
@Component({selector: 'cmp-b11', template: `{{'b11'}}`, standalone: false})
class CmpB11 {}

@Component({selector: 'cmp-b12', template: `{{'b12'}}`, standalone: false})
class CmpB12 {}

@Component({selector: 'cmp-b21', template: `{{'b21'}}`, standalone: false})
class CmpB21 {}

@Component({selector: 'cmp-b22', template: `{{'b22'}}`, standalone: false})
class CmpB22 {}
