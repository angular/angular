  ...
  describe('regressions', () => {
    ...
    describe('empty templates - #15143', () => {
      ...
      modifiedInIvy('Comment node order changed').it(
        'should allow empty embedded templates',
        () => {
    ...
    modifiedInIvy('Static ViewChild and ContentChild queries are resolved in update mode').it(
      ...
      () => {
        ...
        @Component({
          ...
          template: `
            <test></test><br />
            <test><ng-template>Custom as a child</ng-template></test
            ><br />
        ...
        const fixture = TestBed.configureTestingModule({declarations: [App, Test]}).createComponent(
          App
        );
        fixture.detectChanges();

        const testDirs = fixture.debugElement
          .queryAll(By.directive(Test))
    ...
    onlyInIvy('Ivy does not support @ContentChild and @Input on the same property').it(
      'should throw if @ContentChild and @Input are on the same property',
      () => {
        @Directive({selector: 'test'})
        class Test {
          @Input() @ContentChild(TemplateRef, {static: true}) tpl!: TemplateRef<any>;
    ...
    it('should not add ng-version for dynamically created components', () => {
      ...
      const modRef = TestBed.configureTestingModule({imports: [MyModule]}).get(
        NgModuleRef
      ) as NgModuleRef<MyModule>;
      const compRef = modRef.componentFactoryResolver
        .resolveComponentFactory(App)
