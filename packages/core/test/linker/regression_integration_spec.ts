...
describe('regressions', () => {
  ...
  describe('expressions', () => {
    ...
    it('should only update the bound property when using asyncPipe - #15205', fakeAsync(() => {
      @Component({template: '<div myDir [a]="p | async" [b]="2"></div>', standalone: false})
      class MyComp {
        p = Promise.resolve(1);
      }
  ...
  it('should not recreate ViewContainerRefs in queries', () => {
    @Component({template: '<div #vc></div><div *ngIf="show" #vc></div>', standalone: false})
    class MyComp {
  ...
  it('should throw if @ContentChild and @Input are on the same property', () => {
    ...
    @Component({selector: 'my-app', template: `<test></test>`, standalone: false})
    class App {}
...
describe('regressions using bootstrap', () => {
  ...
  if (getDOM().supportsDOMEvents) {
    ...
    it('should keep change detecting if there was an error', (done) => {
      ...
      @Directive({selector: '[dirClick]', standalone: false})
      class EventDir {
...
@Component({selector: 'cmp-content', template: `<ng-content></ng-content>`, standalone: false})
class CmpWithNgContent {}

@Component({selector: 'counting-cmp', template: '', standalone: false})
...
@Pipe({name: 'countingPipe', standalone: false})
