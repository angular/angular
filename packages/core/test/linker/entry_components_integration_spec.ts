...
function declareTests(config?: {useJit: boolean}) {
  describe('@Component.entryComponents', function () {
    ...
    it('should resolve ComponentFactories via ANALYZE_FOR_ENTRY_COMPONENTS', () => {
      ...
      const cfr: ComponentFactoryResolver = compFixture.componentRef.injector.get(
        ComponentFactoryResolver
      );
    ...
    it('should be able to get a component form a parent component (view hierarchy)', () => {
      ...
      expect(childComp.cfr.resolveComponentFactory(NestedChildComp)!.componentType).toBe(
        NestedChildComp
      );
    ...
    obsoleteInIvy('In Ivy, the ComponentFactoryResolver can resolve any component factory').it(
      ...
      () => {
        ...
        expect(nestedChildComp.cfr.resolveComponentFactory(ChildComp)!.componentType).toBe(
          ChildComp
        );
        expect(() => nestedChildComp.cfr.resolveComponentFactory(NestedChildComp)).toThrow(
          noComponentFactoryError(NestedChildComp)
        );
      }
    );
  });
