  ...
  describe(`ChangeDetection`, () => {
    ...
    describe('enforce no new changes', () => {
      it('should throw when a record gets changed after it has been checked', fakeAsync(() => {
        @Directive({selector: '[changed]', standalone: false})
    ...
    describe('nested view recursion', () => {
      it('should recurse into nested components even if there are no bindings in the component view', () => {
        @Component({selector: 'nested', template: '{{name}}', standalone: false})
        class Nested {
          name = 'Tom';
        }
      ...
      it('should recurse into nested view containers even if there are no bindings in the component view', () => {
        @Component({template: '<ng-template #vc>{{name}}</ng-template>', standalone: false})
        class Comp {
          name = 'Tom';
      ...
      describe('projected views', () => {
        ...
        @Directive({selector: '[i]', standalone: false})
    ...
    describe('class binding', () => {
      it('should coordinate class attribute and class host binding', () => {
        @Component({template: `<div class="{{initClasses}}" someDir></div>`, standalone: false})
        class Comp {
          initClasses = 'init';
        }
    ...
    describe('lifecycle asserts', () => {
      ...
      describe('calling init', () => {
        function initialize(options: Options) {
          @Component({selector: 'my-child', template: '', standalone: false})
          class MyChild {
...
@Pipe({name: 'countingImpurePipe', pure: false, standalone: false})
...
@Pipe({name: 'pipeWithOnDestroy', standalone: false})
...
@Pipe({name: 'identityPipe', standalone: false})
...
@Pipe({name: 'multiArgPipe', standalone: false})
...
@Component({selector: 'test-cmp', template: 'empty', standalone: false})
class TestComponent {
  value: any;
  a: any;
  b: any;
...
@Component({selector: 'other-cmp', template: 'empty', standalone: false})
...
@Directive({selector: '[emitterDirective]', standalone: false})
class EmitterDirective {
  @Output('event') emitter = new EventEmitter<string>();
}

@Directive({selector: '[gh9882]', standalone: false})
...
@Directive({selector: '[testDirective]', exportAs: 'testDirective', standalone: false})
class TestDirective
  implements
    OnInit,
    DoCheck,
...
@Directive({selector: '[onDestroyDirective]', standalone: false})
...
@Directive({selector: '[orderCheck0]', standalone: false})
...
@Directive({selector: '[orderCheck1]', standalone: false})
...
@Directive({selector: '[orderCheck2]', standalone: false})
...
@Directive({selector: '[testLocals]', standalone: false})
class TestLocals {
...
@Component({selector: 'root', template: 'empty', standalone: false})
class Person {
