...
@Component({selector: 'simple-cmp', template: '<b>simple</b>', standalone: false})
export class SimpleCmp {}

@Component({selector: 'with-refs-cmp', template: '<div #firstDiv></div>', standalone: false})
export class WithRefsCmp {}

@Component({selector: 'inherited-cmp', template: 'inherited', standalone: false})
export class InheritedCmp extends SimpleCmp {}

@Directive({selector: '[hostBindingDir]', host: {'[id]': 'id'}, standalone: false})
...
@Component({selector: 'inline-template', template: '<p>Hello</p>', standalone: false})
...
describe('TestBed', () => {
  ...
  it('should be able to create a fixture if a test module is reset mid-compilation', async () => {
    ...
    @Component({template: 'hello {{_token}}', standalone: false})
    class TestComponent {
  ...
  describe('resetting ng defs', () => {
    it('should restore ng defs to their initial states', () => {
      @Pipe({name: 'somePipe', pure: true, standalone: false})
      class SomePipe {
        transform(value: string): string {
      ...
      @Directive({selector: 'someDirective', standalone: false})
      class SomeDirective {
        someProp = 'hello';
      }

      @Component({selector: 'comp', template: 'someText', standalone: false})
      class SomeComponent {}
    ...
    it('should cleanup ng defs for classes with no ng annotations (in case of inheritance)', () => {
      @Component({selector: 'someDirective', template: '...', standalone: false})
      class SomeComponent {}
      ...
      @Pipe({name: 'some-pipe', standalone: false})
      class SomePipe {}
    ...
    it('should clean up overridden providers on components whose modules are compiled more than once', async () => {
      ...
      @Component({providers: [SomeInjectable], standalone: false})
  ...
  it('should handle provider overrides when module imports are provided as a function', () => {
    ...
    @Component({template: '{{injectedString.value}}', standalone: false})
    class AppComponent {
