...
import {getReactiveLViewConsumer} from '../../src/render3/reactive_lview_consumer';
...
describe('OnPush components with signals', () => {
  ...
  it('does not generate a reactive consumer for each view', () => {
    @Component({
      template: ``,
      selector: 'child',
      standalone: true,
    ...
    const reactiveIdBeforeViewCreation = nextReactiveId();
    TestBed.createComponent(ParentCmp);
    const reactiveIdAfterViewCreation = nextReactiveId();

    // Only one reactive ID should get generated even though we have at least 4 views (1 parent and
    // 3 children).
    // We would ordinarily expect this to be one. However, it might be zero instead, depending on
    // the order in which the tests are run. This is because the whole point of the optimization is
    // to lazily assign the next reactive consumer, so there might be one already allocated in a
    // previous test, waiting to be claimed.
    expect(reactiveIdAfterViewCreation - reactiveIdBeforeViewCreation).toBeLessThanOrEqual(1);
  });
});
