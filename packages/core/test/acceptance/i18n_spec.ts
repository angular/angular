...
onlyInIvy('Ivy i18n logic').describe('runtime i18n', () => {
  ...
  describe('should support ICU expressions', () => {
    ...
    it('multiple', () => {
      ...
      expect(fixture.nativeElement.innerHTML).toEqual(
        `<div>aucun <b>email</b>!<!--ICU 7--> - (Angular)<!--ICU 13--></div>`
      );
      ...
      fixture.detectChanges();
      expect(fixture.nativeElement.innerHTML).toEqual(
        `<div>aucun <b>email</b>!<!--ICU 7--> - (John)<!--ICU 13--></div>`
      );
    });
  ...
  "ng-reflect-ng-if": "false"
    ...
    it('nested', () => {
      ...
      fixture.detectChanges();
      expect(fixture.nativeElement.innerHTML).toEqual(
        `<div>4 animaux<!--nested ICU 0-->!<!--ICU 5--></div>`
      );
    });
    ...
    it('nested with interpolations in "other" blocks', () => {
      ...
      fixture.detectChanges();
      expect(fixture.nativeElement.innerHTML).toEqual(
        `<div>2 animaux<!--nested ICU 0-->!<!--ICU 5--></div>`
      );
    ...
    it('projection', () => {
      ...
      @Component({
        selector: 'parent',
        template: ` <child i18n
          >{ value // i18n(ph = "blah"), plural, =1 {one} other {at least {{ value }} .} }</child
        >`,
      })
      class Parent {
        value = 3;
    ...
    it('inside a container when creating a view via vcr.createEmbeddedView', () => {
      ...
      expect(fixture.debugElement.nativeElement.innerHTML).toContain(
        '<my-cmp><div>ONE<!--ICU 13--></div><!--container--></my-cmp>'
      );

      fixture.componentRef.instance.count = 2;
      fixture.detectChanges();
      expect(fixture.debugElement.nativeElement.innerHTML).toContain(
        '<my-cmp><div>OTHER<!--ICU 13--></div><!--container--></my-cmp>'
      );
    ...
    it('with nested ICU expression and inside a container when creating a view via vcr.createEmbeddedView', () => {
      ...
      @Component({
        ...
        template: `
          <my-cmp i18n="test"
            >{ count, plural, =1 {ONE} other {{{count}} {name, select, cat {cats} dog {dogs} other
            {animals} }!} }</my-cmp
          >
        `,
      })
      ...
      fixture.detectChanges();
      expect(fixture.debugElement.nativeElement.innerHTML).toBe(
        '<my-cmp><div>ONE<!--ICU 15--></div><!--container--></my-cmp>'
      );
    });
    ...
    it('should support nested ICUs without "other" cases', () => {
      ...
      @Component({
        ...
        template: `
          <div i18n="@@idA"
            >{ type, select, A {{count, select, 1 {one (select)} 2 {two (select)}}} other {} }</div
          >
          -
          <div i18n="@@idB"
    ...
    it('should support ICUs with pipes', () => {
      ...
      @Component({
        ...
        template: `
          <div i18n="@@idA"
            >{count$ | async, select, 1 {{{count$ | async}} item} 2 {two items}}</div
          >
        `,
      })
      class AppComponent {
  ...
  describe('should support attributes', () => {
    ...
    it('multiple attributes', () => {
      ...
      expect(fixture.nativeElement.innerHTML).toEqual(
        `<input title="bonjour Angular" placeholder="bonjour Angular">`
      );

      fixture.componentRef.instance.name = 'John';
      fixture.detectChanges();
      expect(fixture.nativeElement.innerHTML).toEqual(
        `<input title="bonjour John" placeholder="bonjour John">`
      );
    });
    ...
    it('should create corresponding ng-reflect properties', () => {
      ...
      @Component({
        template: `
          <welcome messageText="Hello" i18n-messageText="Welcome message description"> </welcome>
        `,
      })
      class App {}
  ...
  it('should work with directives and host bindings', () => {
    ...
    @Component({
      ...
      template: ` <div i18n test i18n-title title="start {{ exp1 }} middle {{ exp2 }} end" outer>
          trad: {exp1, plural, =0 {no <b title="none">emails</b>!} =1 {one <i>email</i>} other
          {{{exp1}} emails} } </div
        ><div test inner></div>`,
    })
    class MyApp {
  ...
  describe('projection', () => {
    ...
    it('should project a translated i18n block', () => {
      ...
      loadTranslations({
        [computeMsgId('Child of {$INTERPOLATION}')]: 'Enfant de {$INTERPOLATION}',
        [computeMsgId(
          'I am projected from {$INTERPOLATION}'
    ...
    it('should project translated content in i18n blocks', () => {
      ...
      loadTranslations({
        ...
        [computeMsgId(
          'and projection from {$INTERPOLATION}'
