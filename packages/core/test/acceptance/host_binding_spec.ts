...
describe('host bindings', () => {
  ...
  describe('defined in @Component', () => {
    it('should combine the inherited static classes of a parent and child component', () => {
      @Component({template: '...', host: {'class': 'foo bar'}, standalone: false})
      class ParentCmp {}

      @Component({template: '...', host: {'class': 'foo baz'}, standalone: false})
    ...
    it('should render host class and style on the root component', () => {
      @Component({template: '...', host: {class: 'foo', style: 'color: red'}, standalone: false})
      class MyApp {}
  ...
  describe('via @HostBinding', () => {
    it('should render styling for parent and sub-classed components in order', () => {
      ...
      @Component({selector: 'child-and-parent-cmp', template: '...', standalone: false})
      class ChildCmp extends ParentCmp {
    ...
    it('should prioritize styling present in the order of directive hostBinding evaluation, but consider sub-classed directive styling to be the most important', () => {
      @Component({template: '<div child-dir sibling-dir></div>', standalone: false})
      class MyApp {}

      @Directive({selector: '[parent-dir]', standalone: false})
      class ParentDir {
      ...
      @Directive({selector: '[sibling-dir]', standalone: false})
      class SiblingDir {
        @HostBinding('style.width')
        get width3() {
          return '300px';
    ...
    it('should allow class-bindings to be placed on ng-container elements', () => {
      ...
      @Directive({selector: '[dir-that-adds-other-classes]', standalone: false})
  ...
  it('should support host bindings on nodes with providers', () => {
    ...
    @Component({template: '', providers: [ServiceOne, ServiceTwo], standalone: false})
    class App {
      constructor(
  ...
  it('should support host bindings on multiple nodes', () => {
    ...
    @Component({selector: 'host-title-comp', template: '', standalone: false})
    class HostTitleComp {
      @HostBinding() title = 'my-title';
    }
  ...
  it('should support consecutive components with host bindings', () => {
    @Component({selector: 'host-binding-comp', template: '', standalone: false})
    class HostBindingComp {
      @HostBinding() id = 'blue';
    }
  ...
  it('should support dirs with host bindings on the same node as dirs without host bindings', () => {
    ...
    @Component({template: '<div someDir hostBindingDir></div>', standalone: false})
    class App {
      @ViewChild(HostBindingDir) hostBindingDir!: HostBindingDir;
    }
  ...
  it('should support host bindings that rely on values from init hooks', () => {
    @Component({template: '', selector: 'init-hook-comp', standalone: false})
  ...
  it('should support host bindings with the same name as inputs', () => {
    ...
    @Component({template: '<input hostBindingDir [disabled]="isDisabled">', standalone: false})
    class App {
  ...
  it('should support host bindings on second template pass', () => {
    @Component({selector: 'parent', template: '<div hostBindingDir></div>', standalone: false})
    class Parent {}
  ...
  it('should support component with host bindings and array literals', () => {
    ...
    @Component({selector: 'name-comp', template: '', standalone: false})
    class NameComp {
      @Input() names!: string[];
    }
  ...
  it('should support directives with and without allocHostVars on the same component', () => {
    ...
    @Directive({selector: '[hostListenerDir]', standalone: false})
    ...
    @Component({template: '<button hostListenerDir hostDir>Click</button>', standalone: false})
    class App {}
  ...
  it('should support host bindings with literals from multiple directives', () => {
    ...
    @Component({template: '<host-binding-comp hostDir></host-binding-comp>', standalone: false})
    class App {
      @ViewChild(HostBindingComp) hostBindingComp!: HostBindingComp;
      @ViewChild(HostBindingDir) hostBindingDir!: HostBindingDir;
    }
  ...
  it('should support ternary expressions in host bindings', () => {
    ...
    @Component({template: `<host-binding-comp></host-binding-comp>{{ name }}`, standalone: false})
    class App {
      @ViewChild(HostBindingComp) hostBindingComp!: HostBindingComp;
      name = '';
    }
  ...
  it('should merge attributes on host and template', () => {
    @Directive({selector: '[dir1]', host: {id: 'dir1'}, standalone: false})
    class MyDir1 {}
    @Directive({selector: '[dir2]', host: {id: 'dir2'}, standalone: false})
    class MyDir2 {}

    @Component({template: `<div dir1 dir2 id="tmpl"></div>`, standalone: false})
    class MyComp {}
  ...
  it('should work correctly with inherited directives with hostBindings', () => {
    @Directive({selector: '[superDir]', host: {'[id]': 'id'}, standalone: false})
    class SuperDirective {
      id = 'my-id';
    }

    @Directive({selector: '[subDir]', host: {'[title]': 'title'}, standalone: false})
  ...
  it('should support host attributes', () => {
    @Directive({selector: '[hostAttributeDir]', host: {'role': 'listbox'}, standalone: false})
    class HostAttributeDir {}

    @Component({template: '<div hostAttributeDir></div>', standalone: false})
    class App {}
  ...
  it('should support host bindings dependent on content hooks', () => {
    ...
    @Component({template: '<host-binding-comp></host-binding-comp>', standalone: false})
    class App {}
  ...
  describe('styles', () => {
    it('should bind to host styles', () => {
      ...
      @Component({template: '<host-binding-to-styles></host-binding-to-styles>', standalone: false})
      class App {
    ...
    it('should bind to host styles on containers', () => {
      @Directive({selector: '[hostStyles]', host: {'[style.width.px]': 'width'}, standalone: false})
      class HostBindingToStyles {
        width = 2;
      }

      @Directive({selector: '[containerDir]', standalone: false})
      class ContainerDir {
        constructor(public vcr: ViewContainerRef) {}
      }

      @Component({template: '<div hostStyles containerDir></div>', standalone: false})
      class App {
        @ViewChild(HostBindingToStyles) hostBindingDir!: HostBindingToStyles;
      }
    ...
    it('should apply static host classes', () => {
      ...
      @Component({template: '<static-host-class></static-host-class>', standalone: false})
      class App {}
  ...
  describe('sanitization', () => {
    ...
    function verify(
      ...
      it(`should sanitize <${tag} ${prop}> ${isAttribute ? 'properties' : 'attributes'}`, () => {
        ...
        @Component({template: `<${tag} unsafeUrlHostBindingDir></${tag}>`, standalone: false})
        class App {
  ...
  describe('host binding on containers', () => {
    @Directive({selector: '[staticHostAtt]', host: {'static': 'attr'}, standalone: false})
