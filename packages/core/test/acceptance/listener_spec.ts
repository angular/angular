...
describe('event listeners', () => {
  ...
  describe('coalescing', () => {
    ...
    @Directive({selector: '[md-button]', standalone: false})
    class MdButton {
      counter = 0;
      @HostListener('click')
      count() {
    ...
    @Directive({selector: '[likes-clicks]', standalone: false})
    class LikesClicks {
      counter = 0;
      @HostListener('click')
      count() {
    ...
    @Directive({selector: '[returns-false]', standalone: false})
    class ReturnsFalse {
      counter = 0;
      event!: Event;
    ...
    it('should try to execute remaining coalesced listeners if one of the listeners throws', () => {
      @Directive({selector: '[throws-on-clicks]', standalone: false})
  ...
  describe('global host listeners on special element as directive hosts', () => {
    it('should bind global event listeners on an ng-container directive host', () => {
      ...
      @Directive({selector: '[add-global-listener]', standalone: false})
