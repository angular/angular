...
describe('change detection', () => {
  ...
  describe('transplanted views', () => {
    describe('when declaration appears before insertion', () => {
      ...
      // TODO(atscott): test backwards references
      @Component({
        template: `
      ...
      describe('when declaration component is CheckAlways', () => {
        ...
        it('should CD with insertion only', () => {
          ...
          log.length = 0;
          expect(trim(fixture.nativeElement.textContent))
              .toEqual('InsertComp(Hello) Hello Angular!');
        });
      ...
      describe('when declaration component is OnPush', () => {
        ...
        // TODO(atscott): Ensure checkNoChanges works for OnPush transplanted views
        // blocked by https://github.com/angular/angular/pull/34443
    ...
    describe('works for backwards references', () => {
      @Component({
        selector: 'insertion',
        template: `
      ...
      it('when there is a change to declaration and declaration is marked dirty', () => {
        ...
        fixture.detectChanges(false);
        const expectedContent = ivyEnabled ?
        ...
        // VE did not handle this case for backwards referenced templates. Insertion is not dirty
        // but declaration is dirty. We still need to update the transplanted view because the
        // declaration point could have changes that the view cares about.
        expect(appComponent.declaration.transplantedViewRefreshCount).toEqual(ivyEnabled ? 1 : 0);
      });
      ...
      it('when there is a change to insertion and declaration is marked dirty', () => {
        ...
        expect(fixture.nativeElement.textContent)
            .toEqual(
                'Insertion(initial)TemplateDeclaration(initial)TemplateContext(initial)Declaration(initial)');
        // Again, VE did no handle this scenario (see previous test)
        expect(appComponent.declaration.transplantedViewRefreshCount).toEqual(ivyEnabled ? 1 : 0);
      });
      ...
      it('when both declaration and insertion are marked dirty', () => {
        ...
        appComponent.insertion.changeDetectorRef.markForCheck();
        fixture.detectChanges(false);
        // Expect two checks for transplanted view because it's a backwards reference:
        // Insertion is checked because it's dirty and then dirty flag gets cleared. Then
        // declaration gets checked and sees that insertion is not dirty and needs to be checked
        // because it has a dirty transplanted view. Again, VE did not handle not dirty insertion
        // but dirty declaration for backwards references
        expect(appComponent.declaration.transplantedViewRefreshCount).toEqual(ivyEnabled ? 2 : 1);
      });
