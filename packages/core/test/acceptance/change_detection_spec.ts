...
describe('change detection', () => {
  ...
  describe('embedded views', () => {
    @Directive({selector: '[viewManipulation]', exportAs: 'vm', standalone: false})
  ...
  describe('ChangeDetectorRef', () => {
    describe('detectChanges()', () => {
      ...
      it('should check component view when called by directive on component node', () => {
        @Component({template: '<my-comp dir></my-comp>', standalone: false})
        class MyApp {
          @ViewChild(MyComp) myComp!: MyComp;
      ...
      it('should check host component when called by directive on element node', () => {
        @Component({template: '{{ value }}<div dir></div>', standalone: false})
        class MyApp {
          @ViewChild(MyComp) myComp!: MyComp;
          @ViewChild(Dir) dir!: Dir;
          value = '';
      ...
      it('should check the host component when called from EmbeddedViewRef', () => {
        @Component({template: '{{ name }}<div *ngIf="showing" dir></div>', standalone: false})
        class MyApp {
          @ViewChild(Dir) dir!: Dir;
          showing = true;
          name = 'Amelia';
      ...
      it('should support call in ngOnInit', () => {
        @Component({template: '{{ value }}', standalone: false})
      ...
      ['OnInit', 'AfterContentInit', 'AfterViewInit', 'OnChanges'].forEach((hook) => {
        it(`should not go infinite loop when recursively called from children's ng${hook}`, () => {
          @Component({template: '<child-comp [inp]="true"></child-comp>', standalone: false})
          class ParentComp {
            constructor(public cdr: ChangeDetectorRef) {}
          ...
          @Component({template: '{{inp}}', selector: 'child-comp', standalone: false})
          class ChildComp {
            @Input() inp: any = '';
      ...
      it('should support call in ngDoCheck', () => {
        @Component({template: '{{doCheckCount}}', standalone: false})
      ...
      describe('dynamic views', () => {
        @Component({selector: 'structural-comp', template: '{{ value }}', standalone: false})
    ...
    describe('attach/detach', () => {
      @Component({selector: 'detached-comp', template: '{{ value }}', standalone: false})
      ...
      @Component({template: '<detached-comp></detached-comp>', standalone: false})
      class MyApp {
      ...
      it('should detach OnPush components properly', () => {
        ...
        @Component({template: '<on-push-comp [value]="value"></on-push-comp>', standalone: false})
        class OnPushApp {
    ...
    describe('checkNoChanges', () => {
      ...
      @Component({selector: 'no-changes-comp', template: '{{ value }}', standalone: false})
      class NoChangesComp {
        value = 1;
        doCheckCount = 0;
        contentCheckCount = 0;
      ...
      @Component({template: '{{ value }} - <no-changes-comp></no-changes-comp>', standalone: false})
      class AppComp {
        value = 1;
      ...
      it('should throw if bindings in embedded view have changed', () => {
        @Component({template: '<span *ngIf="showing">{{ showing }}</span>', standalone: false})
      ...
      ['ngAfterViewInit', 'ngAfterViewChecked', 'ngAfterContentChecked', 'ngDoCheck'].forEach(
        (hookName) => {
          it(`should be able to mark component as dirty from within ${hookName}`, () => {
            ...
            @Component({template: `<on-push-comp></on-push-comp>`, standalone: false})
            class TestApp {
              @ViewChild(OnPushComp) onPushComp!: OnPushComp;
            }
