...
import {Component, Directive, signal, ViewEncapsulation} from '@angular/core';
...
isBrowser &&
  describe('directive styles', () => {
    ...
    it('should clean up directive styles when the view is destroyed', async () => {
      @Directive({
        selector: '[destroyDir]',
        styles: [':host { color: rgb(12, 34, 56); }'],
      })
      class DestroyDir {}

      @Component({
        template: ` @if (show()) {
          <div destroyDir>Testing destroy</div>
        }`,
        imports: [DestroyDir],
      })
      class App {
      ...
      const styles2 = doc.querySelectorAll('style');
