...
describe('di with forwardRef', () => {
  describe('directive injection', () => {
    it('should throw if directives try to inject each other', () => {
      ...
      @Component({template: '<div dirA dirB></div>', standalone: false})
      class MyComp {}
    ...
    describe('flags', () => {
      describe('@Host', () => {
        it('should find host component on the host itself', () => {
          @Directive({selector: '[dirComp]', standalone: false})
          class DirectiveComp {
            constructor(@Inject(forwardRef(() => MyComp)) @Host() public comp: MyComp) {}
          }

          @Component({selector: 'my-comp', template: '<div dirComp></div>', standalone: false})
          class MyComp {
            @ViewChild(DirectiveComp) dirComp!: DirectiveComp;
          }

          @Component({template: '<my-comp></my-comp>', jit: true, standalone: false})
          class MyApp {
