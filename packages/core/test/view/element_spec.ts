...
import {
  ARG_TYPE_VALUES,
  callMostRecentEventListenerHandler,
  ...
  describe(`View Elements`, () => {
    describe('create', () => {
      ...
      it('should set fixed attributes', () => {
        const rootNodes = createAndGetRootNodes(
          compViewDef([elementDef(0, NodeFlags.None, null, null, 0, 'div', [['title', 'a']])])
        ).rootNodes;
        expect(rootNodes.length).toBe(1);
      ...
      ARG_TYPE_VALUES.forEach((inlineDynamic) => {
        it(`should update via strategy ${inlineDynamic}`, () => {
          const {view, rootNodes} = createAndGetRootNodes(
            compViewDef(
              [
                elementDef(0, NodeFlags.None, null, null, 0, 'input', null, [
      ...
      ARG_TYPE_VALUES.forEach((inlineDynamic) => {
        it(`should update via strategy ${inlineDynamic}`, () => {
          const {view, rootNodes} = createAndGetRootNodes(
            compViewDef(
              [
                elementDef(0, NodeFlags.None, null, null, 0, 'div', null, [
    ...
    if (isBrowser()) {
      describe('listen to DOM events', () => {
        function createAndAttachAndGetRootNodes(
        ...
        it('should listen to DOM events', () => {
          ...
          const {view, rootNodes} = createAndAttachAndGetRootNodes(
            compViewDef([
              elementDef(
                0,
                NodeFlags.None,
