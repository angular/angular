...
import {
  ...
  ɵCssKeyframesDriver as CssKeyframesDriver,
  ɵCssKeyframesPlayer as CssKeyframesPlayer,
...
(function () {
  ...
  describe('animation integration tests using css keyframe animations', function () {
    ...
    it('should cleanup all existing @keyframe <style> objects after the animation has finished', () => {
      @Component({
        ...
        animations: [
          trigger('myAnimation', [
            transition('* => go', [
              query('div', [style({opacity: 0}), animate('1s', style({opacity: 0}))]),
            ]),
          ]),
        ],
      })
    ...
    it('should properly handle easing values that are apart of the sequence', () => {
      @Component({
        selector: 'ani-cmp',
        template: ` <div #elm [@myAnimation]="myAnimationExp"></div> `,
        animations: [
          trigger('myAnimation', [
            transition('* => goSteps', [
              style({opacity: 0}),
            ...
            transition('* => goKeyframes', [
              animate(
                '1s cubic-bezier(0.5, 1, 0.5, 1)',
        ...
        animations: [
          trigger('myAnimation', [
            ...
            transition('* => go', [
              ...
              query('.bar', [style({width: '0px'}), animate('1s', style({width: '100px'}))]),
            ]),
          ]),
        ],
      })
