...
(function () {
  ...
  describe('animation tests', function () {
    ...
    modifiedInIvy('FW-952 - Error recovery is handled differently in Ivy than VE').it(
      'should continue to clean up DOM-related animation artifacts even if a compiler-level error is thrown midway',
      () => {
        @Component({
          selector: 'if-cmp',
          animations: [trigger('foo', [transition('* => something', [])])],
          template: `
            value = {{ foo[bar] }}
            <div #contents>
              <div *ngIf="exp">1</div>
