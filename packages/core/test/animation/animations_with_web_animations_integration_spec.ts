...
(function () {
  ...
  describe('animation integration tests using web animations', function () {
    ...
    it('should compute (*) animation styles for a container that is being removed', () => {
      ...
      let webPlayer = (engine
        .players[0] as TransitionAnimationPlayer).getRealPlayer() as ɵWebAnimationsPlayer;
    ...
    it('should treat * styles as ! for queried items that are collected in a container that is being removed', () => {
      ...
      let queriedPlayers = ((player as TransitionAnimationPlayer).getRealPlayer() as AnimationGroupPlayer)
        .players;
      ...
      player = engine.players[0]! as TransitionAnimationPlayer;
      queriedPlayers = ((player as TransitionAnimationPlayer).getRealPlayer() as AnimationGroupPlayer)
        .players;
      expect(queriedPlayers.length).toEqual(5);
    ...
    it('should compute intermediate styles properly when an animation is cancelled', () => {
      ...
      player = engine.players[0]!;
      webPlayer = (player as TransitionAnimationPlayer).getRealPlayer() as ɵWebAnimationsPlayer;
      expect(approximate(parseFloat(webPlayer.keyframes[0]['width'] as string), 150)).toBeLessThan(
        0.05
      );
      expect(approximate(parseFloat(webPlayer.keyframes[0]['height'] as string), 300)).toBeLessThan(
        0.05
      );
    });
    ...
    it('should compute intermediate styles properly for multiple queried elements when an animation is cancelled', () => {
      ...
      let player = engine.players[0]!;
      let groupPlayer = (player as TransitionAnimationPlayer).getRealPlayer() as AnimationGroupPlayer;
      ...
      for (let i = 0; i < players.length; i++) {
        ...
        expect(approximate(parseFloat(p.keyframes[0]['height'] as string), 500)).toBeLessThan(0.05);
      }
    });
