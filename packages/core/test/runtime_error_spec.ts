...
describe('RuntimeError utils', () => {
  it('should format the error message correctly', () => {
    const RuntimeError = createRuntimeErrorClass(PackageErrorPrefix.CORE);
  ...
  it('should include different package prefix in the error message', () => {
    const RuntimeError = createRuntimeErrorClass(PackageErrorPrefix.FORMS);

    // Error without a guide and an error message.
    const errorInstance = new RuntimeError(RuntimeErrorCode.TEMPLATE_STRUCTURE_ERROR, '');
    expect(errorInstance.toString()).toBe('Error: NG1305');
  });
});
