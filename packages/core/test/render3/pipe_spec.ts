...
describe('pipe', () => {
  ...
  describe('WrappedValue', () => {
    ...
    class WrappedPipe implements PipeTransform {
      ...
      static ngPipeDef = definePipe({
        type: WrappedPipe,
        factory: function WrappedPipe_Factory() { return new WrappedPipe(); },
        pure: false
      });
    }
    ...
    it('should record unwrapped values via ngOnChanges', () => {
      ...
      class TestDirective implements OnChanges {
        ...
        ngOnChanges(changes: SimpleChanges): void { testDirectiveChanges = changes; }
      ...
      class SomeComponent {
        ...
        static ngComponentDef = defineComponent({
          ...
          template: function(ctx: SomeComponent, cm: boolean) {
            if (cm) {
              elementStart(0, 'div', null, [TestDirective]);
              pipe(2, WrappedPipe.ngPipeDef);
              pipe(3, WrappedPipe.ngPipeDef);
              elementEnd();
            }
      ...
      const componentInstance = renderComponent(SomeComponent);
    ...
    it('should support passing wrapped value to the next pure pipe', () => {
      ...
      function Template(person: Person, cm: boolean) {
        ...
        debugger;
        textBinding(0, interpolation1('', pipeBind3(2, pipeBind1(1, person.name), 0, 1), ''));
      }
      person.init('value', null);
