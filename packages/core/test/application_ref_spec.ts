...

{
  describe('bootstrap', () => {
    ...
    it('should bootstrap a component from a child module', async(
    ...
    it('should bootstrap a component with a custom selector', async(
      inject([ApplicationRef, Compiler], (app: ApplicationRef, compiler: Compiler) => {
        @Component({
          selector: 'bootstrap-app',
          template: '',
    ...
    describe('bootstrapModule', () => {
      ...
      it('should call the `ngDoBootstrap` method with `ApplicationRef` on the main module', async(() => {
        ...
        defaultPlatform
          .bootstrapModule(createModule({ngDoBootstrap: ngDoBootstrap}))
          .then((moduleRef) => {
      ...
      it('should auto bootstrap components listed in @NgModule.bootstrap', async(() => {
        defaultPlatform
          .bootstrapModule(createModule({bootstrap: [SomeComponent]}))
          .then((moduleRef) => {
      ...
      it('should error if neither `ngDoBootstrap` nor @NgModule.bootstrap was specified', async(() => {
        defaultPlatform.bootstrapModule(createModule({ngDoBootstrap: false})).then(
          () => expect(false).toBe(true),
          (e) => {
      ...
      onlyInIvy('We only need to define `LOCALE_ID` for runtime i18n').it(
        'should define `LOCALE_ID`',
        async () => {
          @Component({
            selector: 'i18n-app',
            templateUrl: '',
