...
export abstract class PlatformRef {
   ...
   * @experimental APIs related to application bootstrap are currently under review.
   */
  abstract bootstrapModuleFactory<M>(moduleFactory: NgModuleFactory<M>, options?: BoostrapOptions):
  ...
  abstract bootstrapModule<M>(
      moduleType: Type<M>, compilerOptions?: (CompilerOptions&BoostrapOptions)|
      Array<CompilerOptions&BoostrapOptions>): Promise<NgModuleRef<M>>;
...
/**
 * Provides additional options to the boostraping process.
 *
 * @stable
 */
export interface BoostrapOptions {
  /**
   * Optionally specify if which `NgZone` should be used.
   *
   * - Provide your own `NgZone` instance.
   * - `true` - Use default `NgZone` which requires `Zone.js`.
   * - `false` - Use `NoNgZone` which does nothing.
   */
  ngZone?: NgZone|boolean;
}
...
export class PlatformRef_ extends PlatformRef {
  ...
  bootstrapModuleFactory<M>(moduleFactory: NgModuleFactory<M>, options?: BoostrapOptions):
      Promise<NgModuleRef<M>> {
    ...
    const ngZone: NgZone = ngZoneOption == false ?
        new NoNgZone() :
        ((ngZoneOption == true ? undefined : ngZoneOption) ||
