...
function prepareI18nBlockForHydrationImpl(lView, index, parentTNode, subTemplateIndex) {
  ...
  const i18nNodes = hydrationInfo.i18nNodes ?? (hydrationInfo.i18nNodes = new Map());
  const caseQueue = hydrationInfo.data[I18N_DATA]?.[index - HEADER_OFFSET] ?? [];
  const dehydratedIcuData =
    hydrationInfo.dehydratedIcuData ?? (hydrationInfo.dehydratedIcuData = new Map());
