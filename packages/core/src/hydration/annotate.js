...
import {DEFER_BLOCK_STATE as CURRENT_DEFER_BLOCK_STATE} from '../defer/interfaces';
...
function serializeHydrateTriggers(triggerMap) {
  const serializableDeferBlockTrigger = new Set([
    0 /* DeferBlockTrigger.Idle */, 1 /* DeferBlockTrigger.Immediate */,
    2 /* DeferBlockTrigger.Viewport */, 5 /* DeferBlockTrigger.Timer */,
  ]);
  let triggers = [];
...
function appendSerializedNodePath(ngh, tNode, lView, excludedParentNodes) {
  ...
  (_a = ngh[NODES])[noOffsetIndex] ??
    (_a[noOffsetIndex] = calcPathForNode(tNode, lView, excludedParentNodes));
}
/**
  ...
  for (let i = HEADER_OFFSET; i < tView.bindingStartIndex; i++) {
    ...
    if (isLContainer(lView[i])) {
      ...
      if (tNode.type & 8 /* TNodeType.ElementContainer */) {
        ...
        tNode.type &
        (16 /* TNodeType.Projection */ | 128) /* TNodeType.LetDeclaration */
      ) {
