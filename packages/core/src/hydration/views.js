...
export function findAndReconcileMatchingDehydratedViewsImpl(lContainer, templateTNode, hostLView) {
...
export function findAndReconcileMatchingDehydratedViews(lContainer, templateTNode, hostLView) {
...
function removeStaleDehydratedBranch(hostLView, tNode) {
  ...
  while (currentTNode) {
    ...
    if (
      (currentTNode.flags & 256) /* TNodeFlags.isControlFlowStart */ ===
  ...
  while (currentTNode) {
    if (
      (currentTNode.flags & 512) /* TNodeFlags.isInControlFlow */ !==
