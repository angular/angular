 ...
 */
function getDeferBlockId(tNode: TNode, lView: LView): string {
  return getLDeferBlockDetails(lView, tNode)?.[UNIQUE_ID];
}
  ...
  for (let i = CONTAINER_HEADER_OFFSET; i < lContainer.length; i++) {
    ...
    if (!serializedView) {
      ...
      if (isDeferBlock(lView[TVIEW], tNode)) {
        ...
        // We need to annotate defer blocks with their unique id for incremental hydration
