 ...
 */
export function getLViewEnterAnimations(
  lView: LView,
): Map<number, NodeAnimations<RunEnterAnimationFn>> {
 ...
 */
export function getLViewLeaveAnimations(
  lView: LView,
): Map<number, NodeAnimations<RunLeaveAnimationFn>> {
  const animationData = (lView[ANIMATIONS] ??= {});
 ...
 */
export function getClassListFromValue(value: string | (() => string | string[])): string[] | null {
...
export function addAnimationToLView(
  animations: Map<number, NodeAnimations<RunEnterAnimationFn | RunLeaveAnimationFn>>,
  tNode: TNode,
