...
export interface Directive {
   ...
   * class the input should write to, while the `publicName` determines the name under
   ...
   * `'name: publicName'` where `name` is the property on the class that the directive should write
   * to, while the `publicName` determines the name under which the input will be available in
   ...
   *   inputs: ['bankName', {name: 'id', publicName: 'account-id'}],
   *   template: `
   *     Bank Name: {{bankName}}
   *     Account Id: {{id}}
   *   `
   ...
   *
   * The `outputs` property defines a set of `directiveProperty` to `publicName`
   * configuration:
   *
   * - `directiveProperty` specifies the component property that emits events.
   * - `publicName` specifies the DOM property the event handler is attached to.
   *
   * @usageNotes
   *
   * ```typescript
...
export interface InputDecorator {
   ...
   */
  (arg?: string|{publicName?: string, required?: boolean}): any;
  new(arg?: string|{publicName?: string, required?: boolean}): any;
}
...
export const Input: InputDecorator =
    makePropDecorator('Input', (arg?: string|{publicName?: string, required?: boolean}) => {
      if (!arg) {
        return {};
      }
      return typeof arg === 'string' ? {publicName: arg} : arg;
    });
...
export interface OutputDecorator {
   ...
   *
   */
  (publicName?: string): any;
  new(publicName?: string): any;
}
...
export const Output: OutputDecorator =
    makePropDecorator('Output', (publicName?: string) => ({publicName}));
