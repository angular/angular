...
import {FLAGS, INJECTOR, LViewData, LViewFlags, PROJECTABLE_NODES, RootContext, TVIEW} from './interfaces/view';
...
export class ComponentFactory<T> extends viewEngine_ComponentFactory<T> {
  ...
  create(
    ...
    try {
      ...
      if (projectableNodes) {
        let index = 0;
        elementNode.data ![PROJECTABLE_NODES] = projectableNodes.map(nodeList => {
    ...
    const componentRef = new ComponentRef(
        this.componentType, component, rootView,
        ngModule ? ngModule !.injector : parentComponentInjector, hostNode !);
