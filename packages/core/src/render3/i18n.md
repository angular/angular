...
```typescript
<I18nUpdateOpCodes>[
  // The following OpCodes represent: `<div i18n-title="Hello �0�!">`
  // If `changeMask & 0b11`
...
To generate code where the extracted i18n messages have the same ids, the `ngtsc` can be placed int a special compatibility mode which will generated `goog.getMsg` in a special altered format as described next.

Given this Anuglar's template:
```HTML
<div i18n-title="Hello {{name}}!" i18n="Some description.">
  {{count}} is rendered as:
  <b *ngIf="true">
...
const MSG_div = goog.getMsg('{$COUNT_1} is rendered as: {$START_BOLD_TEXT_1}{$ICU}{$END_BOLD_TEXT_1}', {
  ICU: i18nIcuReplaceVar(MSG_div_icu, 'VAR_PLURAL', '�0:1�'),
  COUNT: '�0:1�',
...
The `i18nIcuReplaceVar(MSG_div_icu, 'VAR_PLURAL', '�0:1�')` function is needed to replace `VAR_PLURAL` for  `�0:1�`. 
