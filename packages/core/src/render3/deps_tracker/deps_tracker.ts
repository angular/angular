...
class DepsTracker implements DepsTrackerApi {
  ...
  getComponentDependencies(type: ComponentType<any>, rawImports?: (Type<any>|(() => Type<any>))[]):
    ...
    if (def.standalone) {
      ...
      const moduleType = this.ownerNgModule.get(type);
      if (!moduleType) {
        return {dependencies: []};
      }

      const scope = this.getNgModuleScope(moduleType);
   ...
   * already added to the type itself through methods like ``
   */
  registerNgModule(type: Type<any>, scopeInfo: NgModuleScopeInfoFromDecorator): void {
    ...
    // Lazily process the NgModules later when need be.
