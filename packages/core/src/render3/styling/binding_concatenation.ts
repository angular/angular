...
import {concatString, concatStylingEntry, getBindingPropName, getBindingSanitizer, getConcatenatedValue, getNextBindingIndex, getPreviousBindingIndex, getStyleBindingSuffix, getStylingTail, getValue, hasConfig, hasValueChanged, hyphenate, isComponentHostBinding, isDuplicateBinding, isStylingMap, isStylingValueDefined, setConcatenatedValue, setValue, splitOnWhitespace} from '../util/styling_utils';
...
export function flushBindings(
    ...
    firstUpdatePass: boolean, hostBindingsMode: boolean): void {
  ...
  flushStyleOrClassBindings(
      renderer, native, state, tNode, tData, lView, firstUpdatePass, hostBindingsMode, true,
...
function flushStyleOrClassBindings(
  ...
  if (startIndex !== 0) {
    ...
    const previousValue = concatenateAndGetPrevious(
        tData, lView, tNode, startIndex, tailIndex, firstUpdatePass, isClassBased,
        hostBindingsMode);
    ...
    if (allowValueToBeApplied(tNode, previousValue, newValue, firstUpdatePass)) {
      ...
      ngDevMode &&
          assertNotEqual(previousValue, newValue, `Expected ${previousValue} !== ${newValue}`);
...
function concatenateAndGetPrevious(
    ...
    firstUpdatePass: boolean, isClassBased: boolean, hostBindingsMode: boolean): string {
  ...
  processStylingBindingsUpToEnd(lView, tData, tNode, startIndex, isClassBased, null);
...
export function processStylingBindingsUpToEnd(
    lView: LStylingData, tData: TData, tNode: TNode, bindingIndex: number, isClassBased: boolean,
    sanitizerOverride: StyleSanitizeFn | null): void {
  while (bindingIndex !== 0) {
    const value = getValue(lView, bindingIndex);
    processStylingBinding(
        lView, tNode, tData, value, bindingIndex, isClassBased, sanitizerOverride);
...
function processStylingBinding(
    ...
    isClassBased: boolean, sanitizerOverride: StyleSanitizeFn | null): void {
  ...
  if (isStylingValueDefined(value)) {
    ...
    const sanitizer =
        sanitizerOverride !== null ? sanitizerOverride : getBindingSanitizer(tData, bindingIndex);

    // Case #1: prop-based entries (e.g. [style.prop] or [class.prop])
    if (typeof prop === 'string') {
      const valueToApply =
          normalizeValueForConcatenation(prop, value, suffix, sanitizer, false, isClassBased);
      concatenatedStr = addRemoveStylingEntryInConcatString(
          concatenatedStr, prop, valueToApply, isClassBased, isDuplicateOfPrevious);
    } else if (typeof value === 'string') {  // Case #2: string-based class bindings (e.g. [class]
                                             // or [class.prop])
      ngDevMode && assertEqual(isClassBased, true, 'Expecting to process class');
      if (!isDuplicateOfPrevious) {
        concatenatedStr = concatString(concatenatedStr, value, ' ');
      } else {
        let start = 0;
        ...
        while (start < end) {
          ...
          const index = concatenatedStr.indexOf(token);
          // we found the token, but we need to verify its not a substring of a larger
          // class value (e.g. "disabled" is a substring of "mobile-disabled" but it's a
          // separate token)
          if (index === -1 ||
              (!isWhitespace(concatenatedStr, index - 1, -1) &&
               !isWhitespace(concatenatedStr, index + token.length, concatenatedStr.length))) {
      ...
      const map = value as{[key: string]: string | false | null};
      const props = Object.keys(value);
      const length = props ? props.length : 0;
      for (let i = 0; i < length; i++) {
        let prop = props ![i];
        let valueToApply = map[prop];
        valueToApply = normalizeValueForConcatenation(
            prop, valueToApply, suffix, sanitizer, true, isClassBased);
...
function isWhitespace(value: string, index: number, limit: number): boolean {
  return index === limit || value.charCodeAt(index) <= CharCode.SPACE;
}
