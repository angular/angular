...
- `�{index}(:{block})�`: _Binding placeholder_: Marks a location where an interpolated expression will be rendered.
  - `index`: the index of the binding within this i18n message block.
  - `block` (_optional_): the index of the sub-template block, in which this placeholder was declared.

- `�#{index}(:{block})� ... �/#{index}(:{block})�`: _Element block_: Marks the beginning and end of a DOM element that is embedded in the original translation string.
  - `index`: the index of the element, as defined in the template instructions (e.g. `elementStart(index, ...)`).
  - `block` (_optional_): the index of the sub-template block, in which this element was declared.

- `�*{index}:{block}�`/`�/*{index}:{block}�`: _Sub-template block_: Marks a sub-template block that is translated separately in its own angular template function.
...
```html
<div i18n-title title="Hello {{name}}!"></div>
```
...
<div i18n>
  {{count}} is rendered as:
  <b *ngIf="exp">
    { count, plural, =0 {no <b title="none">emails</b>!} =1 {one <i>email</i>} other {{{count}}
    <span title="{{count}}">emails</span>} } </b
  >.
</div>
```
...
@Component({
  template: `
    {count, plural,
      =0 {zero}
      other {{{count}} {animal, select, cat {cats} dog {dogs} other {animals}}!}
    }
  `,
})
class MyComponent {
 ...
 */
let MSG_div = goog.getMsg(
  `{$COUNT} is rendered as: {$START_BOLD_TEXT_1}{{$COUNT}, plural,
 ...
 */
const MSG_div_raw = goog.getMsg(
  '{$COUNT_1} is rendered as: {$START_BOLD_TEXT_1}{$ICU}{$END_BOLD_TEXT_1}',
  {
    ICU: MSG_div_icu,
    COUNT: '�0:1�',
