...
import {CONTAINER_HEADER_OFFSET, MOVED_VIEWS, NATIVE} from './interfaces/container';
...
export function detachViewFromDOM(tView, lView) {
  ...
  lView[ENVIRONMENT].changeDetectionScheduler?.notify(
    9 /* NotificationSource.ViewDetachedFromDOM */,
  );
...
function hasLeaveAnimations(lView) {
...
export function getClosestRElement(tView, tNode, lView) {
  ...
  if (parentTNode === null) {
    ...
    ngDevMode &&
      assertTNodeType(parentTNode, 3 /* TNodeType.AnyRNode */ | 4 /* TNodeType.Container */);
  ...
  while (tNode != null) {
    ...
    if (!isDetachedByI18n(tNode)) {
      if (tNodeType & 8 /* TNodeType.ElementContainer */) {
        ...
        ngDevMode &&
          assertTNodeType(tNode, 3 /* TNodeType.AnyRNode */ | 4 /* TNodeType.Container */);
        applyToElementOrContainer(
          action,
          renderer,
          parentRElement,
