...
import {RComment, RElement, RText, Renderer3, Renderer3Fn, Renderer3oo} from './renderer';
 ...
 */
export function
renderTemplate<T>(host: LElement, template: ComponentTemplate<T>, context: T) {
  ...
  if (value !== NO_CHANGE) {
    ...
    if (value == null) {
      (renderer as Renderer3Fn).removeStyle ?
          (renderer as Renderer3Fn).removeStyle(lElement.native, styleName) :
          lElement.native.style.removeProperty(styleName);
    } else {
      (renderer as Renderer3Fn).setStyle ?
          (renderer as Renderer3Fn)
              .setStyle(
                  lElement.native, styleName,
                  suffix ? stringify(value) + suffix : stringify(value)) :
