...
import {ComponentFactory, ComponentRef as IComponentRef, ElementRef as IElementRef, EmbeddedViewRef as IEmbeddedViewRef, Injector, NgModuleRef as INgModuleRef, TemplateRef as ITemplateRef, Type, ViewContainerRef as IViewContainerRef, ViewRef as IViewRef} from '@angular/core';
...
export function bloomFindPossibleInjector(injector: LNodeInjector, bloomBit: number): LNodeInjector|
  ...
  while (di) {
    ...
    let value: number = bloomBit < 64 ?
        (bloomBit < 32 ? di.bf0 : di.bf1) :
        (bloomBit < 96 ? di.bf2 : di.bf3) if ((value & mask) === mask) {
      return di;
    }
