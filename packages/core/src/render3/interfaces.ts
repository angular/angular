...
export interface ViewState {
  ...
  /**
   * All element/text/container nodes created inside this view. Stored
   * as an array rather than a linked list so we can look up nodes
   * directly in the case of forward declaration or bindings (e.g. E(1)).
   *
   ...
   */
  readonly nodes: (LText|LElement|LContainer|LProjection)[];
   ...
   * appear in the template. We use `bindingIndex` to internally keep track
   * of which binding is currently active.
   */
 ...
 * Value: object containing minified name and related directive index
 */
 ...
 *
 * Inputs and outputs need separate minified names and indices in case
 * an element has an input and an output with the same name, so they
 * have separate arrays. In each array:
 *
 * Even indices: directive index
 * Odd indices: minified name
 *
 * e.g.
 * {inputs: [0, Comp1], outputs: [1, Comp2]}
 */
export type MinificationDataValue = {
...
export interface LElement extends LNode {
   ...
   * Attributes are stored in the L-tree as reading them from the DOM
