...
import {
  PROJECTED_SIGNAL_NODE,
  type ProjectedSignalNode,
  setProjectedSignalWriter,
} from './internal';

export interface ProjectedSignalOptions {
  debugName?: string;
}

export function projectedSignal<T extends {}, K extends keyof T>(
  parent: WritableSignal<T>,
  key: K | Signal<K>,
  options?: ProjectedSignalOptions,
): WritableSignal<T[K]> {
  type V = T[K];

  let keySignal: Signal<K> | undefined;

  // Create our `PROJECTED_SIGNAL_NODE` and initialize it.
  const node: ProjectedSignalNode = Object.create(PROJECTED_SIGNAL_NODE);
  if (isSignal(key)) {
    keySignal = key;
  ...
  // The projected signal specific write path.
  getter.set = (value: V) => {
    ...
    // exist, we don't use the `projectedSignal` write path. This ensures any `structuralSignal`s will
    // get notified of this change.
    let prevWriter: ProjectedSignalNode | null;
    if ((node.lastProperty as K) in current) {
      prevWriter = setProjectedSignalWriter(node);
    } else {
      prevWriter = setProjectedSignalWriter(null);
    }
    ...
    try {
      ...
      // `projectedSignal`s is updated. We probably want to delay that until we come back up the stack
      // and finish the write path of every `projectedSignal` along the way (marking clean, etc).

      // Unlike normal computed nodes, projected signals handle updating their own value. Therefore, once
      // we set the value we can consider ourselves clean. We could set our value earlier.
      node.value = value;
      producerMarkClean(node);
    } finally {
      setProjectedSignalWriter(prevWriter);
    }
  };
  ...
  if (typeof ngDevMode !== 'undefined' && ngDevMode) {
    ...
    getter.toString = () => `[projectedSignal${debugName}: ${node.value}]`;
  }
