...
export const PROJECTED_SIGNAL_NODE: Omit<ProjectedSignalNode, 'computation' | 'parentNode'> =
  /* @__PURE__ */ (() => {
    return {
      ...COMPUTED_NODE,
      kind: 'projectedSignal',
      propertyNode: undefined,
      lastProperty: undefined,
      rawDirty: false,
      get dirty(): boolean {
        if (isSafeToShortCircuitNotifications(this as ProjectedSignalNode)) {
 ...
 */
function isSafeToShortCircuitNotifications(node: ProjectedSignalNode): boolean {
  // If the current write path doesn't involve any projected signal writes, then we're potentially affected.
  if (projectedSignalWriter === null) {
    return false;
  }

  // `parentNode` is the most recent projected signal being written. If this node isn't also a child
  // of that parent, then we can't guarantee we're not impacted by the write.
  if (projectedSignalWriter.parentNode !== node.parentNode) {
    return false;
  }
  ...
  if (projectedSignalWriter.lastProperty === node.lastProperty) {
    return false;
  }

  // We've proven that:
  //  * another projected signal is changing a specific property of our parent signal
...
export function setProjectedSignalWriter(
  node: ProjectedSignalNode | null,
): ProjectedSignalNode | null {
  const prev = projectedSignalWriter;
  projectedSignalWriter = node;
  return prev;
}

let projectedSignalWriter: ProjectedSignalNode | null = null;
