 ...
 */
@Injectable()
export class DIDebugData implements OnDestroy {
...
export class PlatformDIDebugData extends DIDebugData {
}

@Injectable({providedIn: 'root'})
export class ApplicationDIDebugData extends DIDebugData {
}

export function getFrameworkDIDebugData(injector: Injector): DIDebugData {
  const isPlatformInjector = injector instanceof R3Injector && injector.scopes.has('platform');
  return isPlatformInjector ? injector.get(PlatformDIDebugData) :
                              injector.get(ApplicationDIDebugData);
}
