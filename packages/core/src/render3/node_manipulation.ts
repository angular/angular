...
function trackMovedView(declarationContainer: LContainer, lView: LView) {
  ...
  if (declarationComponentIsCheckAlways) {
    insertionLContainer[CONTAINS_ALWAYS_COUNT]++;
    ...
    while (parent !== null && viewOrContainer[CONTAINS_ALWAYS_COUNT] === 1) {
      parent[CONTAINS_ALWAYS_COUNT]++;
...
function detachMovedView(declarationContainer: LContainer, lView: LView) {
  ...
  if (declarationIsCheckAlways) {
    ...
    // on the parent.
    while (parent !== null && viewOrContainer[CONTAINS_ALWAYS_COUNT] === 0) {
      parent[CONTAINS_ALWAYS_COUNT]--;
      viewOrContainer = parent;
      parent = getLViewParent(parent);
    }
