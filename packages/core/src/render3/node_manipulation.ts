  ...
  if (parentRNode != null) {
    if (Array.isArray(childRNode)) {
      for (let i = 0; i < childRNode.length; i++) {
        nativeAppendOrInsertBefore(renderer, parentRNode, childRNode[i], anchorNode);
      }
    } else {
      nativeAppendOrInsertBefore(renderer, parentRNode, childRNode, anchorNode);
    }
  }
...
function processI18nText(
  ...
  if (i18nParent !== null) {
    for (let i = 1; i < i18nChildren.length; i++) {
      ...
      nativeInsertBefore(renderer, i18nParent, i18nChild, anchorRNode);
    }
  }
}
