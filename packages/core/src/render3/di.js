...
import {NO_PARENT_INJECTOR, NodeInjectorFactory} from './interfaces/injector';
    ...
    try {
      if (moduleInjector) {
        return moduleInjector.get(
          token,
          notFoundValue,
          flags & 8 /* InternalInjectFlags.Optional */,
        );
      } else {
...
function lookupTokenUsingNodeInjector(tNode, lView, token, flags, notFoundValue) {
  ...
  if (typeof bloomHash === 'function') {
    ...
    let hostTElementNode =
      flags & 1 /* InternalInjectFlags.Host */ ? lView[DECLARATION_COMPONENT_VIEW][T_HOST] : null;
    ...
    while (injectorIndex !== -1) {
      ...
      if (
        ...
        shouldSearchParent(
          flags,
          lView[TVIEW].data[injectorIndex + 8 /* NodeInjectorOffset.TNODE */] === hostTElementNode,
        ) &&
...
export function locateDirectiveOrProvider(
  ...
  const injectablesStart =
    nodeProviderIndexes & 1048575; /* TNodeProviderIndexes.ProvidersStartIndexMask */
  const directivesStart = tNode.directiveStart;
  const directiveEnd = tNode.directiveEnd;
  const cptViewProvidersCount =
    nodeProviderIndexes >> 20; /* TNodeProviderIndexes.CptViewProvidersCountShift */
 ...
 */
export function getNodeInjectable(lView, tView, index, tNode, flags) {
...
function shouldSearchParent(flags, isFirstHostTNode) {
  return (
    !((flags & 2) /* InternalInjectFlags.Self */) &&
    !(flags & 1 /* InternalInjectFlags.Host */ && isFirstHostTNode)
  );
}
