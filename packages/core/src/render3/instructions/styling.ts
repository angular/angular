...
import {getActiveDirectiveId, getCheckNoChangesMode, getLView, getSelectedIndex, setElementExitFn} from '../state';
...
/**
 * Shared [style.prop] wrapper for styleProp() and style prop interpolation instructions
 */
export function stylePropInternal(
    elementIndex: number, prop: string, value: string | number | SafeValue | null,
    suffixOrSanitizer: StyleSanitizeFn | string | null | undefined): void {
  const suffix: string|null = typeof suffixOrSanitizer === 'string' ? suffixOrSanitizer : null;
  const sanitizer: StyleSanitizeFn|null =
      typeof suffixOrSanitizer === 'function' ? suffixOrSanitizer : null;
  stylingBindingInternal(elementIndex, prop, value, suffix, sanitizer, false);
}
...
export function ɵɵclassProp(className: string, value: boolean | null): typeof ɵɵclassProp {
  stylingBindingInternal(getSelectedIndex(), className, value, null, null, true);
  return ɵɵclassProp;
}
...
export function ɵɵstyleMap(
    ...
    sanitizer?: StyleSanitizeFn | null): void {
  stylingBindingInternal(getSelectedIndex(), null, styles, null, sanitizer || null, false);
}
...
export function ɵɵclassMap(classes: {[className: string]: any} | NO_CHANGE | string | null): void {
  stylingBindingInternal(getSelectedIndex(), null, classes, null, null, true);
}
...
export function stylingBindingInternal(
  ...
  if (bindingBelongsToDirectiveInput(tNode, prop, isClassBased)) {
    ...
    if (firstUpdatePass) {
      registerBinding(tNode, tData, bindingIndex, state, prop, suffix, sanitizer, isClassBased);
    }
...
function flushStyling(): void {
  ...
  flushBindings(
      renderer, native, lView, tNode, tData, state, tView.firstUpdatePass, hostBindingsMode);
  resetStylingState();
}
