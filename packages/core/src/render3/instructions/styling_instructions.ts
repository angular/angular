...
export function elementHostClassProp(
  ...
  const input = (value instanceof BoundPlayerFactory) ?
      (value as BoundPlayerFactory<boolean|null>) :
      booleanOrNull(value);
  updateElementClassProp(stylingContext, classIndex, input, directiveStylingIndex, forceOverride);
}
...
export function elementHostStylingMap(
  ...
  updateStylingMap(stylingContext, classes, styles, directiveStylingIndex);
}
