...
export function ɵɵdefineComponent<T>(componentDefinition: {
  ...
  return noSideEffects(() => {
    ...
    const rawDependencies = componentDefinition.dependencies;
    const dependencies = rawDependencies && (typeof rawDependencies === 'function') ?
        rawDependencies() :
        rawDependencies;
    ...
    def.directiveDefs =
        dependencies ? (() => dependencies.map(extractDirectiveDef).filter(nonNull)) : null;
    def.pipeDefs = dependencies ? (() => dependencies.map(getPipeDef).filter(nonNull)) : null;
