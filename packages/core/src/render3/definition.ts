...
/**
 * A method that returns a component or directive ID from the definition using a variant of DJB2 hash
 * algorithm.
 */
function getDefinitionId<T>(def: DirectiveDef<T> | ComponentDef<T>): string {
  let hash = 0;
  ...
  const componentDef = def as Partial<ComponentDef<T>>;
  const componentDefConsts = typeof componentDef.consts === 'function' ? '' : componentDef.consts;

  // We cannot rely solely on the component selector as the same selector can be used in different
  // modules.
  //
  // `def.style` is not used, due to it causing inconsistencies. Ex: when server
  ...
  const hashSelectors = [
    ...
    // We cannot use 'def.type.name' as the name of the symbol will change and will not
    // match in the server and browser bundles.
    Object.getOwnPropertyNames(def.type.prototype),
    !!def.contentQueries,
    !!def.viewQuery,
  ];
  ...
  if (
    ...
    if (GENERATED_COMP_IDS.has(compId)) {
      ...
      GENERATED_COMP_IDS.set(compId, def.type);
    }
  }
