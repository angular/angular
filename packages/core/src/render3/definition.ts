...
export function ɵɵdefineComponent<T>(componentDefinition: ComponentDefinition<T>):
  ...
  return noSideEffects(() => {
    ...
    def.features?.forEach((fn) => fn(def));
...
export function ɵɵdefineDirective<T>(directiveDefinition: DirectiveDefinition<T>):
  ...
  return noSideEffects(() => {
    ...
    // TODO(alanagius): the below should not be needed but there are a number of checks
    // that don't handle 'undefined' template for directives.
    (def as any).template = null;
