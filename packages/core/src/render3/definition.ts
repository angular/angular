...
import {ComponentDef, ComponentDefFeature, ComponentTemplate, ComponentType, ContentQueriesFunction, DirectiveDef, DirectiveDefFeature, DirectiveTypesOrFactory, HostBindingsFunction, PipeDef, PipeTypesOrFactory, ProtoPipeDef, ViewQueriesFunction} from './interfaces/definition';
 ...
 *   // Generated by Angular Template Compiler
 *   static ɵpipe = <ProtoPipeDef>{
 *     ...
 *   };
 * }
 * ```
 *
 * NOTE call to `ɵɵdefinePipe` is not necessary as the runtime will automatically call if it
 * detects the property to be of `ProtoPipeDef`.
 *
 * @param pipeDef Pipe definition generated by the compiler
 *
 * @codeGenApi
 */
export function ɵɵdefinePipe<T>(protoPipDef: ProtoPipeDef<T>): PipeDef<T> {
  return new PipeDef<T>(protoPipDef);
}

/**
 * Type representing all of the optional definition properties which could be placed on the type.
 */
export interface DefType<T> {
  [NG_PIPE_DEF]?: PipeDef<T>|ProtoPipeDef<T>;
}
...
export function getPipeDef<T>(type: DefType<any>): PipeDef<T>|null {
  const value = type[NG_PIPE_DEF];
  if (!value) {
    // Don't have definition.
    return null;
  } else if (value instanceof PipeDef) {
    // Already normalized as `PipeDef`
    return value;
  } else {
    // This is `ProtoPipeDef` needs to be converted to `PipeDef`.
    return type[NG_PIPE_DEF] = ɵɵdefinePipe(value);
  }
}
