...
abstract class BaseWritableResource<T> implements WritableResource<T> {
  ...
  constructor(value: Signal<T>, debugName: string | undefined) {
    ...
    this.isLoading = computed(
      ...
      ngDevMode ? createDebugNameObject(debugName, 'isLoading') : {},
    );
  }
...
export class ResourceImpl<T, R> extends BaseWritableResource<T> implements ResourceRef<T> {
  ...
  constructor(
    ...
    super(
      ...
      computed(
        ...
        {equal, ...(ngDevMode ? createDebugNameObject(debugName, 'value') : {})},
      ),
      debugName,
    );
    ...
    this.extRequest = linkedSignal({
      ...
      ...(ngDevMode ? createDebugNameObject(debugName, 'extRequest') : {}),
    });
    ...
    this.state = linkedSignal<WrappedRequest, ResourceState<T>>({
      ...
      ...(ngDevMode ? createDebugNameObject(debugName, 'state') : {}),
    });

    this.effectRef = effect(this.loadEffect.bind(this), {
      injector,
      manualCleanup: true,
      ...(ngDevMode ? createDebugNameObject(debugName, 'loadEffect') : {}),
    });
    ...
    this.status = computed(
      ...
      ngDevMode ? createDebugNameObject(debugName, 'status') : {},
    );
    ...
    this.error = computed(
      ...
      ngDevMode ? createDebugNameObject(debugName, 'error') : {},
    );
  }
  ...
  override set(value: T): void {
    ...
    this.state.set({
      ...
      stream: signal({value}, ngDevMode ? createDebugNameObject(this.debugName, 'stream') : {}),
    });
    ...
    try {
      ...
      this.state.set({
        ...
        stream: signal(
          ...
          ngDevMode ? createDebugNameObject(this.debugName, 'stream') : {},
        ),
      });
    } finally {
  ...
  return async (params) => {
    try {
      return signal(
        ...
        ngDevMode ? createDebugNameObject(options.debugName, 'stream') : {},
      );
    } catch (err) {
      return signal(
        {error: encapsulateResourceError(err)},
