...
import {InjectableDef, InjectableDefType, InjectorDef, InjectorType, InjectorTypeWithProviders} from './defs';
...
export class R3Injector {
  ...
  constructor(def: InjectorType<any>, readonly parent: Injector) {
    ...
    this.isRootScope = this.records.has(APP_ROOT);
    ...
    try {
      ...
      if (!(flags & InjectFlags.SkipSelf)) {
        ...
        if (record === undefined) {
          ...
          const def = couldBeInjectableDefType(token) &&
                  (token as InjectableDefType<any>| InjectableDefToken<any>).ngInjectableDef ||
              undefined;
  ...
  private injectableDefInScope(def: InjectableDef<any>): boolean {
    if (!def.providedIn) {
      ...
      return def.providedIn === 'any' || (def.providedIn === 'root' && this.isRootScope);
    } else {
...
function injectableDefRecord(token: Type<any>| InjectionToken<any>): Record<any> {
  const def = (token as InjectableDefType<any>).ngInjectableDef;
  if (def === undefined) {
...
function couldBeInjectableDefType(value: any): value is Type<any>|InjectionToken<any> {
  return (typeof value === 'function') ||
