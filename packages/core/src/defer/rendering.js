...
import {DEFER_BLOCK_STATE as SERIALIZED_DEFER_BLOCK_STATE} from '../hydration/interfaces';
 ...
 */
export const DEFER_BLOCK_DEPENDENCY_INTERCEPTOR = /* @__PURE__ */ new InjectionToken(
  'DEFER_BLOCK_DEPENDENCY_INTERCEPTOR',
);
/**
 * **INTERNAL**, token used for configuring defer block behavior.
 */
export const DEFER_BLOCK_CONFIG = new InjectionToken(
...
function findMatchingDehydratedViewForDeferBlock(lContainer, lDetails) {
  const dehydratedViewIx =
    lContainer[DEHYDRATED_VIEWS]?.findIndex(
      (view) => view.data[SERIALIZED_DEFER_BLOCK_STATE] === lDetails[DEFER_BLOCK_STATE],
    ) ?? -1;
...
export function renderPlaceholder(lView, tNode) {
 ...
 */
export function renderDeferStateAfterResourceLoading(tDetails, tNode, lContainer) {
  ngDevMode &&
