 ...
 */
export function isDeferBlock(tView, tNode) {
  let tDetails = null;
  const slotIndex = getDeferBlockDataIndex(tNode.index);
 ...
 */
export function trackTriggerForDebugging(tView, tNode, textRepresentation) {
  var _a;
  const tDetails = getTDeferBlockDetails(tView, tNode);
  tDetails.debug ?? (tDetails.debug = {});
  (_a = tDetails.debug).triggers ?? (_a.triggers = new Set());
