...
export async function triggerHydrationFromBlockName(
  ...
  for (let i = 0; i < hydrationQueue.length; i++) {
    const dehydratedBlockId = hydrationQueue[i];
    ...
    if (dehydratedDeferBlock != null) {
      ...
      if (deferBlockHasErrored(dehydratedDeferBlock.lView, dehydratedDeferBlock.tNode)) {
        ...
        const dehydratedViewsToCleanup = dehydratedDeferBlock.lContainer[DEHYDRATED_VIEWS];
        removeDehydratedViewList(dehydratedViewsToCleanup, dehydratedDeferBlock.lContainer);
        cleanupRemainingHydrationQueue(hydrationQueue.slice(i), dehydratedBlockRegistry);
        break;
      }
      ...
      cleanupParentContainer(i, hydrationQueue, dehydratedBlockRegistry);
      cleanupRemainingHydrationQueue(hydrationQueue.slice(i), dehydratedBlockRegistry);
      break;
    }
  }
...
export function deferBlockHasErrored(lView: LView, tNode: TNode): boolean {
  return getLDeferBlockDetails(lView, tNode)[DEFER_BLOCK_STATE] === DeferBlockState.Error;
}
...
function cleanupParentContainer(
  ...
  if (i > 0 && dehydratedBlockRegistry.get(hydrationQueue[i - 1]) != null) {
    const parentDeferBlock = dehydratedBlockRegistry.get(hydrationQueue[i - 1])!;
