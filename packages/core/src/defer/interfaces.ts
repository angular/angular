...
/** A deferred dependency that can be loaded again. */
export interface DeferBlockDependencyLoader<T = DependencyType> {
  /** Loads the module. */
  load: () => Promise<unknown>;

  /** Resolves the dependency from the module. */
  resolve: (module: unknown) => T;
}
...
export type DependencyResolverFn = () => Array<
  Promise<DependencyType> | DeferBlockDependencyLoader | DependencyType
>;

/** Result of loading deferred dependencies. */
export type DeferDependenciesLoadResult =
  {dependencies: DependencyType[]; error: null} | {dependencies: null; error: unknown};

/**
 * Loads deferred dependencies with retry support. Keeping this implementation
 * optional allows applications without retry blocks to tree-shake it.
 */
export type DeferDependencyRetryLoadingFn = (
...
export interface TDeferBlockDetails {
  ...
  /**
   * Number of additional loading attempts configured by `@error (retry N)`, or
   * `null` when retry is disabled.
   */
  maxRetryCount: number | null;

  /**
   * Retry-aware loader installed by `ɵɵdeferEnableRetry`, or `null` when retry
   * is disabled.
   */
  retryLoadingFn: DeferDependencyRetryLoadingFn | null;
