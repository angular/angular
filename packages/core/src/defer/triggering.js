...
import {INJECTOR, TVIEW} from '../render3/interfaces/view';
 ...
 */
export function scheduleDelayedTrigger(scheduleFn) {
 ...
 */
export function scheduleDelayedPrefetching(scheduleFn, trigger) {
 ...
 */
export function scheduleDelayedHydrating(scheduleFn, lView, tNode) {
  if (typeof ngServerMode !== 'undefined' && ngServerMode) return;
  // Only trigger the scheduled trigger on the browser
  // since we don't want to delay the server response.
  const injector = lView[INJECTOR];
    ...
    if (failed) {
      ...
      if (tDetails.errorTmplIndex === null) {
        ...
        const error = new RuntimeError(
          -750 /* RuntimeErrorCode.DEFER_LOADING_FAILED */,
          ngDevMode &&
...
function shouldTriggerDeferBlock(triggerType, lView) {
  ...
  if (
    triggerType === 0 /* TriggerType.Regular */ &&
 ...
 */
export function triggerDeferBlock(triggerType, lView, tNode) {
  const tView = lView[TVIEW];
 ...
 * events.
 */
export async function triggerHydrationFromBlockName(injector, blockName, replayQueuedEventsFn) {
...
export function deferBlockHasErrored(deferBlock) {
  return (
 ...
 */
export function shouldAttachTrigger(triggerType, lView, tNode) {
...
export function hasHydrateTriggers(flags) {
  return (
    flags != null &&
    (flags & 1) /* TDeferDetailsFlags.HasHydrateTriggers */ ===
 ...
 */
export function getHydrateTriggers(tView, tNode) {
  const tDetails = getTDeferBlockDetails(tView, tNode);
  return tDetails.hydrateTriggers ?? (tDetails.hydrateTriggers = new Map());
}
/**
 * Loops through all defer block summaries and ensures all the blocks triggers are
 * properly initialized
 */
export function processAndInitTriggers(injector, blockData, nodes) {
  const idleElements = [];
  const timerElements = [];
...
//# sourceMappingURL=triggering.js.map
