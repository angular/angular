...
export interface ToObservableOptions {
  ...
  /**
   * Prevent `toObservable` from emitting the initial value of the signal synchronously.
   *
   * It will instead be emitted from the effect that watches the signal over time.
   *
   * This was the behavior of `toObservable` during its developer preview. This flag exists to
   * support a more incremental migration to the new behavior.
   *
   * @deprecated
   */
  forceAsyncFirstEmit?: true;
}
...
export function toObservable<T>(source: Signal<T>, options?: ToObservableOptions): Observable<T> {
  ...
  return new Observable<T>((subscriber) => {
    ...
    if (TO_OBSERVABLE_SYNC && options?.forceAsyncFirstEmit !== true) {
      let firstValue: T;
      try {
