...
export class TestBedImpl implements TestBed {
  ...
  createComponent<T>(type: Type<T>, options?: TestComponentOptions): ComponentFixture<T> {
    if (getAsyncClassMetadataFn(type)) {
      const hasOverrides = this.compiler.hasComponentOverrides(type);
      const hasTemplateOverrides = this.compiler.hasComponentTemplateOverrides(type);
      const isCompiled = !!getComponentDef(type);

      if ((hasOverrides && hasTemplateOverrides) || !isCompiled) {
        throw new Error(
