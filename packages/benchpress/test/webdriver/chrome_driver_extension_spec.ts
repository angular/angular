...

{
  describe('chrome driver extension', () => {
    ...
    it('should force gc via window.gc()', inject(
      [AsyncTestCompleter],
    ...
    it('should clear the perf logs and mark the timeline via performance.mark() on the first call', inject(
      [AsyncTestCompleter],
      (async: AsyncTestCompleter) => {
        createExtension()
          .timeBegin('someName')
    ...
    it('should mark the timeline via performance.mark() on the second call', inject(
      [AsyncTestCompleter],
      (async: AsyncTestCompleter) => {
    ...
    it('should mark the timeline via performance.mark()', inject(
      [AsyncTestCompleter],
      (async: AsyncTestCompleter) => {
        createExtension()
    ...
    it('should mark the timeline via performance.mark() with start and end of a test', inject(
      [AsyncTestCompleter],
      (async: AsyncTestCompleter) => {
        createExtension()
          .timeEnd('name1', 'name2')
          .then((_) => {
            expect(log).toEqual([
              [
                'executeScript',
                `performance.mark('name1-bpend');performance.mark('name2-bpstart');`,
              ],
            ]);
            async.done();
          });
    ...
    it('should normalize times to ms and forward ph and pid event properties', inject(
      [AsyncTestCompleter],
      (async: AsyncTestCompleter) => {
    ...
    it('should normalize "tdur" to "dur"', inject(
      [AsyncTestCompleter],
      (async: AsyncTestCompleter) => {
    ...
    it('should report FunctionCall events as "script"', inject(
      [AsyncTestCompleter],
      (async: AsyncTestCompleter) => {
    ...
    it('should report EvaluateScript events as "script"', inject(
      [AsyncTestCompleter],
      (async: AsyncTestCompleter) => {
    ...
    ['Layout', 'UpdateLayerTree', 'Paint'].forEach((recordType) => {
      it(`should report ${recordType} as "render"`, inject(
        [AsyncTestCompleter],
        (async: AsyncTestCompleter) => {
          createExtension([
    ...
    it(`should report UpdateLayoutTree as "render"`, inject(
      [AsyncTestCompleter],
      (async: AsyncTestCompleter) => {
        createExtension([
    ...
    it('should ignore FunctionCalls with empty scriptName', inject(
      [AsyncTestCompleter],
      (async: AsyncTestCompleter) => {
        createExtension([
    ...
    it('should report navigationStart', inject(
      [AsyncTestCompleter],
      (async: AsyncTestCompleter) => {
    ...
    describe('readPerfLog (common)', () => {
      ...
      describe('frame metrics', () => {
        it('should report ImplThreadRenderingStats as frame event', inject(
          [AsyncTestCompleter],
          (async: AsyncTestCompleter) => {
            createExtension([
        ...
        it('should not report ImplThreadRenderingStats with zero frames', inject(
          [AsyncTestCompleter],
          (async: AsyncTestCompleter) => {
            createExtension([
              benchmarkEvents.instant('BenchmarkInstrumentation::ImplThreadRenderingStats', 1100, {
        ...
        it('should throw when ImplThreadRenderingStats contains more than one frame', inject(
          [AsyncTestCompleter],
          (async: AsyncTestCompleter) => {
            createExtension([
              benchmarkEvents.instant('BenchmarkInstrumentation::ImplThreadRenderingStats', 1100, {
      ...
      it('should report begin timestamps', inject(
        [AsyncTestCompleter],
        (async: AsyncTestCompleter) => {
      ...
      it('should throw an error on buffer overflow', inject(
        [AsyncTestCompleter],
        (async: AsyncTestCompleter) => {
          createExtension(
