...
import {LoadedRouterConfig, Route, RunGuardsAndResolvers} from '../config';
...
export function checkGuards(
    ...
    moduleInjector: Injector, preActivation: PreActivation,
    forwardEvent?: (evt: Event) => void): OperatorFunction<RouterStateSnapshot, boolean> {
  return function(source: Observable<RouterStateSnapshot>) {

    return source.pipe(mergeMap(futureSnapshot => {
      ...
      return runCanDeactivateChecks(checks, futureSnapshot, currentSnapshot, moduleInjector)
          .pipe(mergeMap(
              (canDeactivate: boolean) => canDeactivate ?
                  runCanActivateChecks(futureSnapshot, checks, moduleInjector, forwardEvent) :
                  of (false)));
    }));
  };
}
