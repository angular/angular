...
function runCanDeactivateChild(
  ...
  const deactivateParams: Parameters<CanDeactivateChildFn> = [
    component,
    currentARS,
    currRSS,
    futureRSS,
  ];
  return mapGuardsAndPrioritizeResults(routesWithGuards, injector, ...deactivateParams);
}

// TODO(atscott): When the classes are removed from the type union and only functions are allowed on the Route,
// all guards can use this generic mapping function because we no longer have to pull a specific named
// property off the class interface, but can instead just execute call the guard function with the args.
function mapGuardsAndPrioritizeResults<
  T extends (...theArgs: Y) => MaybeAsync<GuardResult>,
  Y extends any[],
>(
  allGuards: Array<{
  ...
  const childGuardsMapped = allGuards.map(({route, guards}) => {
    return defer(() => {
      const guardsMapped = guards.map((guard) => {
        const closestInjector = getClosestRouteInjector(route) ?? injector;
        const guardVal = runInInjectionContext(closestInjector, () => guard(...args));
        return wrapIntoObservable(guardVal).pipe(first());
      });
      return of(guardsMapped).pipe(prioritizedGuardValue());
    });
