...
function noMatch(segmentGroup: UrlSegmentGroup): Observable<UrlSegmentGroup> {
  return new Observable<UrlSegmentGroup>((obs: Observer<UrlSegmentGroup>) =>
    obs.error(new NoMatch(segmentGroup))
  );
}
...
function canLoadFails(route: Route): Observable<LoadedRouterConfig> {
  return new Observable<LoadedRouterConfig>((obs: Observer<LoadedRouterConfig>) =>
    obs.error(
...
class ApplyRedirects {
  ...
  private expandSegmentGroup(
    ...
    if (segmentGroup.segments.length === 0 && segmentGroup.hasChildren()) {
      return this.expandChildren(ngModule, routes, segmentGroup).pipe(
        map((children: any) => new UrlSegmentGroup([], children))
      );
    }
  ...
  private expandChildren(
    ...
    return waitForMap(segmentGroup.children, (childOutlet, child) =>
  ...
  private expandRegularSegmentAgainstRouteUsingRedirect(
    ...
    const {matched, consumedSegments, lastChild, positionalParamSegments} = match(
      segmentGroup,
      route,
      segments
    );
  ...
  private matchSegmentAgainstRoute(
    ...
    return childConfig$.pipe(
      mergeMap((routerConfig: LoadedRouterConfig) => {
...
function createChildrenForEmptySegments(
  routes: Route[],
  primarySegmentGroup: UrlSegmentGroup
): {[name: string]: UrlSegmentGroup} {
  const res: {[name: string]: UrlSegmentGroup} = {};
