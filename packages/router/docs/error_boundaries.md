...
- **Recovery (`retry`) is deferred.** An earlier iteration added `retry?(): void` to `RouterOutletContract` and `RouterOutlet`, reachable from the error component via `inject(RouterOutlet)`. It was removed before shipping (see the TODO in `router_outlet.ts`):
  - It re-activated using the existing `ActivatedRoute` and snapshot, so guards, resolvers, `loadComponent`/`loadChildren` and route resources were all skipped. It could only recover from errors thrown by the component instance itself, and it silently bypassed guards.
  - It destroyed the component outside of `ChildrenOutletContexts`, potentially leaving stale child outlet contexts, and emitted `deactivateEvents` with the error component instance.
  - Re-running the navigation instead is not a drop-in replacement: with the default `RouteReuseStrategy` the route is reused for the same config and params, so the outlet is never re-activated and the error component would stay on screen.

  Adding a recovery API later is not a breaking change, so it is better to ship without one than to commit to these semantics on the public contract.
...
When an error occurs, the `RouterOutlet` instantiates the `errorComponent` and binds the `error` input.
...
2. **Routing Context (DI)**:
   The `errorComponent` is created in an injector that still resolves the enclosing `RouterOutlet` and the failed route's `ActivatedRoute`, so it can inject them if it needs routing context:
   ```ts
   @Component({
     template: `
...
_Design Note_: We deliberately avoid magical reflection on output properties (e.g. subscribing to `output()` instances behind the scenes) and callback closure inputs (`input<() => void>()`), keeping the API direct, standard, and fully type-safe.
...
### 2. Output Event Reflection (e.g. `readonly retry = output()`)

- **Rejected**: Having `RouterOutlet` inspect component output properties dynamically and call `.subscribe()` behind the scenes is roundabout, magical, and violates standard Angular component contracts. This applied to any recovery action the error component might expose; recovery itself is now out of scope (see Non-Goals).

### 3. Callback Closure Inputs (`input<() => void>()`)

- **Rejected**: Passing executable callbacks via component inputs results in awkward template syntax (`(click)="retry()()"`) and runs counter to Angular's architecture. If a recovery action is added later, DI injection is the cleaner, more standard pattern.
