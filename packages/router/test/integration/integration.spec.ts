...
for (const browserAPI of ['navigation', 'history'] as const) {
  describe(`${browserAPI}-based routing`, () => {
    ...
    it('Should work inside ChangeDetectionStrategy.OnPush components', () =>
      fakeAsync(() => {
        @Component({
          selector: 'root-cmp',
    ...
    it('should work when an outlet is added/removed', () =>
      fakeAsync(() => {
        @Component({
          selector: 'someRoot',
    ...
    it('should handle empty or missing fragments', () =>
      fakeAsync(() => {
        const router: Router = TestBed.inject(Router);
        const fixture = createRoot(router, RootCmp);
