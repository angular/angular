...
describe('applyRedirects', () => {
  ...
  describe('lazy loading', () => {
    it('should load config on demand', () => {
      ...
      applyRedirects(testModule.injector, <any>loader, serializer, tree('a/b'), config).forEach(
        (r) => {
    ...
    it('should handle the case when the loader errors', () => {
      ...
      applyRedirects(testModule.injector, <any>loader, serializer, tree('a/b'), config).subscribe(
        () => {},
        (e) => {
    ...
    it('should not load when any canLoad guards return false', () => {
      ...
      applyRedirects(<any>injector, <any>loader, serializer, tree('a/b'), config).subscribe(
        () => {
    ...
    it('should work with absolute redirects', () => {
      ...
      const config: Routes = [
        ...
        {path: 'a', loadChildren: 'children'},
      ];

      applyRedirects(testModule.injector, <any>loader, serializer, tree(''), config).forEach(
        (r) => {
          expectTreeToBe(r, 'a');
    ...
    it('should load the configuration only once', () => {
      ...
      applyRedirects(testModule.injector, <any>loader, serializer, tree('a?k2'), config).subscribe(
        (r) => {
    ...
    it('should load the configuration of a wildcard route', () => {
      ...
      applyRedirects(testModule.injector, <any>loader, serializer, tree('xyz'), config).forEach(
        (r) => {
          expect((config[0] as any)._loadedConfig).toBe(loadedConfig);
        }
      );
    ...
    it('should load the configuration after a local redirect from a wildcard route', () => {
      ...
      const config: Routes = [
        {path: 'not-found', loadChildren: 'children'},
  ...
  describe('empty URL leftovers', () => {
    ...
    it('should error when no children matching and some url is left', () => {
      applyRedirects(testModule.injector, null!, serializer, tree('/a/c'), [
        {path: 'a', component: ComponentA, children: [{path: 'b', component: ComponentB}]},
      ]).subscribe(
        (_) => {
          throw 'Should not be reached';
  ...
  describe('redirecting to named outlets', () => {
    ...
    it('should throw when using non-absolute redirects', () => {
      applyRedirects(testModule.injector, null!, serializer, tree('a'), [
...
function checkRedirect(config: Routes, url: string, callback: any): void {
  applyRedirects(TestBed, null!, new DefaultUrlSerializer(), tree(url), config).subscribe(
    callback,
    (e) => {
      throw e;
    }
