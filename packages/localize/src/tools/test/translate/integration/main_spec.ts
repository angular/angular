...
describe('translateFiles()', () => {
  ...
  it('should copy non-code files to the destination folders', () => {
    ...
    translateFiles({
      ...
      outputPathFn,
      translationFilePaths: resolveAll(__dirname + '/locales', [
        'messages.de.json',
        'messages.es.xlf',
        'messages.fr.xlf',
        'messages.it.xtb',
    ...
    expect(FileUtils.readFile(resolve(testDir, 'fr', 'test-1.txt'))).toEqual(
      'Contents of test-1.txt'
    );
    expect(FileUtils.readFile(resolve(testDir, 'fr', 'test-2.txt'))).toEqual(
      'Contents of test-2.txt'
    );
    expect(FileUtils.readFile(resolve(testDir, 'de', 'test-1.txt'))).toEqual(
      'Contents of test-1.txt'
    );
    expect(FileUtils.readFile(resolve(testDir, 'de', 'test-2.txt'))).toEqual(
      'Contents of test-2.txt'
    );
    expect(FileUtils.readFile(resolve(testDir, 'es', 'test-1.txt'))).toEqual(
      'Contents of test-1.txt'
    );
    expect(FileUtils.readFile(resolve(testDir, 'es', 'test-2.txt'))).toEqual(
      'Contents of test-2.txt'
    );
    expect(FileUtils.readFile(resolve(testDir, 'it', 'test-1.txt'))).toEqual(
      'Contents of test-1.txt'
    );
    expect(FileUtils.readFile(resolve(testDir, 'it', 'test-2.txt'))).toEqual(
      'Contents of test-2.txt'
    );
  });
  ...
  it('should translate and copy source-code files to the destination folders', () => {
    ...
    expect(FileUtils.readFile(resolve(testDir, 'fr', 'test.js'))).toEqual(
      `var name="World";var message="Bonjour, "+name+"!";`
    );
    expect(FileUtils.readFile(resolve(testDir, 'de', 'test.js'))).toEqual(
      `var name="World";var message="Guten Tag, "+name+"!";`
    );
    expect(FileUtils.readFile(resolve(testDir, 'es', 'test.js'))).toEqual(
      `var name="World";var message="Hola, "+name+"!";`
    );
    expect(FileUtils.readFile(resolve(testDir, 'it', 'test.js'))).toEqual(
  ...
  it('should translate and copy source-code files overriding the locales', () => {
    ...
    expect(diagnostics.messages).toContain({
      type: 'warning',
      message: `The provided locale "xde" does not match the target locale "de" found in the translation file "${resolve(
        __dirname,
        'locales',
        'messages.de.json'
      )}".`,
    ...
    expect(FileUtils.readFile(resolve(testDir, 'xde', 'test.js'))).toEqual(
      `var name="World";var message="Guten Tag, "+name+"!";`
    );
    expect(FileUtils.readFile(resolve(testDir, 'es', 'test.js'))).toEqual(
      `var name="World";var message="Hola, "+name+"!";`
  ...
  it('should transform and/or copy files to the destination folders', () => {
    ...
    translateFiles({
      sourceRootPath: resolve(__dirname, 'test_files'),
      sourceFilePaths: resolveAll(__dirname + '/test_files', [
        'test-1.txt',
        'test-2.txt',
        'test.js',
      ]),
