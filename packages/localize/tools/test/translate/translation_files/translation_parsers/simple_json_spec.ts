...
describe('SimpleJsonTranslationParser', () => {
  ...
  describe('analyze()', () => {
    ...
    it('should return a failure object if the file is not a valid format', () => {
      const parser = new SimpleJsonTranslationParser();
      expect(parser.analyze('/some/file.xlf', '')).toEqual(
        jasmine.objectContaining({
          canParse: false,
        }),
      );
      expect(parser.analyze('/some/file.json', '{}')).toEqual(
        jasmine.objectContaining({
          canParse: false,
        }),
      );
      expect(parser.analyze('/some/file.json', '{ "translations" : {} }')).toEqual(
        jasmine.objectContaining({canParse: false}),
      );
      expect(parser.analyze('/some/file.json', '{ "locale" : "fr" }')).toEqual(
        jasmine.objectContaining({canParse: false}),
      );
    });
  });
