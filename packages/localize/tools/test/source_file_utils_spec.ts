  ...
  describe('utils', () => {
    ...
    describe('isGlobalIdentifier()', () => {
      it('should return true if the identifier is at the top level and not declared', () => {
        const taggedTemplate = getTaggedTemplate('$localize ``;');
        expect(isGlobalIdentifier(taggedTemplate.get('tag') as babel.NodePath<t.Identifier>)).toBe(true);
      });
      ...
      it('should return false if the identifier is declared locally', () => {
        ...
        expect(isGlobalIdentifier(taggedTemplate.get('tag') as babel.NodePath<t.Identifier>)).toBe(false);
      });
    ...
    describe('unwrapMessagePartsFromLocalizeCall', () => {
      ...
      it('should remove a lazy load template helper', () => {
        ...
        $localize(_templateObject(), 1, 2)`);
        const localizeStatement = localizeCall.parentPath as babel.NodePath<t.ExpressionStatement>;
...
function getTaggedTemplate(
    code: string, options?: babel.TransformOptions): babel.NodePath<t.TaggedTemplateExpression> {
...
function getFirstExpression<T extends t.Expression>(
    code: string, options?: babel.TransformOptions): babel.NodePath<T> {
  return getExpressions<T>(code, options)[0];
}

function getExpressions<T extends t.Expression>(
    code: string, options?: babel.TransformOptions): babel.NodePath<T>[] {
  const expressions: babel.NodePath<t.Expression>[] = [];
  babel.transformSync(code, {
    ...
    plugins: [{
      visitor: {
        Expression: (path: babel.NodePath<t.Expression>) => {
  ...
  return expressions as babel.NodePath<T>[];
}

function getLocalizeCall(code: string): babel.NodePath<t.CallExpression> {
  let callPaths: babel.NodePath<t.CallExpression>[] = [];
  babel.transformSync(code, {
    code: false,
    filename: 'test/file.js',
    cwd: '/',
