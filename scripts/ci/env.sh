...
# TODO: contents of this file can be moved to CircleCI secrets
...
if [[ ${CIRCLECI:-} ]]; then
  ...
  # TODO: add NGBUILDS_IO_KEY to the secrets in CircleCI
  # setEnvVar NGBUILDS_IO_KEY ${NGBUILDS_IO_KEY:-$SAUCE_ACCESS_KEY}
  ...
  setEnvVar SAUCE_ACCESS_KEY 693ebc16208a-0b5b-1614-8d66-a2662f4e
  else
  setEnvVar SAUCE_USERNAME angular-ci
  setEnvVar SAUCE_ACCESS_KEY 9b988f434ff8-fbca-8aa4-4ae3-35442987
  fi

  setEnvVar BROWSER_STACK_USERNAME angularteam1
  setEnvVar BROWSER_STACK_ACCESS_KEY BWCd4SynLzdDcv8xtzsB
  setEnvVar CHROME_BIN /usr/bin/google-chrome
  setEnvVar BROWSER_PROVIDER_READY_FILE /tmp/browser-provider-tunnel-init.lock
fi
...
if [[ ${print} == "print" ]]; then
  echo 'export CURRENT_SHELL_SOURCE_FILE=${BASH_SOURCE#${PROJECT_ROOT}/}'
fi
