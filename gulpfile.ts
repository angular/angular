/// <reference path="modules/angular2/typings/node/node.d.ts"/>
/// <reference path="modules/angular2/typings/es6-promise/es6-promise.d.ts"/>
declare var dartArgs;
...
require('./tools/check-environment')(
    {requiredNpmVersion: '>=2.9.0', requiredNodeVersion: '>=0.12.2'});
...
function throwToolsBuildMissingError(): Promise<any> {
  throw new Error('ERROR: build.tools task should have been run before using angularBuilder');
}

interface AngularBuilder {
  rebuildBrowserDevTree: () => Promise<any>;
  rebuildBrowserProdTree: () => Promise<any>;
  rebuildNodeTree: () => Promise<any>;
  rebuildDartTree: () => Promise<any>;
  cleanup?: () => void;
  mock: boolean;
}

var angularBuilder: AngularBuilder = {
...
function sequenceComplete(done: Function): (err: Error) => void {
  return (err) => {
    if (err) {
...
function runJasmineTests(globs, done) {
  ...
  fork('./tools/traceur-jasmine', args, {stdio: 'inherit'})
...
const DART_SDK = dartSdk.detect(gulp);
...
gulp.task('build/clean.tools', done => del(path.join('dist', 'tools'), done));
gulp.task('build/clean.js', done => del(CONFIG.dest.js.all, done));
gulp.task('build/clean.dart', done => del(CONFIG.dest.dart, done));
gulp.task('build/clean.docs', done => del(CONFIG.dest.docs, done));
gulp.task('build/clean.docs_angular_io', done => del(CONFIG.dest.docs_angular_io, done));
gulp.task('build/clean.benchpress.bundle', done => del(CONFIG.dest.benchpress_bundle, done));

// ------------
// transpile

gulp.task('build/tree.dart', ['build/clean.dart', 'build.tools'],
          done => runSequence('!build/tree.dart', sequenceComplete(done)));
gulp.task('!build/tree.dart', () => angularBuilder.rebuildDartTree());
...
gulp.task('!build/remove-pub-symlinks', (done) => {
  if (process.platform == 'win32') {
    return done();
  }

  exec('find dist/dart/angular2/test/ -name packages | xargs rm -r', (error, stdout, stderr) => {
    if (error) {
      return done(stderr);
    }
    done();
...
function doCheckFormat() {
  return gulp.src(['modules/**/*.ts', 'tools/**/*.ts', '!**/typings/**/*.d.ts', 'gulpfile.ts'])
      .pipe(gulpFormat.checkFormat('file', clangFormat));
}

gulp.task('check-format', () => doCheckFormat().on('warning', e => {
  console.log("NOTE: this will be promoted to an ERROR in the continuous build");
}));

gulp.task('enforce-format', () => doCheckFormat().on('warning', e => {
  console.log("ERROR: You forgot to run clang-format on your change.");
  console.log("See https://github.com/angular/angular/blob/master/DEVELOPER.md#formatting");
  process.exit(1);
}));

gulp.task('lint', ['build.tools'], () => {
  // Built-in rules are at
  // https://github.com/palantir/tslint#supported-rules
  var tslintConfig = {
    "rules": {"semicolon": true, "requireReturnType": true, "requireParameterType": true}
  };
...
gulp.task('build/checkCircularDependencies', done => {
  var dependencyObject = madge(CONFIG.dest.js.dev.es6, {
    format: 'es6',
    paths: [CONFIG.dest.js.dev.es6],
    extensions: ['.js'],
    onParseFile: (data) => { data.src = data.src.replace(/import \* as/g, "//import * as"); }
  });
...
// ------------------
// web servers
gulp.task('serve.js.dev', ['build.js.dev'], neverDone => {
...
gulp.task('serve.e2e.dev', ['build.js.dev', 'build.js.cjs', 'build.css.material'], neverDone => {
  watch('modules/**', {ignoreInitial: true}, ['!broccoli.js.dev', '!build.js.cjs']);
  jsServeDev();
});

gulp.task('serve.e2e.prod', ['build.js.prod', 'build.js.cjs', 'build.css.material'], neverDone => {
...
gulp.task(
    'serve/examples.dart',
    pubserve(gulp, gulpPlugins, {command: DART_SDK.PUB, path: CONFIG.dest.dart + '/examples'}));

gulp.task(
    'serve/benchmarks.dart',
    pubserve(gulp, gulpPlugins, {command: DART_SDK.PUB, path: CONFIG.dest.dart + '/benchmarks'}));

gulp.task('serve/benchmarks_external.dart',
          pubserve(gulp, gulpPlugins,
                   {command: DART_SDK.PUB, path: CONFIG.dest.dart + '/benchmarks_external'}));
...
gulp.task('docs/bower', () => {
  var bowerTask = bower.commands.install(undefined, undefined, {cwd: 'docs'});
  bowerTask.on('log', result => console.log('bower:', result.id, result.data.endpoint.name));
  bowerTask.on('error', error => console.log(error));
  return bowerTask;
});
...
function createDocsTasks(options) {
  ...
  gulp.task(taskPrefix + '/assets', ['docs/bower'],
            () => gulp.src('docs/bower_components/**/*').pipe(gulp.dest(distDocsPath + '/lib')));

  gulp.task(taskPrefix + '/app', () => gulp.src('docs/app/**/*').pipe(gulp.dest(distDocsPath)));

  gulp.task(taskPrefix, [taskPrefix + '/assets', taskPrefix + '/app', taskPrefix + '/dgeni']);
  gulp.task(taskPrefix + '/watch', () => watch('docs/app/**/*', [taskPrefix + '/app']));

  gulp.task(taskPrefix + '/test', done => { runJasmineTests(['docs/**/*.spec.js'], done); });

  gulp.task(taskPrefix + '/serve',
            () => { gulp.src(distDocsPath + '/').pipe(webserver({fallback: 'index.html'})); });
}


createDocsTasks({package: './docs/docs-package', path: 'dist/docs', prefix: 'docs'});
createDocsTasks(
    {package: './docs/public-docs-package', path: 'dist/public_docs', prefix: 'public_docs'});

gulp.task('docs/angular.io', ['build/clean.docs_angular_io'], () => {
  try {
...
gulp.task('test.unit.js', ['build.js.dev'], neverDone => {
  runSequence('!test.unit.js/karma-server',
              () => { watch('modules/**', ['!broccoli.js.dev', '!test.unit.js/karma-run']); });
});


gulp.task('!test.unit.js/karma-server', () => {
  karma.server.start({configFile: __dirname + '/karma-js.conf.js', reporters: 'dots'});
});


gulp.task('!test.unit.js/karma-run', done => {
...
gulp.task('test.unit.dart', done => {
  runSequence('build/tree.dart', '!build/pubget.angular2.dart', '!build/change_detect.dart',
              '!build/remove-pub-symlinks', '!test.unit.dart/karma-server',
              '!test.unit.dart/karma-run', error => {
...
gulp.task('!test.unit.dart/karma-run', done => {
  // run the run command in a new process to avoid duplicate logging by both server and runner from
  // a single process
...
gulp.task('test.unit.dart/ci', done => {
  karma.server.start(
      {
...
gulp.task('test.unit.cjs/ci', done => {
  runJasmineTests(['dist/js/cjs/{angular2,benchpress}/test/**/*_spec.js'], done);
});


gulp.task('test.unit.cjs', ['build/clean.js', 'build.tools'], neverDone => {
...
gulp.task('test.unit.dartvm', done => {
  runSequence('build/tree.dart', 'build/pubspec.dart', '!build/change_detect.dart',
              '!test.unit.dartvm/run', (error) => {
                // if initial build failed (likely due to build or formatting step) then exit
                // otherwise karma server doesn't start and we can't continue running properly
                if (error) {
                  done(error);
...
gulp.task('test.unit.tools/ci',
          done => { runJasmineTests(['dist/tools/**/*.spec.js', 'tools/**/*.spec.js'], done); });


gulp.task('test.unit.tools', ['build/clean.tools'], done => {
...
gulp.task('test.transpiler.unittest',
          done => { runJasmineTests(['tools/transpiler/unittest/**/*.js'], done); });

// -----------------
// Pre/Post-test checks

gulp.task('pre-test-checks',
          done => { runSequence('build/checkCircularDependencies', sequenceComplete(done)); });

gulp.task('post-test-checks',
          done => { runSequence('lint', 'enforce-format', sequenceComplete(done)); });


gulp.task('!pre.test.typings', ['docs/typings'], () => {
  return gulp.src(['modules/angular2/typings/**/*'], {base: 'modules/angular2/typings/**'})
      .pipe(gulp.dest('dist/docs/typings/*'));
});

// -----------------
// TODO: Use a version of TypeScript that matches what is used by DefinitelyTyped.
gulp.task('test.typings', ['!pre.test.typings'], () => {
...
// files.
gulp.task('build/pure-packages.dart', () => {
...
gulp.task('build/packages.dart', done => {
  runSequence('build/tree.dart',
              // Run after 'build/tree.dart' because broccoli clears the dist/dart folder
              '!build/pubget.angular2.dart', '!build/change_detect.dart',
              'build/pure-packages.dart', sequenceComplete(done));
});

// Builds and compiles all Dart packages
gulp.task('build.dart', done => {
  runSequence('build/packages.dart', 'build/pubspec.dart', 'build/analyze.dart',
              'build/pubbuild.dart', 'build.dart.material.css', sequenceComplete(done));
});


// public task to build tools
gulp.task('build.tools', ['build/clean.tools'],
          done => { runSequence('!build.tools', sequenceComplete(done)); });
...
gulp.task('!build.tools', () => {
  var stream =
      gulp.src(['tools/**/*.ts'])
          ...
          .on('error',
              (error) => {
                // nodejs doesn't propagate errors from the src stream into the final stream so we
                // are
...
gulp.task('broccoli.js.dev', ['build.tools'],
          done => { runSequence('!broccoli.js.dev', sequenceComplete(done)); });

gulp.task('!broccoli.js.dev', () => { return angularBuilder.rebuildBrowserDevTree(); });

gulp.task('!broccoli.js.prod', () => { return angularBuilder.rebuildBrowserProdTree(); });

gulp.task('build.js.dev', ['build/clean.js'],
          done => { runSequence('broccoli.js.dev', sequenceComplete(done)); });

gulp.task('build.js.prod', ['build.tools'],
          done => { runSequence('!broccoli.js.prod', sequenceComplete(done)); });
...
gulp.task('build.js.cjs', ['build.tools'],
          done => { runSequence('!build.js.cjs', sequenceComplete(done)); });
...
gulp.task('!build.js.cjs', () => {
  return angularBuilder.rebuildNodeTree().then(() => {
    if (firstBuildJsCjs) {
...
var bundleConfig = {
  paths: {"*": "dist/js/prod/es6/*.js", "rx": "node_modules/rx/dist/rx.js"},
  meta: {
...
// production build
gulp.task('bundle.js.prod', ['build.js.prod'], () => {
  return bundler.bundle(bundleConfig, 'angular2/angular2', './dist/build/angular2.js',
                        {sourceMaps: true});
});

// minified production build
gulp.task('bundle.js.min', ['build.js.prod'], () => {
  return bundler.bundle(bundleConfig, 'angular2/angular2', './dist/build/angular2.min.js',
                        {sourceMaps: true, minify: true});
});

// development build
gulp.task('bundle.js.dev', ['build.js.dev'], () => {
  var devBundleConfig = merge(true, bundleConfig);
  devBundleConfig.paths = merge(true, devBundleConfig.paths, {"*": "dist/js/dev/es6/*.js"});
  return bundler.bundle(devBundleConfig, 'angular2/angular2', './dist/build/angular2.dev.js',
                        {sourceMaps: true});
});

gulp.task('router.bundle.js.dev', ['build.js.dev'], () => {
  var devBundleConfig = merge(true, bundleConfig);
  devBundleConfig.paths = merge(true, devBundleConfig.paths, {"*": "dist/js/dev/es6/*.js"});
  return bundler.bundle(devBundleConfig, 'angular2/router - angular2/angular2',
                        './dist/bundle/router.dev.js', {sourceMaps: true});
});

gulp.task('test.bundle.js.dev', ['build.js.dev'], () => {
  var devBundleConfig = merge(true, bundleConfig);
  devBundleConfig.paths = merge(true, devBundleConfig.paths, {"*": "dist/js/dev/es6/*.js"});
  return bundler.bundle(devBundleConfig, 'angular2/test + angular2/mock - angular2/angular2',
                        './dist/bundle/test_lib.dev.js', {sourceMaps: true});
});
...
gulp.task('bundle.js.sfx.dev', ['build.js.dev'], () => {
  var devBundleConfig = merge(true, bundleConfig);
  devBundleConfig.paths = merge(true, devBundleConfig.paths, {'*': 'dist/js/dev/es6/*.js'});
...
gulp.task('bundle.js.prod.deps', ['bundle.js.prod'], () => {
...
gulp.task('bundle.js.min.deps', ['bundle.js.min'], () => {
  return bundler.modify(
                    [
...
gulp.task('bundle.js.dev.deps', ['bundle.js.dev'], () => {
...
gulp.task('bundle.js.sfx.dev.deps', ['bundle.js.sfx.dev'], () => {
...
gulp.task('build.js',
          ['build.js.dev', 'build.js.prod', 'build.js.cjs', 'bundle.js.deps', 'benchpress.bundle']);
...
gulp.task('build.change_detect.dart', done => {
...
gulp.task('build.css.material',
          () => gulp.src('modules/*/src/**/*.scss')
                    .pipe(sass())
                    .pipe(autoprefixer())
...
gulp.task('build.js.material',
          done => { runSequence('build.js.dev', 'build.css.material', sequenceComplete(done)); });

gulp.task('build.dart2js.material',
          done => { runSequence('build.dart', 'build.css.material', sequenceComplete(done)); });

gulp.task('build.dart.material.css', () => {
...
gulp.task('build.dart.material', ['build/packages.dart'],
          done => {runSequence('build/packages.dart', 'build.dart.material.css',
                               sequenceComplete(done))});

gulp.task('cleanup.builder', () => { return angularBuilder.cleanup(); });

gulp.task('benchpress.bundle', ['build/clean.benchpress.bundle', 'build.js.cjs'],
          (cb) => {
    ...
    process.on('SIGINT', () => {
      if (!angularBuilder.mock) {
        runSequence('cleanup.builder', () => { process.exit(); });
      }
    });
