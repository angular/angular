module.exports = function processNavigationMap(versionInfo, log) {
  return {
    ...
    $process: function(docs) {
      ...
      if (!navigationDoc) {
        log.warn('Missing navigation map document (docType="navigation-map"). Did you forget to add it to the readFileProcessor?');
      }
      ...
      // Add in the version data to be rendered in the docs app
      navigationDoc.data['version-info'] = versionInfo.currentVersion;
    }
  }
};
...
function walk(node, map, path) {
  ...
  for(const key in node) {
    ...
    if (key === 'url') {
      const url = child.replace(/#\w*$/, ''); // strip hash
