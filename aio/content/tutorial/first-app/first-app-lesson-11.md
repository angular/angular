...
1.  In `src/app/housing-location/housing-location.component.ts`, add an anchor tag to the `section` element and include the `routerLink` directive:
    ...
    For the `routerLink` to work in the template, add `RouterModule` at the file level in `housing-location` component and update the `imports` in `@Component` as well.
