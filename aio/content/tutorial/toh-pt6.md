...
-->
그리고 이 패키지에서 제공하는 `HttpClientInMemoryWebApiModule`을 로드하고, 앞으로 만들 `InMemoryDataService` 클래스를 로드합니다.
...
이제 사용자가 검색창에 내용을 입력하면 *keyup* 이벤트에 바인딩된 `search()` 메소드가 실행될 것입니다.
...
`search()` 메소드는 텍스트 입력박스에서 발생하는 `keyup` 이벤트와 바인딩되었기 때문에, `keyup` 이벤트가 발생할 때마다 실행됩니다.
