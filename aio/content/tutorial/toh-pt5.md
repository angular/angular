# Add in-application navigation with routing
...
First, `AppRoutingModule` imports `RouterModule` and `Routes` so the application can have routing functionality. The next import, `HeroesComponent`, will give the Router somewhere to go once you configure the routes.
