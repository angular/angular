...
@description
Two or more components use the same [element selector](guide/component/component-create#specify-the-css-selector "Specify the CSS selector - Create an Angular component | Angular"). Because there can only be a single component associated with an element, selectors must be unique strings to prevent ambiguity for Angular.
