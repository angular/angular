...
In Angular CLI, a project is a standalone application or [library](#library) that can be created or modified by a CLI command.

By default, the [`ng new`](cli/new) command creates an initial application project at the root of the [workspace](#workspace),
which contains the set of source files, resources, and configuration files that you need to develop and test the application.
Projects can also be created with [`ng generate application`](cli/generate#application-command),
[`ng generate library`](cli/generate#library-command) commands.
