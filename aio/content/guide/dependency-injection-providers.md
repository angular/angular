...
You can also create multiple provider, Angular uses lot of multiple providers like `HTTP_INTERCEPTORS` and provides it to be used in applications. The advantage is you can add values into an array and use them whenever you need it, a simple example is, in case you want a mechanism to register external components within your framework, you can get those values via a multi provider and load them.
