...
1. Enter the CLI command `ng new` and provide the name `feature-modules` as shown:

   <code-example language="sh" class="code-shell">
       ng new feature-modules
   </code-example>

   1. The command displays the request `Would you like to add Angular routing?`
   Respond by typing **N** for no routing.

   1. The command then displays a second request: `Which stylesheet format would you like to use?`
   Press Enter/Return to accept the default stylesheet.

   The `ng new` command generates a new workspace with a root folder named `feature-modules` and a skeleton app project in its `src` subfolder.

1. Change the app's title by opening `app.component.ts` in the `src/app` folder and changing the value of the `title` property to `'app works!'`.

1. Open the component template file (`app.component.html`), and
...
1. Open the app module in `src/app/app.module.ts` and add the [`HttpClientModule`](api/common/http/HttpClientModule "HttpClientModule API") and [`FormsModule`](api/forms/FormsModule "FormsModule API") imports to the import section (which already includes `NgModule` and `BrowserModule`):
   ...
   The features in this example app need `FormsModule` and `HttpClientModule` to work, but NgModules do not require them.

1. In the same file, add `HttpClientModule` and `FormsModule` to the imports array in the `@NgModule()` metadata, which already includes `BrowserModule`:
...
1. Enter the following CLI command, using a name such as `CustomerDashboard` as the name of your new NgModule (omit the `Module` suffix because the CLI automatically appends it to the name):
...
Note that for new NgModules, you should import `CommonModule` rather than `BrowserModule`.
The [root NgModule](guide/bootstrapping "Launching your app with a root module"), generated by the CLI `ng new` command, already imports `BrowserModule` in order to configure the entire app for the browser.

Use the `declarations` array in the `customer-dashboard.module.ts` file to declare the components, directives, and pipes that belong exclusively to this particular NgModule.

## Generate a new component in the NgModule

To add a component to the new NgModule, enter the following CLI command, which specifies the folder for the NgModule, `customer-dashboard`, and the component name `CustomerDashboard`:
...
This command adds the `CustomerDashboardComponent` to the import list at the top of the `customer-dashboard.module.ts` file, and also adds it to the `declarations` array, which lets Angular know to associate this new component with this NgModule.
The command also generates a folder for the new component within the `customer-dashboard` folder, and updates the `customer-dashboard.module.ts` file with the `CustomerDashboardComponent` info:
...
* Specify only [declarables](guide/glossary#declarable "Definition of a declarable")&mdash;component, directives, and pipes&mdash;in the `declarations` array.
* Declare these classes in an NgModule _only_ if they belong to that NgModule.
...
You can add service providers to the new NgModule, as described in [Providing dependencies in modules](guide/providers "Providing dependencies in modules").
`AppModule` would know about those too, so that they can be used by other NgModules.
For more about service dependency injection, see [Dependency injection in Angular](guide/dependency-injection "Dependency injection in Angular").

## Render the new component's template

When the CLI generated the `CustomerDashboardComponent` for the new NgModule, the component included a template, `customer-dashboard.component.html`, with the following HTML:
...
1. In the `AppComponent` template, `app.component.html`, add the  `<app-customer-dashboard>` tag:
...
As a result, the `CustomerDashboardComponent` template renders along with the title:
...
* For more about NgModules, see [NgModules](guide/ngmodules "NgModules").
* To learn more about the root NgModule, see [Launching your app with a root module](guide/bootstrapping "Launching your app with a root module").
