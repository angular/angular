...
In the following example, the `searchItems()` method uses a JSONP request to query for items whose names contain the search term.
...
searchItems(term: string): Observable {
  ...
  let itemsURL = `${this.itemsURL}?${term}`;
  return this.http.jsonp(itemsUrl, 'callback').pipe(
      catchError(this.handleError('searchItems', []) // then handle the error
    );
};
```

This request passes the `itemsURL` as the first parameter and the callback function name as the second parameter.
...
The sample app for this guide includes a simplified version of the "Tour of Items" example
that fetches items and enables users to add, delete, and update them.

The following sections excerpt methods of the sample's `ItemsService`.

### Making a POST request

Apps often POST data to a server. They POST when submitting a form.
In the following example, the `ItemsService` posts when adding an item to the database.

<code-example
  path="http/src/app/items/items.service.ts"
  region="addItem"
  header="app/items/items.service.ts (addItem)">
</code-example>

The `HttpClient.post()` method is similar to `get()` in that it has a type parameter
(you're expecting the server to return the new item)
and it takes a resource URL.

It takes two more parameters:

1. `item` - the data to POST in the body of the request.
...
The `ItemsComponent` initiates the actual POST operation by subscribing to
the `Observable` returned by this service method.

<code-example
  path="http/src/app/items/items.component.ts"
  region="add-item-subscribe"
  header="app/items/items.component.ts (addItem)">
</code-example>

When the server responds successfully with the newly added item, the component adds
that item to the displayed `items` list.

### Making a DELETE request

This application deletes a item with the `HttpClient.delete` method by passing the item's id
in the request URL.

<code-example
  path="http/src/app/items/items.service.ts"
  region="deleteItem"
  header="app/items/items.service.ts (deleteItem)">
</code-example>

The `ItemsComponent` initiates the actual DELETE operation by subscribing to
the `Observable` returned by this service method.

<code-example
  path="http/src/app/items/items.component.ts"
  region="delete-item-subscribe"
  header="app/items/items.component.ts (deleteItem)">
</code-example>
...
You must call _subscribe()_ or nothing happens. Just calling `ItemsService.deleteItem()` **does not initiate the DELETE request.**
...
<code-example
  path="http/src/app/items/items.component.ts"
  region="delete-item-no-subscribe">
</code-example>
...
```javascript
const req = http.get<Items>('/api/items');
...
The following `ItemsService` example is just like the POST example.

<code-example
  path="http/src/app/items/items.service.ts"
  region="updateItem"
  header="app/items/items.service.ts (updateItem)">
</code-example>

For the reasons [explained above](#always-subscribe), the caller (`ItemsComponent.update()` in this case) must `subscribe()` to the observable returned from the `HttpClient.put()`
...
The `ItemsService` defines such headers in an `httpOptions` object that will be passed
to every `HttpClient` save method.

<code-example
  path="http/src/app/items/items.service.ts"
  region="http-options"
  header="app/items/items.service.ts (httpOptions)">
</code-example>
...
The following example, the `searchItems()` method queries for items whose names contain the search term.
...
<code-example
  path="http/src/app/items/items.service.ts"
  region="searchItems" linenums="false">
</code-example>

If there is a search term, the code constructs an options object with an HTML URL-encoded search parameter.
If the term is "cat", for example, the GET request URL would be `api/items?name=cat`.
...
`src/app/items/items.service.spec.ts`.
