...
1.  Run this command to sync the `main` branch of the clone on your local computer with the upstream `angular/angular` repo.
...
1.  In the output of the previous `git log` command, find the entry that contains `upstream/main`.
    ...
    1.  **Confirm that your commit messages are in the correct format**
        ...
        The commit message format is tested by the CI/CD tests and they must be in the correct format before the pull request is approved.
