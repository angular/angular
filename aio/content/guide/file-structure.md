...
| `main.ts`                 | The main entry point for your application. Compiles the application with the [AOT compiler](guide/aot-compiler) and bootstraps the application's root module \(AppModule\) to run in the browser. You can also use the [JIT compiler](guide/glossary#jit) without changing any code by appending the `--aot=false` flag to the CLI `build` and `serve` commands since AOT compiler is the default starting in Angular 9, see [Choosing a compiler](https://angular.io/guide/aot-compiler#choosing-a-compiler). |
