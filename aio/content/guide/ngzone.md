...
This guide describes how to use Angular's NgZone to automatically detect changes in the component to update DOM.
...
In Angular, you can [display data](guide/displaying-data) by binding controls in a template to the properties of an Angular component.
...
However, the DOM is also updated automatically.
This guide describes how and when Angular renders the DOM based on the data from the Angular component.
...
If the data changed, you render the DOM to reflect the updated data.

In Angular, this step is unnecessary. Whenever you update the data, your DOM is updated automatically.

### When apps update DOM

To understand how change detection works, first consider when the application needs to update the DOM. Typically, updates occur for one of the following reasons:

1. Component initialization. For example, when bootstrapping an Angular application, Angular loads the bootstrap component and triggers the [ApplicationRef.tick()](api/core/ApplicationRef#tick) to call change detection and View Rendering. Just as in the [displaying data](guide/displaying-data) sample, the `AppComponent` is the bootstrap component. This component has the properties `title` and `myHero`, which the application renders in the DOM.
