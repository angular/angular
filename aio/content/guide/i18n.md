...
-->
템플릿 파일 작업을 끝내면 Angular CLI 명령 중 `xi18n`을 실행해서 번역할 대상으로 지정한 문자열을 _소스 언어_ 파일로 추출해야 합니다.
...
`xi18n` 명령을 실행할 때 옵션을 지정하면 소스 파일의 위치, 형식, 파일 이름을 지정할 수 있습니다:
...
Ivy 컴파일러는 아직 `ng xi18n` 명령과 완벽하게 호환되지 않기 때문에 아직까지는 ViewEngine 컴파일러를 활용합니다.
이 이슈는 현재 처리중이며 진행과정을 확인하거나 직접 기여하고 싶다면 [PR 32912](https://github.com/angular/angular/pull/32912)링크를 참고하세요.
...
-->
`ng xi18n` 명령을 옵션없이 실행하면 프로젝트 `src` 폴더에 `messages.xlf`라는 이름으로 소스 언어 파일이 생성됩니다.
