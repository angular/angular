...
-->
*Internationalization(국제화)*은 애플리케이션 설계 단계 중 하나이며, 애플리케이션을 여러 언어로 제공할 수 있도록 준비하는 것을 의미합니다.
그리고 *Localization(현지화)*은 i18n이 적용된 앱을 특정 언어로 번역하는 단계를 의미합니다.
...
-->
## Angular CLI로 Localization 설정하기
...
-->
Angular CLI로 localization을 설정하려면 프로젝트에 `@angular/localize` 패키지를 추가하면 됩니다.
그러면 패키지가 설치되면서 Angular가 제공하는 localization 기능도 자동으로 적용됩니다.
...
그리고 Ivy는 `i18nLocale`을 활용해서 추가 locale 데이터를 자동으로 로드합니다.
...
4. 원하는 언어를 애플리케이션 설정에 추가합니다.

5. 번역한 파일을 애플리케이션에 머지합니다. 이 때 Angular CLI `build` 명령으로 앱을 빌드할 때 [locale 설정](#merge)을 선택하거나 `--localize` [옵션](#localize-config)을 지정해야 합니다. 그러면 명령이 실행될 때 원래 있던 메시지가 번역된 메시지로 대체되면서 해당 언어로 앱이 빌드됩니다.
    * locale마다 환경설정을 따로 구성하면 한 번에 여러 언어로 앱을 빌드하고 배포할 수 있습니다.
    * `--localize` 옵션을 사용하면 애플리케이션이 지원하는 언어를 Angular CLI가 자동으로 찾아서 빌드합니다. 따라서 이 옵션을 사용하면 빌드 프로세스를 단축할 수 있으며 각 언어로 빌드하기 위해 애플리케이션 전체를 다시 빌드할 필요도 없습니다. 다만, 배포는 따로 해야 합니다.
...
-->
번역한 문구를 컴포넌트 템플릿에 머지하려면 온전한 번역 파일로 앱을 컴파일해야 합니다.
이 때 번역 파일의 위치를 정확하게 지정해야 하며 번역 파일이 사용하는 locale도 제대로 설정해야 합니다.
커맨드 라인에서 작업하는 방법 외에 프로젝트 환경 설정 파일인 `angular.json` 파일에 설정하는 방법도 있습니다.

`.xlf` 포맷이나 `.xtb` 등 Angular가 지원하는 포맷을 사용하더라도 컴파일 과정은 이전과 동일합니다.
...
-->
[AOT 컴파일러](guide/glossary#aot)는 애플리케이션을 바로 실행할 수 있는 상태로 빌드하기 때문에 빌드 결과물의 크기가 작고 실행속도도 빠릅니다.
그리고 Angular 버전 9부터는 Ivy가 적용되면서 개발 단계와 운영용 빌드 단계에서 모두 AOT를 기본으로 사용합니다.
...
-->
JIT 모드로 i18n 번역을 머지하면 Ivy를 지원하지 않습니다.
그래서 반드시 JIT 모드를 사용해야 한다면 [Ivy를 비활성화](guide/ivy#opting-out-of-ivy-in-version-9)하거나 번역과 관련된 정보를 추가해야 합니다.
[이 가이드 문서](https://v8.angular.io/guide/i18n#merge-with-the-jit-compiler)를 참고하세요.
...
-->
AOT 컴파일러로 i18n을 적용하려면 각 언어별로 애플리케이션을 빌드해야 하며 서버-사이드에서 언어를 자동으로 인식하는 방법을 사용하거나 URL 인자를 전달해야 합니다.
각 언어별로 locale 빌드 설정이 되어있다면 Angular CLI만으로도 처리할 수 있습니다.
...
-->
Angular CLI 설정 파일에 프로젝트 옵션으로 `i18n`을 지정하면 프로젝트마다 locale을 지정해둘 수 있습니다.
이 옵션 안에서 세부 옵션으로 프로젝트에 적용할 번역 파일을 지정하면 됩니다.

  * `sourceLocale` - 애플리케이션의 소스 코드에서 사용할 locale을 지정합니다. 기본값은 `en-US`입니다.
  * `locales` - 번역 파일의 위치를 지정합니다.
...
-->
그리고 번역과 관련된 설정이 있다는 것을 AOT 컴파일러에게 알리기 위해 `angular.json` 파일에 `localize` 빌드 옵션을 지정해야 합니다. 이 옵션에 지정할 수 있는 값은 이런것들이 있습니다:

  * `true` - 지원하는 locale 버전별로 애플리케이션을 빌드합니다.
  * `false` - (기본값) 현지화를 적용하지 않고 일반적인 방법으로 애플리케이션을 빌드합니다.
  * locale 배열 - 지정된 locale 설정으로만 애플리케이션을 빌드합니다.

`localize` 옵션을 사용하면 Angular CLI가 만드는 빌드 결과물이 locale 폴더마다 생성됩니다. 이 폴더는 프로젝트에 설정한 `outputPath`의 하위 폴더로 생성되며, HTML 파일에 지정하는 HREF도 각 locale에 맞게 `baseHref` 값이 지정됩니다.

`--localize` 옵션은 `ng build` 명령을 `production` 설정으로 빌드할 때도 사용할 수 있습니다.
이 경우에도 Angular CLI는 프로젝트 설정에 지정된 모든 locale에 맞게 애플리케이션을 빌드합니다.
...
-->
특정 언어로 빌드할 때만 빌드 옵션을 추가할 수도 있습니다. 이 경우에는 `angular.json` 파일의 빌드 설정을 다음과 같이 수정하면 됩니다.
...
-->
이 환경 설정은 `ng serve`나 `ng build` 명령을 실행할 때 적용됩니다.
...
-->
Angular CLI가 실행하는 개발 서버(`ng serve`)에는 한가지 locale로만 동작합니다.
...
-->
번역된 언어로 운영용 빌드하려면 다음과 같이 실행하면 됩니다:
...
Angular CLI 환경 설정 파일 `angular.json`의 `options` 항목을 지정하면 경고 레벨을 조정할 수 있습니다. 그래서 다음과 같은 옵션을 지정하면 에러 레벨로 경고가 표시됩니다.
...
-->
애플리케이션을 여러 언어로 빌드하고 이 앱을 Apache 2나 NGINX에 적용하는 방법에 대해 자세하게 알아보려면 [Philippe Martin이 작성한 이 튜토리얼](https://dev.to/angular/deploying-an-i18n-angular-app-with-angular-cli-2fb9)을 참고하세요.