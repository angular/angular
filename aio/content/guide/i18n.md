...
<div class="alert is-helpful">
  ...
  The `--i18nFormat` option is only required if you are using an earlier version of Angular, or if you [disable Ivy](guide/ivy#opting-out-of-ivy-in-version-9)
...
If you don't use the CLI, you can use the `ng-xi18n` tool directly from the `@angular/compiler-cli` package.
...
Ivy does not support i18n translation with JIT mode.
AOT is the default within Ivy for both production and development configurations.
