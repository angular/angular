...
export class Rule extends Rules.AbstractRule {
  apply(sourceFile: ts.SourceFile): RuleFailure[] {
    return this.applyWithWalker(
        new SwitchTemplateAttributeSelectorsWalker(sourceFile, this.getOptions()));
  }
}

export class SwitchTemplateAttributeSelectorsWalker extends ComponentWalker {
          ...
          .forEach(replacement => {
            replacements.push({
              message: `Found deprecated attribute selector` +
                  ` "${red('[' + selector.replace + ']')}" which has been renamed to` +
                  ` "${green('[' + selector.replaceWith + ']')}"`,
              replacement
            });
          });
    });
