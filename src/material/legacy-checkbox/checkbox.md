...
```
providers: [
  {provide: MAT_CHECKBOX_DEFAULT_OPTIONS, useValue: { clickAction: 'noop' } as MatLegacyCheckboxDefaultOptions}
]
```
...
`MatLegacyCheckbox` uses an internal `<input type="checkbox">` to provide an accessible experience.
...
descriptive text content. For dynamic labels, `MatLegacyCheckbox` provides input properties for binding
