...
const config = {
  'commitMessage': {
    'maxLineLength': 120,
    'minBodyLength': 0,
    'types': ['feat', 'fix', 'refactor', 'release', 'style'],
    'scopes': ['common', 'compiler', 'core', 'packaging'],
  },
};
    ...
    describe('(squash)', () => {
      ...
      describe('with `disallowSquash`', () => {
        it('should fail', () => {
          ...
          expect(
            validateCommitMessage('squash! fix(core): something', {
              disallowSquash: true,
            })
          ).toBe(INVALID);
          expect(lastError).toContain(
    ...
    describe('(fixup)', () => {
      ...
      describe('with `nonFixupCommitHeaders`', () => {
        ...
        it('should fail if `nonFixupCommitHeaders` is empty', () => {
          ...
          expect(
            validateCommitMessage('fixup! foo', {disallowSquash: false, nonFixupCommitHeaders: []})
          ).toBe(INVALID);
          expect(lastError).toContain(
            `Unable to find match for fixup commit among prior commits: -`
          );
