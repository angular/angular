...
The MV3 service worker is terminated after about 30s of inactivity, which would drop a live
connection. The content script posts `__NG_DEVTOOLS_BEAT` over its port every 20s to keep the
worker alive ([`content-script.ts`](../projects/shell-browser/src/app/content-script.ts)).
