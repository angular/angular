...
const fakeNgGlobal = (framework) => {
  const base = {
    getComponent(_element) {
      return {};
    },
    getDirectiveMetadata(_directiveOrComponentInstance) {
      return {
        framework,
      };
    },
  ...
  if (framework === Framework.Angular || framework === Framework.ACX) {
    return {
      ...base,
      ɵgetLoadedRoutes(route) {
        return [];
      },
      ɵnavigateByUrl(router, url) {},
      ɵgetRouterInstance(injector) {
        return {};
      },
    };
  }
...
describe('ng-debug-api', () => {
  ...
  describe('ngDebugDependencyInjectionApiIsSupported', () => {
    ...
    it('returns false when any required API is missing', () => {
      globalThis.ng = {...goldenNg, getInjector: undefined};
      expect(ngDebugDependencyInjectionApiIsSupported()).toBeFalse();
      globalThis.ng = {...goldenNg, ɵgetInjectorResolutionPath: undefined};
      expect(ngDebugDependencyInjectionApiIsSupported()).toBeFalse();
      globalThis.ng = {...goldenNg, ɵgetDependenciesFromInjectable: undefined};
      expect(ngDebugDependencyInjectionApiIsSupported()).toBeFalse();
      globalThis.ng = {...goldenNg, ɵgetInjectorProviders: undefined};
      expect(ngDebugDependencyInjectionApiIsSupported()).toBeFalse();
      globalThis.ng = {...goldenNg, ɵgetInjectorMetadata: undefined};
      expect(ngDebugDependencyInjectionApiIsSupported()).toBeFalse();
    });
  });
  describe('ngDebugProfilerApiIsSupported', () => {
    ...
    it('should support Profiler API', () => {
      globalThis.ng = fakeNgGlobal(Framework.Angular);
      expect(ngDebugProfilerApiIsSupported()).toBeTrue();
      globalThis.ng = fakeNgGlobal(Framework.ACX);
      expect(ngDebugProfilerApiIsSupported()).toBeTrue();
    });
    it('should NOT support Profiler API', () => {
      globalThis.ng = fakeNgGlobal(Framework.Wiz);
...
//# sourceMappingURL=ng-debug-api.spec.js.map
