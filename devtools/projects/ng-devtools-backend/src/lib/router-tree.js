...
export function parseRoutes(router) {
  ...
  const rootName = router.rootComponentType?.name || 'no-name';
...
function getGuardNames(child, type) {
...
function assignChildrenToParent(parentPath, children, currentUrl) {
  return children.map((child) => {
    ...
    const loadedRoutes = window.ng?.ɵgetLoadedRoutes?.(child);
 ...
 */
export function getElementRefByName(type, routes, name) {
  for (const element of routes) {
    ...
    const loadedRoutes = element?.['_loadedRoutes'];
    if (loadedRoutes) {
 ...
 * @returns - the element reference if found, otherwise null
 */
export function getComponentRefByName(routes, name) {
  for (const element of routes) {
