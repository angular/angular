...
export function highlightSelectedElement(el: Node): void {
  ...
  if (selectedElementOverlays[0]?.isConnected === false) {
    // If the overlay is detached, we should clear it.
    // Actually the HEAD checked selectedElementOverlay?.isConnected
  }
...
function addHighlightForElement(
  ...
  } else if (showLabel && directiveName) {
