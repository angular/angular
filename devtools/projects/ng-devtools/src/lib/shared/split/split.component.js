...
let SplitComponent = class SplitComponent {
  constructor(elRef, cdRef, renderer) {
    ...
    this.directionInput = input('horizontal', {alias: 'direction'});
    // This allows the direction to be writable from the outside
    this.direction = linkedSignal(this.directionInput);
    this.unit = input('percent');
    this.gutterSize = input(11, {transform: (v) => getInputPositiveNumber(v, 11)});
    this.gutterStep = input(1, {transform: (v) => getInputPositiveNumber(v, 1)});
    this.restrictMove = input(false, {transform: booleanAttribute});
    this.useTransition = input(false, {transform: booleanAttribute});
    this.disabled = input(false, {transform: booleanAttribute});
    this.dir = input('ltr');
    this.gutterDblClickDuration = input(0, {transform: (v) => getInputPositiveNumber(v, 0)});
    ////
    this.dragStart = output();
    this.dragEnd = output();
    this.gutterClick = output();
    this.gutterDblClick = output();
    this.transitionEndSubscriber = null;
    this.transitionEnd = outputFromObservable(
  ...
  startDragging(event, gutterOrder, gutterNum) {
    ...
    this.displayedAreas.forEach((area) => {
      const areaSnapshot = {
        ...
        sizePercentAtStart: untracked(this.unit) === 'percent' ? area.size : -1, // If pixel mode, anyway, will not be used.
      };
...
__decorate([ViewChildren('gutterEls')], SplitComponent.prototype, 'gutterEls', void 0);
