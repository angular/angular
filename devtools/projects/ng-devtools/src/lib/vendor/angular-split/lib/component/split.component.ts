...
export class SplitComponent implements OnDestroy {
  ...
  readonly unit = input<'percent' | 'pixel'>('percent');
  readonly dir = input<'ltr' | 'rtl'>('ltr');
  readonly gutterSize = input(11, {transform: (v) => getInputPositiveNumber(v, 11)});
  readonly gutterStep = input(1, {transform: (v) => getInputPositiveNumber(v, 1)});
  ...
  readonly gutterDblClickDuration = input(0, {transform: (v) => getInputPositiveNumber(v, 0)});

  readonly klass = computed(() => [`as-${this.direction()}`, `as-${this.unit()}`]);
  ...
  readonly gutterEls = viewChildren<ElementRef>('gutterEls');
  ...
  constructor() {
    effect(() => {
      const _ = this.gutterSize();
      const _1 = this.direction();
  ...
  public startDragging(
    ...
    if (this.startPoint === null || this.disabled()) {
      return;
    }
