...
describe('SettingsStore', () => {
  ...
  beforeEach(() => {
    ...
    TestBed.configureTestingModule({
      providers: [SettingsStore, {provide: ApplicationOperations, useValue: appOperationsMock}],
    });
  ...
  it('should return a settings item with an initial value', async () => {
    const item = settingsStore.create('item@test', 'foo');
    expect(item()).toEqual('foo');
  });

  it('should set a settings item value', async () => {
    const value = settingsStore.create('item@test', 'foo');
  ...
  it('should set values to multiple settings items', async () => {
    const first = settingsStore.create('first@test', 'not_set');
    const second = settingsStore.create('second@test', 'not_set');
  ...
  it('should keep in sync multiple instances of the same settings item', async () => {
    const foo = settingsStore.create('item@test', 'foo');
    const bar = settingsStore.create('item@test', 'bar');
