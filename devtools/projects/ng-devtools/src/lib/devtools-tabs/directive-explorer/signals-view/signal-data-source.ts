...
export class SignalDataSource extends DataSource<FlatNode> {
  ...
  constructor(
    ...
    private _entityPosition: SignalNodePosition,
    private _messageBus: MessageBus<Events>,
  ) {
    super();
    this._data.next(
      this._treeFlattener.flattenNodes([
        {
          descriptor: props,
          parent: null,
          name: 'value',
  ...
  update(props: Descriptor): void {
    const newData = this._treeFlattener.flattenNodes([
      {
        descriptor: props,
        parent: null,
        name: 'value',
  ...
  private _toggleNode(node: FlatNode, expand: boolean): void {
    ...
    this._messageBus.emit('getSignalNestedProperties', [this._entityPosition, parentPath]);
