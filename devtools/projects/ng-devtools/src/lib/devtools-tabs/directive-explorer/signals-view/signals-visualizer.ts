...
export class SignalsGraphVisualizer {
  ...
  render(injectorGraph: DebugSignalGraph): void {
    ...
    const createNode = (node: DebugSignalGraphNode) => {
      ...
      outer.onclick = () => this._selected.set(node.id);
    ...
    const g = d3.select(this._svg).select('g');

    this.drender(g, this.graph);

    g.select('.output').attr(
      'transform',
      `translate(${-this.graph.graph().width! / 2}, ${-this.graph.graph().height! / 2})`,
    );
  }
  ...
  resize() {
    const svg = d3.select(this._svg);
    svg.attr('viewBox', [
      -this._svg.clientWidth / 2,
      -this._svg.clientHeight / 2,
