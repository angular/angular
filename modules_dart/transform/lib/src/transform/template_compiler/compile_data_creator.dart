...
Future<CompileDataResults> createCompileData(AssetReader reader,
    AssetId assetId, List<String> platformDirectives, List<String> platformPipes) async {
  return logElapsedAsync(() async {
    final creator =
        await _CompileDataCreator.create(reader, assetId, platformDirectives, platformPipes);
...
class _CompileDataCreator {
  ...
  _CompileDataCreator(
      this.reader, this.entryPoint, this.ngMeta, this.platformDirectives, this.platformPipes);
  ...
  static Future<_CompileDataCreator> create(AssetReader reader, AssetId assetId,
    ...
    return new _CompileDataCreator(reader, assetId, ngMeta, platformDirectives, platformPipes);
  }
  ...
  Future<CompileDataResults> createCompileData() async {
    ...
    final platformDirectives = await _readPlatformTypes(this.platformDirectives, 'directives');
      ...
      if (ngMeta.types.containsKey(reflectable.name)) {
        ...
        if (compileDirectiveMetadata is CompileDirectiveMetadata &&
          ...
          final compileDatum = new NormalizedComponentWithViewDirectives(
              compileDirectiveMetadata, <CompileDirectiveMetadata>[], <CompilePipeMetadata>[]);
          compileDatum.directives.addAll(platformDirectives);
          compileDatum.directives.addAll(_resolveTypeMetadata(ngMetaMap, reflectable.directives));
          compileDatum.pipes.addAll(platformPipes);
          compileDatum.pipes.addAll(_resolveTypeMetadata(ngMetaMap, reflectable.pipes));
  ...
  List<dynamic> _resolveTypeMetadata(Map<String, NgMeta> ngMetaMap, List<PrefixedType> prefixedTypes) {
  ...
  Future<List<dynamic>> _readPlatformTypes(List<String> inputPlatformTypes, String configOption) async {
    ...
    for (var pd in inputPlatformTypes) {
      ...
      res.addAll(await _readPlatformTypeFromUri(parts[0], parts[1]));
    }
    return res;
  }

  Future<List<dynamic>> _readPlatformTypeFromUri(
