 ...
 *
 * If the above checks fails, the script will throw an error, otherwise it will complete
 * successfully.
 *
 * **Context:**
 ...
 * is using a lockfile (such as `pnpm-lock.yaml`) to pin the dependencies to exact versions. This works
 * as long as the lockfile itself is in-sync with the corresponding `package.json`, which specifies
 * the dependencies.
 *
 * Ideally, we would run `pnpm install` with the `--frozen-lockfile` option to verify that the
