...
-   **Conflict Detection**: It actively prevents "double binding". If you bind `[formField]`, you are *banned* from also binding `[value]`, `[disabled]`, `[required]`, etc., as the signal form should be the single source of truth.
...
-   **`ControlDirectiveHost`**: These instructions provide the directive with a `ControlDirectiveHost`. This is a privileged interface that allows the `FormField` directive to:
    ...
    -   **Set Inputs**: Write directly to inputs of *other directives* on the same node (e.g., setting the `value` input of a custom control).
