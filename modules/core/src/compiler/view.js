...
export class View {
  ...
  constructor(proto:ProtoView, nodes:List<Node>, protoRecordRange:ProtoRecordRange) {
    this.proto = proto;
    this.nodes = nodes;
    ...
    if (isPresent(this.proto) && MapWrapper.size(this.proto.variableBindings) > 0) {
      var localBindings = MapWrapper.create();
      MapWrapper.forEach(this.proto.variableBindings, (tmplName, ctxName) => {
        MapWrapper.set(localBindings, tmplName, null);
      });
      this.contextWithLocals = new ContextWithVariableBindings(null, localBindings);
    } else {
...
export class ProtoView {
  ...
  instantiate(hostElementInjector: ElementInjector):View {
    ...
    var view = new View(this, viewNodes, this.protoRecordRange);
