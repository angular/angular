...
function validateNode(route: Route): void {
  ...
  if (route.component === undefined && (route.outlet && route.outlet !== PRIMARY_OUTLET)) {
    throw new Error(`Invalid route configuration of route '${route.path
                    }': a componentless route cannot have a named outlet set`);
  }
  if (!!route.redirectTo && !!route.children) {
    throw new Error(`Invalid configuration of route '${route.path
  ...
  if (route.redirectTo === undefined && !route.component && !route.children &&
      !route.loadChildren) {
    throw new Error(
        `Invalid configuration of route '${route.path
        }': one of the following must be provided (component or redirectTo or children or loadChildren)`);
  }
  ...
  if (route.path === '' && route.redirectTo !== undefined && route.pathMatch === undefined) {
    ...
    throw new Error(
        `Invalid route configuration of route '{path: "${route.path
        }", redirectTo: "${route.redirectTo}"}': please provide 'pathMatch'. ${exp}`);
  }
