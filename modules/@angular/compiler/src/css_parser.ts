...
export class CssParser {
  ...
  /**
   * @param sourceContent the CSS code that will be parsed
   * @param sourceUrl the name of the CSS file containing the CSS source code
   */
  parse(sourceContent: string, sourceUrl: string): ParsedCssResult {
    var lexer = new CssLexer();
    this._file = new ParseSourceFile(sourceContent, sourceUrl);
    this._scanner = lexer.scan(sourceContent, false);

    const delimiters: number = EOF_DELIM_FLAG;
  ...
  /** @internal */
  _generateSourceSpan(startSubject: CssToken|CssAst, endSubject: CssToken|CssAst = null):
      ParseSourceSpan {
    var startLoc: ParseLocation;
    if (startSubject instanceof CssAst) {
      startLoc = startSubject.location.start;
    } else {
      var token = startSubject;
      if (!isPresent(token)) {
    ...
    if (!isPresent(endSubject)) {
      endSubject = this._lastToken;
    }

    var endLine: number;
    var endColumn: number;
    var endIndex: number;
    if (endSubject instanceof CssAst) {
      endLine = endSubject.location.end.line;
      endColumn = endSubject.location.end.col;
      endIndex = endSubject.location.end.offset;
    } else if (endSubject instanceof CssToken) {
      endLine = endSubject.line;
      endColumn = endSubject.column;
      endIndex = endSubject.index;
    }
  ...
  _parseSelector(delimiters: number): CssSelectorAst {
    ...
    var lastSelector = firstSelector;
    ...
    if (limit > 0) {
      lastSelector = simpleSelectors[limit];
    }
