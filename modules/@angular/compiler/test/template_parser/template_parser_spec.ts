...
2. If 'un-known' is a Web Component then add "CUSTOM_ELEMENTS_SCHEMA" to the '@NgModule.schema' of this component to suppress this message. ("[ERROR ->]<un-known></un-known>"): TestComp@0:0`);
            ...
            it('should parse bound properties via bind-animate- and not report them as animation properties',
               () => {
                 expect(humanizeTplAst(parse('<div bind-animate-something="value2">', [])))
                     .toEqual([
                       [ElementAst, 'div'],
                       [
                         BoundElementPropertyAst, PropertyBindingType.Animation, 'something',
                         'value2', null
                       ]
                     ]);
               });
            ...
            it('should parse bound properties via [@] and not report them as attributes', () => {
              expect(humanizeTplAst(parse('<div [@something]="value2">', []))).toEqual([
                [ElementAst, 'div'],
                [
                  BoundElementPropertyAst, PropertyBindingType.Animation, 'something', 'value2',
                  null
                ]
              ]);
            });
...
Multiple components matched on one element. Make sure that only one component is active on any given element. Conflicting components are: DirB, DirA ("[ERROR ->]<div>"): TestComp@0:0`);
          });
