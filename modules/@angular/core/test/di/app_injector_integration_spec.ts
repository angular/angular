...
import {CompilerConfig, InjectorResolver} from '@angular/compiler';

import {MockInjectorResolver} from '@angular/compiler/testing';
...
function declareTests(isJit: boolean) {
  describe('generated injector integration tests', function () {
    var injectorResolver: MockInjectorResolver;
    var compiler: ComponentResolver;

    beforeEach(inject([InjectorResolver, ComponentResolver], (_injectorResolver, _compiler) => {
      injectorResolver = _injectorResolver;
      compiler = _compiler;
    }));

    it('should work without mocks', () => {
      var factory = compiler.createInjectorFactory(SomeConfig);
      var injector = factory.create();
      expect(injector.get(SomeService)).toBeAnInstanceOf(SomeService);
    });

    it('should support extra providers', () => {
      var factory = compiler.createInjectorFactory(SomeConfig, [Engine]);
      var injector = factory.create();
      expect(injector.get(SomeService)).toBeAnInstanceOf(SomeService);
      expect(injector.get(Engine)).toBeAnInstanceOf(Engine);
    });

    function createInjector(providers: any[],
      parent: Injector = null): Injector {
      injectorResolver.setProvidersOverride(SomeConfig, providers);
      var factory = compiler.createInjectorFactory(SomeConfig);
      ...
      describe("@Self()", () => {
        ...
        it("should throw when not requested provider on self", () => {
          expect(() => createInjector([
            provide(Car, { useFactory: (e) => new Car(e), deps: [[Engine, new SelfMetadata()]] })
          ])).toThrowError(/No provider for Engine/);
        });
      });
    ...
    describe('provider properties', () => {
      ...
      it('should support provider properties', () => {
        var inj = createInjector([new Provider(Engine, { useProperty: 'a' })], new SomeConfig());
        expect(inj.get(Engine)).toBe('aValue');
      });

      it('should support multi providers', () => {
        var inj = createInjector([new Provider(Engine, { useValue: 'bValue', multi: true }), new Provider(Engine, { useProperty: 'a', multi: true })], new SomeConfig());
        expect(inj.get(Engine)).toEqual(['bValue', 'aValue']);
      });

      it('should throw if the config is missing when the injector is created', () => {
        expect(() => createInjector([new Provider(Engine, { useProperty: 'a' })])).toThrowError('This injector needs a config object!');
      });
    });
  });
