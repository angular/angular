...
export function main() {
  describe('dom adapter', () => {
    ...
    it('should not coalesque text nodes', () => {
      const el1 = el(defaultDoc, '<div>a</div>');
      const el2 = el(defaultDoc, '<div>b</div>');
    ...
    it('should clone correctly', () => {
      const el1 = el(defaultDoc, '<div x="y">a<span>b</span></div>');
    ...
    it('should be able to create text nodes and use them with the other APIs', () => {
      const t = getDOM().createTextNode(defaultDoc, 'hello');
      expect(getDOM().isTextNode(t)).toBe(true);
      const d = getDOM().createElement(defaultDoc, 'div');
    ...
    if (getDOM().supportsDOMEvents()) {
      describe('getBaseHref', () => {
        ...
        it('should return the value of the base element', () => {
          const baseEl = getDOM().createElement(defaultDoc, 'base');
