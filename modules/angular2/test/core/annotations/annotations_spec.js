...
import {DirectiveAnnotation, onChange} from 'angular2/src/core/annotations/annotations';

export function main() {
  describe("Directive", () => {
    describe("lifecycle", () => {
      it("should be false when no lifecycle specified", () => {
        var d = new DirectiveAnnotation();
        expect(d.hasLifecycleHook(onChange)).toBe(false);
      });

      it("should be false when the lifecycle does not contain the hook", () => {
        var d = new DirectiveAnnotation({lifecycle:[]});
        expect(d.hasLifecycleHook(onChange)).toBe(false);
      });

      it("should be true otherwise", () => {
        var d = new DirectiveAnnotation({lifecycle:[onChange]});
