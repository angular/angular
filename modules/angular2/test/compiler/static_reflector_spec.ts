...
import {StaticReflector, StaticReflectorHost, StaticType, ModuleContext} from 'angular2/src/compiler/static_reflector';
...
export function main() {
  ...
  describe('StaticReflector', () => {
    ...
    it('should get constructor for NgFor', () => {
      let NgFor = host.findDeclaration('angular2/src/common/directives/ng_for', 'NgFor');
      let ViewContainerRef = host.findDeclaration('angular2/src/core/linker/view_container_ref', 'ViewContainerRef');
      let TemplateRef = host.findDeclaration('angular2/src/core/linker/template_ref', 'TemplateRef');
      let IterableDiffers = host.findDeclaration('angular2/src/core/change_detection/differs/iterable_differs',
          'IterableDiffers');
      let ChangeDetectorRef = host.findDeclaration('angular2/src/core/change_detection/change_detector_ref',
          'ChangeDetectorRef');
    ...
    it('should get annotations for HeroDetailComponent', () => {
      ...
      expect(annotation.directives)
          .toEqual([
            [
              host.findDeclaration('angular2/src/common/directives/ng_for',
                                      'NgFor')
            ]
          ]);
    });
    ...
    it('should simplify an array index', () => {
      expect(singleModuleSimplify(noContext, ({__symbolic: "index", expression: [1, 2, 3], index: 2})))
          .toBe(3);
    });
    ...
    it('should simplify a module reference without crossing modules', () => {
      expect(singleModuleSimplify({moduleId: '', filePath: '/src/cases'},
                                  ({__symbolic: "reference", module: "./extern", name: "s"})))
          .toEqual(host.getStaticType('', '/src/extern.d.ts', 's'));
    });
  });
}

class MockReflectorHost implements StaticReflectorHost {
  private staticTypeCache = new Map<string, StaticType>();
  ...
  findDeclaration(modulePath: string,
    ...
    if (modulePath.indexOf('.') === 0) {
      return this.getStaticType(`mod/${symbolName}`, pathTo(containingFile, modulePath) + '.d.ts', symbolName);
    }
    return this.getStaticType(`mod/${symbolName}`, '/tmp/' + modulePath + '.d.ts', symbolName);
  }
