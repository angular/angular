...
export class Router {
  ...
  navigate(url: string, _skipLocationChange: boolean = false): Promise<any> {
    return this._currentNavigation = this._currentNavigation.then((_) => {
      ...
      return this._afterPromiseFinishNavigating(this.recognize(url).then((instruction) => {
        ...
        return this._nagivate(instruction, _skipLocationChange);
      }));
    });
  }
  ...
  navigateInstruction(instruction: Instruction,
    ...
    return this._currentNavigation = this._currentNavigation.then((_) => {
      this._startNavigating();
      return this._afterPromiseFinishNavigating(this._nagivate(instruction, _skipLocationChange));
    });
  }

  _nagivate(instruction: Instruction, _skipLocationChange: boolean): Promise<any> {
