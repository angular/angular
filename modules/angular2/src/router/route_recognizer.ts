...
export class RouteRecognizer {
  ...
  config(config: RouteDefinition): boolean {
    ...
    if (isPresent(config.name) && config.name[0].toUpperCase() != config.name[0]) {
      var suggestedAlias = config.name[0].toUpperCase() + config.name.substring(1);
      throw new BaseException(
          `Route '${config.path}' with alias '${config.name}' does not begin with an uppercase letter. Route aliases should be CamelCase like '${suggestedAlias}'.`);
    }
