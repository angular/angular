...
class ViewBuilderVisitor implements TemplateAstVisitor {
  ...
  visitBoundText(ast: BoundTextAst, parent: CompileElement): any {
    ...
    this.view.createMethod.resetDebugInfo(nodeIndex, ast);
    var renderNode = this._visitText(ast, '', ast.ngContentIndex, parent);
    return null;
  }
  visitText(ast: TextAst, parent: CompileElement): any {
    this.view.createMethod.resetDebugInfo(this.view.nodes.length, ast);
  ...
  private _visitText(ast: TemplateAst, value: string, ngContentIndex: number,
    ...
    this.view.createMethod.addStmtWithoutAddingDebugInfo(createRenderNode);
...
function createViewClass(view: CompileView, renderCompTypeVar: o.ReadVarExpr, nodeDebugInfosVar: o.Expression): o.ClassStmt {
  ...
  var viewMethods = [
    ...
    new o.ClassMethod(
        ...
        o.DYNAMIC_TYPE),
    new o.ClassMethod('detectChangesInternal', [new o.FnParam(DetectChangesVars.throwOnChange.name, o.BOOL_TYPE)],
...
function generateDetectChangesMethod(view: CompileView): o.Statement[] {
  ...
  if (view.detectChangesInInputsMethod.isEmpty() && view.updateContentQueriesMethod.isEmpty() &&
     view.afterContentLifecycleCallbacksMethod.isEmpty() && view.detectChangesHostPropertiesMethod.isEmpty() &&
  ...
  if (afterViewStmts.length > 0) {
    stmts.push(new o.IfStmt(o.not(DetectChangesVars.throwOnChange),
      afterViewStmts
    ));
  }
  ...
  if (SetWrapper.has(readVars, DetectChangesVars.currVal.name)) {
    varStmts.push(DetectChangesVars.currVal.set(o.NULL_EXPR).toDeclStmt());
  }
  if (SetWrapper.has(readVars, DetectChangesVars.valUnwrapper.name)) {
    varStmts.push(DetectChangesVars.valUnwrapper.set(o.importExpr(Identifiers.ValueUnwrapper).instantiate([])).toDeclStmt(null, [o.StmtModifier.Final]));
  }
...
function getChangeDetectionMode(view: CompileView):ChangeDetectionStrategy {
  ...
  if (view.viewType === ViewType.COMPONENT) {
      mode = isDefaultChangeDetectionStrategy(view.component.changeDetection) ? ChangeDetectionStrategy.CheckAlways :
