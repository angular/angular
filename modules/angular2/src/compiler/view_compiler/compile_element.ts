...
export class CompileElement extends CompileNode {
  ...
  private _componentInjector: o.Expression;
  ...
  public contentNodesByNgContentIndex: Array<o.Expression>[];
    ...
    if (isPresent(this.component)) {
      ...
      this.view.constructorMethod.addStmt(
          this.getOrCreateAppElement()
              .callMethod('initComponent', [compExpr, componentConstructorViewQueryList])
              .toStmt());
    }
  }
  ...
  afterChildren(childNodeCount: number) {
    this._resolvedProviders.values().forEach((resolvedProvider) => {
      ...
      // matches first.
      var providerExpr = this._instances.get(resolvedProvider.token);
      if (resolvedProvider.providerType !== ProviderAstType.PrivateService) {
      ...
      if (resolvedProvider.providerType === ProviderAstType.PrivateService ||
        ...
        this.view.injectorPrivateGetMethod.addStmt(
            createInjectInternalCondition(this.nodeIndex, 0, resolvedProvider, providerExpr));
      }
    });
  ...
  getOrCreateComponentInjector(): o.Expression {
    if (isBlank(this._componentInjector)) {
      ...
      this.view.fields.push(new o.ClassField(fieldName, o.importType(Identifiers.Injector),
                                             [o.StmtModifier.Private]));
      var statement = o.THIS_EXPR.prop(fieldName).set(this._createInjector(true)).toStmt();
      this.view.constructorMethod.addStmt(statement);
  ...
  getOrCreateDefaultInjector(): o.Expression {
    if (isBlank(this._defaultInjector)) {
      var fieldName = `_inj_${this.nodeIndex}`;
      this.view.fields.push(new o.ClassField(fieldName, o.importType(Identifiers.Injector),
                                             [o.StmtModifier.Private]));
      var statement = o.THIS_EXPR.prop(fieldName).set(this._createInjector(false)).toStmt();
      this.view.constructorMethod.addStmt(statement);
  ...
  private _createInjector(readPrivate: boolean): o.Expression {
    return o.THIS_EXPR.callMethod('injector', [
      o.literal(this.nodeIndex),
      o.literal(readPrivate),
    ]);
  }
    ...
    if (isPresent(dep.token)) {
      ...
      if (isBlank(result)) {
        if (dep.token.equalsTo(identifierToken(Identifiers.Renderer))) {
          ...
          if (requestingProviderType === ProviderAstType.Directive || isBlank(this.component)) {
            return o.THIS_EXPR.prop('ref');
          } else {
            return this.getOrCreateAppElement().prop('componentChangeDetectorRef');
          }
