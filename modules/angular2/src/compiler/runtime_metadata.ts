...
export class RuntimeMetadataResolver {
  ...
  getRuntimeIdentifier(value: any): cpl.CompileIdentifierMetadata {
    return new cpl.CompileIdentifierMetadata({runtime: value, name: runtimeIdentifierName(value)});
  }
...
function runtimeIdentifierName(value: any): string {
  var name = stringify(value);
  var parts = StringWrapper.split(name, /[\s-{]/g);
  return parts[0];
}
