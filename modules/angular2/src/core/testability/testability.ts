...
export class Testability {
  ...
  _registerStabilityCallbacks(_ngZone: NgZone): void {
    _ngZone.overrideOnTurnStart(() => { this._isAngularStable = false; });
    _ngZone.overrideOnEventDone(() => {
      this._isAngularStable = true;
  ...
  _runCallbacksIfReady(): void {
    if (this._pendingCount != 0 || !this._isAngularStable) {
      return;  // Not ready
    }
  ...
  // check for stability.
  isAngularStable(): boolean { return this._isAngularStable; }
