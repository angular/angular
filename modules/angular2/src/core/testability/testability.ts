  ...
  _watchAngularEvents(): void {
    ...
    this._ngZone.runOutsideAngular(() => {
      ObservableWrapper.subscribe(this._ngZone.onStable, (_) => {
        ...
        scheduleMicrotask(() => {
          // Because the operation we just executed was caused by MacroTask (such as setTimeout),
          // when the function gets called we are still inside of the MacroTask. This will make
          // _ngZone.hasPendingMacrotask report outstanding tasks. We need to drain the task by
          // jumping to a microtask.
          //
          // TODO(misko): Consider having NgZone expos onMacroTaskEmpty hook. (As Zone.js already
          // provides this.)
