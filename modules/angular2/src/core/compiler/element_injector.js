...
import {Directive, onChange, onDestroy} from 'angular2/src/core/annotations/annotations' import {
  BindingPropagationConfig
}
from 'angular2/src/core/compiler/binding_propagation_config'
...
class TreeNode {
  ...
  set parent(node: TreeNode) { this._parent = node; }
...
export class ElementInjector extends TreeNode {
  ...
  get(token) { return this._getByKey(Key.get(token), 0, null); }

  hasDirective(type: Type): boolean { return this._getDirectiveByKeyId(Key.get(type).id) !== _undefined; }
  ...
  forElement(el): boolean { return this._preBuiltObjects.element.domElement === el; }

  /** Gets the NgElement associated with this ElementInjector */
  getNgElement() { return this._preBuiltObjects.element; }
  ...
  _isComponentKey(key: Key) { return this._proto._binding0IsComponent && key.id === this._proto._keyId0; }
  ...
  _shouldIncludeSelf(depth: int) { return depth === 0; }
  ...
  hasEventEmitter(eventName: string) { return this._proto.hasEventEmitter(eventName); }
