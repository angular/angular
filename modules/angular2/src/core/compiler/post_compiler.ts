...
import {TemplateCmd, BeginElementCmd, EmbeddedTemplateCmd, BeginComponentCmd, createEndComponentCmd, TextCmd,
...
import {mergeTemplates, MergeResult} from './template_merger';
...
export class PostCompiler {
  ...
  compileHost(componentType: Type): ProtoViewRef {
    ...
    if (isBlank(protoView)) {
      var annotation = <Component> this._directiveResolver.resolve(componentType);
      var cmds = [
        // TODO: support components with arbitrary css selectors!
        new BeginComponentCmd(protoViewId, annotation.selector, [], [], [], [componentType], false, null),
        createEndComponentCmd()
      ];
      protoView = this._createAppProtoView(protoViewId, renderApi.ViewType.HOST, null, cmds,  ProtoPipes.fromBindings([]));
    }
  ...
  private _createComponentProtoView(bc:BeginComponentCmd): AppProtoView {
    ...
    if (isBlank(protoView)) {
      ...
      protoView = this._createAppProtoView(bc.templateId, renderApi.ViewType.COMPONENT, null, this._templateRegistry.getTemplate(bc.templateId), ProtoPipes.fromBindings(pipes));
  ...
  private _createEmbeddedProtoView(et:EmbeddedTemplateCmd, parentProtoView:AppProtoView) {
    var embeddedProtoView = this._createAppProtoView(et.templateId, renderApi.ViewType.EMBEDDED,
      arrayToMap(et.variables), et.content, new ProtoPipes(parentProtoView.pipes.config));
    if (et.transitiveNgContentCount > 0) {
      this._initializeProtoView(embeddedProtoView);
    }
    return embeddedProtoView;
  }

  private _createAppProtoView(protoViewId: string, protoViewType: renderApi.ViewType,
                     embeddedPvVariableBindings: Map<string, string>,
                     templateCmds: TemplateCmd[], protoPipes: ProtoPipes): AppProtoView {
    var protoChangeDetector = this._changeDetection.createProtoChangeDetector(new ChangeDetectorDefinition(protoViewId, null, null, null, null, null));
    var protoView = new AppProtoView(
        protoViewType, null, null,
        protoChangeDetector, embeddedPvVariableBindings, null,
        null, protoPipes);
    protoView.templateCmds = templateCmds;
    protoView.templateCmds = templateCmds;
  ...
  initializeProtoViewIfNeeded(protoView: AppProtoView) {
    if (!protoView.isInitialized()) {
      var mergeResult = mergeTemplates(this._templateRegistry, protoView.templateCmds);
      this._renderer.addSharedStyles(mergeResult.sharedStyles);
      var renderProtoViewRef = this._renderer.createProtoView(mergeResult.fragments);      
      protoView.mergeMapping = new AppProtoViewMergeMapping(
        mergeResult.fragments.length,
        mergeResult.nestedViewIndicesByElementIndex,
        mergeResult.hostElementIndicesByViewIndex,
        mergeResult.nestedViewCountByViewIndex,
        renderProtoViewRef      
      );      
      this._initializeProtoView(protoView);
    }
  ...
  private _initializeProtoView(protoView: AppProtoView) {
    ...
    // TODO: pass all data into the constructor into AppProtoView
    // instead of using the bind... methods!
    protoView.variableLocations = variableLocations;
    protoView.textBindingCount = boundTextCount;    
  }
...
function _createElementBinder(protoView: AppProtoView, nestedProtoView: AppProtoView, elementBinderStack:ElementBinder[], boundElementIndex: number, distanceToParentBinder: number, distanceToParentPei: number,
                              beginElementCmd:BeginElementCmd):ElementBinder {
  ...
  } else if (beginElementCmd.type === RenderTemplateCmdType.TEMPLATE_ANCHOR) {
  ...
  var elBinder = protoView.bindElement(parentElementBinder, distanceToParentBinder,
  ...
  for (var i=0; i<beginElementCmd.variables.length; i+=2) {
    protoView.protoLocals.set(beginElementCmd.variables[i+1], null);
  }
  return elBinder;
}
