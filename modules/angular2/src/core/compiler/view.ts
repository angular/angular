...
export class View {
  ...
  constructor(proto:ProtoView, nodes:List<any>, protoLocals:Map<any,any>) {
    this.proto = proto;
    this.nodes = nodes;
  ...
  init(changeDetector:ChangeDetector, elementInjectors:List<any>, rootElementInjectors:List<any>, textNodes: List<any>, bindElements:List<any>,
    viewContainers:List<any>, contentTags:List<any>, preBuiltObjects:List<any>, componentChildViews:List<any>, lightDoms:List<LightDom>) {
  ...
  hydrate(appInjector: Injector, hostElementInjector: ElementInjector, hostLightDom: LightDom,
    ...
    for (var i = 0; i < binders.length; ++i) {
      ...
      if (isPresent(componentDirective)) {
        var services = (<any>componentDirective.annotation).services;
  ...
  private onAllChangesDone(directiveMemento:DirectiveMemento) {
    var dir = directiveMemento.directive(this.elementInjectors);
    dir.onAllChangesDone();
  }

  private onChange(directiveMemento:DirectiveMemento, changes) {
    var dir = directiveMemento.directive(this.elementInjectors);
...
export class ProtoView {
  ...
  bindingRecords:List<any>;
  parentProtoView:ProtoView;
  _variableBindings:List<any>;

  _directiveMementosMap:Map<any,any>;
  _directiveMementos:List<any>;
   ...
   * @returns {Function}
   */
  static buildEventHandler(eventMap: Map<any,any>, injectorIdx: int) {
...
export class ElementBindingMemento {
  ...
  invoke(currentValue:any, bindElements:List<any>) {
