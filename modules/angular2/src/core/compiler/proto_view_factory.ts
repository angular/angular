...
export class ProtoViewFactory {
  ...
  createProtoView(componentAnnotation:ComponentAnnotation, renderProtoView: renderApi.ProtoView, directives:List<DirectiveBinding>):ProtoView {
    return this._createProtoView(null, componentAnnotation, renderProtoView, directives);
  }

  _createProtoView(parent:ProtoView, componentAnnotation:ComponentAnnotation, renderProtoView: renderApi.ProtoView, directives:List<DirectiveBinding>):ProtoView {
    var protoChangeDetector = this._changeDetection.createProtoChangeDetector('dummy',
      componentAnnotation.changeDetection);
    var domProtoView = this._getDomProtoView(<DirectDomProtoViewRef>renderProtoView.render);
  ...
  private _getDomProtoView(protoViewRef: DirectDomProtoViewRef) {
    return protoViewRef.delegate;
  }

  private _findParentProtoElementInjectorWithDistance(binderIndex, elementBinders, renderElementBinders) {
    var distance = 0;
    do {
