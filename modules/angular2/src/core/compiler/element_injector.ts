...
export class ElementInjector extends TreeNode<ElementInjector> {
  ...
  dehydrate(): void {
    ...
    this._injector.strategy.dehydrate();
  }
  ...
  hydrate(injector: Injector, host: ElementInjector, preBuiltObjects: PreBuiltObjects): void {
    ...
    this._hydrateInjector(injector, host);
  ...
  private _hydrateInjector(injector: Injector, host: ElementInjector): void {
    if (isPresent(this._parent)) {
      this._reattachInjector(this._injector, this._parent._injector, false);
    } else {
      if (isPresent(injector) && isPresent(host)) {
        this._reattachInjector(injector, host._injector, true);
      }
      var parent = this._getHostInjector(injector, host);
      this._reattachInjector(this._injector, parent, true);
    }
  }

  private _getHostInjector(injector: Injector, host: ElementInjector): Injector {
  ...
  private _reattachInjector(injector: Injector, parentInjector: Injector, isBoundary: boolean) {
    injector.strategy.attach(parentInjector, isBoundary);
    injector.strategy.hydrate();
  }
