  ...
  toString(): string {
    ...
    if (isPresent(this.attrs)) {
      for (var i = 0; i < this.attrs.length;) {
        ...
        var attrValue = this.attrs[i++] res += '[' + attrName;
...
export class SelectorMatcher {
  ...
  addSelectable(cssSelector: CssSelector, selectable) {
    ...
    if (isPresent(attrs)) {
      for (var index = 0; index < attrs.length;) {
        ...
        var valuesMap = MapWrapper.get(map, attrName) if (isBlank(valuesMap)) {
  ...
  _addTerminal(map: Map<string, string>, name: string, selectable) {
    var terminalList = MapWrapper.get(map, name) if (isBlank(terminalList)) {
  ...
  _addPartial(map: Map<string, string>, name: string) {
    var matcher = MapWrapper.get(map, name) if (isBlank(matcher)) {
  ...
  */
  match(cssSelector: CssSelector, matchedCallback: Function) {
    ...
    if (isPresent(attrs)) {
      for (var index = 0; index < attrs.length;) {
        ...
        valuesMap = MapWrapper.get(this._attrValuePartialMap, attrName) this._matchPartial(
  ...
  _matchTerminal(map: Map<string, string> = null, name, matchedCallback) {
    ...
    var selectables = MapWrapper.get(map, name) if (isBlank(selectables)) { return; }
  ...
  _matchPartial(map: Map<string, string> = null, name, cssSelector, matchedCallback) {
    ...
    var nestedSelector = MapWrapper.get(map, name) if (isBlank(nestedSelector)) { return; }
