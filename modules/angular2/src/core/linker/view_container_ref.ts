...
export class ViewContainerRef_ implements ViewContainerRef {
  ...
  createHostView(hostViewFactoryRef: HostViewFactoryRef, index: number = -1,
                 ...
                 projectableNodes: any[][] = null, contextElRef: ElementRef = null): HostViewRef {
    var s = this._createHostViewInContainerScope();
    if (index == -1) index = this.length;
    var contextEl;
    var contextInjector;
    if (isPresent(contextElRef)) {
      contextEl = (<ElementRef_>contextElRef).internalElement;
    ...
    var view = hostViewFactory.viewFactory(contextEl.parentView.viewManager, childInjector,
                                           contextEl, projectableNodes, null);
    this._element.attachView(view, index);
    return wtfLeave(s, view.ref);
  }
