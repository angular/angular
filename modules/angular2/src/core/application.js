...
function _injectorBindings(appComponentType): List<Binding> {
  return [
    ...
    bind(appComponentAnnotatedTypeToken)
        .toFactory((reader) =>
                   {
    ...
    bind(appElementToken)
        .toFactory((appComponentAnnotatedType, appDocument) =>
                   {
    ...
    bind(appViewToken)
        .toAsyncFactory(
            (changeDetection, compiler, injector, appElement, appComponentAnnotatedType, strategy, eventManager) =>
            {
              return compiler.compile(appComponentAnnotatedType.type)
                  .then((protoView) => {
                    var appProtoView =
                        ProtoView.createRootProtoView(protoView, appElement, appComponentAnnotatedType,
                                                      changeDetection.createProtoChangeDetector('root'), strategy);
    ...
    bind(appChangeDetectorToken).toFactory((rootView) => rootView.changeDetector, [appViewToken]),
    bind(appComponentType).toFactory((rootView) => rootView.elementInjectors[0].getComponent(), [appViewToken]),
    bind(LifeCycle).toFactory((exceptionHandler) => new LifeCycle(exceptionHandler, null, assertionsEnabled()),
                                  [ExceptionHandler]),
    bind(EventManager)
        .toFactory((zone) =>
                   {
...
export function bootstrap(appComponentType: Type, bindings: List < Binding >= null,
                          givenBootstrapErrorReporter: Function = null): Promise {
  ...
  zone.run(() => {
    ...
    PromiseWrapper.then(appInjector.asyncGet(appViewToken), (rootView) =>
                                                            {
                                                              // retrieve life cycle: may have already been created if
                                                              // injected in root component
                        ...
                        (err) => {bootstrapProcess.reject(err)});
  });
