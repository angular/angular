...
export class BrowserDomAdapter extends GenericBrowserDomAdapter {
  ...
  getHost(el:HTMLElement): HTMLElement {
    return <HTMLElement>(<any>el).host;
  }
  clone(node:Node) {
  ...
  importIntoDoc(node:Node) {
    ...
    if (this.isTemplateElement(result) &&
      !this.content(<HTMLElement>result).childNodes.length && this.content(<HTMLElement>node).childNodes.length) {
      var childNodes = this.content(<HTMLElement>node).childNodes;
      for (var i = 0; i < childNodes.length; ++i) {
        this.content(<HTMLElement>result).appendChild(
