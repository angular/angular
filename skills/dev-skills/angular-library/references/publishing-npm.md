...
Update the version in the library's source `package.json` (`projects/my-lib/package.json`), which ng-packagr copies to `dist/` on every build:
...
Or use `npm version` from the library's source folder, then rebuild:
...
Do not run `npm version` inside `dist/`: the next `ng build` regenerates `dist/my-lib/package.json` from the source and the bump is lost.
