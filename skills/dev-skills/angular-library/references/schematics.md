...
Install the DevKit schematics packages (the separate `@angular-devkit/schematics-cli` is not required):
...
Schematics must be compiled to JavaScript before they are usable. Add a `tsconfig.schematics.json` next to the library's `tsconfig.lib.json`:
...
`rootDir` and `outDir` are resolved relative to this file, so the compiled schematics land in `dist/my-lib/schematics`, next to the output of `ng build my-lib`.

Build command, run after `ng build my-lib` (which cleans `dist/my-lib`):

```bash
tsc -p projects/my-lib/tsconfig.schematics.json
```

> ⚠️ **Important:** `tsc` only compiles TypeScript files. It does **not** copy JSON assets (`collection.json`, `schema.json`) or schematic templates (files in `files/` folders) to the output directory. You must add a copying step to your build pipeline.
>
> For example, you can install the `cpx` package and run:
>
> ```bash
> cpx "projects/my-lib/schematics/**/*.json" dist/my-lib/schematics && cpx "projects/my-lib/schematics/**/files/**/*" dist/my-lib/schematics
> ```

Add these compilation and copying steps to your library's publish pipeline (e.g., in a post-build or custom build script in `package.json`).
