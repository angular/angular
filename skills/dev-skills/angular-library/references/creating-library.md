...
The output is placed in `dist/my-lib/`, as set by the `dest` option in the generated `ng-package.json`.
...
>
> Fortunately, the Angular CLI automatically configures a path mapping for the primary entry point (`"my-lib": ["./dist/my-lib"]`) pointing to the `dist/` folder in the root `tsconfig.json` when generating a library.
>
> This mapping does not cover secondary entry points such as `my-lib/testing`. To import those from an application in the same workspace, add a wildcard mapping as described in [secondary-entrypoints.md](secondary-entrypoints.md#using-a-secondary-entry-point-in-the-same-workspace).

---

## Using the Library in the Same Workspace

After building the library, the consuming application can import from it by name using the automatically configured path mappings:
...
> Accessing browser-specific APIs (such as `window` or `document`) directly is not recommended if you intend to support Server-Side Rendering (SSR). This should be reserved as a last resort when other better options aren't possible, such as providing a different implementation based on Dependency Injection (DI) or using the `afterNextRender` or `afterEveryRender` lifecycle hooks.
