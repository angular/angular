...
> Libraries cannot use browser-specific APIs directly in their module code if you intend to support SSR consumers. This pattern is usually a last resort when other better options aren't possible, such as providing a different implementation based on DI or using the `afterNextRender` or `afterEveryRender` hooks.
