...
With the Vite-based dev server (`@angular/build:dev-server`):
- `"/api"` matches only `/api` exactly.
- `"/api/*"` matches `/api/users` but not `/api/users/123`.
- `"/api/**"` matches `/api/users` and `/api/users/123` (recommended for API subpaths).
...
2. Update `angular.json` (or CLI flag) to point to `src/proxy.conf.js`.
