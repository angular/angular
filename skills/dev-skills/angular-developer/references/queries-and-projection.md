...
- **Querying Components**: You can query by component class name. Type inference is automatic in this case.
  ```ts
  readonly childComponent = viewChild(ChildComponent); // Inferred as Signal<ChildComponent | undefined>
  ```
...
Use a self-closing `<ng-content />` tag to project all children into a single location.
...
## Common agent pitfalls and misconceptions

- **Do NOT pass `{static: true}` to signal queries**: In legacy decorator queries (`@ViewChild`), `{static: true}` determined whether the query resolved before `ngOnInit`. Signal queries are always reactive and lazily evaluated; passing `static` causes a TypeScript compilation error.
- **Always invoke signal queries as functions (`()`)**: `viewChild` and `contentChild` return signals (`Signal<T>`). You must call them with parentheses (`this.inputEl()?.nativeElement`), not as plain properties (`this.inputEl.nativeElement`).
- **String locators only match template reference variables**: `viewChild('myInput')` queries `<input #myInput>`, **not** CSS element selectors (`<myInput>`) or class names (`.myInput`). To query components or directives by type, pass the class directly: `viewChild(MyComponent)`.
- **`descendants` defaults differ**: `contentChild` defaults to `{descendants: true}` (deep search), while `contentChildren` defaults to `{descendants: false}` (direct children only). If you need `contentChildren` to find deeply nested children, explicitly pass `{descendants: true}`.
- **Do not rely on `ngAfterViewInit` to read signals**: Signal queries are reactive. Instead of imperatively reading them in lifecycle hooks, compose them reactively in `computed()` or monitor them in `effect()`.
...
- **Reactive derivations**: Since signal queries return signals, compose them inside `computed` or monitor them in `effect` blocks without needing lifecycle hooks like `ngAfterViewInit`.
