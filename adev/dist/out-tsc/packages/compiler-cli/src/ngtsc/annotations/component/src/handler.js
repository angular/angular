...
import {assertSuccessfulReferenceEmit, Reference} from '../../../imports';
...
import {isNamedClassDeclaration, reflectObjectLiteral} from '../../../reflection';
import {
  ComponentScopeKind,
...
export class ComponentDecoratorHandler {
  ...
  analyze(node, decorator) {
    ...
    for (const styleUrl of styleUrls) {
      try {
        ...
        if (
          styleUrl.source === 2 /* ResourceTypeForDiagnostics.StylesheetFromDecorator */ &&
  ...
  resolve(node, analysis, symbol) {
    ...
    if (scope === null) {
      ...
      if (this.semanticDepGraphUpdater !== null) {
        const getSemanticReference = (decl) =>
  ...
  locateDeferBlocksWithoutScope(template) {
    const deferBlocks = new Map();
    const directivelessBinder = new R3TargetBinder(null);
...
function removeDeferrableTypesFromComponentDecorator(analysis, deferrableTypes) {
  if (analysis.classMetadata) {
    ...
    const rewrittenDecoratorsNode = removeIdentifierReferences(
      analysis.classMetadata.decorators.node,
      deferrableSymbols,
    );
