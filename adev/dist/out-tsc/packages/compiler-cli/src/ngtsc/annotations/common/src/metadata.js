...
export function extractClassMetadata(
  ...
  angularDecoratorTransform = (dec) => dec,
) {
...
function ctorParameterToMetadata(param, isCore) {
  ...
  const mapEntries = [{key: 'type', value: type, quoted: false}];
...
function decoratorToMetadata(decorator, wrapFunctionsInParens) {
  ...
  const properties = [ts.factory.createPropertyAssignment('type', decorator.identifier)];
 ...
 */
export function removeIdentifierReferences(node, names) {
  const result = ts.transform(node, [
    (context) => (root) =>
      ts.visitNode(root, function walk(current) {
