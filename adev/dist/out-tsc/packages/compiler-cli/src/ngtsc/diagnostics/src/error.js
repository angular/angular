...
export class FatalDiagnosticError extends Error {
  ...
  constructor(code, node, diagnosticMessage, relatedInformation) {
    super(
      `FatalDiagnosticError: Code: ${code}, Message: ${ts.flattenDiagnosticMessageText(diagnosticMessage, '\n')}`,
    );
    this.code = code;
    this.node = node;
...
export function makeDiagnosticChain(messageText, next) {
  return {
...
export function addDiagnosticChain(messageText, add) {
 ...
 */
export function isLocalCompilationDiagnostics(diagnostic) {
  return (
