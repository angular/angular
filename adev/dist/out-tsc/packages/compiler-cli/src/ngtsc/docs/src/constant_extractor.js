...
export function extractConstant(declaration, typeChecker) {
...
export function isSyntheticAngularConstant(declaration) {
...
function extractLiteralPropertiesAsEnumMembers(declaration) {
  ...
  if (initializer === undefined || !ts.isObjectLiteralExpression(initializer)) {
    throw new Error(
      `Declaration tagged with "${LITERAL_AS_ENUM_TAG}" must be initialized to an object literal, but received ${initializer ? ts.SyntaxKind[initializer.kind] : 'undefined'}`,
    );
  }
...
//# sourceMappingURL=constant_extractor.js.map
