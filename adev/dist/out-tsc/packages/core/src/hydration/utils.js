...
import {HEADER_OFFSET, HYDRATION, TVIEW} from '../render3/interfaces/view';
 ...
 */
export const NGH_DEFER_BLOCKS_KEY = makeStateKey(TRANSFER_STATE_DEFER_BLOCKS_INFO);
/**
 * Checks whether a given key is used by the framework for transferring hydration data.
 */
export function isInternalHydrationTransferStateKey(key) {
  ...
  const commentNodesIterator = doc.createNodeIterator(node, NodeFilter.SHOW_COMMENT, {
    acceptNode(node) {
      ...
      const isTextNodeMarker =
        content === 'ngetn' /* TextNodeMarker.EmptyNode */ ||
        content === 'ngtns'; /* TextNodeMarker.Separator */
...
export function countBlocksSkippedByHydration(injector) {
...
export function isIncrementalHydrationEnabled(injector) {
  return injector.get(IS_INCREMENTAL_HYDRATION_ENABLED, false, {
    optional: true,
  });
}
/** Throws an error if the incremental hydration is not enabled */
export function assertIncrementalHydrationIsConfigured(injector) {
  if (!isIncrementalHydrationEnabled(injector)) {
    throw new RuntimeError(
      508 /* RuntimeErrorCode.MISCONFIGURED_INCREMENTAL_HYDRATION */,
...
export function assertSsrIdDefined(ssrUniqueId) {
  assertDefined(
    ssrUniqueId,
 ...
 */
export function canHydrateNode(lView, tNode) {
...
export function convertHydrateTriggersToJsAction(triggers) {
  let actionList = [];
  if (triggers !== null) {
 ...
 */
export function getParentBlockHydrationQueue(deferBlockId, injector) {
...
export function appendDeferBlocksToJSActionMap(doc, injector) {
...
export function retrieveDeferBlockDataImpl(injector) {
  const transferState = injector.get(TransferState, null, {optional: true});
  if (transferState !== null) {
    const nghDeferData = transferState.get(NGH_DEFER_BLOCKS_KEY, {});
    ngDevMode &&
 ...
 */
export function retrieveDeferBlockData(injector) {
 ...
 */
export function processBlockData(injector) {
 ...
 */
export function verifySsrContentsIntegrity(doc) {
  ...
  throw new RuntimeError(
    -507 /* RuntimeErrorCode.MISSING_SSR_CONTENT_INTEGRITY_MARKER */,
