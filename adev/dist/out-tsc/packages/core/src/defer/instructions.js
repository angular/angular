...
import {renderPlaceholder} from './rendering';
import {
  getHydrateTriggers,
...
function logHmrWarning(injector) {
  if (!_hmrWarningProduced) {
    ...
    console.log(
      formatRuntimeError(
        -751 /* RuntimeErrorCode.DEFER_IN_HMR_MODE */,
...
export function ɵɵdefer(
  ...
  storeTriggerCleanupFn(0 /* TriggerType.Regular */, lDetails, () =>
 ...
 * @codeGenApi
 */
export function ɵɵdeferHydrateWhen(rawValue) {
  const lView = getLView();
  const tNode = getSelectedTNode();
  if (ngDevMode) {
 ...
 * @codeGenApi
 */
export function ɵɵdeferHydrateOnTimer(delay) {
  const lView = getLView();
  const tNode = getCurrentTNode();
  if (ngDevMode) {
