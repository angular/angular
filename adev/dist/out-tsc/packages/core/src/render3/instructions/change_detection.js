...
export function detectChangesInternal(lView, mode = 0 /* ChangeDetectionMode.Global */) {
...
export function checkNoChangesInternal(lView, exhaustive) {
  setIsInCheckNoChangesMode(
...
export function refreshView(tView, lView, templateFn, context) {
  ...
  try {
    ...
    const hooksInitPhaseCompleted =
      (flags & 3) /* LViewFlags.InitPhaseStateMask */ === 3; /* InitPhaseState.InitPhaseCompleted */
      ...
      if (hooksInitPhaseCompleted) {
        ...
        if (preOrderHooks !== null) {
          executeInitAndCheckHooks(
            lView,
            preOrderHooks,
            0 /* InitPhaseState.OnInitHooksToBeRun */,
            null,
          );
        }
      ...
      if (hooksInitPhaseCompleted) {
        ...
        if (contentHooks !== null) {
          executeInitAndCheckHooks(
            lView,
            contentHooks,
            1 /* InitPhaseState.AfterContentInitHooksToBeRun */,
          );
        }
      ...
      if (hooksInitPhaseCompleted) {
        ...
        if (viewHooks !== null) {
          executeInitAndCheckHooks(
            lView,
            viewHooks,
            2 /* InitPhaseState.AfterViewInitHooksToBeRun */,
          );
        }
...
function detectChangesInView(lView, mode) {
  ...
  let shouldRefreshView = !!(
    (mode === 0 /* ChangeDetectionMode.Global */ && flags & 16) /* LViewFlags.CheckAlways */
  );
  ...
  lView[FLAGS] &= ~(
    (8192 /* LViewFlags.HasChildViewsToRefresh */ | 1024) /* LViewFlags.RefreshView */
  );
