...
import {HEADER_OFFSET, HYDRATION, RENDERER, TVIEW} from '../interfaces/view';
 ...
 * @codeGenApi
 */
export function ɵɵdomElementContainerStart(index, attrsIndex, localRefsIndex) {
  const lView = getLView();
  ngDevMode && assertTNodeCreationIndex(lView, index);
  const tView = lView[TVIEW];
  const adjustedIndex = index + HEADER_OFFSET;
 ...
 * @codeGenApi
 */
export function ɵɵdomElementContainerEnd() {
  const initialTNode = getCurrentTNode();
  ngDevMode && assertDefined(initialTNode, 'No parent node to close.');
  const currentTNode = elementLikeEndShared(initialTNode);
  ngDevMode && assertTNodeType(currentTNode, 8 /* TNodeType.ElementContainer */);
 ...
 * @codeGenApi
 */
export function ɵɵdomElementContainer(index, attrsIndex, localRefsIndex) {
...
//# sourceMappingURL=element_container.js.map
