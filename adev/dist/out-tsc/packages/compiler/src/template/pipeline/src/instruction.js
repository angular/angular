...
export function deferOn(trigger, args, modifier, sourceSpan) {
...
export function deferWhen(modifier, expr, sourceSpan) {
...
export function ariaProperty(name, expression, sourceSpan) {
...
export function attribute(name, expression, sanitizer, namespace, sourceSpan) {
  const args = [o.literal(name)];
  if (expression instanceof ir.Interpolation) {
    args.push(interpolationToExpression(expression, sourceSpan));
  } else {
...
export function domElement(slot, tag, constIndex, localRefIndex, sourceSpan) {
  return elementOrContainerBase(
...
export function domElementStart(slot, tag, constIndex, localRefIndex, sourceSpan) {
  return elementOrContainerBase(
...
export function domElementEnd(sourceSpan) {
  return call(Identifiers.domElementEnd, [], sourceSpan);
}
export function domElementContainerStart(slot, constIndex, localRefIndex, sourceSpan) {
  return elementOrContainerBase(
...
export function domElementContainer(slot, constIndex, localRefIndex, sourceSpan) {
  return elementOrContainerBase(
...
export function domElementContainerEnd() {
  return call(Identifiers.domElementContainerEnd, [], null);
}
export function domListener(name, handlerFn, eventTargetResolver, sourceSpan) {
  const args = [o.literal(name), handlerFn];
  if (eventTargetResolver !== null) {
    args.push(o.importExpr(eventTargetResolver));
  }
...
export function domProperty(name, expression, sanitizer, sourceSpan) {
  return propertyBase(Identifiers.domProperty, name, expression, sanitizer, sourceSpan);
}
export function animation(animationKind, handlerFn, sanitizer, sourceSpan) {
  const args = [handlerFn];
  if (sanitizer !== null) {
    args.push(sanitizer);
  }
  const identifier =
    animationKind === 'enter' /* ir.AnimationKind.ENTER */
      ? Identifiers.animationEnter
      : Identifiers.animationLeave;
  return call(identifier, args, sourceSpan);
}
export function animationString(animationKind, expression, sanitizer, sourceSpan) {
  const value =
    expression instanceof ir.Interpolation
      ? interpolationToExpression(expression, sourceSpan)
      : expression;
...
export function animationListener(animationKind, handlerFn, eventTargetResolver, sourceSpan) {
  const args = [handlerFn];
  if (eventTargetResolver !== null) {
    args.push(o.importExpr(eventTargetResolver));
  }
...
export function attachSourceLocation(templatePath, locations) {
