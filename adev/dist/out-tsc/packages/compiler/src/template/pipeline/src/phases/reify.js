...
import {TemplateCompilationMode, ViewCompilationUnit} from '../compilation';
    ...
    switch (op.kind) {
      ...
      case ir.OpKind.Element:
        ir.OpList.replace(
          op,
          unit.job.mode === TemplateCompilationMode.DomOnly
            ? ng.domElement(op.handle.slot, op.tag, op.attributes, op.localRefs, op.wholeSourceSpan)
            : ng.element(op.handle.slot, op.tag, op.attributes, op.localRefs, op.wholeSourceSpan),
        );
        break;
      ...
      case ir.OpKind.Container:
        ir.OpList.replace(
          ...
          unit.job.mode === TemplateCompilationMode.DomOnly
            ...
            : ng.elementContainer(op.handle.slot, op.attributes, op.localRefs, op.wholeSourceSpan),
        );
        break;
      ...
      case ir.OpKind.DeferOn:
        ...
        switch (op.trigger.kind) {
          ...
          default:
            throw new Error(
              `AssertionError: Unsupported reification of defer trigger kind ${op.trigger.kind}`,
            );
        }
...
function reifyIrExpression(expr) {
  ...
  switch (expr.kind) {
    ...
    default:
      throw new Error(
        `AssertionError: Unsupported reification of ir.Expression kind: ${ir.ExpressionKind[expr.kind]}`,
      );
  }
}
/**
