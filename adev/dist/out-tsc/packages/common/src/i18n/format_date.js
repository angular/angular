...
function assertValidDateFormat(parts) {
  if (parts.some((part) => /^Y+$/.test(part)) && !parts.some((part) => /^w+$/.test(part))) {
    ...
    if (parts.length === 1) {
      ...
      console.error(
        formatRuntimeError(2300 /* RuntimeErrorCode.SUSPICIOUS_DATE_FORMAT */, message),
      );
    } else {
      throw new RuntimeError(2300 /* RuntimeErrorCode.SUSPICIOUS_DATE_FORMAT */, message);
    }
  }
}
/**
  ...
  switch (part) {
    ...
    default:
      throw new RuntimeError(
        2301 /* RuntimeErrorCode.UNKNOWN_DATE_TYPE_VALUE */,
...
function getDateTranslation(date, locale, name, width, form, extended) {
  switch (name) {
    ...
    default:
      ...
      throw new RuntimeError(
        2302 /* RuntimeErrorCode.UNEXPECTED_TRANSLATION_TYPE */,
...
function timeZoneGetter(width) {
  return function (date, locale, offset) {
    ...
    switch (width) {
      ...
      default:
        throw new RuntimeError(
          2310 /* RuntimeErrorCode.UNKNOWN_ZONE_WIDTH */,
...
export function toDate(value) {
  ...
  if (!isDate(date)) {
    throw new RuntimeError(
      2311 /* RuntimeErrorCode.INVALID_TO_DATE_CONVERSION */,
