...
export class FakeEmbeddedTutorialManager {
  ...
  previousTutorial = signal(undefined);
  nextTutorial = signal(undefined);
  tutorialId = signal('fake-tutorial');
  tutorialFiles = signal(
    // use a reverse map to test openFiles order
    Object.fromEntries(filesReverse.map((file) => [file, ''])),
  );
  openFiles = signal(files);
  tutorialFilesystemTree = signal(
    Object.fromEntries(
      files.map((file) => [
        file,
        {
...
//# sourceMappingURL=code-mirror-editor.service.spec.js.map
