...
import {createCompilerHost, NgtscProgram} from '@angular/compiler-cli';
...
function main() {
  ...
  const program = new NgtscProgram(filesWithoutExamples, compilerOptions, compilerHost);
  const extraEntries = (extraEntriesSrcs ?? '')
    .split(',')
    .filter((path) => !!path)
    .reduce((result, path) => {
      return result.concat(JSON.parse(readFileSync(path, {encoding: 'utf8'})));
    }, []);
