...
export function addHtmlDescription(entry) {
  const firstParagraphRule = /(.*?)(?:\n\n|$)/s;
  let jsDocDescription = '';
  if ('jsdocTags' in entry) {
    jsDocDescription =
      entry.jsdocTags.find((tag) => tag.name === JS_DOC_DESCRIPTION_TAG)?.comment ?? '';
  }
 ...
 */
export function addHtmlJsDocTagComments(entry) {
  return {
    ...entry,
...
export function addHtmlAdditionalLinks(entry) {
  return {
    ...entry,
    additionalLinks: getHtmlAdditionalLinks(entry),
  };
}
export function addHtmlUsageNotes(entry) {
...
export function setEntryFlags(entry) {
...
//# sourceMappingURL=jsdoc-transforms.mjs.map
