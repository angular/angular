 ...
 */
export async function addRenderableCodeToc(entry) {
...
function getCodeTocData(members, isDeprecated) {
  const initialMetadata = {
    contents: '',
    codeLineNumbersWithIdentifiers: new Map(),
...
function getMethodCodeLine(member, memberTags) {
...
function getFunctionCodeLine(member, memberTags) {
 ...
 */
export function addApiLinksToHtml(htmlString) {
  const result = htmlString.replace(
    ...
    (type, span1, potentialSymbolName, span2) => {
 ...
 */
export function makeGenericsText(generics) {
  if (!generics?.length) {
    return '';
  }
  const parts = ['<'];
...
function isClassLikeDecorator(entry) {
  return !!entry.members;
}
//# sourceMappingURL=code-transforms.mjs.map
