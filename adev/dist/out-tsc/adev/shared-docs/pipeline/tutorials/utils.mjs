 ...
 */
export async function addDirectoryToFilesRecord(files, dir) {
 ...
 */
export async function findAllConfigs(dir) {
  const configs = {};
  ...
  for (const path of paths) {
    const content = await getFileContents(join(dir, path));
    configs[dirname(path)] = JSON.parse(content);
  }
  return configs;
}
/**
