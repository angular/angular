 ...
 */
export function getFileSystemTree(files, filesContents) {
...
function buildFileSystemTree(fileSystemTree, fileDirectories, filename, fileContents) {
  if (fileDirectories.length === 1) {
    ...
    fileSystemTree[directory] = {
      ...fileSystemTree[directory],
      directory: {
        ...(fileSystemTree[directory] ? fileSystemTree[directory].directory : undefined),
        [filename]: fileNode,
      },
    };
    return;
...
export function shouldUseFileInWebContainer(filename) {
  return ['.md', '.png', '.jpg'].includes(extname(filename)) === false;
}
//# sourceMappingURL=webcontainers.mjs.map
