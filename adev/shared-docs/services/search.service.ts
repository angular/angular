...
import type {Environment, SearchResult, SnippetResult} from '../interfaces/index';
...
export class Search {
  ...
  private parseResult(response: any): SearchResultItem[] | undefined {
    ...
    return this.getUniqueSearchResultItems(response.results[0].hits).map(
      (hitItem: SearchResult) => {
