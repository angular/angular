...
import {DestroyRef, Injectable, inject, resource, signal} from '@angular/core';
import {ENVIRONMENT} from '../providers/index';
import {SearchResult, SnippetResult} from '../interfaces/index';
...
export class Search {
  ...
  private parseResult(response: any): SearchResultItem[] | undefined {
    if (!response) {
      return;
    }

    const uniqueHits = this.getUniqueSearchResultItems(response.results[0].hits);

    return uniqueHits.map((hitItem: SearchResult) => {
...
/**
 * Temperory helper to implement the debounce functionality on the search resource
 */
