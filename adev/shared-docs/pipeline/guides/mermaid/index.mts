...
function getMermaidScriptTagData() {
  ...
  return (mermaidScriptTagData = {
    path: resolve(join(runfilesDir, 'npm/node_modules/mermaid/dist/mermaid.js')),
  });
}
...
export async function processMermaidCodeBlock(token: DocsCodeToken) {
  ...
  const browser = await chromium.launch({
    ...
    executablePath: join(
      runfilesDir,
      // We need to remove the ../ prefix placed on the chrome binary path env variable.
      process.env['CHROME_HEADLESS_BIN']!.substring(3),
    ),
    args: ['--no-sandbox'],
  });
