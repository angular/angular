...
### Используйте одно и то же имя файла для TypeScript, шаблона и стилей компонента {#use-the-same-file-name-for-a-components-typescript-template-and-styles}
...
### Весь код приложения помещается в директорию с именем `src` {#all-the-applications-code-goes-in-a-directory-named-src}
...
### Запускайте приложение в файле с именем `main.ts` непосредственно внутри `src` {#bootstrap-your-application-in-a-file-named-maints-directly-inside-src}
...
### Группируйте тесно связанные файлы вместе в одной директории {#group-closely-related-files-together-in-the-same-directory}
...
### Организуйте проект по функциональным областям {#organize-your-project-by-feature-areas}
...
### Сохраняйте фокус компонентов и директив на представлении {#keep-components-and-directives-focused-on-presentation}
...
### Используйте `protected` для членов класса, которые используются только шаблоном компонента {#use-protected-on-class-members-that-are-only-used-by-a-components-template}
...
### Используйте `readonly` для свойств, которые не должны изменяться {#use-readonly-for-properties-that-shouldnt-change}
...
### Называйте обработчики событий по тому, что они _делают_, а не по событию, которое их вызывает {#name-event-handlers-for-what-they-do-not-for-the-triggering-event}
