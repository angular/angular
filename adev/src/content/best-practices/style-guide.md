...
### В случае сомнений отдавайте предпочтение единообразию {#when-in-doubt-prefer-consistency}
...
### Разделяйте слова в именах файлов дефисами {#separate-words-in-filenames-with-hyphens}

Разделяйте слова в имени файла дефисами (`-`). Например, компонент с именем `UserProfile` должен иметь имя файла
`user-profile.ts`.

### Используйте то же имя для тестов файла с `.spec` в конце {#use-the-same-filename-for-tests-with-spec-suffix}
...
### Сопоставляйте имена файлов с идентификатором TypeScript внутри {#match-filenames-to-the-typescript-identifier-inside}
...
### Используйте одно и то же имя файла для TypeScript, шаблона и стилей компонента {#use-the-same-filename-for-component-typescript-template-and-styles}
...
### Весь код приложения помещается в директорию с именем `src` {#all-app-code-goes-in-a-directory-named-src}
...
### Запускайте приложение в файле с именем `main.ts` непосредственно внутри `src` {#bootstrap-the-app-in-a-file-named-main-ts}
...
### Группируйте тесно связанные файлы вместе в одной директории {#group-closely-related-files-together-in-one-directory}
...
### Организуйте проект по функциональным областям {#organize-the-project-by-feature-areas}
...
### Группируйте специфичные для Angular свойства перед методами {#group-angular-specific-properties-before-methods}
...
### Сохраняйте фокус компонентов и директив на представлении {#keep-components-and-directives-focused-on-the-view}
...
### Избегайте слишком сложной логики в шаблонах {#avoid-overly-complex-logic-in-templates}
...
### Используйте `protected` для членов класса, которые используются только шаблоном компонента {#use-protected-for-class-members-only-used-in-the-component-template}
...
### Используйте `readonly` для свойств, которые не должны изменяться {#use-readonly-for-properties-that-should-not-be-reassigned}
...
### Называйте обработчики событий по тому, что они _делают_, а не по событию, которое их вызывает {#name-event-handlers-by-what-they-do-not-the-event-that-triggers-them}
...
### Сохраняйте методы жизненного цикла простыми {#keep-lifecycle-methods-simple}
