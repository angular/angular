...
Building an accessible web experience often involves setting [Accessible Rich Internet Applications \(ARIA\) attributes](https://web.dev/learn/accessibility/aria-html/) to provide semantic meaning where it might otherwise be missing.
...
* The `cdkTrapFocus` directive traps Tab-key focus within an element.
    Use it to create an accessible experience for components such as modal dialogs, where focus must be constrained.
