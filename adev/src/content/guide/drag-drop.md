...
## Создание перетаскиваемых элементов {#creating-draggable-elements}
...
## Создание списка сортируемых перетаскиваемых элементов {#creating-a-sortable-list}
...
## Перемещение перетаскиваемых элементов между списками {#transferring-items-between-lists}
...
### Выборочное перетаскивание {#enter-predicate}
...
возвращает предикат true или false, элемент может быть разрешён или запрещён для помещения в новый контейнер.
...
## Прикрепление данных {#attaching-data}
...
## Настройка перетаскивания {#customizing-drag}

### Настройка элемента захвата (ручки) {#customizing-the-drag-handle}

По умолчанию пользователь может перетаскивать весь элемент `cdkDrag` целиком. Чтобы ограничить пользователя возможностью
перетаскивания только за определённый элемент захвата, добавьте директиву `cdkDragHandle` к элементу внутри `cdkDrag`.
...
### Настройка превью при перетаскивании {#customizing-the-drag-preview}
...
Это приведёт к тому, что любой CSS, нацеленный на этот id, не будет применён.
...
### Настройка точки вставки при перетаскивании {#customizing-the-drag-preview-insertion-point}
...
### Настройка плейсхолдера (заполнителя) {#customizing-the-placeholder}

Пока элемент `cdkDrag` перетаскивается, директива создаёт элемент-плейсхолдер, который показывает, где элемент будет
размещён при сбросе. По умолчанию плейсхолдер — это клон перетаскиваемого элемента. Вы можете заменить плейсхолдер на
...
### Настройка корневого элемента перетаскивания {#customizing-the-drag-root-element}
...
Атрибут принимает селектор и ищет вверх по DOM, пока не найдёт элемент, соответствующий селектору. Если элемент найден,
...
### Установка DOM-позиции перетаскиваемого элемента {#setting-the-dom-position-of-a-draggable}
...
пользователь ушёл со страницы и вернулся.
...
### Ограничение перемещения внутри элемента {#restricting-movement-within-an-element}

Чтобы запретить пользователю перетаскивать элемент `cdkDrag` за пределы другого элемента, передайте CSS-селектор в
атрибут `cdkDragBoundary`. Этот атрибут принимает селектор и ищет вверх по DOM, пока не найдёт соответствующий элемент.
...
### Ограничение перемещения по оси {#locking-movement-along-an-axis}

По умолчанию `cdkDrag` разрешает свободное перемещение во всех направлениях. Чтобы ограничить перетаскивание
определённой осью, установите `cdkDragLockAxis` в значение "x" или "y" на `cdkDrag`. Чтобы ограничить перетаскивание для
...
### Задержка перетаскивания {#delaying-dragging}
...
### Отключение перетаскивания {#disabling-dragging}
...
## Настройка сортировки {#customizing-sorting}
...
### Перенос элементов списка {#list-wrapping}
...
### Выборочная сортировка {#sort-predicate}
...
перемещён на новый индекс, в противном случае он сохранит свою текущую позицию.
...
### Отключение сортировки {#disabling-sorting}
...
создать элемент-"якорь", который остаётся в исходном контейнере и не перемещается вместе с элементом. Если пользователь
...
| .cdk-drop-list-disabled  | Селектор для отключённых контейнеров `cdkDropList`.                                                                                                                                                                                                                                                 |
...
| .cdk-drag-anchor         | Селектор для элемента-якоря, который создаётся, когда включён `cdkDropListHasAnchor`. Этот элемент указывает позицию, с которой начал движение перетаскиваемый элемент.                                                                                                                             |
...
автоматическая прокрутка не будет работать, если у контейнера нет директивы `cdkScrollable`. Без неё CDK не сможет
...
Функциональность Drag and Drop из CDK может быть интегрирована с различными компонентами. Распространённые случаи
