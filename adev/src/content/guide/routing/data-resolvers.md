  ...
  providers: [
    provideRouter(routes, withNavigationErrorHandler((error) => {
      ...
      if (error.message.includes('resolver')) {
        router.navigate(['/error'], { queryParams: { type: 'data-load-failed' } });
      } else {
  ...
  private navigationErrors = toSignal(
    this.router.events.pipe(
      ...
      map(event => {
        ...
        // Check if error is from a resolver
        if (event.error?.message?.includes('resolver')) {
          return 'Failed to load required data. Please try again.';
        } else {
