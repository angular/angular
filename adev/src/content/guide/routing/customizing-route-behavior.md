...
- `'always'`: Forces a trailing slash on all URLs, both during serialization and during parsing.
- `'never'`: Removes trailing slashes from all URLs.
- `'preserve'`: Keeps the trailing slash if present, and omits it if not.

Both 'always' and 'preserve' instruct the `Route` matching logic to consume the trailing slash segment. A route with `path: 'a'` will consume both the 'a' segment and the terminating empty segment for the path 'a/'.

NOTE: By default, the `DefaultUrlSerializer` preserves trailing slashes, but `Location.path()` and `Location.normalize()` strip them. Route matching also prevents trailing slashes from matching route configs other than wildcard routes.
