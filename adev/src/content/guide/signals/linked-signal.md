...
The `source` can be any signal, such as a `computed` or component `input`. When the value of `source` changes or any signal referenced in the `computation` changes, `linkedSignal` updates its value to the result of the provided `computation`.
