...
1. **Automatic provision** - Using `providedIn` in the `@Injectable` decorator or `InjectionToken` configuration
...
The string parameter (e.g., `'api.url'`) is a description purely for debugging — Angular identifies tokens by their object reference, not this string.

### InjectionToken with `providedIn: 'root'`

Just like services use `providedIn: 'root'` for automatic global provision, `InjectionToken` can do the same with a factory function:
...
Even services with `providedIn: 'root'` can be overridden at the component level:
...
1. [Class name](#class-name-tokens)
...
class ApiClient {
  constructor(
    private http: any, // Would normally be HttpClient from @angular/common/http
...
const apiClientFactory = (http: any, userService: UserService) => {
  const config = {
    free: { baseUrl: 'https://api.example.com/v1', rateLimitMs: 1000 },
    premium: { baseUrl: 'https://api.example.com/v2', rateLimitMs: 100 },
    enterprise: { baseUrl: 'https://api.example.com/enterprise', rateLimitMs: 0 }
  };

  const userTier = userService.getUserTier();
  const { baseUrl, rateLimitMs } = config[userTier] || config.free;
...
export const apiClientProvider = {
  ...
  deps: [/* HttpClient would go here */, UserService]
};
...
`useExisting` creates an alias for an existing provider. Both tokens return the same instance:
