...
A service is a broad category that encompasses any value, function, or feature that an application needs.
A service is typically a class with a focused and well-defined purpose.
A component is one type of class that can use dependency injection (DI).

Angular distinguishes components from services to improve modularity and reusability.
By separating a component's view-related features from other types of processing, component classes can remain lean and efficient.

Ideally, a component's responsibility is to enable the user experience and nothing more.
...
A component can delegate tasks to services, such as fetching data from a server, validating user input, or logging to the console.
By defining such tasks in an injectable service class, those capabilities become available to any component.
...
Angular helps you follow these principles by making it easy to organize application logic into services and make those services available to components through DI.
...
The `providedIn: 'root'` metadata specifies that the `HeroService` is available throughout the application.
...
For clarity and maintainability, it is recommended to define components and services in separate files.
