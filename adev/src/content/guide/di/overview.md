...
Внедрение зависимостей — популярный шаблон, поскольку он позволяет разработчикам решать распространённые задачи, такие
как:
...
- **Масштабируемость**: Модульную функциональность можно повторно использовать в различных контекстах, что обеспечивает
  более лёгкое масштабирование.
- **Улучшенное тестирование**: DI позволяет юнит-тестам легко
  использовать [тестовые дублёры (test doubles)](https://en.wikipedia.org/wiki/Test_double) для ситуаций, когда
...
создаёт сам. Другими словами, это создаёт связь между различными частями вашего приложения, поскольку оно не работало бы
без зависимости.
...
«Значения» в данном контексте могут быть любым значением JavaScript, включая объекты и функции. Распространённые типы
...
для внедрения в качестве зависимости. Сервисы — это наиболее распространённый способ совместного использования данных и
функциональности во всём приложении.

Распространённые типы сервисов включают:
...
NOTE: Опция `providedIn: 'root'` делает этот сервис доступным во всём вашем приложении как синглтон. Это рекомендуемый
подход для большинства сервисов.

## Внедрение зависимостей с помощью `inject()` {#dependency-injection-with-inject}
...
### Где можно использовать `inject()`? {#where-can-inject-be-used}

Вы можете внедрять зависимости во время создания компонента, директивы или сервиса. Вызов [`inject`](/api/core/inject) может находиться либо в `constructor`, либо в инициализаторе поля. Вот несколько распространённых примеров:
...
Angular использует термин «контекст внедрения» (injection context) для описания любого места в вашем коде, где вы можете вызвать [`inject`](/api/core/inject). Хотя создание компонента, директивы и сервиса является наиболее распространённым случаем, см. [контексты внедрения](guide/di/dependency-injection-context) для получения дополнительных сведений.
...
Это охватывает наиболее распространённый вариант использования сервисов в приложениях Angular.
