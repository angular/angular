...
The `minDate()` and `maxDate()` validators support string values for date and time inputs. This works with `<input type="date">`, `<input type="time">`, `<input type="datetime-local">`, and similar inputs that produce ISO-formatted string values:
...
export class BookingFormComponent {
  ...
  bookingForm = form(this.bookingModel, (schemaPath) => {
    ...
    minDate(schemaPath.checkIn, new Date().toISOString().split('T')[0], {
      message: 'Check-in date cannot be in the past',
    });

    // Appointment must be during business hours (HH:MM format)
    minDate(schemaPath.appointmentTime, '09:00', {message: 'Appointments start at 9 AM'});
    maxDate(schemaPath.appointmentTime, '17:00', {message: 'Appointments end at 5 PM'});
  });
}
```
