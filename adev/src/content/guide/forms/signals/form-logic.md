...
For complete details on `FieldContext` properties and methods, see the [Validation guide](guide/forms/signals/validation#field-context).
...
IMPORTANT: Unlike `disabled()` and `readonly()`, the `hidden()` state is **not automatically bound** to the field input. The `[field]` directive does not apply a `hidden` attribute. You must use `@if` or custom CSS in your template to conditionally render fields based on the `hidden()` state.
...
// Accumulate hints and compute total count
export const HINTS = createManagedMetadataKey(
  ...
  }), // Transform function computes new value from accumulated result
  (acc, value) => [...(acc ?? []), value] // Reducer function accumulates values into array
)
...
The managed metadata key takes two functions:

1. **Transform function** - Computes a new value from the accumulated result (receives a signal of the reduced value)
2. **Reducer function** - Accumulates values (receives `acc` and `value`, returns new accumulated value)

Common reducer patterns include:

- Arrays: `(acc, value) => [...(acc ?? []), value]`
