<docs-decorative-header title="Form Input Bindings" imgSrc="adev/src/assets/images/signals.svg"> <!-- markdownlint-disable-line -->
...
When you create a form, you start by creating a signal that holds the state of your entire form:
...
As a result, any user input automatically updates the form, and any programmatic changes update the input as well:
...
NOTE: The `[field]` directive also allows Signal Forms to sync attributes like `required`, `disabled`, and `readonly` when needed. You can read more in the upcoming in-depth guide.

### 4. Read form inputs with `value()`
