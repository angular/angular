...
## Автоматическое развертывание с помощью CLI {#automatic-deployment-with-the-cli}
...
## Ручное развертывание на удаленный сервер {#manual-deployment-to-a-remote-server}
...
### Приложения с маршрутизацией должны возвращаться к `index.html` {#routed-apps-must-fall-back-to-indexhtml}
...
### Запрос данных с другого сервера (CORS) {#requesting-services-from-a-different-server-cors}
...
### Функции только для разработки {#development-only-features}
