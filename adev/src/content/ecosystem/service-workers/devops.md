# DevOps для Service Worker {#service-worker-devops}

Эта страница является справочником по развертыванию и поддержке production-приложений, использующих Angular
Service Worker.
...
## Service Worker и кэширование ресурсов приложения {#service-worker-and-app-resources}
...
В контексте Angular Service Workerа «версия» — это набор ресурсов, представляющих конкретную сборку (build) приложения
Angular.
...
Благодаря такому поведению версионирования Angular Service Workerа, сервер приложений может гарантировать, что
приложение Angular всегда имеет согласованный набор файлов.
...
При необходимости Service Worker переходит в безопасный режим (safe mode), где запросы перенаправляются в сеть.
Service Worker не использует свой кэш, если существует высокий риск обслуживания сломанного, устаревшего или невалидного
контента.
...
Если у Angular Service Workerа нет хеша для проверки валидности ресурса, он всё равно кэширует его содержимое. При этом
...
IMPORTANT: Эта гарантия **сильнее**, чем та, что предоставляется обычной моделью веб-развертывания. Без Service Workerа
...
Большинство обновлений Angular Service Workerа прозрачны для приложения. Старые кэши остаются действительными, и контент
по-прежнему обслуживается нормально.
Иногда исправление ошибки или новая функция в Angular Service Workerе может потребовать аннулирования старых кэшей.
...
Примером может служить использование функции, которая в настоящее время не поддерживается в Service Workerе,
...
### Запросы Service Worker, когда сервер недоступен {#service-worker-requests-when-server-is-unavailable}
...
Браузеры предоставляют встроенные инструменты для отладки Service Workerов, а сам Angular Service Worker включает полезные
функции отладки.
...
#### Состояние драйвера (Driver state) {#driver-state}
...
Браузер иногда завершает работу простаивающего Service Workerа для экономии памяти и мощности процессора и создает новый
экземпляр Service Workerа в ответ на сетевые события.
Новый экземпляр запускается в режиме `NORMAL`, независимо от состояния предыдущего экземпляра.

#### Хеш последнего манифеста (Latest manifest hash) {#latest-manifest-hash}
...
#### Последняя проверка обновлений (Last update check) {#last-update-check}
...
В этом примере у Service Workerа есть одна кэшированная версия приложения, которая используется для обслуживания двух
разных вкладок.
...
#### Очередь фоновых задач (Idle Task Queue) {#idle-task-queue}
...
Idle Task Queue — это очередь всех ожидающих задач, которые выполняются в фоновом режиме в Service Workerе.
Если в очереди есть какие-либо задачи, они перечисляются с описанием.
В этом примере у Service Workerа запланирована одна такая задача — операция после инициализации, включающая проверку
...
#### Журнал отладки (Debug log) {#debug-log}
...
Ошибки, возникающие внутри Service Workerа, регистрируются здесь.

### Инструменты разработчика {#developer-tools}

Браузеры, такие как Chrome, предоставляют инструменты разработчика для взаимодействия с Service Workerами.
...
- Остановка и запуск Service Workerа на панели Service Worker инициирует проверку обновлений.
...
Когда запрос Service Workerа к `ngsw.json` вернет `404`, Service Worker удалит все свои кэши и отменит свою регистрацию,
...
При загрузке он отменяет свою регистрацию в браузере и удаляет кэши Service Workerа.
Этот скрипт можно использовать как крайнюю меру для избавления от нежелательных Service Workerов, уже установленных на
страницах клиентов.
...
Вместо этого вы должны отдавать содержимое `safety-worker.js` по URL-адресу скрипта Service Workerа, регистрацию которого
...
Для большинства сайтов это означает, что вы должны отдавать safety worker по старому URL-адресу Service Workerа навсегда.
Этот скрипт можно использовать для деактивации `@angular/service-worker` и удаления соответствующих кэшей. Он также
удаляет любые другие Service Workerы, которые могли обслуживаться на вашем сайте в прошлом.

### Изменение местоположения вашего приложения {#changing-your-apps-location}
...
Кроме того, перенаправление даже не сработает для пользователей, которые загружают сайт полностью из Service Workerа.
...
## Дополнительно об Angular Service Worker {#more-on-angular-service-workers}
