...
```ts
import {Directive, HostBinding, HostListener, input, signal, computed} from '@angular/core';
```
...
<docs-step title="Bind the computed signal to the host">
Use HostBinding to apply the computed style to the element.
...
This automatically updates the element's background color when signals change.
</docs-step>

<docs-step title="Handle user interactions">
Add mouse event handlers that update the hover signal.
...
These handlers update the signal, which automatically triggers the computed signal to recalculate.
</docs-step>
...
- **Host bindings** - Connecting computed signals to DOM properties
- **Event handlers** - Updating signals in response to user interactions
