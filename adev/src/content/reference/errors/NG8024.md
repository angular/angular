...
one of its bindings under a different alias name. This also applies when a model is exposed through
the `models` shorthand: the model input and its corresponding `Change` output must use the same
alias in every declaration.
...
`hostDirectives` declaration that includes that shared directive and inspect the `inputs`,
`outputs`, or `models` arrays for the conflicting binding.
...
- **Remove the alias from one or both declarations.** If neither declaration needs to expose the
  binding to consumers, omit it from the `inputs`, `outputs`, or `models` arrays entirely.
