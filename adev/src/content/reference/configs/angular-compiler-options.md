...
Exact property names are also preserved in translated property interpolation marked with `i18n-*`.
...
The VS Code language server reports manifest diagnostics on the project's `tsconfig` and updates them when resources change, including unsaved manifest edits. These diagnostics do not currently link to individual entries inside a manifest.
