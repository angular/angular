...
export class InteractiveTerminal extends Terminal {
  ...
  constructor() {
    ...
    // bypass command validation if sudo=true is present in the query string in development
    if (environment.production || !this.window.location.search.includes('sudo=true')) {
