...
const DEFAULT_STATUS = STATUSES.stable;
...
export default class ApiReferenceList {
  ...
  constructor() {
    ...
    effect(() => {
      const params: Params = {
        'query': this.query() || null, // on empty string we remove the query param
        'type': this.type(),
        'status': this.status(),
      };
  ...
  setStatus(status: number): void {
    this.status.update((previousStatus) =>
      this.isStatusSelected(previousStatus) ? status & ~previousStatus : previousStatus | status,
    );
  }
