...
type TutorialAnswerComparisonFile = {
  path: string;
  content: string;
};
...
export default class Tutorial {
  ...
  readonly answerComparisonFiles = computed<TutorialAnswerComparisonFile[]>(() => {
    const answerFiles = this.embeddedTutorialManager.answerFiles();
    const hiddenFiles = new Set(this.embeddedTutorialManager.hiddenFiles());
    const orderedPaths = Array.from(
      new Set([...this.embeddedTutorialManager.openFiles(), ...Object.keys(answerFiles)]),
    );

    return orderedPaths
      .filter((path) => !hiddenFiles.has(path) && answerFiles[path] !== undefined)
      .map((path) => ({
        path,
        content: this.getAnswerFileContent(answerFiles[path]),
      }));
  });
  ...
  private getAnswerFileContent(content: string | Uint8Array | undefined): string {
