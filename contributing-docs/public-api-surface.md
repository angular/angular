...
- Any class members or symbols marked as `private`, or prefixed with
  underscore (`_`), [barred latin o](https://en.wikipedia.org/wiki/%C6%9F) (`ɵ`), and double barred latin o (`ɵɵ`). While these private APIs are exempt from SemVer's traditional definition of a breaking change, we maintain compatibility for them within a given minor version, across patches.
- Extending any of our classes unless the support for this is specifically documented in the API
  reference.
- The contents and API surface of the code generated by Angular's compiler. While the format of generated code is not part of the public API, Angular maintains stability for the internal runtime APIs it calls (the **Compiler ABI**) across patch versions of the same minor version.
