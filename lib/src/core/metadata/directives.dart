 ...
 * removed, or moved by a directive that uses a [ViewContainerRef] such as a `ngFor`, an
 * `ngIf`, or an `ngSwitch`.
 *
 * ```
 * @Directive({ selector: '[my-directive]' })
 ...
 * runtime.
 * The [ViewContainerRef] is created as a result of `<template>` element, and represents a
 ...
 * ## Lifecycle hooks
 *
 * When the directive class implements some [angular2/lifecycle_hooks] the callbacks are
...
class DirectiveMetadata extends InjectableMetadata {
  ...
  List<String> get outputs {
    return isPresent(this._events) && this._events.length > 0
        ? this._events
        : this._outputs;
  }
  ...
  List<dynamic> get providers {
    return isPresent(this._bindings) && this._bindings.length > 0
        ? this._bindings
        : this._providers;
  }
 ...
 * ## Lifecycle hooks
 *
 * When the component class implements some [angular2/lifecycle_hooks] the callbacks are
...
class ComponentMetadata extends DirectiveMetadata {
   ...
   * ```
   */
  List<dynamic> get viewProviders {
    return isPresent(this._viewBindings) && this._viewBindings.length > 0
        ? this._viewBindings
        : this._viewProviders;
  }

  List<dynamic> get viewBindings {
