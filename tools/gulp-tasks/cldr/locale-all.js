...
module.exports = (gulp, done) => {
  ...
  // additional "all" file that will be used by closure
  console.log(`Writing file ${I18N_DATA_FOLDER}/all.ts`);
  fs.writeFileSync(`${RELATIVE_I18N_DATA_FOLDER}/all.ts`, generateAllLocalesFile(GOOG_LOCALES));

  console.log(`Formatting ${I18N_DATA_FOLDER}/all.ts..."`);
  const format = require('gulp-clang-format');
  const clangFormat = require('clang-format');
  return gulp.src([`${I18N_DATA_FOLDER}/all.ts`], {base: '.'})
...
${LOCALES.map(locale => `import ${toCamelCase(locale)} from './locale_${locale}';\n`).join('')}
