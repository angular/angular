...
/**
 * Makes writting diffing plugins easy.
 *
 ...
 *
 * @param pluginClass
 * @returns {function(): DiffingPluginWrapper}
 */
export function wrapDiffingPlugin(pluginClass): (inputTrees: (BroccoliTree | BroccoliTree[]),
                                                 options?) => BroccoliTree {
...
class DiffingPluginWrapper implements BroccoliTree {
  ...
  rebuild() {
    ...
    if (rebuildPromise) {
      return (<Promise<any>>rebuildPromise).then(this.relinkOuputAndCachePaths.bind(this));
    }

    this.relinkOuputAndCachePaths();
  }


  private relinkOuputAndCachePaths() {
