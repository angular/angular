...
function makeBrowserTree(options) {
  ...
  var es6Tree = new TraceurCompiler(modulesTree, '.es6', {
    sourceMaps: true,
  ...
  es6Tree = stew.rename(es6Tree, function (relativePath) {
    return relativePath.replace(/\.(js|es6)\.map$/, '.map').replace(/\.js$/, '.es6');
  });

  // Call Traceur again to lower the ES6 build tree to ES5
  var es5DevTree = new TraceurCompiler(es6Tree, '.js', {modules: 'instantiate', sourceMaps: true});
  ...
  return mergeTrees([
    ...
    stew.mv(es5DevTree, 'js/' + options.name + '/es5')
  ]);
}
...
function makeCjsTree() {
  ...
  var extras = new Funnel(modulesTree, {include: ['**/*.md', '**/*.png'], exclude: ['**/*.dart.md']});
