...
describe('typescript.previewer', () => {
  it('Should ignore hyphens after a param tag', () => {
    expect(
      (0, text_render_1.tagsToMarkdown)(
        [{name: 'param', text: [{kind: 'text', text: 'a - b'}]}],
        () => undefined,
      ),
    ).toBe('*@param* `a` — b');
  });
  it('Should parse url jsdoc @link', () => {
    expect(
      (0, text_render_1.documentationToMarkdown)(
        [
          {'text': 'x ', 'kind': 'text'},
          {'text': '{@link ', 'kind': 'link'},
  ...
  it('Should render @example blocks as code', () => {
    expect(
      (0, text_render_1.tagsToMarkdown)(
        [{name: 'example', text: [{kind: 'text', text: 'code();'}]}],
        () => undefined,
      ),
    ).toBe('*@example*  \n```\ncode();\n```');
  });
  ...
  it('Should render @linkcode symbol name as code', () => {
    expect(
      (0, text_render_1.asPlainTextWithLinks)(
        [
          {'text': 'a ', 'kind': 'text'},
          {'text': '{@linkcode ', 'kind': 'link'},
          {
...
//# sourceMappingURL=text_render_spec.js.map
